...
namespace FinTrakBanking.ThirdPartyIntegration
{
    ...
    public class IntegrationWithFlexcube : IIntegrationWithFinacle
    {
        ...
        public ResponseMessageViewModel OverDraftExtend(OverDraftExtendViewModel model, TwoFactorAutheticationViewModel twoFADetails = null)
        {
            ...
            //===================This is a make shift pending when the API for Overdraft top up is provided. the below code should be Uncommented.========================
        ...
        {
            {
                ...
                if (result.APIResponse != null)
                {
                    if (result.APIResponse.responseCode == "00")
                    {
                        ...
                        //return new PostingResult { posted = true, responseCode = str.Trim() };
                        return new PostingResult { posted = true, responseCode = result.APIResponse.responseCode, responseMessage = result.APIResponse.message };
                    }
                    else
                    {
                        //var message = result.responseMessage.Replace("[", "").Replace("]", "").Replace("{", "").Replace("}", "").Replace(@"""", "");
                        throw new APIErrorException("Core Banking API error - Response Code:" + result.APIResponse.responseCode + ". Response Message:" + result.APIResponse.message);
                    }
                }
                else
                {
        ...
        public bool AddCustomerAccounts(int customerId, string customerCode)
        {
            ...
            {
                var productId = context.TBL_PRODUCT.FirstOrDefault().PRODUCTID;
                ...
                addCustomerAcct.PRODUCTID = productId;//(short)DefaultProductEnum.CASA; //(short)(item.productCode != "" ? 8 : 8);
                addCustomerAcct.COMPANYID = 1;
                addCustomerAcct.BRANCHID = context.TBL_BRANCH.Where(x => x.BRANCHCODE == item.branchCode).Select(x => x).FirstOrDefault() == null ? (short)94 : context.TBL_BRANCH.Where(x => x.BRANCHCODE == item.branchCode).Select(x => x.BRANCHID).FirstOrDefault();
        ...
        public bool AddCustomerAccounts(string customerCode)
        {
            ...
            foreach (var item in data)
            {
                ...
                if (item.accountStatusName.ToLower() == "regular") { accountStatusId = 1; }
                if (item.accountStatusName.ToLower() == "postnodebit") { accountStatusId = 5; }
                TBL_CASA addCustomerAcct = new TBL_CASA();
                addCustomerAcct.CUSTOMERID = customerId;
                addCustomerAcct.AVAILABLEBALANCE = item.availableBalance;
                addCustomerAcct.LEDGERBALANCE = item.ledgerBalance;
                addCustomerAcct.PRODUCTACCOUNTNAME = item.productName;//item.productAccountName;
                addCustomerAcct.PRODUCTACCOUNTNUMBER = item.productAccountNumber;
                addCustomerAcct.PRODUCTID = context.TBL_PRODUCT.FirstOrDefault().PRODUCTID; //(short)DefaultProductEnum.CASA; //(short)(item.productCode != "" ? 8 : 8);
                addCustomerAcct.COMPANYID = 1;
        ...
        public List<CustomerTurnoverViewModel> GetCustomerAccountTurnover(string accountNumber, DateTime startDate, DateTime endDate)
        {
            if (!USE_THIRD_PARTY_INTEGRATION) return null;
            List<CustomerTurnoverViewModel> accounts = new List<CustomerTurnoverViewModel>();

            try {
                Task.Run(async () => accounts = await customer.GetCustomerTransactions(accountNumber, startDate, endDate))?.GetAwaiter().GetResult();
                return accounts;
            }
            catch (Exception) {
