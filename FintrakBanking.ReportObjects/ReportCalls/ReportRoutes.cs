...
namespace FintrakBanking.ReportObjects.ReportCalls
{
    ...
    public class ReportRoutes : IReportRoutes
    {
       ...
       string cryptoKey = "sqluser10$";
        ...
        public async Task<int> GetLoanApplicationIdByReferenceNumber(string applicationRefNumber)
        {
            int loanAppId;
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                var appl = await context.TBL_LOAN_APPLICATION.Where(c => c.APPLICATIONREFERENCENUMBER == applicationRefNumber).FirstOrDefaultAsync();
