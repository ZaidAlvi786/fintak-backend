...
namespace FintrakBanking.ReportObjects
{
    ...
    public class LoanReportObjects
    {
        ...
        public IEnumerable<InitiatedLoansViewModel> GetReferredLoans(DateTime startDate, DateTime endDate, int companyId, string loanRefNo, short? branchId, int? productClassId, int? staffId)
        {
            using (FinTrakBankingStagingContext stagecontext = new FinTrakBankingStagingContext())
            {
                using (FinTrakBankingContext context = new FinTrakBankingContext())
                {
                    var data = (
                        from l in context.TBL_LOAN_APPLICATION
                        join d in context.TBL_LOAN_APPLICATION_DETAIL on l.LOANAPPLICATIONID equals d.LOANAPPLICATIONID
                        join trail in context.TBL_APPROVAL_TRAIL on l.LOANAPPLICATIONID equals trail.TARGETID
                        where trail.SYSTEMARRIVALDATETIME >= startDate && trail.SYSTEMARRIVALDATETIME <= endDate
                        && trail.APPROVALSTATUSID == (short)ApprovalStatusEnum.Referred
                        && trail.APPROVALSTATEID != (short)ApprovalState.Ended
                        && trail.RESPONSESTAFFID != null
                        orderby trail.SYSTEMARRIVALDATETIME descending
                        select new InitiatedLoansViewModel
                        {
                            applicationReferenceNumber = l.APPLICATIONREFERENCENUMBER,
                            solId = l.TBL_BRANCH.BRANCHCODE,
                            customerName = l.TBL_CUSTOMER.FIRSTNAME + " " + l.TBL_CUSTOMER.MIDDLENAME + " " + l.TBL_CUSTOMER.LASTNAME,
                            staffName = l.TBL_STAFF.FIRSTNAME + " " + l.TBL_STAFF.MIDDLENAME + " " + l.TBL_STAFF.LASTNAME,
                            approvedTenor = d.APPROVEDTENOR,
                            proposedAmount = d.PROPOSEDAMOUNT,
                            exchangeRate = d.EXCHANGERATE,
                            product = d.TBL_PRODUCT.PRODUCTNAME,
                ...
                trialBal = (from ft in context.TBL_FINANCE_TRANSACTION
                            ...
                            select new TrialBalanceViewModel()
                            {
                                ...
                                if (hasdetailid)
                                {
                                    ...
                                    {
                                        string message = TranslateHelper.get(ex.Message);
                                    }
        ...
        public List<CollateralRegisterReportViewModel> CollateralRegister(DateTime startDate, DateTime endDate, int companyid, short? branchId)
        {
                ...
                List<CollateralRegisterReportViewModel> collateralRegisterReportData = null;
                using (FinTrakBankingContext context = new FinTrakBankingContext())
                {
                    ...
                    var data = (from l in context.TBL_COLLATERAL_CUSTOMER
                     ...
                     collateralRegisterReportData = data.ToList();
                    ...
                    foreach (var k in collateralRegisterReportData)
                    {
                        ...
                        if (k.collateralTypeId == (int)CollateralTypeEnum.PlantAndMachinery)
                        {
                            ...
                            if (cim != null)
                            {
                                ...
                                k.collateralValueOmv = k.collateralValue;
                                k.collateralValueEfsv = cim.REPLACEMENTVALUE;
                            }
                        }
        ...
        public List<CollateralAdequacyViewModel> CollateralAdequacy(DateTime startDate, DateTime endDate, int companyid, short? branchId)
        {
                using (FinTrakBankingContext context = new FinTrakBankingContext())
                {
                    ...
                    foreach (var k in collateralAdequacyReportData)
                    {
                            ...
                            if (k.totalTangibleCollateral > 0)
                            {
                                k.percentageTangibleCoverage = (k.totalDirectExposure / k.totalTangibleCollateral);
                            }
                            if (k.totalIntangibleCollateral > 0)
                            {
                                k.percentageIntangibleCoverage = (k.totalDirectExposure / k.totalIntangibleCollateral);
                            }
