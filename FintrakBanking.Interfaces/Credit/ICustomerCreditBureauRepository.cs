...
namespace FintrakBanking.Interfaces.Credit
{
    public interface ICustomerCreditBureauRepository
    {
        ...
        #region Integration
        Task<XDSSearchResult> GetCustomerXDSCreditMatch(CreditBureauSearchViewModel searchInfoList);

        Task<XDSSearchResult> GetXDSFullSearchResultInPDF(SearchInput searchInput);

        Task<string> GetCRCFullCreditMergeReport(MultiHitRequestViewModel request);

        Task<CRCSearchResult> GetCustomerCRCCreditMatch(CRCRequestViewModel searchInfo);
        #endregion
    }
}
