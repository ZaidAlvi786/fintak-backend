    ...
    public class FinanceTransactionRepository : IFinanceTransactionRepository
    {
        ...
        public string PostTransactionBulk(List<FinanceTransactionViewModel> inputTransactions)
        {
            ...
            if (transactionCount < 2) //transaction.transactionDetails.Count() < 2
                throw new ConditionNotMetException(TranslateHelper.get("Specify both debit and credit transactions"));
            ...
            if (debitSum != creditSum)
                throw new ConditionNotMetException(TranslateHelper.get("Total Debit Amount should equal Total Credit Amount"));
            ...
            foreach (var item in inputTransactions)
            {
                if (item.debitAmount != 0 && item.creditAmount != 0)
                    throw new ConditionNotMetException(TranslateHelper.get("Debit or Credit Amount should be 0"));

                if (item.debitAmount < 0)
                    throw new ConditionNotMetException(TranslateHelper.get("Debit Amount should NOT be less than 0"));

                if (item.creditAmount < 0)
                    throw new ConditionNotMetException(TranslateHelper.get("Credit Amount should NOT be less than 0"));
                ...
                if (glClass == GLClassEnum.CASA)
                {
                    if (item.casaAccountId == null)
                        throw new ConditionNotMetException($"{TranslateHelper.get("Specify the CASA Account Number in this transaction for GL Code")} {glInfo.ACCOUNTCODE}");
                ...
                else if (glClass == GLClassEnum.LoanSchedule)
                {
                    ...
                    if (referenceCount <= 0)
                        throw new ConditionNotMetException($"{TranslateHelper.get("Loan reference number")} {item.sourceReferenceNumber} {TranslateHelper.get("does not exist in the loan table for this transaction for GL Code")} {glInfo.ACCOUNTCODE}");
                }
                else
                { item.casaAccountId = null; }
        ...
        public string PostTransaction(List<FinanceTransactionViewModel> inputTransactions, bool isBulkPosting = false, TwoFactorAutheticationViewModel twoFADetails = null)
        {
            ...
            foreach (var item in inputTransactions)
            {
                ...
                if (glClass == GLClassEnum.CASA)
                {
                    ...
                    if (casa == null)
                        throw new ConditionNotMetException($"{TranslateHelper.get("CASA account number")} {item.sourceReferenceNumber} {TranslateHelper.get("does not exist in the CASA table for this transaction for GL Code")} {glInfo.ACCOUNTCODE}");
            ...
            if (USE_THIRD_PARTY_INTEGRATION && isBulkPosting == false && globalIntegrationSetting.USE_THIRPARTY_POSTING)
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Transaction Failed."));
                }
            }
            else
            {
        ...
        public CurrencyExchangeRateViewModel GetExchangeRate(DateTime date, short currencyId, int companyId)
        {
            ...
            if (USE_THIRD_PARTY_INTEGRATION)
            {
                ...
                else
                {
                    ...
                    if (exRate != null && today > exRate.date.Date)
                    {
                    ...
                    if (exRate != null && today == exRate.date.Date && exRate.exchangeRate == 1)
                    {
                        var rate = integration.GetExchangeRate(fromCurrencyCode, toCurrencyCode, rateCode);
                        var currencyRate = context.TBL_CURRENCY_EXCHANGERATE.FirstOrDefault(r => r.CURRENCYID == currencyId && r.DELETED == false);
                        if (currencyRate != null && rate != null && rate.sellingRate > 1)
                        {
                            ...
                            currencyRate.EXCHANGERATE = rate.exchangeRate;
        ...
        private CurrencyExchangeRateViewModel GetExchangeRateStaging(DateTime date, short currencyId, short baseCurrency, string rateCode)
        {
            ...
            else
            {
                ...
                if (rateInfo == null)
                {
                    if (currency == null)
                    {
                        throw new ConditionNotMetException($"{TranslateHelper.get("Currency is not defined. Define the Currency First and try again")}");
                    }
                    throw new ConditionNotMetException($"{TranslateHelper.get("Exchange rate for")} {currency.CURRENCYNAME} {TranslateHelper.get("is not defined. Define the exchange rate and try again")}");
                    //throw new ConditionNotMetException($"Exchange rate for {generalSetup.GetApplicationDate()} is not defined. Define the exchange rate and try again");
                }
...

        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Loan Daily Write-off Interest Accrual Posting:")} {model.referenceNumber}",
...

        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = (int)SystemStaff.System,//model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Loan Daily Interest Accrual Posting:")} {model.referenceNumber}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = model.date,//generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
...

        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Daily Amortized Fee Posting:")} {model.referenceNumber}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = model.date,//generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public FinanceTransactionViewModel PostDailyAuthorisedOverdraftInterestAccrual(DailyInterestAccrualViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Authorised Overdraft Daily Interest Accrual Posting:")} {model.referenceNumber}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = model.date,//generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public FinanceTransactionViewModel PostDailyUnauthorisedOverdraftInterestAccrual(DailyInterestAccrualViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Unauthorised Overdraft Daily Interest Accrual Posting:")} {product.PRODUCTCODE}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = model.date,//generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
...

        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Interval Fees and Commission charge Posting :")} {model.loanReferenceNumber}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
...

        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Interest Suspension Posting:")} {model.referenceNumber}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = model.date,//generalSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public TrialBalanceViewModel GenerateTrialBalanceData(ReportSearchEntity entity, int companyId)
        {
            ...
            if (record == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For Download"));
