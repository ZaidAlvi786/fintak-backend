...
namespace FintrakBanking.Repositories.WorkFlow
{
    public class JobRequestRepository : IJobRequestRepository
    {
        ...
        #region Basic
        private async Task<string> saveJobRequest(JobRequestViewModel model)
        {
            if (model.jobSourceId == null || model.jobSourceId <= 0) { throw new ConditionNotMetException(TranslateHelper.get("Could not resolve job source. Please contact admin.")); }
            if (model.operationsId == 0)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Job request could not resolve the operation."));
            }
            ...
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Added JobRequest")} '{ model.jobRequestCode }' ",
            ...
            {
                throw new Exception(TranslateHelper.get("An error occured while saving records."));
            }
        }

        public async Task<string> AddGlobalJobRequest(JobRequestViewModel model)
        {
            if (model.receiverStaffId == model.createdBy)
                throw new SecureException(TranslateHelper.get("You cannot assign a job to yourself"));
            ...
            if (model.isApplicationLevel)
            {
                ...
                if (!applicationDetail.Any())
                    throw new ConditionNotMetException(TranslateHelper.get("Targeted for this record could not be resolved"));
                ...
                foreach (var item in applicationDetail)
                {
                    ...
                    code = await saveJobRequest(model);
                    ctr = ctr + 1;
                }
                if (ctr > 1) { code = code + "..."; }
                return code;
            ...
            {
                model.jobRequestCode = model.jobTypeId + "" + model.createdBy + "" + model.receiverStaffId + "" + this.RequestCode();
                return await saveJobRequest(model);
            }
        }

        public async Task<bool> AddJobComment(JobRequestMessageViewModel model)
        {
        ...
        public async Task<bool> ReplyJobRequest(JobRequestViewModel model, int jobRequestId)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Reply JobRequest")} '{ data.JOBREQUESTCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = applicationDate,
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> ReRouteJobRequest(JobRequestViewModel model)
        {
            var applicationDate = general.GetApplicationDate();
            ...
            if (existingUnit == null || newUnit == null) throw new ConditionNotMetException(TranslateHelper.get("Missing unit. Contact admin."));
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Job request with code")} '{ data.JOBREQUESTCODE }' {TranslateHelper.get("was re-routed from")} '{existingUnit.UNITNAME}' {TranslateHelper.get("to")} '{newUnit.UNITNAME}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = applicationDate,
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> ReassignJobRequest(JobRequestViewModel model, int jobRequestId)
        {
            ...
            if (data.REQUESTSTATUSID == (short)JobRequestStatusEnum.approved)
                throw new BadLogicException(TranslateHelper.get("You cannot assign/reassign this job. Job has already been responded and/or closed."));
            ...
            if (fromStaffData != null)
            {
                if (toStaffCode == fromStaffData.STAFFCODE)
                    throw new ConditionNotMetException(TranslateHelper.get("This job is currently assigned to the selected staff. Choose another staff to reassign."));

                info = $"{TranslateHelper.get("Reassigned JobRequest with code")} '{ model.jobRequestCode }' {TranslateHelper.get("from staff with code")} '{fromStaffData.STAFFCODE}' {TranslateHelper.get("to staff with code")} '{toStaffCode}'";
            }
            else { info = $"{TranslateHelper.get("Assigned JobRequest with code")} '{ model.jobRequestCode }' {TranslateHelper.get("to staff")} '<strong>{toStaffName}</strong>' {TranslateHelper.get("with code")} '<strong>{toStaffCode}</strong>'"; }
            ...
            if (data.JOBTYPEID == (short)JobTypeEnum.middleOfficeVerification)
            {
                ...
                string messageBoby = $"{TranslateHelper.get("Attention!")}, <br /><br />{TranslateHelper.get("Please note that a job request assignment/reassignment occured with the following details:")}<br /><br /> 'Job Request Code:' " +
                    $" {data.JOBREQUESTCODE} <br /><br /> {TranslateHelper.get("Previously Assigned Staff:")} {from} " +
                    $"<br /><br />  {TranslateHelper.get("Current Staff Assigned:")} {to} <br /><br />  {TranslateHelper.get("Regards.")} ";
                string alertSubject = $"{TranslateHelper.get("REQUEST FOR DOCUMENT VERIFICATION")}";
                ...
                if (loanDetails != null)
                {
                    ...
                    var accountLine = casaInfo != null ? $"<br /><br /> <strong>{TranslateHelper.get("Account Number:")}</strong>  {casaInfo.PRODUCTACCOUNTNUMBER} " : null;
                    var customerNameLine = $"<br /><br /> <strong>{TranslateHelper.get("Customer Name:")}</strong>  {customerInfo.FIRSTNAME}  {customerInfo.LASTNAME}  ";
                    var loantTypeLine = facilityInfo != null ? $"<br /><br /> <strong>{TranslateHelper.get("Facility Type:")}</strong>  {facilityInfo.PRODUCTNAME} " : null;
                    var applicationRef = loanDetails.TBL_LOAN_APPLICATION != null ? $"<br /><br /> <strong>{TranslateHelper.get("Application Reference:")}</strong>  {loanDetails.TBL_LOAN_APPLICATION.APPLICATIONREFERENCENUMBER} " : null;
                    var accountNumberLine = casaInfo != null ? $"<br /><br /> <strong>{TranslateHelper.get("Account Number:")}</strong>  {casaInfo.PRODUCTACCOUNTNUMBER} " : null;
                    var currencyTypeLine = currencyInfo != null ? $"<br /><br /> <strong>Currency:</strong>  {currencyInfo.CURRENCYCODE} " : null;

                    var principalNameLine = invoiceInfo.FirstOrDefault() != null ? $"<br /><br /> <strong>{TranslateHelper.get("Principal Name:")}</strong>  {invoiceInfo.FirstOrDefault().TBL_LOAN_PRINCIPAL.NAME} " : null;
                    var rmCommentLine = $"<br /><br /><br /> {TranslateHelper.get("kindly proceed with verification and provide timely feedback")} ";
                    ...
                    var poAmountLine = invoiceInfo.FirstOrDefault() != null ? $"<br /><br /> <strong>{TranslateHelper.get("Total amount on PO/Contract/Invoice:")}</strong>  {string.Format("{0:#,0.00}", amount) }  " : null;
                    var poNumber = invoiceInfo.FirstOrDefault() != null ? $"<br /><br />  <strong>{TranslateHelper.get("PO/Contract/Invoice No:")}</strong>  {numbers }  " : null;
                    messageBoby = $"{TranslateHelper.get("Attention!")} <br /><br />{TranslateHelper.get("Please note that a job has been assigned with the following details:")}" +
                           $"{ principalNameLine} " +
                           $"{ customerNameLine} " +
                           $"{ accountLine} " +
                           $"{ loantTypeLine} " +
        ...
        public async Task<bool> AcknowledgeJob(JobRequestViewModel entity, int jobRequestId)
        {
            var data = context.TBL_JOB_REQUEST.Find(jobRequestId);
            if (data == null)
                return false;
            ...
            if (data.ISACKNOWLEDGED)
                throw new BadLogicException(TranslateHelper.get("Job already ackowledged."));
        ...
        public async Task<IEnumerable<ApplicationJobRequest>> GetLoanApplicationJobsById(int loanApplicationId, int companyId)
        {
            ...
            foreach (var i in data)
            {
                ...
                var invoiceDiscountDetail = await (from ap in context.TBL_LOAN_APPLICATION_DETL_INV.Where(x => x.LOANAPPLICATIONDETAILID == i.loanApplicationDetailId)
                ...
                var firstEducationtDetail = await (from ed in context.TBL_LOAN_APPLICATION_DETL_EDU.Where(x => x.LOANAPPLICATIONDETAILID == i.loanApplicationDetailId)
                ...
                var firstTradderDetail = await (from tr in context.TBL_LOAN_APPLICATION_DETL_TRA.Where(x => x.LOANAPPLICATIONDETAILID == i.loanApplicationDetailId)
        ...
        private async Task<IEnumerable<JobRequestViewModel>> GetAllGlobalJobRequest(int staffId, bool isFilter = false)
        {
            ...
            bool isTeamLead = await (from s in context.TBL_JOB_TYPE_HUB_STAFF where s.STAFFID == staffId select s.ISTEAMLEAD).FirstOrDefaultAsync();
            ...
            allData = await (from x in context.TBL_JOB_REQUEST
            ...
            allData = await setCustomerName(allData);

            allData = await setJobRequestOtherDetails(allData);

            allData = await PadModelWithLMSReference(allData);


            return allData;
        }

        private async Task<List<JobRequestViewModel>> setJobRequestOtherDetails(List<JobRequestViewModel> models)
        {
            foreach (var item in models)
            {
                ...
                var jobStatusFeedBackRecord = await context.TBL_JOB_REQUEST_STATUS_FEEDBAK.Where(x => x.JOB_STATUS_FEEDBACKID == item.jobStatusFeedBackId).FirstOrDefaultAsync();
                if (jobStatusFeedBackRecord != null) item.jobStatusFeedBack = jobStatusFeedBackRecord.JOB_STATUS_FEEDBACK_NAME;
            }
            return models;
        }

        private async Task<List<JobRequestViewModel>> setCustomerName(List<JobRequestViewModel> models)
        {
            foreach (var model in models)  //(applicationDet != null && model.jobSourceId == (short)JobSourcesEnum.LoanApplicationDetail)
            {
                var applicationDet = await context.TBL_LOAN_APPLICATION_DETAIL.FindAsync(model.targetId);
        ...
        private async Task<IEnumerable<JobRequestViewModel>> GetGlobalJobRequestByStatus(int staffId, short statusId, int? startNumber = null)
        {
            var thisStaff = await this.context.TBL_STAFF.FindAsync(staffId);
            var staffAdmin = this.context.TBL_JOB_TYPE_REASSIGNMENT.Where(x => x.STAFFID == staffId);
            var staffHub = this.context.TBL_JOB_TYPE_HUB_STAFF.Where(x => x.STAFFID == staffId);
            var middleOfficeUnit = from x in context.TBL_JOB_TYPE_UNIT
                                   join t in context.TBL_JOB_TYPE_HUB_STAFF on x.JOBTYPEUNITID equals t.JOBTYPEUNITID
        ...
        public async Task<IEnumerable<JobRequestViewModel>> GetJobRequestBySearchString(int staffId, string searchString)
        {
            List<int> applicationDetailsIds = await SearchFacilityApplicationEntries(searchString.ToLower());
            List<int> loanIds = await SearchBookedFacilityEntries(searchString.ToLower());

            loanIds.AddRange(await SearchCustomerLoanFacilities(searchString.ToLower()));
            applicationDetailsIds.AddRange(await SearchCustomerFacilityApplication(searchString.ToLower()));
        ...
        public async Task<IEnumerable<JobRequestViewModel>> GetJobRequestByStaffId(int staffId, int branchId)
        {
            var allGlobalJobRequest = await GetAllGlobalJobRequest(staffId);
            return allGlobalJobRequest.OrderByDescending(x => x.jobRequestId);
        }

        public async Task<IEnumerable<JobRequestViewModel>> GetJobRequestByFilter(int staffId, int branchId, string filter, int? startNumber)
        {
            ...
            {
                case "completed":
                    var allGlobalJobRequestStatus = await GetGlobalJobRequestByStatus(staffId, (short)RequestStatusEnum.Approved);
                    return allGlobalJobRequestStatus.OrderByDescending(x => x.jobRequestId);

                case "approved":
                    allGlobalJobRequestStatus = await GetGlobalJobRequestByStatus(staffId, (short)RequestStatusEnum.Approved);
                    return allGlobalJobRequestStatus.OrderByDescending(x => x.jobRequestId);

                case "pending":

                    return await GetGlobalJobRequestByStatus(staffId, (short)RequestStatusEnum.Pending);
                case "in-progress":
                    var allGlobalJobRequest = await GetAllGlobalJobRequest(staffId, true);
                    return allGlobalJobRequest.Where(x => x.requestStatusId == (short)RequestStatusEnum.Processing).OrderByDescending(x => x.jobRequestId);

                case "cancelled":
                    allGlobalJobRequest = await GetGlobalJobRequestByStatus(staffId, (short)RequestStatusEnum.Cancel);
                    return allGlobalJobRequest.OrderByDescending(x => x.jobRequestId);

                case "disapproved":
                    allGlobalJobRequest = await GetGlobalJobRequestByStatus(staffId, (short)RequestStatusEnum.Disapproved);
                    return allGlobalJobRequest.OrderByDescending(x => x.jobRequestId);

                default:
                    allGlobalJobRequest = await GetAllGlobalJobRequest(staffId);
                    return allGlobalJobRequest.OrderByDescending(x => x.jobRequestId);
            }
        }

        public async Task<IEnumerable<JobRequestViewModel>> GetAllGlobalJobRequestByFacilityRef(string facilityRef)
        {
            var application = await context.TBL_LOAN_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.TBL_LOAN_APPLICATION.APPLICATIONREFERENCENUMBER == facilityRef);

            var targetId = application != null ? application.LOANAPPLICATIONDETAILID : 0;
            var data = await (from x in context.TBL_JOB_REQUEST
                              join s in context.TBL_JOB_TYPE_SUB on x.JOB_SUB_TYPEID equals s.JOB_SUB_TYPEID
                              join t in context.TBL_JOB_TYPE on x.JOBTYPEID equals t.JOBTYPEID
            ...
            //requests.Add(data);
            data = await setCustomerName(data);

            data = await setJobRequestOtherDetails(data);

            data = await PadModelWithLMSReference(data);

            return data;
        }

        private async Task<List<int>> SearchFacilityApplicationEntries(string searchString)
        {
            List<int> applicationDetailsIds = new List<int>();
            var applicationSearchResult = await context.TBL_LOAN_APPLICATION.Where(x => x.APPLICATIONREFERENCENUMBER.Contains(searchString)).ToListAsync();
        ...
        private async Task<List<int>> SearchBookedFacilityEntries(string searchString)
        {
            ...
            var revolvingSearchResult = await context.TBL_LOAN_REVOLVING.Where(x => x.LOANREFERENCENUMBER.Contains(searchString)).ToListAsync();
        ...
        private async Task<List<int>> SearchCustomerFacilityApplication(string searchString)
        {
            ...
            List<int> loanIds = new List<int>();
            var loanSearchResult = await context.TBL_LOAN.Where(x => customerSearchResult.Select(c => c.CUSTOMERID).Contains(x.CUSTOMERID)).ToListAsync();
            loanIds.AddRange(loanSearchResult.Select(x => x.TERMLOANID));
            ...
            var contingentSearchResult = await context.TBL_LOAN_CONTINGENT.Where(x => customerSearchResult.Select(c => c.CUSTOMERID).Contains(x.CUSTOMERID)).ToListAsync();
            loanIds.AddRange(contingentSearchResult.Select(x => x.CONTINGENTLOANID));
        ...
        private async Task<List<int>> SearchCustomerLoanFacilities(string searchString)
        {
            List<int> customerIds = new List<int>();
            ...
            var applicationSearchResult = await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => customerSearchResult.Select(c => c.CUSTOMERID).Contains(x.CUSTOMERID)).ToListAsync();
            customerFacilityIds.AddRange(applicationSearchResult.Select(x => x.LOANAPPLICATIONDETAILID));

            return customerFacilityIds;
        }

        private async Task<List<JobRequestViewModel>> PadModelWithLMSReference(List<JobRequestViewModel> models)
        {
                ...
                if (model.applicationReferenceNumber == null)
                {
                    ...
                    if (model.jobSourceId == (short)JobSourcesEnum.LoanApplicationDetail)
                    {
                        ...
                        if (losApplication != null)
                        {
                            ...
                            var customer = await context.TBL_CUSTOMER.FindAsync(losApplication.CUSTOMERID);
                            if (customer != null)
                            {
                    ...
                    {
                        var lmsoperation = await context.TBL_LOAN_REVIEW_OPERATION.FindAsync(model.targetId);

                        if (lmsoperation != null && lmsoperation.LOANSYSTEMTYPEID == (short)LoanSystemTypeEnum.TermDisbursedFacility)
                        {
                            var loan = await context.TBL_LOAN.FindAsync(lmsoperation.LOANID);
                            if (loan != null)
                            {
                                ...
                                var customer = await context.TBL_CUSTOMER.FindAsync(loan.CUSTOMERID);
                                if (customer != null)
                                {
                                    model.customerName = customer.FIRSTNAME;
                                    model.customerId = customer.CUSTOMERID;
                        ...
                        {
                            var loan = await context.TBL_LOAN_REVOLVING.FindAsync(lmsoperation.LOANID);
                            if (loan != null)
                            {
                                model.refNo = loan.LOANREFERENCENUMBER;
                                var appDetail = loan.TBL_LOAN_APPLICATION_DETAIL;
                        ...
                        {
                            var loan = await context.TBL_LOAN_CONTINGENT.FindAsync(lmsoperation.LOANID);
                            if (loan != null)
                            {
                                model.refNo = loan.LOANREFERENCENUMBER;
                                var appDetail = loan.TBL_LOAN_APPLICATION_DETAIL;
                        ...
                        {
                            var applicationRequests = await this.context.TBL_LOAN_APPLICATION_DETAIL.FindAsync(lmsoperation.LOANID);
                            if (applicationRequests != null)
                            {
                                ...
                                var customer = await context.TBL_CUSTOMER.FindAsync(applicationRequests.CUSTOMERID);
                                if (customer != null)
                                {
                                    model.customerName = customer.FIRSTNAME;
                                    model.customerId = customer.CUSTOMERID;
                    ...
                    if (model.jobSourceId == (short)JobSourcesEnum.LMSApplication)
                    {
                        ...
                        var customer = await context.TBL_CUSTOMER.FindAsync(lmsoperation?.CUSTOMERID);
                        if (customer != null) model.customerName = customer.FIRSTNAME;
        ...
        public async Task<jobRequestCountViewModel> GetJobRequestStatusCount(int staffId, int branchId)
        {
            var jobRequests = await GetAllGlobalJobRequest(staffId, false);

            var pendingCount = jobRequests.Where(x => x.requestStatusId == (short)JobRequestStatusEnum.pending).Count();
            var finishedCount = jobRequests.Where(x => x.responseComment != null).Count();
            var inProgresCount = jobRequests.Where(x => x.requestStatusId == (short)JobRequestStatusEnum.processing).Count();
            var cancelledCount = jobRequests.Where(x => x.requestStatusId == (short)JobRequestStatusEnum.cancel).Count();
            var assignedCount = jobRequests.Where(x => x.receiverStaffId != null || x.reassignedTo != null).Count();
            var unAssignedCount = jobRequests.Where(x => x.reassignedTo == null && x.receiverStaffId == null).Count();

            return new jobRequestCountViewModel()
            {
                pendingCount = pendingCount,
                finishedCount = finishedCount,
                inProgresCount = inProgresCount,
        ...
        public async Task<List<JobRequestDetailViewModel>> GetJobRequestDetailsById(int jobRequestId)
        {
            var jbDetails = await GetJobRequestDetails();
            return jbDetails.Where(x => x.jobRequestId == jobRequestId).ToList();
        }

        private async Task<List<JobRequestDetailViewModel>> GetJobRequestDetails()
        {
            var details = await (from x in this.context.TBL_JOB_REQUEST_DETAIL
        ...
        public async Task<List<JobRequestDetailViewModel>> GetLegalJobRequestDetails()
        {
            ...
            {
                var jobDetail = await context.TBL_JOB_REQUEST_DETAIL.Where(x => x.JOBREQUESTID == i.JOBREQUESTID && x.ACCREDITEDCONSULTANTPAID == false && x.TRANSACTIONREVERSED != true && x.JOB_SUB_TYPEID == x.TBL_JOB_TYPE_SUB_CLASS.JOB_SUB_TYPEID).ToListAsync();
                ...
                foreach (var item in jobDetail)
                {
                    ...
                    var a = await context.TBL_LOAN_APPLICATION_DETAIL.Where(z => z.LOANAPPLICATIONDETAILID == i.TARGETID).ToListAsync();
        ...
        public async Task<List<JobRequestViewModel>> GetJobRequestLegalJobDetail()
        {
            var details = await (from x in this.context.TBL_JOB_REQUEST
        ...
        public async Task<List<JobRequestViewModel>> GetApplicationJobRequest(int targetId, int operationId, short jobSourceId)
        {
            ...
            {
                var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
                requests.AddRange(applicationRequests);
            }

            if (jobSourceId == (short)JobSourcesEnum.LoanBookingAndApproval)
            {
                var loan = await context.TBL_LOAN.FindAsync(targetId);
                var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.LoanBookingAndApproval).ToListAsync();
                var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
            ...
            {
                var loan = await context.TBL_LOAN_CONTINGENT.FindAsync(targetId);
                var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.ContingentLiabilityBookingAndApproval).ToListAsync();
                var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
            ...
            {
                var loan = await context.TBL_LOAN_REVOLVING.FindAsync(targetId);
                var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.OverdraftBookingAndApproval).ToListAsync();
                var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
            ...
            if (jobSourceId == (short)JobSourcesEnum.LMSOperationAndApproval)
            {
                ...
                {
                    var lmsoperation = await context.TBL_LOAN_REVIEW_OPERATION.FindAsync(targetId);
                    var lmsoperationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.JOBSOURCEID == (short)JobSourcesEnum.LMSOperationAndApproval).ToListAsync();

                    if (lmsoperation.LOANSYSTEMTYPEID == (short)LoanSystemTypeEnum.TermDisbursedFacility)
                    {
                        var loan = await context.TBL_LOAN.FindAsync(lmsoperation.LOANID);
                        var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.TERMLOANID && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.LoanBookingAndApproval).ToListAsync();
                        var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
                        requests.AddRange(disbursementRequests);
                        requests.AddRange(applicationRequests);
                    }
                    ...
                    {
                        var loan = await context.TBL_LOAN_REVOLVING.FindAsync(lmsoperation.LOANID);
                        var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.REVOLVINGLOANID && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.LoanBookingAndApproval).ToListAsync();
                        var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
                        requests.AddRange(disbursementRequests);
                        requests.AddRange(applicationRequests);
                    }
                    ...
                    if (lmsoperation.LOANSYSTEMTYPEID == (short)LoanSystemTypeEnum.ContingentLiability)
                    {
                        ...
                        var disbursementRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.CONTINGENTLOANID && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.LoanBookingAndApproval).ToListAsync();
                        var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == loan.LOANAPPLICATIONDETAILID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
                        requests.AddRange(disbursementRequests);
                        requests.AddRange(applicationRequests);
                    }
                    ...
                    {
                        var applicationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == lmsoperation.LOANID && x.JOBSOURCEID == (short)JobSourcesEnum.LoanApplicationDetail).ToListAsync();
                        requests.AddRange(applicationRequests);
                    }
            ...
            if (jobSourceId == (short)JobSourcesEnum.LMSApplication)
            {
                if (operationId == (short)OperationsEnum.TermLoanBooking)
                {
                    var lmsoperation = await context.TBL_LMSR_APPLICATION_DETAIL.FindAsync(targetId);
                    var lmsoperationRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.JOBSOURCEID == (short)JobSourcesEnum.LMSApplication).ToListAsync();
            ...
            {
                var collateralReleaseJobRequests = await this.context.TBL_JOB_REQUEST.Where(x => x.TARGETID == targetId && x.OPERATIONSID == operationId && x.JOBSOURCEID == (short)JobSourcesEnum.CollateralReleaseApproval).ToListAsync();
            ...
            foreach (var request in requests)
            {
                ...
                var jobDocumentsList = await GetJobRequestDocuments(request.JOBREQUESTCODE);
                TBL_JOB_REQUEST_STATUS_FEEDBAK feedback;

                feedback = await context.TBL_JOB_REQUEST_STATUS_FEEDBAK.Where(c => c.JOB_STATUS_FEEDBACKID == request.JOB_STATUS_FEEDBACKID).FirstOrDefaultAsync();
                var requestModel = new JobRequestViewModel
                {
                    ...
                    msgExchangeTrail = await (from y in context.TBL_JOB_REQUEST_MESSAGE
                ...
                var fromData = await context.TBL_STAFF.Where(b => b.STAFFID == request.SENDERSTAFFID).FirstOrDefaultAsync();
                requestModel.fromSender = fromData != null ? fromData.FIRSTNAME + " " + fromData.MIDDLENAME + " " + fromData.LASTNAME : "n/a";

                var toData = await context.TBL_STAFF.Where(b => b.STAFFID == request.RECEIVERSTAFFID).FirstOrDefaultAsync();
                requestModel.to = toData != null ? toData.FIRSTNAME + " " + toData.MIDDLENAME + " " + toData.LASTNAME : "n/a";

                var asigneeData = await context.TBL_STAFF.Where(b => b.STAFFID == request.REASSIGNEDTO).FirstOrDefaultAsync();
        ...
        public async Task<JobRequestViewModel> GetJobRequest(int jobRequestId)
        {
            var x = await this.context.TBL_JOB_REQUEST.FindAsync(jobRequestId);
            ...
            request = setCustomerName(requests).Result.FirstOrDefault();

            request = setJobRequestOtherDetails(requests).Result.FirstOrDefault();

            request = PadModelWithLMSReference(requests).Result.FirstOrDefault();

            return request;

        }

        public async Task<IEnumerable<JobRequestMessageViewModel>> GetJobComments(int jobRequestId)
        {
            var data = await (from x in context.TBL_JOB_REQUEST_MESSAGE
        ...
        public async Task<IEnumerable<ApprovalStatusViewModel>> GetJobRequestApprovaStatus()
        {
            var data = await (from x in context.TBL_JOB_REQUEST_STATUS
        ...
        public async Task<IEnumerable<JobRequestStatusFeedbackViewModel>> GetJobRequestStatusFeedback(short statusId, short jobTypeId)
        {
            return await this.context.TBL_JOB_REQUEST_STATUS_FEEDBAK.Select(x => new JobRequestStatusFeedbackViewModel
            {
                jobTypeId = x.JOBTYPEID,
        ...
        private bool PlaceChargeOnCustomerForCollateralSearch2(JobRequestCollateralSearchViewModel model)
        {
            ...
            if (model.isInitiation)
            {
                ...
                if (casa == null && !model.debitBusiness)
                    throw new ConditionNotMetException(TranslateHelper.get("Customer account number is not supplied"));
                ...
                if (!model.debitBusiness)
                {
                    ...
                    auditDetail = $"{TranslateHelper.get("Customer account number")} '{casa.PRODUCTACCOUNTNUMBER}' {TranslateHelper.get("debited with collateral search fees")}";
                }
            ...
            else
            {
                ...
                if ( b == null || b == string.Empty || b == " ")
                    throw new ConditionNotMetException(TranslateHelper.get("The solicitor's account number is not found. No account number has been mapped to this solicitor."));
                ...
                model.feeNarration = $"{TranslateHelper.get("Payment to solicitor")}";
                auditDetail = $"{TranslateHelper.get("Solicitor account number")} '{accountNumber}' {TranslateHelper.get("credited for collateral search job with")} '{currency.CURRENCYCODE}{witholdingAmount}'";
            }
            ...
            if (model.isInitiation && model.totalChargeAmount > accountBalance && !model.debitBusiness )
                throw new ConditionNotMetException(TranslateHelper.get("The customer's Account is not funded."));
            ...
            if (model.totalChargeAmount > 0)
            {
                ...
                if(model.isInitiation)
                {
                    if (model.debitBusiness)
                    {
                        ...
                        if (bizAccount == null) throw new ConditionNotMetException(TranslateHelper.get("No Account has been mapped for charges on business"));
                ...
                if (inputTransactions.Count > 0)
                {
                    ...
                    if (model.isInitiation) //When RM apply fee on customer's account
                    {
                        if (consultantRecord.Any())
                        {
                            ...
                            string messageBoby = $"{TranslateHelper.get("Dear")} {solicitor.FIRMNAME}, " +
                                $"<br /><br />{TranslateHelper.get("Kindly conduct a comprehensive search on the title document")}" +
                                $" {TranslateHelper.get("– DEED OF SUB-LEASE REGD AS 22/22/594 IN")} {state.STATENAME}  {TranslateHelper.get("and dated July 15, 1994")} " + //to work on this dynamic line
                                $"{TranslateHelper.get("with a view to determining who is the current holder/owner of the legal estate interest in the property and whether or")} " +
                                $"{TranslateHelper.get("not the title of the said current holder is free from encumbrance(s) and encroachment(s) of any nature and/or")} " +
                                $"{TranslateHelper.get("Government acquisition (please note that for this purpose it is sufficient to state that there is no annotation to")} " +
                                $"{TranslateHelper.get("that effect on the title document/survey plan/land registry file).")}" +
                                $"<br /> <br />" +
                                $"{TranslateHelper.get("We will be much obliged to receive your report of search and certificate of good title (if applicable) for the attention of the undersigned, within 24 hours.")}" +
                                $"<br /> <br />" +
                                $"{TranslateHelper.get("Where the search report is favourable, kindly proceed to carry out a charting  exercise in respect of the survey plan attached to the title document  at the Surveyor General Office to confirm the following:-")}" +
                                $"<ol type= i>" +
                                $"<li>{TranslateHelper.get("That the property is free from acquisition or revocation of any kind</li>")}" +
                                $"<li>{TranslateHelper.get("That  the survey plan is within the right coordinates,")}</li>" +
                                $"<li>{TranslateHelper.get("That  the survey plan is within the right coordinates,")}</li>" +
                                $"<li>{TranslateHelper.get("That the survey plan attached to the title document corresponds/matches with the Surveyor General’s copy and the Assignor’s  Survey (where applicable)")}</li>" +
                                $"<li>{TranslateHelper.get("That the address of the Property stated on the Survey Plan is the same with the address stated on the title document.  (here we may state the actual addresses for clarity)")}</li>" +
                                $"<li> {TranslateHelper.get("That the survey plan is without any other defect that may lead to queries which may stall or delay subsequent perfection exercises")}</li>" +
                                $"<br /> <br />" +
                                $"{TranslateHelper.get("Thank you.")}" +
                                $"<br /> <br /> <br />" +
                                $"{TranslateHelper.get("Please acknowledge receipt of this mail.")}";

                            string alertSubject = $"{TranslateHelper.get("Loan Collateral Search")}";
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public async Task<bool> ReverseChargeOnCustomerForCollateralSearch(JobRequestCollateralSearchViewModel model)
        {
            var jobRequestDetail =await context.TBL_JOB_REQUEST_DETAIL.Where(x => x.JOBREQUESTID == model.jobRequestId && x.JOB_SUB_TYPEID == (short)JobSubTypeEnum.CollateralRelated).ToListAsync();
            var consultantId = jobRequestDetail.FirstOrDefault().ACCREDITEDCONSULTANTID;
            var consultantRecord = context.TBL_ACCREDITEDCONSULTANT.Where(x => x.ACCREDITEDCONSULTANTID == consultantId);
            var twoFADetails = new TwoFactorAutheticationViewModel
            {
           ...
           List<TBL_JOB_REQUEST_DETAIL> jobRequestDetail, TwoFactorAutheticationViewModel twoFADetails, IEnumerable<TBL_ACCREDITEDCONSULTANT> consultantRecord)
        {
            ...
            if (casa == null && !model.debitBusiness) { throw new ConditionNotMetException(TranslateHelper.get("Customer account number missing")); }
            ...
            if (model.totalChargeAmount > 0)
            {
                ...
                if (inputTransactions.Count > 0)
                {
                    ...
                    if (jobRequestData.RESPONSECOMMENT == null) jobRequestData.RESPONSECOMMENT = TranslateHelper.get("This request has been cancelled. The collateral search by solicitor is not confirmed");
                     // Audit Section ---------------------------
                     var auditDetail = !model.debitBusiness ? $"{TranslateHelper.get("Customer account number")} '{casa.PRODUCTACCOUNTNUMBER}' {TranslateHelper.get("credited with collateral search fees")}" : $"{ TranslateHelper.get("Bank account collateral search fees debit reversal")}";
                    var audit = new TBL_AUDIT
                    {
                        AUDITTYPEID = (short)AuditTypeEnum.CollateralSearchJob,
                        STAFFID = model.createdBy,
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public async Task<bool> ChargeCustomerForOnSearchJobs(JobRequestCollateralSearchViewModel model)
        {
            var jobRequestDetail =await context.TBL_JOB_REQUEST_DETAIL.Where(x => x.JOBREQUESTID == model.jobRequestId && x.JOB_SUB_TYPEID == model.jobSubTypeId).ToListAsync();
            var subJobRecord =await context.TBL_JOB_TYPE_SUB.FindAsync(model.jobSubTypeId);
            var consultantId = jobRequestDetail.FirstOrDefault().ACCREDITEDCONSULTANTID;
            var consultantRecord = context.TBL_ACCREDITEDCONSULTANT.Where(x => x.ACCREDITEDCONSULTANTID == consultantId);
            ...
            List<TBL_JOB_REQUEST_DETAIL> jobRequestDetail, TwoFactorAutheticationViewModel twoFADetails, IEnumerable<TBL_ACCREDITEDCONSULTANT> consultantRecord)
        {
            ...
            if (!model.debitBusiness)
            {
                model.casaAccountId = casa.CASAACCOUNTID;
                accountNumber = casa.PRODUCTACCOUNTNUMBER;
                auditDetail =  $"{TranslateHelper.get("Customer account number")} '{casa.PRODUCTACCOUNTNUMBER}' {TranslateHelper.get("debited with")} {subTypeClass.ToLower()} {TranslateHelper.get("fees")}";
            }
            else { auditDetail = $"{TranslateHelper.get("Bank account debited with")} {subTypeClass} {TranslateHelper.get("fees")}"; }
            ...
            List<TBL_JOB_REQUEST_DETAIL> jobRequestDetail, TwoFactorAutheticationViewModel twoFADetails, IEnumerable<TBL_ACCREDITEDCONSULTANT> consultantRecord)
        {
            ...
            auditDetail = $"{TranslateHelper.get("Solicitor account number")} '{accountNumber}' {TranslateHelper.get("credited for")} {subTypeClass.ToLower()} {TranslateHelper.get("job with")} '{currency.CURRENCYCODE}{witholdingAmount}'";
        ...
        public async Task<bool> saveCollateralJobsChargesSpecifiedByLegal(JobRequestCollateralSearchViewModel model)
        {
            var jobRequest =await context.TBL_JOB_REQUEST.FindAsync(model.jobRequestId);
            var baseApplication = await context.TBL_LOAN_APPLICATION_DETAIL.FindAsync(jobRequest.TARGETID);
            var state =await context.TBL_STATE.FindAsync(model.collateralStateId);
            var company =await context.TBL_COMPANY.FindAsync(model.companyId);
            //var casa = context.TBL_CASA.Find(model.casaAccountId);
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("You must add at least one charge."));
            }
            ...
            string messageBoby = $"{TranslateHelper.get("Dear RM,")} <br /><br />{TranslateHelper.get("This is to bring to your attention that legal has specified charges for collateral on job request with code")} '{jobRequest.JOBREQUESTCODE}'. <br /><br /> {TranslateHelper.get("Your attention is required to effect the charges.")} <br /><br />";
            string alertSubject = $"{TranslateHelper.get("Collateral Search Request")}";
        ...
        #region job-type
        public async Task<bool> ConfirmLegalCollateralJobSearch(int jobRequestDetailId, bool payStatus, UserInfo model)
        {
            var data = await this.context.TBL_JOB_REQUEST_DETAIL.FindAsync(jobRequestDetailId);
            if (data == null)
            {
                return false;
            }
        ...
        public async Task<IEnumerable<JobTypeViewModel>> GetAllJobType()
        {
            return await this.context.TBL_JOB_TYPE.Select(x => new JobTypeViewModel
            {
                jobTypeId = x.JOBTYPEID,
                jobTypeName = x.JOBTYPENAME,
                inUse = x.INUSE,
                canBeReasigned = x.CANBEREASSIGNED
            }).Where(c => c.inUse == true).ToListAsync();
        }

        public async Task<IEnumerable<JobTypeHubViewModel>> GetAllJobTypeHub(short jobTypeId)
        {
            return await this.context.TBL_JOB_TYPE_HUB.Select(x => new JobTypeHubViewModel
            {
                jobTypeId = x.JOBTYPEID,
                jobTypeHubId = x.JOBTYPEHUBID,
                jobTypeHubName = x.HUBNAME,
                deleted = x.DELETED
            }).Where(c => c.jobTypeId == jobTypeId && c.deleted == false).ToListAsync();
        }

        public async Task<IEnumerable<HubStaffViewModel>> GetHubStaffByHubId(short jobTypeHubId)
        {
            return await this.context.TBL_JOB_TYPE_HUB_STAFF.Select(x => new HubStaffViewModel
            {
            ...
            }).Where(c => c.jobTypeHubId == jobTypeHubId && c.deleted == false).ToListAsync();
        }

        public async Task<IEnumerable<HubStaffViewModel>> GetHubStaffByHubTypeUnitId(short jobTypeUnitId)
        {
            return await this.context.TBL_JOB_TYPE_HUB_STAFF.Select(x => new HubStaffViewModel
            {
                hubStaffId = x.STAFFID,
                ...
                isTeamLead = x.ISTEAMLEAD,
                deleted = x.DELETED,
            }).Where(c => c.jobTypeHubId == jobTypeUnitId && c.deleted == false).ToListAsync();
        }

        public async Task<IEnumerable<JobTypeUnitViewModel>> GetAllJobTypeUnit(short jobTypeId)
        {
            return await this.context.TBL_JOB_TYPE_UNIT.Select(x => new JobTypeUnitViewModel
            {
                jobTypeId = x.JOBTYPEID,
                jobTypeUnitId = x.JOBTYPEUNITID,
                unitName = x.UNITNAME
            }).Where(c => c.jobTypeId == jobTypeId).ToListAsync();
        }

        public async Task<List<jobReasignment>> GetJobTypeReasignmentAdmin(int companyId)
        {
            var details = await (from x in context.TBL_JOB_TYPE_REASSIGNMENT
        ...
        public async Task<bool> mapJobTypeHubStaff(JobTypeHubViewModel model)
        {
            var applicationDate = general.GetApplicationDate();
            ...
            var staff = await context.TBL_STAFF.FindAsync(model.staffId);
            var hub = await context.TBL_JOB_TYPE_HUB.FindAsync(model.jobTypeHubId);
            var unit = await context.TBL_JOB_TYPE_UNIT.FindAsync(model.jobTypeUnitId);
            // Audit Section ---------------------------
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Staff with staff code")} '{ staff.STAFFCODE }' {TranslateHelper.get("of")} '{unit.UNITNAME}' {TranslateHelper.get("unit was added to")} '{hub.HUBNAME}' {TranslateHelper.get("hub")}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = applicationDate,
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteMappedJobTypeHubStaff(int hubStaffId, int staffId)
        {
            var hubStaffRecord = await context.TBL_JOB_TYPE_HUB_STAFF.FindAsync(hubStaffId);
        ...
        public async Task<bool> UpdatemappedJobTypeHubStaff(JobTypeHubViewModel model)
        {
            var data = await context.TBL_JOB_TYPE_HUB_STAFF.FindAsync(model.hubStaffId);
            if (data == null) throw new ConditionNotMetException(TranslateHelper.get("Could not find record to update"));
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("New update made to")} 'job type hub staff'  {TranslateHelper.get("thus: staff code")} - '{ staff.STAFFCODE }', {TranslateHelper.get("hub")} - '{hub.HUBNAME}', {TranslateHelper.get("unit")} - '{unit.UNITNAME}'.",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AssignJobTypeToStaff(jobReasignment model)
        {
            var applicationDate = general.GetApplicationDate();
            if (context.TBL_JOB_TYPE_REASSIGNMENT.Any(x => x.JOBTYPEID == model.jobTypeId && x.STAFFID == model.staffId))
                throw new ConditionNotMetException(TranslateHelper.get("This job type exist for this staff"));
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Job Type has been assigned to a staff with ID")} '{ model.staffId }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = applicationDate,
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteJobTypeForAStaff(jobReasignment model)
        {
            var data = await context.TBL_JOB_TYPE_REASSIGNMENT.FindAsync(model.reasignmentId);
            if (data == null) throw new ConditionNotMetException(TranslateHelper.get("No record selected."));

            data.DATETIMEDELETED = model.dateTimeDeleted;
            data.DELETED = true;

            var jobType = await context.TBL_JOB_TYPE.FindAsync(data.JOBTYPEID);
            var staff = await context.TBL_STAFF.FindAsync(data.STAFFID);
            var audit = new TBL_AUDIT
            {
                AUDITTYPEID = (short)AuditTypeEnum.StaffJobTypeDeleted,
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Deleted job Type admin with detail: JobType")} - '{jobType.JOBTYPENAME}' {TranslateHelper.get("staff code")} '{ staff.STAFFCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateAsignedJobTypeToStaff(jobReasignment model)
        {
            var data = await context.TBL_JOB_TYPE_REASSIGNMENT.FindAsync(model.reasignmentId);
            if (data != null)
            {
            ...
            var jobType = await context.TBL_JOB_TYPE.FindAsync(model.jobTypeId);
            var staff = await context.TBL_STAFF.FindAsync(model.staffId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"'{jobType?.JOBTYPENAME}' {TranslateHelper.get("staff admin has been modified. New admin staff code :")} '{ staff.STAFFCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<List<jobReasignment>> GetJobReasignmentStaffById(int staffId, int companyId)
        {
            var jobTypeAdminStaff = await GetJobTypeReasignmentAdmin(companyId);
            return jobTypeAdminStaff.Where(x => x.staffId == staffId).ToList(); ;
        }

        public async Task<IEnumerable<JobTypeHubViewModel>> GetJobTypeHubStaff()
        {
            return await this.context.TBL_JOB_TYPE_HUB_STAFF.Where(x => x.DELETED == false).Select(x => new JobTypeHubViewModel
            {
        ...
        public async Task<IEnumerable<JobTypeViewModel>> GetJobSubType(short jobId)
        {
            return await context.TBL_JOB_TYPE_SUB.Select(x => new JobSubTypeViewModel
            {
                jobTypeId = x.JOBTYPEID,
            ...
            }).Where(x => x.jobTypeId == jobId).ToListAsync();
        }

        public async Task<IEnumerable<JobSubTypeClassViewModel>> GetJobSubTypeClass(short jobSubTypeId)
        {
            return await context.TBL_JOB_TYPE_SUB_CLASS.Select(x => new JobSubTypeClassViewModel
            {
                jobSubTypeclassId = x.JOB_SUB_TYPE_CLASSID,
                jobSubTypeclassName = x.JOB_SUB_TYPE_CLASS_NAME,
                jobSubTypeId = x.JOB_SUB_TYPEID,
                defaultChargeAmount = x.DEFAULTCHARGEAMOUNT,
            }).Where(x => x.jobSubTypeId == jobSubTypeId).ToListAsync();
        }
        ...
        public async Task<bool> UpdateInvoiceStatus(JobRequestInvoiceViewModel model)
        {
            var job = await this.context.TBL_JOB_REQUEST.FindAsync(model.jobRequestId);
            var invoice = await this.context.TBL_LOAN_APPLICATION_DETL_INV.Where(x => x.INVOICEID == model.invoiceId).FirstOrDefaultAsync();
            if (invoice != null)
            {
                if (!model.status)
                {
        ...
        public async Task<bool> AddJobReplyAndDocument(RequestDocumentViewModel model, byte[] file)
        {
            if (await AddJobDocumentOnly(model, file))
            {
                ...
                return await ReplyJobRequest(jb, jb.jobRequestId);
            }
            else return false;
        }

        public async Task<bool> AddJobDocumentOnly(RequestDocumentViewModel model, byte[] file)
        {
            string[] chanelArray = new string[] { "docx", "pdf", "jpg", "jpeg", "png", "txt", "xlsx", "xls", "doc", "xml" };
            if (!chanelArray.Contains(model.fileExtension))
            {
                throw new ConditionNotMetException(TranslateHelper.get("Kindly Upload Valid File with Accepted Extention ") + chanelArray);
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added Loan Document")} '{ model.documentTitle }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<string> AddJobDocument(RequestDocumentViewModel model, JobRequestViewModel requestModel, byte[] file)
        {
            string[] chanelArray = new string[] { "docx", "pdf", "jpg", "jpeg", "png", "txt", "xlsx", "xls", "doc", "xml" };
            if (!chanelArray.Contains(model.fileExtension))
            {
            ...
            var code = await AddGlobalJobRequest(requestModel);
            ...
            var aud = await context.SaveChangesAsync() != 0;
        ...
        public async Task<bool> UpdateJobDocument(RequestDocumentViewModel model, int documentId)
        {
            string[] chanelArray = new string[] { "docx", "pdf", "jpg", "jpeg", "png", "txt", "xlsx", "xls", "doc", "xml" };
            if (!chanelArray.Contains(model.fileExtension))
            {
            ...
            var data = await this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.FindAsync(documentId);
            if (data == null)
            {
                return false;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.lastUpdatedBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("LoanDocument")} '{ model.documentTitle }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<RequestDocumentViewModel>> GetAllJobDocument()
        {
            return await this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.Select(x => new RequestDocumentViewModel
            {
        ...
        public async Task<RequestDocumentViewModel> GetJobDocument(int documentId)
        {
            var data = await this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.FindAsync(documentId);
        ...
        public async Task<IEnumerable<LMSOperationListViewModel>> getLMSRApplicationDetail(int targetId)
        {
            var data = await (from x in this.context.TBL_LMSR_APPLICATION_DETAIL
        ...
        public async Task<IEnumerable<LMSOperationListViewModel>> getLMSROperation(int targetId)
        {
            var data = await (from x in this.context.TBL_LOAN_REVIEW_OPERATION
        ...
        public async Task<IEnumerable<LMSOperationListViewModel>> getLOSOperationLoanData(int loanId, int operationId)
        {
            ...
            {
                data = await (from x in this.context.TBL_LOAN
            ...
            {
                data = await (from x in this.context.TBL_LOAN_REVOLVING
            ...
            {
                data = await (from x in this.context.TBL_LOAN_CONTINGENT
        ...
        public async Task<bool> deleteJobDocument(int documentId, int staffId)
        {
            var data = await this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.FindAsync(documentId);
            if (data.CREATEDBY != staffId)
                throw new ConditionNotMetException(TranslateHelper.get("You did not upload the document you are trying to delete."));

            this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.Remove(data);
            return await docContext.SaveChangesAsync() > 0;
        }

        public async Task<IEnumerable<RequestDocumentViewModel>> GetJobRequestDocuments(string jobRequestCode)
        {

            return await (from x in this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT
        ...
        public async Task<IEnumerable<RequestDocumentViewModel>> GetJobRequestDocumentById(int documentId)
        {
            return await this.docContext.TBL_MEDIA_JOB_REQUEST_DOCUMENT.Where(x => x.DOCUMENTID == documentId).Select(x => new RequestDocumentViewModel
            {
                documentId = x.DOCUMENTID,
                jobRequestCode = x.JOBREQUESTCODE,
                documentTitle = x.DOCUMENTTITLE,
                ...
                foreach (var post in postingGroups)
                {
                    ...
                    foreach (var debits in feeDetails.Where(a => a.POSTINGTYPEID == (int)GLPostingTypeEnum.Debit))
                    {
                        ...
                        if (!model.debitBusiness && context.TBL_PRODUCT.FirstOrDefault(x => x.PRODUCTID == casa.PRODUCTID).PRINCIPALBALANCEGL == null)
                            throw new BadLogicException($"{TranslateHelper.get("No GL is currently mapped to this product code")} '{casa.TBL_PRODUCT.PRODUCTCODE}'.");
                    ...
                    foreach (var credits in feeDetails.Where(a => a.POSTINGTYPEID == (int)GLPostingTypeEnum.Credit))
                    {
                        ...
                        {
                            throw new ConditionNotMetException(TranslateHelper.get("There is no GL defined for ")+ credits.DESCRIPTION + TranslateHelper.get(" record(")+ credits.CHARGEFEEDETAILID+").");
                        }
        ...
        public async Task<bool> UpdateJobType(JobTypeViewModel model, short jobTypeId)
        {
            var data = await this.context.TBL_JOB_TYPE.FindAsync(jobTypeId);
            if (data == null)
            {
                return false;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.lastUpdatedBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated JobType")} '{ model.jobTypeName }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddJobType(JobTypeViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added JobType")} '{ model.jobTypeName }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<LookupViewModel>> GetJobRequestStatus()
        {
            var status = await (from rs in context.TBL_JOB_REQUEST_STATUS
        ...
        public async Task<IEnumerable<JobRequestStatusFeedbackViewModel>> GetAllJobRequestStatusFeedback()
        {
            var feedback = await (from x in context.TBL_JOB_REQUEST_STATUS_FEEDBAK
        ...
        public async Task<bool> AddUpdateJobRequestFeedBack(JobRequestStatusFeedbackViewModel feedback)
        {
            ...
            try
            {
                ...
                {
                    jobFeedback = await context.TBL_JOB_REQUEST_STATUS_FEEDBAK.FindAsync(feedback.jobStatusFeedbackId);
                    if (jobFeedback != null)
                    {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Added/Updated new job request feedback:")} {feedback.jobStatusFeedbackName}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> ValidateJobRequestFeedBack(string feedback)
        {
            var isExist = await (from a in context.TBL_JOB_REQUEST_STATUS_FEEDBAK
