...
namespace FintrakBanking.Repositories.Setups.Credit
{
    ...
    public class BulkDisbursementPackageRepository : IBulkDisbursementPackageRepository
    {
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> GetAllBulkDisburseSchemeByApplicationReferenceNumber(string applicationReferenceNumber)
        {

            var data = await (from a in context.TBL_LOAN_BULK_DISBURSE_SCHEME
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> GetAllBulkDisbursementSchemeByProductId(int productId)
        {
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> GetAllBulkDisbursementSchemeByDisburseSchemeId(int disburseSchemeId)
        {
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> GetAllBulkDisbursementScheme()
        {
            var data =await  (from a in context.TBL_LOAN_BULK_DISBURSE_SCHEME
        ...
        public async Task<bool> AddBulkDisbursementScheme(BulkDisbursementSetupSchemeViewModel model)
        {
            var schedulTypeData = await context.TBL_LOAN_SCHEDULE_TYPE.Where(s => s.SCHEDULETYPEID == model.scheduleMethodId).FirstOrDefaultAsync();
            if (schedulTypeData == null) {
                throw new ConditionNotMetException(TranslateHelper.get("The LOANAPPLICATIONDETAILID does not exist"));
            }

            var productData = context.TBL_PRODUCT.Where(p => p.PRODUCTID == model.productId);
            if (productData == null) {
                throw new ConditionNotMetException(TranslateHelper.get("The PRODUCTID does not exist"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Bulk Disbursement Scheme for")}   ",
        ...
        public async Task<bool> AddMultipleBulkDisbursementScheme(List<BulkDisbursementSetupSchemeViewModel> models)
        {
            if (models.Count <= 0)
                return false;
            ...
            {
                await AddBulkDisbursementScheme(model);
            }
            return true;
        }

        public async Task<bool> UpdateBulkDisbursementScheme(int disbursementSchemeId, BulkDisbursementSetupSchemeViewModel model)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("The PRODUCTPRICEINDEXID does not exist"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Bulk Disbursement scheme with code")} '{model.createdBy}' ",
                IPADDRESS = model.userIPAddress,
                URL = model.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now
        ...
        public async Task<bool> DeleteBulkDisbursementScheme(int disbursementPackageId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Deleted Bulk Disbursement Scheme with id")} '{data.DISBURSESCHEMEID}'",
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> SearchLoanApplicationDetails(int companyId, string searchQuery)
        {
            searchQuery = searchQuery?.Trim()?.ToLower();

            var allApplicationDetails =await (from d in context.TBL_LOAN_APPLICATION_DETAIL
        ...
        public async Task<IEnumerable<BulkDisbursementSetupSchemeViewModel>> SchemeSearch(string searchString)
        {
            var search = searchString.Trim().ToLower();

            var applications = await (from a in context.TBL_LOAN_BULK_DISBURSE_SCHEME
                                join x in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONDETAILID equals x.LOANAPPLICATIONDETAILID
                                join b in context.TBL_LOAN_APPLICATION on x.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
                                join c in context.TBL_CUSTOMER on x.CUSTOMERID equals c.CUSTOMERID
