...
namespace FintrakBanking.Repositories.Setups.Credit
{
    public class FXAccountCreationRepository : IFXAccountCreationRepository
    {
        ...
        public async Task<List<FXAccountCreationViewModel>> GetAllGLSubHead(string schemeCode)
        {
            var freecode1 = await (from a in context.STG_GL_SUBHEAD_TBL
            ...
            try
            {
                ...
                if (USE_THIRD_PARTY_INTEGRATION)
                {
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        DETAIL = $"{TranslateHelper.get("Added New Foreign Loan Account for")} {accountInfo.customerName} {TranslateHelper.get("with account number")} {accountInfo.accountNumber} ",
