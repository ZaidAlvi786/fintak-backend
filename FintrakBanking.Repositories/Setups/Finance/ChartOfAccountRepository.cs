    ...
    public class ChartOfAccountRepository : IChartOfAccountRepository
    {
        ...
        public GLAccountDetailsViewModel ValidateGLNumber(string glNumber)
        {
            ...
            if (data  != null)
                ...
                throw new SecureException(TranslateHelper.get("Office Account Does Not Exist in Finacle"));
        }
        ...
        private bool ApproveAccount(int accountId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved Account")} '{accountModel.ACCOUNTNAME}' {TranslateHelper.get("with staff code")} '{accountModel.ACCOUNTCODE}'",
        ...
        private int AddAccount2(ChartOfAccountViewModel account)
        {
            ...
            if (account.currencies.Count < 1)
                throw new SecureException(TranslateHelper.get("Chart of Account Currency must be specified"));
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added New Account:")} {account.accountName} {TranslateHelper.get("with code:")} {account.accountCode}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> AddTempAccount(ChartOfAccountViewModel accountModel)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("Account Information already exist and is undergoing approval"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Initiated Chart of Account Creation for")} '{accountModel.accountName}' {TranslateHelper.get("with code")} '{accountModel.accountCode}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> IsAccountCodeAlreadyExist(string accountCode)
        {
            return context.TBL_CHART_OF_ACCOUNT.Any(x => x.ACCOUNTCODE.ToLower() == accountCode.ToLower());
        }

        public async Task<bool> IsTempAccountExist(string accountCode)
        {
        ...
        public async Task<IEnumerable<ChartOfAccountViewModel>> GetAllAccounts()
        {
        ...
        public async Task<string> GetAccountNameByAccountCode(string accountCode)
        {
            return await (from c in context.TBL_CHART_OF_ACCOUNT
                    where c.ACCOUNTCODE.Trim() == accountCode.Trim()
                    select c.ACCOUNTNAME).FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<ChartOfAccountViewModel>> GetAccountsByCategory(short accountCategoryId)
        {
            var data = await GetAllAccountsDetails().Where(x => x.accountCategoryId == accountCategoryId).ToListAsync();

            return data;
        }

        public async Task<ChartOfAccountViewModel> GetAccountByAccountId(short accountId)
        {
        ...
        public async Task<int> GetAccountDefaultCurrency(int glAccountId, int companyId)
        {
            int output;
            var setup = await context.TBL_SETUP_GLOBAL.FirstOrDefaultAsync();
            ...
            {
                 output = await (from gl in context.TBL_CUSTOM_CHART_OF_ACCOUNT
                                   ...
                                   select cur.CURRENCYID).FirstOrDefaultAsync();
            }
        ...
        private bool UpdateAccount2(short accountId, ChartOfAccountViewModel account)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = (int)account.createdBy,
                BRANCHID = (short)account.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated New Account:")} {accountModel.ACCOUNTNAME} {TranslateHelper.get("with code:")} {accountModel.ACCOUNTCODE}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = account.applicationUrl,
                SYSTEMDATETIME = DateTime.Now,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
        ...
        public async Task<bool> UpdateAccount(short accountId, ChartOfAccountViewModel accountModel)
        {
            if (accountModel == null)
                return false;
            ...
            {
                throw new SecureException(TranslateHelper.get("Chart of Account is already undergoing approval"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = accountModel.createdBy,
                BRANCHID = (short)accountModel.userBranchId,
                DETAIL = $"{TranslateHelper.get("Initiated updated of Chart Of Account")} '{accountModel.accountName}' {TranslateHelper.get("with code")} '{accountModel.accountCode}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = accountModel.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<ChartOfAccountViewModel>> GetAccountsAwaitingApprovals(int staffId, int companyId)
        {
        ...
        public async Task<bool> DeleteAccount(short accountId, UserInfo user)
        {
            var accountModel = await this.context.TBL_CHART_OF_ACCOUNT.FindAsync(accountId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Deleted New Account:")} {accountModel.ACCOUNTNAME} {TranslateHelper.get("with code:")} {accountModel.ACCOUNTCODE}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                SYSTEMDATETIME = DateTime.Now,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
        ...
        public async Task<IEnumerable<LookupViewModel>> GetFinancialSatementCaptionLookup()
        {
        ...
        public async Task<IEnumerable<ChartOfAccountClassViewModel>> GetChartOfAccountClasses()
        {
