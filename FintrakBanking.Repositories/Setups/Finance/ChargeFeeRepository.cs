...
namespace FintrakBanking.Repositories.Setups.Finance
{
    public class ChargeFeeRepository : IChargeFeeRepository
    {
        ...
        public bool AddTempChargeFee(ChargeFeeViewModel chargeFeemodel)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("Charge Fee Information already exist and is undergoing approval"));
            }
        ...
        public async Task<bool> AddChargeFee(ChargeFeeViewModel model)
        {
            if (model != null)
            {
                try
                {
                    ...
                    {
                        temChargeFee = await context.TBL_TEMP_CHARGE_FEE.FindAsync(model.chargeFeeId);
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        DETAIL = $"{TranslateHelper.get("Added Charge Fee")} '{temChargeFee.CHARGEFEENAME }' ",
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    ...
                    using (var trans = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            workFlow.Comment = TranslateHelper.get("Charge Fee Creation");
        ...
        public async Task<bool> UpdateChargeFee(ChargeFeeViewModel model, int chargeFeeId)
        {
            var data = await this.context.TBL_CHARGE_FEE.FindAsync(chargeFeeId);
            if (data == null)
            {
                return false;
            }
            ...
            var chargeFeeData = this.context.TBL_CHARGE_FEE_DETAIL.Find(model.chargeFeeDetailId);
            if (chargeFeeData != null)
            {
                foreach (var item in model.chargeFeeDetails)
                {
                    if (item.feeTypeId > 0)
                    {
                        chargeFeeData.DESCRIPTION = item.description;
                        chargeFeeData.GLACCOUNTID1 = item.glAccountId1;
                        chargeFeeData.GLACCOUNTID2 = item.glAccountId2;
                        chargeFeeData.DETAILTYPEID = item.detailTypeId;
                        chargeFeeData.POSTINGTYPEID = item.postingTypeId;
                        chargeFeeData.VALUE = item.rate;
                        chargeFeeData.FEETYPEID = item.feeTypeId;
                        chargeFeeData.REQUIREAMORTISATION = item.requireAmortization;
                        chargeFeeData.POSTINGGROUP = item.postingGroup;
                        chargeFeeData.CREATEDBY = model.createdBy;
                        chargeFeeData.DATETIMECREATED = DateTime.Now;
                        chargeFeeData.DELETED = false;
                        chargeFeeData.CHARGEFEEID = item.chargeFeeId;
                    }
                }
            }
            else
            {
                foreach (var item in model.chargeFeeDetails)
                {
                    context.TBL_CHARGE_FEE_DETAIL.Add(new TBL_CHARGE_FEE_DETAIL
                    {
                        DESCRIPTION = item.description,
                        GLACCOUNTID1 = item.glAccountId1,
                        GLACCOUNTID2 = item.glAccountId2,
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated ChargeFee")} '{ data.CHARGEFEENAME }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<ChargeFeeViewModel>> GetChargeFeeAwaitingApprovals(int staffId, int companyId)
        {
            //var ids = general.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.Fee_chargeChange).ToList();

            //var result =  await (from a in context.TBL_LOAN_REVIEW_OPERATION
            ...
            var result = await context.TBL_TEMP_CHARGE_FEE.Select(a => new ChargeFeeViewModel
            {
                chargeName = a.CHARGEFEENAME,
                operationId = a.OPERATIONID.HasValue ? a.OPERATIONID : 0,
                amount = a.AMOUNT,
                rate = a.RATE,
                tempChargeFeeId = a.TEMPCHARGEFEEID,
                chargeFeeId = a.CHARGEFEEID.HasValue ? a.CHARGEFEEID.Value : 0
            }).ToListAsync();
        ...
        public async Task<IEnumerable<ChargeFeeViewModel>> GetChargeFeeAwaitingAdminApprovals(int staffId, int companyId)
        {
            var ids = general.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.Fee_chargeChange).ToList();
        ...
        private bool ApproveChargeFee(int loanChargeFeeId, decimal feeAmount, double feeRate, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = TranslateHelper.get("Approved Charge Fee"),
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        private bool ApproveNewChargeFee(int loanChargeFeeId, decimal feeAmount, double feeRate, short approvalStatusId, UserInfo user, int targetId)
        {
            var tempCharge = (from a in context.TBL_TEMP_CHARGE_FEE where a.TEMPCHARGEFEEID == targetId select a).FirstOrDefault();
            var tempChargeDetails = (from a in context.TBL_TEMP_CHARGE_FEE_DETAIL where a.TEMPCHARGEFEEID == targetId select a).ToList();
            ...
            if (tempCharge != null)
            {
                    ...
                    var newTempCharge = new TBL_CHARGE_FEE()
                    {
                        CHARGEFEENAME = tempCharge.CHARGEFEENAME,
                        SHORTNAME = tempCharge.CHARGEFEENAME,
                        FEEINTERVALID = tempCharge.FEEINTERVALID,
                        PRODUCTTYPEID = tempCharge.PRODUCTTYPEID,
                        FEETARGETID = tempCharge.FEETARGETID,
                        FEEAMORTISATIONTYPEID = tempCharge.FEEAMORTISATIONTYPEID,
                        ...
                        RECURRING = tempCharge.RECURRING,
                        COMPANYID = tempCharge.COMPANYID,
                        CREATEDBY = tempCharge.CREATEDBY,
                        LASTUPDATEDBY = tempCharge.LASTUPDATEDBY,
                        DATETIMECREATED = tempCharge.DATETIMECREATED,
                        DATETIMEUPDATED = tempCharge.DATETIMEUPDATED,
                        DELETED = tempCharge.DELETED,
                        DELETEDBY = tempCharge.DELETEDBY,
                        DATETIMEDELETED = tempCharge.DATETIMEDELETED,
                        CRMSREGULATORYID = tempCharge.CRMSREGULATORYID
                    };
                    context.TBL_CHARGE_FEE.Add(newTempCharge);
             }
            ...
            if (tempChargeDetails != null)
            {
                foreach (var item in tempChargeDetails)
                {
                    ...
                    context.TBL_CHARGE_FEE_DETAIL.Add(newDetails);
                }
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Newly Created Charge Fee"),
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        private bool RejectChargeFee(int loanChargeFeeId, decimal feeAmount, double feeRate, short approvalStatusId, UserInfo user, int targetId)
        {
            var tempCharge = (from a in context.TBL_TEMP_CHARGE_FEE where a.TEMPCHARGEFEEID == targetId select a).FirstOrDefault();
            var tempChargeDetails = (from a in context.TBL_TEMP_CHARGE_FEE_DETAIL where a.TEMPCHARGEFEEID == targetId select a).ToList();
            ...
            {
              tempCharge.ISCURRENT = false;
              tempCharge.APPROVALSTATUSID = (short)ApprovalStatusEnum.Disapproved;
              context.SaveChanges();
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Rejected Newly Created Charge Fee"),
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public bool GoForApproval(ApprovalViewModel entity)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        trans.Rollback();
                        throw new SecureException(TranslateHelper.get("Approval Failed"));
                    }
        ...
        public bool GoForFeeApproval(ApprovalViewModel entity)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var tempCharge = (from a in context.TBL_TEMP_CHARGE_FEE where a.TEMPCHARGEFEEID == entity.targetId select a).FirstOrDefault();
                        var response = RejectChargeFee(entity.loanChargeFeeId, entity.feeAmount, entity.feeRate, (short)workFlow.StatusId, entity, entity.targetId);
                    }
                    else
                    {
                        workFlow.StaffId = entity.staffId;
                        ...
                        if (workFlow.NewState == (int)ApprovalState.Ended)
                        {
                            var response = ApproveNewChargeFee(entity.loanChargeFeeId, entity.feeAmount, entity.feeRate, (short)workFlow.StatusId, entity, entity.targetId);
        ...
        public async Task<ChargeFeeViewModel> GetChargeFee(int chargeProductFeeId)
        {
            var data = await this.context.TBL_CHARGE_FEE.FindAsync(chargeProductFeeId);
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllChargeFeeDetailClass()
        {
        ...
        public async Task<IEnumerable<ChargeFeeViewModel>> GetAllChargeFeeByCompanyId(int companyId)
        {
            return  this.GetAllChargeFee().Where(x => x.companyId == companyId);
        }

        public async Task<bool> DeleteChargeFee(int chargeProductFeeId, UserInfo user)
        {
            var data = await this.context.TBL_CHARGE_FEE.FindAsync(chargeProductFeeId);
            if (data == null)
            {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Deleted ChargeFee")} '{ data.CHARGEFEENAME }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
