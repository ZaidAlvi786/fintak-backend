...
namespace FintrakBanking.Repositories.Setups.Approval
{
    public class ApprovalLevelRepository : IApprovalLevelRepository
    {
        ...
        private IEnumerable<ApprovalLevelViewModel> GetApprovalLevel(int companyId)
        {
            var data = (from x in this.context.TBL_APPROVAL_LEVEL
                        ...
                        select new ApprovalLevelViewModel
                        {
                            ...
                            renewalLimit = x.RENEWALLIMITAMOUNT,
                            feeRate = x.FEERATE,
                            ...
                            isExternalSystemApprover = x.ISEXTERNALSYSTEMAPPROVER,
        ...
        private IEnumerable<ApprovalLevelViewModel> GetAllDetailedApprovalLevel(int companyId)
        {
            var data = (from a in context.TBL_APPROVAL_LEVEL
                        ...
                        select new ApprovalLevelViewModel
                        {
                            ...
                            renewalLimit = a.RENEWALLIMITAMOUNT,
                            feeRate = a.FEERATE,
                            ...
                            isExternalSystemApprover = a.ISEXTERNALSYSTEMAPPROVER,
        ...
        public async Task< IEnumerable<ApprovalLevelViewModel>> GetAllApprovalLevel(int companyId)
        {
            return GetApprovalLevel(companyId);
        }

        public async Task <IEnumerable<ApprovalLevelViewModel>> GetAllApprovalLevelDetails(int companyId)
        {
            return GetAllDetailedApprovalLevel(companyId);
        }

        public async Task< IEnumerable<ApprovalLevelViewModel>> GetApprovalLevelById(int ApprovalLevelId, int companyId)
        {
            return GetApprovalLevel(companyId).Where(c => c.approvalLevelId == ApprovalLevelId);
        }

        public async Task<IEnumerable<ApprovalLevelViewModel>> GetApprovalLevelByGroupId(int groupId, int companyId)
        { 
            return GetApprovalLevel(companyId).Where(c => c.groupId == groupId);
        }

        public async Task<IEnumerable<ApprovalLevelViewModel>> GetApprovalLevelByOperationId(int operationId, int companyId)
        {
        ...
        public async Task<List<FintrakDropDownSelectList>> GetApprovalLevelsByOperationIdAndProductClassId(int operationId, int? classId)
        {
            var levels = await context.TBL_APPROVAL_GROUP_MAPPING.Where(x => x.OPERATIONID == operationId && x.PRODUCTCLASSID == classId)
        ...
        public async Task<List<FintrakDropDownSelectList>> GetTranchDisbursmentApprovalLevels()
        {
            ...
            var levels = await context.TBL_APPROVAL_GROUP_MAPPING.Where(x => x.OPERATIONID == operationId && x.PRODUCTCLASSID == null)
                .Join(context.TBL_APPROVAL_GROUP, m => m.GROUPID, g => g.GROUPID, (m, g) => new { m, g })
        ...
        public bool AddApprovalLevel(ApprovalLevelViewModel model)
        {
            if (admin.IsSuperAdmin(model.createdBy) == true)
            {
                var data = new TBL_APPROVAL_LEVEL
                {
                    ...
                    RENEWALLIMITAMOUNT = model.renewalLimit,
                    ...
                    ISEXTERNALSYSTEMAPPROVER = model.isExternalSystemApprover,
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("New approval Level")} '{ model.levelName }' {TranslateHelper.get("created by this super-admin")} {audit_staff}",
            ...
            else
            {
                ...
                workflow.Comment = $"{TranslateHelper.get("New approval Level")} '{ model.levelName }' {TranslateHelper.get("request for approval initiated")}";
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("New approval Level")} '{ model.levelName }' {TranslateHelper.get("request for approval initiated")} ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public bool UpdateApprovalLevel(int approvalLevelId, ApprovalLevelViewModel model)
        {
            ...
            var staffRoleCode = this.context.TBL_STAFF_ROLE.Where(r => r.STAFFROLEID == model.roleId).FirstOrDefault()?.STAFFROLECODE;
            if (staffRoleCode != null && model.roleCode != null)
            {
                var remoteUsers = this.context.TBL_STAFF_REMOTE_NT_IDS.Where(s => s.STAFFROLECODE == staffRoleCode).ToList();
                foreach (var staff in remoteUsers)
                {
                    staff.STAFFROLECODE = model.roleCode;
                }
            }
            ...
            if (admin.IsSuperAdmin(model.createdBy) == true)
            {
                ...
                data.RENEWALLIMITAMOUNT = model.renewalLimit;
                ...
                data.ISEXTERNALSYSTEMAPPROVER = model.isExternalSystemApprover;
                data.SUBSIDIARYID = model.subsidiaryId;
                data.USERROLEID = model.userRoleId;

                //Remote Approver Update Section---------------
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated Approval Level")} '{model.levelName}'. ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
            ...
            else
            {
                ...
                workflow.Comment = $"{TranslateHelper.get("Request to Update Approval Level")} '{model.levelName}' ";
                workflow.OperationId = (int)OperationsEnum.ApprovalWorkflowLevelModification;
                workflow.DeferredExecution = true;
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated Approval Level")} '{model.levelName}' {TranslateHelper.get("to go for approval.")} ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteApprovalLevel(int id, UserInfo user)
        {
            ...
            if (admin.IsSuperAdmin(user.createdBy) == true)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Workflow approval level")} '{model.LEVELNAME}' {TranslateHelper.get("was deleted by this super-admin")} {audit_staff}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
            ...
            else
            {
                if (model != null)
                {
                    var values = new TBL_TEMP_APPROVAL_LEVEL
                    {
                        ...
                        INVESTMENTGRADEAMOUNT = model.INVESTMENTGRADEAMOUNT,
                        STANDARDGRADEAMOUNT = model.STANDARDGRADEAMOUNT,
                        RENEWALLIMITAMOUNT = model.RENEWALLIMITAMOUNT,
                        FEERATE = model.FEERATE,
                        INTERESTRATE = model.INTERESTRATE,
                        NUMBEROFUSERS = model.NUMBEROFUSERS,
                        NUMBEROFAPPROVALS = model.NUMBEROFAPPROVALS,
                ...
                workflow.Comment = $"{TranslateHelper.get("Approval request to delete workflow approval level")} {model.LEVELNAME}";
                workflow.OperationId = (int)OperationsEnum.ApprovalWorkflowLevelModification;
                workflow.DeferredExecution = true;
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.createdBy,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = $"{TranslateHelper.get("Approval request to delete workflow approval level")} '{model.LEVELNAME}'",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
            ...
            {
                throw new SecureException(TranslateHelper.get("Can not delete this level because it is being used. You can de activate it."));
            }
            else
            {
        ...
        public async Task<bool> UpdateApprovalTrail(TBL_APPROVAL_TRAIL model)
        {
            bool result = false;
            var update = await context.TBL_APPROVAL_TRAIL.SingleOrDefaultAsync(m => m.OPERATIONID == model.OPERATIONID
        ...
        public async Task<IEnumerable<TBL_STAFF>> GetStaffOrganogram(int companyId)
        {
        ...
        private void UpdateMainApprovalLevel(ApprovalLevelViewModel ApprovalModel, short status)
        {
            ...
            if (data != null)
            {
                ...
                if (data.OPERATION == "create")
                {
                    ...
                    audit.DETAIL = $"{TranslateHelper.get("Workflow approval level")} '{ApprovalModel.levelName}' {TranslateHelper.get("has been added successfully")}";
                ...
                else if (data.OPERATION == "update")
                {
                    ...
                    audit.AUDITTYPEID = (short)AuditTypeEnum.ApprovalLevelAdded;
                    audit.DETAIL = $"{TranslateHelper.get("Workflow approval level")} '{ApprovalModel.levelName}' {TranslateHelper.get("has been updated successfully")}";
                ...
                else if (data.OPERATION == "delete")
                {
                    ...
                    audit.DETAIL = $"{TranslateHelper.get("Workflow approval level")} '{ApprovalModel.levelName}' {TranslateHelper.get("has been deleted successfully")}";
                }
        ...
        private void CreateApprovalLevel(TBL_TEMP_APPROVAL_LEVEL data)
        {
            var entity = new TBL_APPROVAL_LEVEL
            {
                ...
                RENEWALLIMITAMOUNT = data.RENEWALLIMITAMOUNT,
                ...
                ISEXTERNALSYSTEMAPPROVER = data.ISEXTERNALSYSTEMAPPROVER,
        ...
        private void UpdateApprovalLevel(TBL_TEMP_APPROVAL_LEVEL data)
        {
            ...
            if (updateData != null)
            {
                ...
                updateData.RENEWALLIMITAMOUNT = data.RENEWALLIMITAMOUNT;
                ...
                updateData.ISEXTERNALSYSTEMAPPROVER = data.ISEXTERNALSYSTEMAPPROVER;
                updateData.SUBSIDIARYID = data.SUBSIDIARYID;
        ...
        public async Task<List<ApprovalLevelViewModel>> GetTempApprovalApprovalLevel(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.ApprovalWorkflowLevelModification).ToList();

            var insurance = await (from x in context.TBL_TEMP_APPROVAL_LEVEL
                             ...
                             select new ApprovalLevelViewModel
                             {
                                 ...
                                 numberOfApprovals = x.NUMBEROFAPPROVALS,
                                 slaInterval = x.SLAINTERVAL,
                                 isPoliticallyExposedValue = x.ISPOLITICALLYEXPOSED ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canEscalateValue = x.CANESCALATE ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canApproveUntenoredValue = x.CANAPPROVEUNTENORED ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canResolveDisputeValue = x.CANRESOLVEDISPUTE ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 isActiveValue = x.ISACTIVE ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 slaNotificationInterval = x.SLANOTIFICATIONINTERVAL,
                                 canViewDocumentValue = x.CANVIEWDOCUMENT ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canEditValue = x.CANEDIT ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canViewUploadedFileValue = x.CANVIEWUPLOAD ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canUploadFileValue = x.CANUPLOAD ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canViewApprovalValue = x.CANVIEWAPPROVAL ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canApproveValue = x.CANAPPROVE ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canRecieveEmailValue = x.CANRECIEVEEMAIL ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 canRecieveSmsValue = x.CANRECIEVESMS ? TranslateHelper.get("Yes") : TranslateHelper.get("No"),
                                 routeViaStaffOrganogram = x.ROUTEVIASTAFFORGANOGRAM,
                                 createdBy = x.CREATEDBY,
                                 dateTimeCreated = x.DATETIMECREATED,
                                 dateTimeUpdated = x.DATETIMEUPDATED,
        ...
        public async Task<bool> PresetRoute(PresetRouteViewModel entity)
        {
            if (entity.moduleId == 1)
            {
                var appl = await context.TBL_LOAN_APPLICATION.FindAsync(entity.applicationId);
            ...
            {
                var appl = await context.TBL_LMSR_APPLICATION.FindAsync(entity.applicationId);
                appl.NEXTAPPLICATIONSTATUSID = (short)entity.nextApplicationStatusId;
                appl.FINALAPPROVAL_LEVELID = entity.finalApprovalLevelId;
            }
        ...
        public async Task<PresetRouteViewModel> GetPresetRouteCollection(int operationId, int? classId)
        {
            var preset = new PresetRouteViewModel();

            var process = await context.TBL_LOAN_APPLICATION_STATUS.Select(x => new FintrakDropDownSelectList
                {
        ...
        public async Task<List<FintrakDropDownSelectList>> GetRoutableOperations(List<int> operationIds)
        {
            return await context.TBL_OPERATIONS
        ...
        public async Task<List<ApprovalLevelViewModel>> GetRerouteApprovalLevels(int operationId)
        {
            var levels = await context.TBL_APPROVAL_GROUP_MAPPING.Where(x => x.OPERATIONID == operationId && x.DELETED == false)
                    .Join(context.TBL_APPROVAL_GROUP, m => m.GROUPID, g => g.GROUPID, (m, g) => new { m, g })
        ...
        public async Task<bool> RerouteOperation(ForwardViewModel model) 
        {
            //workflow.StaffId = model.createdBy;
            ...
            var currentTrail = await context.TBL_APPROVAL_TRAIL.FirstOrDefaultAsync(x =>
        ...
        public async Task<IEnumerable<ApprovalTrailViewModel>> GenericApprovalTrail(ApprovalTrailRequestViewModel entity)
        {
            ...
            var data = await trail.Select(x => new ApprovalTrailViewModel
            {
            ...
            }).OrderByDescending(x => x.approvalTrailId).ToListAsync();

            return data.OrderByDescending(p => p.approvalTrailId);
        }

        public async Task<IEnumerable<ApprovalTrailViewModel>> GenericLMSApprovalTrail(int targetId, int operationId)
        {
            var staffRoles = context.TBL_STAFF_ROLE.ToList();
            var staffs = from s in context.TBL_STAFF select s;

            var allstaff = this.GetAllStaffNames();
            var data = await (from x in context.TBL_APPROVAL_TRAIL
                         where
              ...
              select new ApprovalTrailViewModel
              {
                ...
                toStaffName = allstaff.FirstOrDefault(s => s.id == x.RESPONSESTAFFID) == null ? "N/A" : allstaff.FirstOrDefault(s => s.id == x.RESPONSESTAFFID).name,
                fromStaffName = allstaff.FirstOrDefault(s => s.id == x.REQUESTSTAFFID) == null ? "N/A" : allstaff.FirstOrDefault(s => s.id == x.REQUESTSTAFFID).name,
            }).OrderBy(x => x.approvalTrailId).ToListAsync();
        ...
        public async Task<IQueryable<WorkflowNotificationViewModel>> GetWorkflowMappingNotifications(int MappingId)
        {
        ...
        public async Task<bool> DeleteWorkflowMappingNotification(int MappingId, UserInfo user)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.createdBy,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = $"{TranslateHelper.get("Workflow mapping notification for mappingId")} '{MappingId}' {TranslateHelper.get("was deleted by")} {audit_staff}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
