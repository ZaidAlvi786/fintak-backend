...
namespace FintrakBanking.Repositories.Setups.Approval
{
    public class ApprovalReliefRepository : IApprovalReliefRepository
    {
        ...
        public async Task<ApprovalReliefViewModel> AddApprovalRelief(ApprovalReliefViewModel model)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Please select a staff to relieve"));
            }

            if(model.reliefStaffId <= 0)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Please select a relieving staff"));
            }

            if(model.startDate > model.endDate)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Start Date cannot be later than End Date"));
            }

            if (model.startDate == model.endDate)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Start Date and End Date cannot be the same"));
            }
            if (model.reliefStaffId == model.relievedStaffId)
            {
                throw new ConditionNotMetException(TranslateHelper.get("A Staff cannot relief himself!"));
            }
            ...
            tempStaffRelief = new TBL_TEMP_STAFF_RELIEF()
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        DETAIL = $"{TranslateHelper.get("Added")} '{model.reliefStaffName}' {TranslateHelper.get("as Staff Relief for:")} '{model.staffName}'",
                    ...
                    using (var trans = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            var entity = new ApprovalViewModel
                            {
                                ...
                                comment = TranslateHelper.get("Please approve this Approval Relief"),
        ...
        public async Task<IEnumerable<ApprovalReliefViewModel>> GetAllApprovalRelief(int companyId)
        {
        ...
        public async Task<bool> UpdateApprovalRelief(int reliefId, ApprovalReliefViewModel model)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("Approval Relief is already undergoing approval"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Staff Relief of")} '{model.staffName}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    workFlow.Comment = $"{TranslateHelper.get("Update approval request for Staff Relief")} : {targetReliefId}";
        ...
        public async Task<IEnumerable<ApprovalReliefViewModel>> GetApprovalReliefAwaitingApprovals(int staffId, int companyId)
        {
            var ids = general.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.StaffReliefCreation).ToList();

            var charge = await (from a in context.TBL_TEMP_STAFF_RELIEF
    ...
    //        var data = charge.ToList();
        ...
        private bool ApproveStaffRelief(int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = TranslateHelper.get("Approved Approval Relief ") + previousReliefForAudit,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async  Task<IEnumerable<ApprovalReliefViewModel>> GetAllStaffRelief(int companyId, int staffId)
        {
        ...
        public bool AddStaffRelief(ApprovalReliefViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added")} '{staffRecord}' {TranslateHelper.get("as Staff Relief for")}: '{reliefStaffRecord}'",
