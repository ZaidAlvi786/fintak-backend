...
namespace FintrakBanking.Repositories.Setups.Approval
{
    public class ApprovalGroupRepository : IApprovalGroupRepository
    {
        ...
        public async Task<IEnumerable<ApprovalGroupViewModel>> GetAllApprovalGroup(int companyId)
        {
            return GetApprovalGroup(companyId);
        }

        public async Task<IEnumerable<ApprovalGroupViewModel>> GetApprovalGroupById(int GroupId, int companyId)
        {
        ...
        public bool AddApprovalGroup(ApprovalGroupViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Approval Group")} '{model.groupName}'",
        ...
        public bool UpdateApprovalGroup(int GroupId, ApprovalGroupViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Approval Group")} '{model.groupName}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public bool DeleteApprovalGroup(int GroupId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Deleted Approval Group")} '{data.GROUPNAME}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
