...
namespace FintrakBanking.Repositories.Setups.Approval
{
    public class ApprovalLevelStaffRepository : IApprovalLevelStaffRepository
    {
        ...
        public async Task<bool> AddApprovalLevelStaff(ApprovalLevelStaffViewModel model)
        {
            if (admin.IsSuperAdmin(model.createdBy) == true)
            {
                ...
                var audit_staff_level = await (context.TBL_APPROVAL_LEVEL.FirstOrDefaultAsync(x => x.APPROVALLEVELID == data.APPROVALLEVELID));
                var audit_staff = await (context.TBL_STAFF.FirstOrDefaultAsync(x => x.STAFFID == data.STAFFID));
        ...
        public async Task<IEnumerable<ApprovalLevelStaffViewModel>> GetTempApprovalLevelStaff(int staffId)
        {
            var ids = _genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.ApprovalLevelStaffModification).ToList();


            var data = await (from a in context.TBL_TEMP_APPROVAL_LEVEL_STAFF
        ...
        public async Task<bool> UpdateApprovalLevelStaff(int StaffLevelId, ApprovalLevelStaffViewModel model)
        {
            if (admin.IsSuperAdmin(model.createdBy) == true)
            {
                var data = await context.TBL_APPROVAL_LEVEL_STAFF.Where(x => x.STAFFLEVELID == StaffLevelId).Select(x => x).FirstOrDefaultAsync();
                ...
                var audit_staff_level = await (context.TBL_APPROVAL_LEVEL.FirstOrDefaultAsync(x => x.APPROVALLEVELID == StaffLevelId));
        ...
        public async Task<bool> DeleteApprovalLevelStaff(int StaffLevelId, UserInfo user)
        {
            ...
            if (model != null)
            {
                ...
                else
                {
                    ...
                    workflow.Comment = $"Approval Level for staff with code '{audit_staff.STAFFCODE}' to level {model.STAFFLEVELID}' is deleted and the action is going for approval";
                    workflow.OperationId = (int)OperationsEnum.ApprovalLevelStaffModification;
                    workflow.DeferredExecution = true;
                    workflow.ExternalInitialization = true;
                    workflow.LogActivity();
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        STAFFID = user.createdBy,
                        BRANCHID = (short)user.BranchId,
                        DETAIL = $"Approval Level for staff with code '{audit_staff.STAFFCODE}' to level {model.STAFFLEVELID}' is deleted and the action is going for approval ",
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                        URL = user.applicationUrl,
                        APPLICATIONDATE = _genSetup.GetApplicationDate(),
                        SYSTEMDATETIME = DateTime.Now,
        ...
        private void UpdateMainApprovalLevelStaff(ApprovalLevelStaffViewModel ApprovalModel, short status)
        {
            ...
            if (data != null)
            {
                ...
                else if (data.OPERATION == "delete")
                {
                    ...
                    audit.DETAIL = $"The Approval-Level '{audit_staff_level?.LEVELNAME}' for user code '{audit_staff?.STAFFCODE}' has been deleted successfully";
                }
        ...
        public async Task<List<WorkflowTrackerViewModel>> GetAllApprovalStatus()
        {
            var result = await (from x in context.TBL_APPROVAL_STATUS
                         select new WorkflowTrackerViewModel
                         {
        ...
        public async Task<List<WorkflowTrackerViewModel>> GetAllApprovalOperations()
        {
            var result = await (from x in context.TBL_OPERATIONS
                         select new WorkflowTrackerViewModel
                         {
        ...
        public int GetWorkingDays(DateTime from, DateTime to, int countryId)
        {
            var totalDays = 0;
            var holidays = context.TBL_PUBLIC_HOLIDAY.Where(x => x.COUNTRYID == countryId).Select(x => x.DATE).ToList();
        ...
        public List<WorkflowTrackerViewModel> GetTurnAroundMointoring(DateRange param)
        {
            ...
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                ...
                var record = (from a in context.TBL_APPROVAL_TRAIL
                              ...
                              select (new WorkflowTrackerViewModel
                              {
                                  ...
                                  approvalTrailId = a.APPROVALTRAILID,
                                  toStaffId = a.TOSTAFFID,
                                  productClassProcessId = d.PRODUCT_CLASS_PROCESSID,

                              })
                              ).GroupBy(o => o.applicationReferenceNumber).Select(r => r.FirstOrDefault()).ToList()
                ...
                {
                    item.countryId = 1;
                    item.applicationDate2 = item.applicationDate.ToString("dd-MM-yyyy hh:mm:ss");
                    var ProductName = "";

                    int count = serial;
                    decimal amount = 0;
                    var loanDetails = context.TBL_LOAN_APPLICATION_DETAIL.Where(a => a.LOANAPPLICATIONID == item.loanApplicationId).ToList();
                    var approvallTrails = context.TBL_APPROVAL_TRAIL.Where(a => a.TARGETID == item.loanApplicationId && (a.OPERATIONID == 6 || a.OPERATIONID == 153 || a.OPERATIONID == 133)).ToList();
                    var excludeDays = context.TBL_PUBLIC_HOLIDAY.Where(x => x.COUNTRYID == item.countryId).Select(x => x.DATE).ToList();
                    DateTime? firstRecord;
                    DateTime? lastRecord;


                    if (approvallTrails.Count() != 0)
                    {
                        firstRecord = approvallTrails.Where(c => c.TARGETID > 0).OrderBy(c => c.SYSTEMARRIVALDATETIME).First()?.SYSTEMARRIVALDATETIME;
                        lastRecord = approvallTrails.Where(c => c.TARGETID > 0).OrderBy(c => c.SYSTEMARRIVALDATETIME).Last()?.SYSTEMARRIVALDATETIME;
                        var lastRecordFilter = approvallTrails.Where(c => c.TARGETID > 0).OrderBy(c => c.SYSTEMARRIVALDATETIME).Last();
                        if (lastRecordFilter?.SYSTEMRESPONSEDATETIME == null && lastRecordFilter.APPROVALSTATUSID != (int)ApprovalStatusEnum.Approved)
                        {
                            lastRecord = DateTime.Now;
                        }
                        if (lastRecordFilter?.SYSTEMRESPONSEDATETIME != null)
                        {
                            lastRecord = lastRecordFilter?.SYSTEMRESPONSEDATETIME;
                        }
                        item.firstApprovertime = firstRecord;
                        item.lastApprovertime = lastRecord;
                        int actualDays = GetWorkingDays((DateTime)item.firstApprovertime, (DateTime)item.lastApprovertime, item.countryId);
                        item.excludedDays = actualDays;

                    }


                    var expectedTAT = "";
                    if (item.productClassProcessId == 1)
                    {
                        expectedTAT = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 1).FirstOrDefault().TURNAROUNDTIME.ToString() + " days";
                        item.isCreditProgramme = false;
                        item.slaDays = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 1).FirstOrDefault().TURNAROUNDTIME.ToString();
                        item.useHolidays = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 1).FirstOrDefault().EXCLUDEPUBLICHOLIDAYS;
                    }
                    else
                    {
                        expectedTAT = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 2).FirstOrDefault().TURNAROUNDTIME.ToString() + " days";
                        item.isCreditProgramme = true;
                        item.slaDays = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 2).FirstOrDefault().TURNAROUNDTIME.ToString();
                        item.useHolidays = context.TBL_TAT_SETUP.Where(t => t.PRODUCTCLASSPROCESSID == 2).FirstOrDefault().EXCLUDEPUBLICHOLIDAYS;
                    }
                    foreach (var rec in loanDetails)
                    {
                        if (loanDetails.Count > 1)
        ...
        public List<WorkflowTrackerViewModel> GetBookingTATMointoring(DateRange param)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                ...
                var preBookingrecord = (from a in context.TBL_APPROVAL_TRAIL
                           ...
                           ).GroupBy(o => o.loanApplicationDetailId).Select(r => r.FirstOrDefault());
                ...
                {
                    var loanDetails = context.TBL_LOAN_APPLICATION_DETAIL.Where(a => a.LOANAPPLICATIONID == item.loanApplicationId).ToList();
                    var approvallTrails = context.TBL_APPROVAL_TRAIL.Where(a => a.TARGETID == item.loanApplicationId && (a.OPERATIONID == 6 || a.OPERATIONID == 153)).ToList();
                    DateTime? firstRecord;
                    DateTime? lastRecord;
        ...
        public async Task<List<WorkflowTrackerViewModel>> GetContractReviewMointoring(DateRange param)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                ...
                var preBookingrecord = await (from a in context.TBL_APPROVAL_TRAIL
                                        join b in context.TBL_APPROVAL_STATUS on a.APPROVALSTATUSID equals b.APPROVALSTATUSID
                                        join c in context.TBL_APPROVAL_STATE on a.APPROVALSTATEID equals c.APPROVALSTATEID
                ...
                var record = await (from a in context.TBL_APPROVAL_TRAIL
                              join b in context.TBL_APPROVAL_STATUS on a.APPROVALSTATUSID equals b.APPROVALSTATUSID
                              join c in context.TBL_APPROVAL_STATE on a.APPROVALSTATEID equals c.APPROVALSTATEID
        ...
        public async Task<List<WorkflowTrackerViewModel>> GetApprovalTrailByTargetId(int targetId, int companyId)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                // int[] operations = new int[] { 6, 37, 38 };
                var operations = await context.TBL_LOAN_APPLICATION.Select(x => x.OPERATIONID).Distinct().ToListAsync();
                ...
                var result = await (from a in context.TBL_APPROVAL_TRAIL
                              ...
                              select new WorkflowTrackerViewModel
                              {
                                  ...
                                  approvalStatus = a.TBL_APPROVAL_STATUS.APPROVALSTATUSNAME,
                                  approvalTrailId = a.APPROVALTRAILID,
                              }).Distinct().OrderByDescending(i => i.approvalTrailId).ToListAsync();
        ...
        public async Task<List<WorkflowTrackerViewModel>> GetBookingApprovalTrailByTargetId(int targetId, int companyId)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
        ...
        public async Task<WorkflowTrackerViewModel> GenerateApprovalMonitoringReport(DateRange param)
        {

            return await GenerateWorkflowRecord(param);

        }
        private async Task<WorkflowTrackerViewModel> GenerateWorkflowRecord(DateRange param)
        {
        ...
        public async Task<bool> AddTATSetup(TurnAroundTimeViewModel entity)
        {
            try
            {
                var data = context.TBL_TAT_SETUP.Add(new TBL_TAT_SETUP
                {
                    PRODUCTCLASSPROCESSID = entity.productClassProcessId,
                    TURNAROUNDTIME = entity.turnAroundTime,
                    EXCLUDEPUBLICHOLIDAYS = entity.excludePublicHolidays,
                ...
                var audit = new TBL_AUDIT
                {
                    AUDITTYPEID = (short)AuditTypeEnum.TatSetupAdded,
                    STAFFID = entity.createdBy,
                    BRANCHID = (short)entity.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Added Turn arround time information")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public IEnumerable<TurnAroundTimeViewModel> GetTATSetup()
        {
            try
            {
                var data = (from a in context.TBL_TAT_SETUP
                            where a.DELETED == false
                            select new TurnAroundTimeViewModel
                            {
                                tatId = a.TATID,
                                productClassProcessId = a.PRODUCTCLASSPROCESSID,
                                productClassProcessName = context.TBL_PRODUCT_CLASS_PROCESS.Where(p => p.PRODUCT_CLASS_PROCESSID == a.PRODUCTCLASSPROCESSID).FirstOrDefault().PRODUCT_CLASS_PROCESS_NAME,
                                turnAroundTime = a.TURNAROUNDTIME,
                                excludePublicHolidays = a.EXCLUDEPUBLICHOLIDAYS,
        ...
        public async Task<bool> UpdateTATSetup(int tatId, TurnAroundTimeViewModel entity)
        {
            try
            {

                var tat = context.TBL_TAT_SETUP.FirstOrDefault(x => x.TATID == tatId);
                if (tat == null) return false;

                tat.PRODUCTCLASSPROCESSID = entity.productClassProcessId;
                tat.TURNAROUNDTIME = entity.turnAroundTime;
                tat.EXCLUDEPUBLICHOLIDAYS = entity.excludePublicHolidays;
                tat.LASTUPDATEDBY = (int)entity.createdBy;
                ...
                var audit = new TBL_AUDIT
                {
                    AUDITTYPEID = (short)AuditTypeEnum.TatSetupUpdated,
                    STAFFID = entity.createdBy,
                    BRANCHID = (short)entity.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated turn around time information")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = entity.applicationUrl,
                    DEVICENAME = CommonHelpers.GetDeviceName(),
                    OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> DeleteTATSetup(int tatId, UserInfo user)
        {
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    AUDITTYPEID = (short)AuditTypeEnum.TatSetupDeleted,
                    STAFFID = user.createdBy,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = $"{TranslateHelper.get("Deleted turn around time information")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    DEVICENAME = CommonHelpers.GetDeviceName(),
                    OSNAME = CommonHelpers.FriendlyName(),
