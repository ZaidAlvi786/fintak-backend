...
namespace FintrakBanking.Repositories.Setups.Approval
{
    public class ApprovalGroupMappingRepository : IApprovalGroupMappingRepository
    {
        ...
        public async Task <int> AddApprovalGroupMapping(ApprovalGroupMappingViewModel model)
        {
            ...
            var recordExist = await context.TBL_TEMP_APPROVAL_GRP_MAPPING.Where(x => x.OPERATIONID == model.operationId && x.GROUPID == model.groupId && x.POSITION == model.position).AnyAsync();
            if (recordExist)
                throw new ConditionNotMetException(TranslateHelper.get("This operation has already been initiated and is approval pending"));
            ...
            if (admin.IsSuperAdmin(model.staffId) == true)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Added Approval Group Mapping for Operation")}: {operationName} {TranslateHelper.get("in Group")}: {groupName}",
            ...
            else
            {
                ...
                workflow.Comment = $"{TranslateHelper.get("New approval request for group operation mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}";
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = (int)model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName} {TranslateHelper.get("was added and is going for approval")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    SYSTEMDATETIME = DateTime.Now,
                    APPLICATIONDATE = generalSetup.GetApplicationDate(),
        ...
        public bool DeleteApprovalGroupMapping(int operationMappingId, UserInfo model)
        {
            ...
            if (dataExist)
                throw new ConditionNotMetException(TranslateHelper.get("This operation has already been initiated and is pending approval"));
            ...
            if (admin.IsSuperAdmin(model.staffId) == true)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Delete Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    SYSTEMDATETIME = DateTime.Now,
                    APPLICATIONDATE = generalSetup.GetApplicationDate(),
            ...
            else
            {
                ...
                workflow.Comment = $"{TranslateHelper.get("Request to Delete Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}";
                workflow.OperationId = (int)OperationsEnum.ApprovalWorkflowGroupModification;
                workflow.DeferredExecution = true;
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = (int)model.createdBy,
                    BRANCHID = (short)model.BranchId,
                    DETAIL = $"{TranslateHelper.get("Request to Delete Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    SYSTEMDATETIME = DateTime.Now,
                    APPLICATIONDATE = generalSetup.GetApplicationDate(),
        ...
        public async Task<ApprovalGroupMappingViewModel> GetApprovalGroupMapping(int operationMappingId)
        {
            return  await (from data in GetAllApprovalGroupMapping()
                    where data.groupOperationMappingId == operationMappingId
                    select data).FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<ApprovalGroupMappingViewModel>> GetApprovalGroupMapping(
            int operationId,
            short? productClassId,
            short? productId
            )
        {
            var operationGroups = await context.TBL_APPROVAL_GROUP_MAPPING
        ...
        public bool UpdateApprovalGroupMapping(int operationMappingId, ApprovalGroupMappingViewModel model)
        {
            ...
            if (admin.IsSuperAdmin(model.staffId) == true)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName} {TranslateHelper.get("was updated by a super-admin")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    SYSTEMDATETIME = DateTime.Now,
                    APPLICATIONDATE = generalSetup.GetApplicationDate(),
            ...
            else
            {
                ...
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = model.tempGroupOperationMappingId;
                workflow.Comment = $"{TranslateHelper.get("Update approval request for group operation mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}";
                workflow.OperationId = (int)OperationsEnum.ApprovalWorkflowGroupModification;
                workflow.DeferredExecution = true;
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = (int)model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    SYSTEMDATETIME = DateTime.Now,
                    APPLICATIONDATE = generalSetup.GetApplicationDate(),
        ...
        private void UpdateMainApprovalGroupMapping(ApprovalGroupMappingViewModel ApprovalModel, short status)
        {
            ...
            if (data != null)
            {
                ...
                if (data.OPERATION == "create")
                {
                    ...
                    audit.DETAIL = $"{TranslateHelper.get("Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName} {TranslateHelper.get("is added successfully")}";
                ...
                else if (data.OPERATION == "delete")
                {
                    ...
                    audit.DETAIL = $"{TranslateHelper.get("Approval Group Mapping for Operation:")} {operationName} {TranslateHelper.get("in Group:")} {groupName} {TranslateHelper.get("is deleted successfully")}";
        ...
        public async Task<List<ApprovalGroupMappingViewModel>> GetTempApprovalGroupForApproval(int staffId)
        {
            var ids = generalSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.ApprovalWorkflowGroupModification).ToList();

            var insurance = await (from x in context.TBL_TEMP_APPROVAL_GRP_MAPPING
