...
namespace FintrakBanking.Repositories.Setups.General
{
    public class AccreditedConsultantsRepository : IAccreditedConsultantsRepository
    {
        ...
        public async Task<IEnumerable<AccreditedConsultantsViewModel>> GetAccreditedStateConsultants(int companyId)
        {
            var data = await (from m in context.TBL_ACCREDITEDCONSULTANT
        ...
        public async Task<IEnumerable<AccreditedConsultantsViewModel>> GetAccreditedConsultants(int companyId)
        {
            var data = await (from m in context.TBL_ACCREDITEDCONSULTANT
                        where m.COMPANYID == companyId
                        && m.ACCREDITEDCONSULTANTTYPEID == (int)AccreditedConsultantTypeEnum.RecoveryAgent
        ...
        public async Task<IEnumerable<AccreditedConsultantsViewModel>> GetSearchedAgent(string search)
        {

            var agencies = await (from m in context.TBL_ACCREDITEDCONSULTANT
        ...
        public async Task<IEnumerable<AccreditedConsultantsViewModel>> GetAccreditedStateConsultantsByStateId(int companyId, int stateId)
        {
            var data = await (from m in context.TBL_ACCREDITEDCONSULTANT
                        join c in context.TBL_ACCREDITEDCONSULTANT_STATE on m.ACCREDITEDCONSULTANTID equals c.ACCREDITEDCONSULTANTID
        ...
        public async Task<bool> AddConsultantType(AccreditedConsultantTypeViewModel entity)
        {
            bool output = false;
        ...
        public async Task<AccreditedConsultantsViewModel> AddAccreditedConsultants(AccreditedConsultantsViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added tbl_AccreditedConsultant with Id:")} {entity.accreditedConsultantId} ",
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var appTrail = new ApprovalViewModel
                    {
                        ...
                        comment = TranslateHelper.get("Please approve this Accredited Consultant"),
      ...
      .FirstOrDefault(x => x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved && x.ACCREDITEDCONSULTANTID == entity.accreditedConsultantId);
            ...
            {
                throw new SecureException(TranslateHelper.get("Accredited Approval is already undergoing approval"));
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Accredited Consultant")} '{entity.accreditedConsultantName}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public bool UpdateAccreditedConsultants1(AccreditedConsultantsViewModel entity, int id)
        {
        ...
        public int GoForApproval(ApprovalViewModel entity)
        {
            ...
            if (workFlow.NewState == (int)ApprovalState.Ended && entity.approvalStatusId != (short)ApprovalStatusEnum.Disapproved)
            {
        ...
        public async Task<bool> AddAccreditedPrincipals(AccreditedPrincipalsViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added tbl_Collateral_Principals with Id:")} {entity.principalsId} ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateAccreditedPrincipals(AccreditedPrincipalsViewModel entity, int id)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated tbl_Collateral_Principals with Id:")} {entity.principalsId} ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<List<LoanConsultantViewModel>> GetLoanConsultant(int applicationId)
        {
            return await context.TBL_LOAN_APPLICATION_DETL_CON.Where(x => x.DELETED == false && x.TBL_LOAN_APPLICATION_DETAIL.LOANAPPLICATIONID == applicationId)
        ...
        public async Task<bool> RemoveLoanConsultant(int id, UserInfo user)
        {
            var data = await this.context.TBL_LOAN_APPLICATION_DETL_CON.FindAsync(id);
            if (data == null) return false;
            ...
            {
                throw new SecureException(TranslateHelper.get("The agent is currently on recovery collection list. Kindly unassign before deleting"));
            }
            ...
            {
                var staffId = 0;
                var staf = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.LOCALNETWORKID == user.createdBy).FirstOrDefault();
                if (staf != null)
                {
                    staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.LOCALNETWORKID == user.createdBy).FirstOrDefault().REMOTENETWORKID;
                }
                else
                {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = staffId,
                    BRANCHID = 94,
                    DETAIL = $"{TranslateHelper.get("Deleted loan consultant Precedent")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
            ...
            else
            {
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Deleted Condition Precedent")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
