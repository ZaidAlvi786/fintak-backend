    ...
    public class StaffRoleRepository : IStaffRoleRepository
    {
        ...
        public bool AddUpdateStaffRole(StaffRoleViewModel entity)
        {
            ...
            auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                DETAIL = TranslateHelper.get("Added/Modified Staff Role"),
            ...
            {
                throw new SecureException(TranslateHelper.get("There is an operation that is yet to be approved on this item!"));
            }
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    if (context.SaveChanges() > 0)
                    {
                        ...
                        workFlow.Comment = TranslateHelper.get("Create/Update Staff Role");
        ...
        private void ApproveUserRoleUpdate(int staffRoleId, UserInfo user)
        {
            ...
            auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved Staff Role Group and Activities with staffRoleId:")} '{staffRoleId}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public IEnumerable<ApprovalFlowTypeViewModel> GetAllApprovalFlowType()
        {
            var data = (from b in context.TBL_APPROVAL_FLOW_TYPE
                        select new ApprovalFlowTypeViewModel()
                        {
                            approvalFlowTypeId = b.APPROVALFLOWTYPEID,
                            flowTypeName = b.FLOWTYPENAME,
                        }).ToList();
