...
namespace FintrakBanking.Repositories.Setups.General
{
    ...
    public class AuthenticationRepository : IAuthenticationRepository
    {
        ...
        public async Task<ActiveUserDetails> GetUserInformation(string username)
        {
            return await (from u in context.TBL_PROFILE_USER
        ...
        public async Task<bool> UpdateUser(int userId, UserViewModel user)
        {
            try
            {
                var targetUser = await context.TBL_PROFILE_USER.FindAsync(userId);
                if (targetUser == null)
                {
                    return false;
                }
        ...
        public async Task<int> ConcurrentUsers()
        {
            var result = await context.TBL_PROFILE_USER.Where(x => 
                x.ISACTIVE == true &&
                x.ISLOCKED == false &&
                x.LOGINCODE != null 
                ).CountAsync();
        ...
        public async Task<ActiveUserDetails> GetUserAuthenticationInfo(string username)
        {
            username = username.ToLower();
            ActiveUserDetails result = new ActiveUserDetails();
            // var user = GetAllUsers().FirstOrDefault(c => c.username.ToLower() == username);

            var user = await (from u in context.TBL_PROFILE_USER
                        join st in context.TBL_STAFF on u.STAFFID equals st.STAFFID
                        where u.USERNAME.ToLower() == username
            ...
            if (user == null) throw new SecureException(TranslateHelper.get("Invalid Username or Password"));

            result.grantMessage = "valid";
            result.companyId = user.companyId;
            if (!user.isActive)
            {
                result.grantMessage = TranslateHelper.get("This account is INACTIVE");
            }
            else if (await IsAccountLocked(user.username))
            {
                result.grantMessage = TranslateHelper.get("This account is LOCKED");
            }
            else if (!ResumptionClosingTime(user))
            {
                result.grantMessage = TranslateHelper.get("You cannot login at this time");
            }
            else
            {
        ...
        public async Task<UserViewModel> FindUserByUserName(string username)
        {
            ...
            var user = await db.TBL_PROFILE_USER.FirstOrDefaultAsync(x => x.USERNAME.ToLower() == username);

            if (user != null)
            {
                var data = await (from p in db.TBL_PROFILE_USER
            ...
            throw new SecureException(TranslateHelper.get("1001 Login Failure."));
        ...
        public async Task<SessionStatusInfo> CheckSessionState(string username, string ipAddress)
        {
            ...
            var LoginCode = await (from a in context.TBL_PROFILE_USER
                    where a.USERNAME.ToLower() == username
                    select a.LOGINCODE).FirstOrDefaultAsync();
                ...
                else if (loginCodeStr != null)
                {
                    ...
                    else
                    {
                        ...
                        result = new SessionStatusInfo
                        {
                            ...
                            state = 1,
                            ipaddress = ipAddressStr,
                            errorMessage = TranslateHelper.get("You already have an active session."),
                        };
                    }
                }
            }
        ...
        public async Task<UserViewModel> FindUserByUserNameAndPassword(string username, string password) // ERROR POINT 2 - 
        {
            UserViewModel data = null;
            var result = _sessionInfo;

            data = await UserLoginDetails(username, password);

            result.isPasswordExpired = IsPasswordExpired(username);
            result.isFirstLogin = IsFirstLogin(username);
            if (result.state > 0)
            {
                if (data == null) throw new SecureException(TranslateHelper.get("1001 Login Failure."));
            }
        ...
        public async Task<bool>  IsAccountLocked(string userName) // ERROR POINT 1 - 
        {
            var data2 = await GetAllUsers();
            ...
            if (data.isLocked)
            {
                ...
                {
                    //UnlockUser(userName); Please uncomment and change return to false if you want above maths to apply 
                    return true;
                }
                else
                {
                    var loginInfo = await GetUserLoginInfoByUserName(userName);
                    _auditTrail.AddAuditTrail(new TBL_AUDIT
                    {
                        AUDITTYPEID = (short)AuditTypeEnum.LoginFailed,
                        ...
                        DETAIL = $"{userName} - {TranslateHelper.get("This account is LOCKED")}",
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),// CommonHelpers.GetUserIP(),
        ...
        public async Task<bool> IsAccountActive(string userName)
        {
            ...
            var data2 = data.FirstOrDefault(c => c.username.ToLower() == userName);
            if(data2 != null)
            {
                var loginInfo = await GetUserLoginInfoByUserName(userName);
                ...
                var audit = new TBL_AUDIT()
                {
                    ...
                    DETAIL = $"{loginInfo.username} - {TranslateHelper.get("This account is INACTIVE")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),// CommonHelpers.GetUserIP(),
        ...
        public async Task<bool> ResumptionClosignTime(string userName, bool logAudit = true)
        {
            var presentTime = DateTime.Now.TimeOfDay;
            ...
            if (logAudit)
            {
                ...
                var audit = new TBL_AUDIT()
                {
                    ...
                    STAFFID = loginInfo.staffId,
                    BRANCHID = (short)context.TBL_STAFF.Where(x => x.STAFFID == loginInfo.staffId).Select(x => x.BRANCHID).FirstOrDefault(),
                    DETAIL = $"{loginInfo.username} - {TranslateHelper.get("You cannot resume now.")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),// CommonHelpers.GetUserIP(),
                    //URL = Request.RequestUri.AbsoluteUri,
                    APPLICATIONDATE = context.TBL_FINANCECURRENTDATE.FirstOrDefault().CURRENTDATE,
                    SYSTEMDATETIME = DateTime.Now,
            ...
            throw new SecureException(TranslateHelper.get("You cannot resume now."));
        }
        ...
        public bool PasswordStandard(string password)
        {
            ...
            if (setting.MINREQUIREDPASSWORDLENGTH > password.Length)
                throw new SecureException($"{TranslateHelper.get("Password should not be")} {setting.MINREQUIREDPASSWORDLENGTH} {TranslateHelper.get("less characters")}");

            if (setting.MINREQUIREDNONALPHANUMERICCHAR > 0)
            {
                if (!CommonHelpers.isAlphaNumeric(password))
                {
                    throw new SecureException(TranslateHelper.get("Password should alphanumeric."));
                }
                else
                    throw new SecureException($"{TranslateHelper.get("Password should not be")} {setting.MINREQUIREDNONALPHANUMERICCHAR} {TranslateHelper.get("less characters")}");
        ...
        private async Task<UserViewModel> UserLoginDetails(string username, string password) // ERROR POINT 3 - underlying provider...
        {
            TBL_PROFILE_USER profile = new TBL_PROFILE_USER();

            profile = await context.TBL_PROFILE_USER.FirstOrDefaultAsync(c => c.USERNAME.ToUpper() == username.ToUpper()); // && c.PASSWORD == password);

            var record = await (from a in context.TBL_PROFILE_USER
            ...
            else
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("1001 Not Found., Kindly Contact Administrator"));
                }
            }
        }
        ...
        {
           var data =  await context.TBL_PROFILE_USER.AnyAsync(x => x.USERNAME.ToLower() == username);
            return data;
        }

        public async Task<bool> IsUserAccountValid(string username)
        {
            //  var isValid = await GetAllUsers().FirstOrDefault(x => x.username.ToLower() == username.ToLower() && x.isLocked == false && x.isActive == true);
            var users = await(from u in context.TBL_PROFILE_USER
                              join st in context.TBL_STAFF on u.STAFFID equals st.STAFFID
        ...
        public async Task<UserViewModel> GetSingleUser(int userId)
        {
            var user = await (from u in context.TBL_PROFILE_USER
                        join st in context.TBL_STAFF
                        on u.STAFFID equals st.STAFFID
            ...
            if (user != null)
            {
                ...
                user.groupId = await context.TBL_PROFILE_USERGROUP.Where(x => x.USERID == user.user_id)
                                        .Select(x => new UserGroupId
                                        {
                                            groupId = x.GROUPID,
                                            groupKey = x.TBL_PROFILE_GROUP.GROUPNAME
        ...
        public async Task<UserViewModel> GetSingleUserByUserName(string userName)
        {
            var setting = GetProfileSettings();
        ...
        public async Task<UserViewModel> GetUserLoginInfoByUserName(string userName)
        {
            var setting = GetProfileSettings();
        ...
        public async Task<bool> ClearLoginToken(string userName)
        {
            bool result = false;
           // var _user = context.TBL_PROFILE_USER.FirstOrDefault(x => x.USERNAME.ToLower() == userName);
            ...
            _user = await (from a in context.TBL_PROFILE_USER
                     where a.USERNAME.ToLower() == userName
                     select a).FirstOrDefaultAsync();
        ...
        public async Task<LookupViewModel> GetDashboardStaffRole(int staffId)
        {
            var dash = await (from st in context.TBL_STAFF
                        ...
                        select new LookupViewModel
                        {
                            ...
                            lookupCode = sr.STAFFROLECODE,
                        }).FirstOrDefaultAsync();
            return dash;
        }

        public async Task<bool> PasswordChange(PasswordChangeViewModel pwdChange)
        {
            var setting = GetProfileSettings();

            if (pwdChange.currentPassword != pwdChange.newPassword)
            {
                var data = await  context.TBL_PROFILE_USER.Where(u => u.USERNAME.ToUpper() == pwdChange.username.ToUpper()).FirstOrDefaultAsync();
        ...
        public async Task<bool> ValidateOldPassword(string username, string oldPassword)
        {
            bool isOldPasswordValid = false;
            var data = await  context.TBL_PROFILE_USER.Where(u => u.USERNAME.ToUpper() == username.ToUpper()).FirstOrDefaultAsync();
            if (data != null)
            {
