...
namespace FintrakBanking.Repositories.Setups.General
{
    public class EmployerRepository : IEmployerRepository
    {
        ...
        public async Task<string> addEmployer(EmployerViewModel employer)
        {
            if (employer != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Loan employer with")} {employer.companyId} {TranslateHelper.get("id is added")}",
                ...
                return TranslateHelper.get("The record has been added successful");

            }
            return TranslateHelper.get("The record has not been added");
        }

        public async Task<string> deleteEmployer(int employerId, EmployerViewModel employer)
        {
          var employerDel =  await _context.TBL_CUSTOMER_EMPLOYER.FindAsync(employerId);
            if (employerDel!=null)
            {
                ...
                return TranslateHelper.get("The record has been deleted successful");

            }
            return TranslateHelper.get("The record has not been deleted");
        }

        public async Task<IEnumerable<EmployerViewModel>> getEmployer(int companyId)
        {
            var employerData = await (from a in _context.TBL_CUSTOMER_EMPLOYER
        ...
        public async Task<IEnumerable<EmployerViewModel>> getAllPendingEmployers(int companyId)
        {
            var employerData = await (from a in _context.TBL_CUSTOMER_EMPLOYER
                                join c in _context.TBL_CITY on a.CITYID equals c.CITYID
                                join sb in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB on a.EMPLOYER_SUB_TYPEID equals sb.EMPLOYER_SUB_TYPEID
        ...
        public async Task<IEnumerable<EmployerViewModel>> getAllApprovedEmployers(int companyId)
        {
            var employerData = await (from a in _context.TBL_CUSTOMER_EMPLOYER
                                join c in _context.TBL_CITY on a.CITYID equals c.CITYID
                                join sb in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB on a.EMPLOYER_SUB_TYPEID equals sb.EMPLOYER_SUB_TYPEID
        ...
        public async Task<IEnumerable<EmployerViewModel>> GetRelatedEmployersWaitingForApproval(int staffId)
        {
            ...
            var relatedEmployers = await (from a in _context.TBL_CUSTOMER_EMPLOYER
                                    join c in _context.TBL_CITY on a.CITYID equals c.CITYID
                                    join sb in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB on a.EMPLOYER_SUB_TYPEID equals sb.EMPLOYER_SUB_TYPEID
        ...
        public async Task<WorkflowResponse> ForwardRelatedEmployerForApproval(EmployerViewModel model)
        {
            int operationId = (int)OperationsEnum.RelatedEmployer;
            var employer = await _context.TBL_CUSTOMER_EMPLOYER.FindAsync(model.employerId);
        ...
        public String ResponseMessage(WorkflowResponse response, string itemHeading)
        {
            if (response.stateId != (int)ApprovalState.Ended)
            {
                ...
                else
                {
                    ...
                    {
                        return $"{TranslateHelper.get("The")} {itemHeading} {TranslateHelper.get("request has been SENT to")} {response.nextLevelName}";
                    }
                }
            }
            else
        ...
        public async Task<EmployerViewModel> getEmployer(int employerId, int companyId)
        {
            var employerData = await (from a in _context.TBL_CUSTOMER_EMPLOYER
                                join c in _context.TBL_CITY on a.CITYID equals c.CITYID
        ...
        public async Task<string> updateEmployer(int employerId, EmployerViewModel employer)
        {
            var employerDel = await _context.TBL_CUSTOMER_EMPLOYER.FindAsync(employerId);
            if (employerDel != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = employer.staffId,
                    BRANCHID = (short)employer.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Loan employer with")} {employer.companyId} {TranslateHelper.get("id is updated")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = employer.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<EmployerSubType>> getEmployerSunType(int emplyerTypeId)
        {
            var subType = await (from a in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB
        ...
        public async Task<IEnumerable<EmployerSubType>> getAllEmployerSubTypes()
        {
            var subType = await (from a in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB
                          select new EmployerSubType
                          {
        ...
        public async Task<IEnumerable<EmployerType>> getEmployerType()
        {
            var type = await (from a in _context.TBL_CUSTOMER_EMPLOYER_TYPE
        ...
        public async Task<string> addEmployerType(EmployerViewModel employerType)
        {
            if (employerType != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Employer Type with")} {employerType.employerTypeName} {TranslateHelper.get("name is added")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<string> deleteEmployerType(int employerId, EmployerViewModel employerType)
        {
            var employerDel = await _context.TBL_CUSTOMER_EMPLOYER_TYPE.FindAsync(employerType.employerTypeId);
            if (employerDel != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = employerType.staffId,
                    BRANCHID = (short)employerType.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Loan employer with")} {employerType.companyId} {TranslateHelper.get("id is deleted")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = employerType.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<string> updateEmployerType(int employerTypeId, EmployerViewModel employerType)
        {
            var employerDel = await _context.TBL_CUSTOMER_EMPLOYER_TYPE.FindAsync(employerTypeId);
            if (employerDel != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = employerType.staffId,
                    BRANCHID = (short)employerType.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Loan employer with")} {employerType.companyId} {TranslateHelper.get("id is updated")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = employerType.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        #region Employer Type
        public async Task<string> addEmployerSubType(EmployerViewModel employerSubType)
        {
            if (employerSubType != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Employer Sub Type with")} {employerSubType.employerSubTypeName} {TranslateHelper.get("name is added")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<string> deleteEmployerSubType(int employerId, EmployerViewModel employerSubType)
        {
            var employerDel = await _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB.FindAsync(employerSubType.employerSubTypeId);
            if (employerDel != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = employerSubType.staffId,
                    BRANCHID = (short)employerSubType.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Loan employer Sub Type with")} {employerSubType.employerSubTypeName} {TranslateHelper.get("name is deleted")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = employerSubType.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<string> updateEmployerSubType(int employerTypeId, EmployerViewModel employerSubType)
        {
            var employerDel = await _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB.FindAsync(employerTypeId);
            if (employerDel != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = employerSubType.staffId,
                    BRANCHID = (short)employerSubType.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Loan employer Sub Type with")} {employerSubType.companyId} {TranslateHelper.get("name is updated")}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = employerSubType.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<EmployerSubType>> getEmployerSubType(int employerSubTypeId)
        {
            var type = await (from a in _context.TBL_CUSTOMER_EMPLOYER_TYPE_SUB
