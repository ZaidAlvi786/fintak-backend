...
namespace FintrakBanking.Repositories.Setups.General
{
    ...
    public class PublicHolidayRepository : IPublicHolidayRepository
    {
        ...
        public async Task<IEnumerable<PublicHolidayViewModel>> GetAllPublicHoliday()
        {
            var holidays = await (from x in context.TBL_PUBLIC_HOLIDAY
                            join k in context.TBL_COUNTRY on x.COUNTRYID equals k.COUNTRYID
                            select new PublicHolidayViewModel
                            {
                                publicHolidayId = x.PUBLICHOLIDAYID,
        ...
        public async Task<bool> AddWeekendsInTheYear(PublicHolidayViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added all weekends in the year:")} '{model.date.Year}",
        ...
        public async Task<bool> AddPublicHoliday(PublicHolidayViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = (int)model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added Holiday:")} '{model.description}' {TranslateHelper.get("with Id:")} {model.publicHolidayId}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        {
            var response = 0;
            var holiday = await context.TBL_PUBLIC_HOLIDAY.FindAsync(id);
            ...
            if (holiday != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = (int)model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated branch:")} '{model.description}' {TranslateHelper.get("with CountryId:")} {model.countryId}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeletePublicHoliday(UserInfo model, int id)
        {
            var response = 0;
            var holiday = await context.TBL_PUBLIC_HOLIDAY.Where(o=>o.PUBLICHOLIDAYID==id).FirstOrDefaultAsync();
            ...
            if (holiday != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Delete Public holyday with public Holiday Id:")} {id}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
