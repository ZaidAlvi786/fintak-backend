...
namespace FintrakBanking.Repositories.Setups.General
{
    public class StaffRepository : IStaffRepository
    {
        ...
        public async Task<staffBulkFeedbackViewModel> UploadBulkPrepaymentData(StaffDocumentViewModel model, byte[] file)
        {
            ...
            var setupGlobal = await context.TBL_SETUP_GLOBAL.FirstOrDefaultAsync();
            ...
            for (int j = range.FirstRowIndex; j <= range.LastRowIndex; j++)
            {
                ...
                for (int i = range.FirstColumnIndex; i <= range.LastColumnIndex; i++)
                {
                    ...
                    switch (cellColumn)
                    {
                        case "A":
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Loan Reference Number Does Not Exist. ");
                            }
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Transaction Reference Number Does Not Exist Today. ");
                            }
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Transaction Reference Number Does Has Past Due Abligation. ");
                            }
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Irregular Schedule Cannot Form Part Of Bulk Irregular Schedule. ");
                            }
                            break;
                        ...
                        case "B":
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Prepayment Amount Does Exist Against This Transaction Reference Number Today. ");
                            }
                            ...
                            {
                                rowSuccess = false;
                                staffRowData.message = staffRowData.message + TranslateHelper.get("Prepayment Amount Transaction Reference Number Already Exist. ");
                            }
                ...
                {
                    staffRowData.message = TranslateHelper.get("Success");
            ...
            else
            {
                ...
                foreach (var staffInfoRow in staffInfo)
                {
                    ...
                    var response = await AddBulkPrepaymentData(staffInfoRow, (int)batchCodeInt, applicationDate);
                ...
                if (rec != null)
                {
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        DETAIL = $"{TranslateHelper.get("Initiated Bulk Prepayment with code")} '{batchCodeInt}'",
        ...
        public async Task<bool> AddBulkPrepaymentData(StaffInfoViewModel staffModel, int batchCode, DateTime applicationDate)
        {
        ...
        public  async Task<bool> UpdatePrepayment(int staffid, StaffInfoViewModel staffModel)
        {
            bool isUpdate = false;
        ...
        public async Task<bool> DeleteBulkPrepayment(int bulkPrepaymentId, UserInfo user)
        {
            ...
            var staff = (from c in context.TBL_STAFF
                         ...
                         select new StaffInfoViewModel()
                         {
                            ...
                            // supervisorStaffName = c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME,
                             ...
                             businessUnitId =c.TBL_PROFILE_BUSINESS_UNIT.BUSINESSUNITID,
                             misCode = c.MISCODE,
                             businessUnitName = c.BUSINESSUNITID != null ? context.TBL_PROFILE_BUSINESS_UNIT.Where(b=>b.BUSINESSUNITID == c.BUSINESSUNITID).Select(b => b.BUSINESSUNITNAME).FirstOrDefault() : null,
                         }).ToList();
        ...
        public async Task<StaffInfoViewModel> GetStaffById(int staffId)
        {
            var staff =  await (from c in context.TBL_STAFF
        ...
        public bool UpdateStaff(int staffid, StaffInfoViewModel staffModel)
        {
            ...
            var tempStaffForPassword = context.TBL_PROFILE_USER.FirstOrDefault(u => u.STAFFID == staffid);
            var currentSuperVisorId = context.TBL_STAFF.FirstOrDefault(u => u.STAFFID == staffid).SUPERVISOR_STAFFID;
            var currentStaffProf = context.TBL_STAFF.FirstOrDefault(u => u.STAFFID == staffid);
            if (staffModel.supervisorStaffId == null)
            {
                staffModel.supervisorStaffId = currentSuperVisorId;
            }
            ...
            var existingTempStaff = context.TBL_TEMP_STAFF.FirstOrDefault(x => x.STAFFCODE.ToLower() == currentStaffProf.STAFFCODE.ToLower() && x.ISCURRENT == true && x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Pending);
            ...
            if (existingTempStaff != null)
            {
                ...
                if (existingTempUser != null)
                {
                    ...
                    {
                        existingTempUser.PASSWORD = context.TBL_PROFILE_USER.Where(a => a.USERNAME.ToLower() == tempStaffForPassword.USERNAME.ToLower()).FirstOrDefault().PASSWORD;
                        existingTempUser.NEXTPASSWORDCHANGEDATE = context.TBL_PROFILE_USER.Where(a => a.USERNAME.ToLower() == tempStaffForPassword.USERNAME.ToLower()).FirstOrDefault().NEXTPASSWORDCHANGEDATE;
                    }
            ...
            if (existingTempStaff != null)
            {
                ...
                tempStaffToUpdate.STAFFCODE = currentStaffProf != null ? currentStaffProf.STAFFCODE : staffModel.StaffCode.ToLower().Trim();
            ...
            else
            {
                ...
                try
                {
                    tempStaff = new TBL_TEMP_STAFF()
                    {
                        ...
                        MIDDLENAME = staffModel.MiddleName,
                        LASTNAME = staffModel.LastName,
                        STAFFCODE = currentStaffProf != null ? currentStaffProf.STAFFCODE : staffModel.StaffCode.ToLower().Trim(),
                        //JOBTITLEID = staffModel.JobTitleId,
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Updated Staff")} '{staffModel.StaffFullName}' {TranslateHelper.get("with code")} '{staffModel.StaffCode}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    workflow.Comment = TranslateHelper.get("Update Staff Creation");
        ...
        public async Task<IEnumerable<BatchPrepaymentViewModel>> GetAllUnprocessedBulkPrepayment()
        {
            var data = await (from a in context.TBL_BULK_PREPAYMENT
        ...
        public async Task<IEnumerable<BatchPrepaymentViewModel>> GetAllUnprocessedBulkPrepaymentBatch(int staffId)
        {
            ...
            var processedBatch = await (from a in context.TBL_BULK_PREPAYMENT
        ...
        public async Task<bool> SubmitPrepaymentBatchForApproval(ApprovalViewModel model)
        {
            bool response = false;
        ...
        public async Task<IEnumerable<BatchPrepaymentViewModel>> GetBulkPrepaymentsAwaitingApprovalBatch(int staffId, int companyId)
        {
        ...
        public async Task<bool> SubmitPrepaymentBatchForWorkflowApproval(ApprovalViewModel model)
        {
            bool response = false;
        ...
        public async Task<bool> LogDeleteRequestStaff(int staffId, UserInfo user)
        {
            var targetStaff = await context.TBL_STAFF.FindAsync(staffId);
            ...
            if(existingApprovalEntry.Count() <= 0)
            {
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    ...
                    {
                        trans.Rollback();
                        throw new ConditionNotMetException(TranslateHelper.get("User delete failed. Contact Administrator"));
                    }
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("This staff record is currently undergoing approval"));
            }
        }

        public async Task<int> GoForStaffDeleteApproval(ApprovalViewModel entity)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                try
        ...
        private bool DeleteStaff(int staffId, int approvalStatusId, ApprovalViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved Deleted Staff")} '{targetStaff?.FIRSTNAME}' {TranslateHelper.get("with code")} '{targetStaff?.STAFFCODE}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<StaffInfoViewModel>> GetStaffDeleteRequestAwaitingApprovals(int staffId, int companyId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.DeleteStaff).ToList();

            var staff =  await (from c in context.TBL_TEMP_STAFF
                         join br in context.TBL_BRANCH on c.BRANCHID equals br.BRANCHID
                         join coy in context.TBL_COMPANY on br.COMPANYID equals coy.COMPANYID
        ...
        public async Task<int> GoForApproval(ApprovalViewModel entity)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                try
        ...
        private bool ApproveStaff(int staffid, short approvalStatusId, UserInfo user)
        {
            ...
            if (tempStaff != null)
            {
                targetStaff = context.TBL_STAFF.FirstOrDefault(x => x.STAFFCODE.ToLower().TrimEnd().Trim() == tempStaff.STAFFCODE.ToLower());
            ...
            if (targetStaff != null) //Update existing staff with tempStaff record
            {
                ...
                targetStaff.STAFFCODE = tempUser != null ? tempUser.USERNAME : tempStaff.STAFFCODE;
                ...
                targetStaff.BUSINESSUNITID = tempStaff.BUSINESSUNITID;
            }
            else //Insert a new staff record into the real staff table
            {
                targetStaff = new TBL_STAFF()
                {
                    ...
                    STAFFCODE = tempUser != null ? tempUser.USERNAME : tempStaff.STAFFCODE,
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved Staff")} '{tempStaff?.FIRSTNAME + " " + tempStaff?.LASTNAME}' {TranslateHelper.get("with staff code")} '{tempStaff?.STAFFCODE}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
            ...
            try
            {
                ...
                if (isUpdate == false && tempUser != null)
                {
        ...
        public async Task<bool> GoForBulkApproval(List<ApprovalViewModel> model, UserInfo userInfo)
        {
        ...
        private bool ApproveBulkStaff(int staffid, short approvalStatusId, UserInfo user)
        {
            ...
            if (temp != null)
            {
                if (entity != null)
                {
                    ...
                    entity.STAFFCODE = tempUser != null ? tempUser.USERNAME : temp.STAFFCODE;
                ...
                else
                {
                    entity = new TBL_STAFF()
                    {
                        ...
                        STAFFCODE = tempUser != null ? tempUser.USERNAME : temp.STAFFCODE,
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Approved Staff")} '{temp?.FIRSTNAME + " " + temp?.LASTNAME}' {TranslateHelper.get("with staff code")} '{temp?.STAFFCODE}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public bool AddTempStaff(StaffInfoViewModel staffModel)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("Staff Information already exist and is undergoing approval"));
            }
            ...
            //var dateValidation = DateTime.Today - DateTime.Today.AddYears(-18);
            //if (staffModel.DateOfBirth. <= dateValidation)
            //{
            //    throw new SecureException(TranslateHelper.get("Staff must not be less than 18 years of age"));
            //}
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = staffModel.createdBy,
                BRANCHID = (short)staffModel.BranchId,
                DETAIL = $"{TranslateHelper.get("Updated Staff Creation for")} '{staffModel?.StaffFullName}' {TranslateHelper.get("with code")} '{staffModel?.StaffCode}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = staffModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            workflow.Comment = TranslateHelper.get("New Staff Creation");
            workflow.OperationId = (int)OperationsEnum.StaffCreation;
            workflow.DeferredExecution = true; // false by default will call the internal SaveChanges()
            workflow.ExternalInitialization = true;
            workflow.LogActivity();
        ...
        public async Task<IEnumerable<StaffInfoViewModel>> GetStaffAwaitingApprovals(int staffId, int companyId)
        {
        ...
        public async Task<StaffDetailsModel> GetTempStaffDetail(int staffId)
        {
            //return GetTempStaffDetails().Where(x => x.StaffId == staffId).Single();

            return  await (from c in context.TBL_TEMP_STAFF
                    join br in context.TBL_BRANCH on c.BRANCHID equals br.BRANCHID
                    join coy in context.TBL_COMPANY on br.COMPANYID equals coy.COMPANYID
                    //join dept in context.TBL_DEPARTMENT on c.TBL_DEPARTMENT_UNIT.DEPARTMENTID equals dept.DEPARTMENTID
        ...
        public async Task<IEnumerable<simpleStaffModel>> GetStaffRelationshipManagerByStaffId(int staffId)
        {
            var data = await (from st in context.TBL_STAFF
        ...
        public async Task<IEnumerable<ApprovalStatusViewModel>> GetApprovalStatus()
        {
            return await(from ap in context.TBL_APPROVAL_STATUS
        ...
        public async Task<IEnumerable<simpleStaffModel>> SearchApprovers(int levelId, string searchQuery ="", int companyId=0)
        {
        ...
        public async Task<int> GetNextApprovalLvlRoleId(int roleId, int groupId)
        {
            //if (groupId == 0) groupId = 261; //ApprovalGroupEnum.Business
            var levels =  await context.TBL_APPROVAL_LEVEL.Where(l => l.GROUPID == groupId).ToListAsync();
        ...
        public async Task<staffBulkFeedbackViewModel> UploadStaffData(StaffDocumentViewModel model, byte[] file)
        {
            var staffInfo = new List<StaffInfoViewModel>();
            ...
            for (int j = range.FirstRowIndex; j <= range.LastRowIndex; j++)
            {
                ...
                for (int i = range.FirstColumnIndex; i <= range.LastColumnIndex; i++)
                {
                    ...
                    {
                        case "A":
                            try { staffRowData.username = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "B":
                            try { staffRowData.StaffCode = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "C":
                            try { staffRowData.staffRoleCode = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "D":
                            try { staffRowData.branchCode = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "E":
                            try { staffRowData.FirstName = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "F":
                            try { staffRowData.LastName = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "G":
                            try { staffRowData.MiddleName = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "H":
                            try { staffRowData.Email = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                        case "I":
                            try { staffRowData.supervisorsCode = cell.Value.ToString(); } catch (Exception e) { throw e; }
                            break;
                    }
                }
        ...
        public bool AddSimpleTempStaff(StaffInfoViewModel staffModel)
        {
            ...
            var user = new TBL_TEMP_PROFILE_USER()
            {
                ...
                ISLOCKED = true,
                FAILEDLOGONATTEMPT = 0,
                SECURITYQUESTION = TranslateHelper.get("What is my firstname"),
        ...
        public async Task<bool> UpdateStaffSignature(StaffDocumentViewModel model, int documentId)
        {
            var data = await documentsContext.TBL_MEDIA_STAFF_SIGNATURE.FindAsync(documentId);
            if (data == null)
            {
                return false;
            }
        ...
        public async Task<bool> UpdateSupervisor(SupervisorViewModel model)
        {
            ...
            if (pending.Any()) throw new SecureException(TranslateHelper.get("Staff is already undergoing approval"));
        ...
        public async Task<List<simpleStaffModel>> StaffReportingLine(int staffId, string staffCode, int companyId)
        {
            ...
            list = await context.TBL_STAFF.Where(x => x.SUPERVISOR_STAFFID == staffId && x.COMPANYID == companyId)
        ...
        public async Task<simpleStaffModel> StaffInformation(int staffId, string staffCode, int companyId)
        {
            if (staffCode != "null")
            {
                staffId = context.TBL_STAFF.Where(a => a.STAFFCODE.ToLower() == staffCode.Trim().ToLower() && a.COMPANYID == companyId).Select(a => a.STAFFID).FirstOrDefault();
            ...
            return await context.TBL_STAFF.Where(x => x.STAFFID == staffId && x.COMPANYID == companyId)
                  .Select(x => new simpleStaffModel
                  {
                      staffCode = x.STAFFCODE,
                      firstName = x.FIRSTNAME + TranslateHelper.get(" ") + x.LASTNAME + TranslateHelper.get(" ") + x.MIDDLENAME,
                      branchCode = context.TBL_BRANCH.Where(a => a.BRANCHID == x.BRANCHID).Select(a => a.BRANCHCODE).FirstOrDefault(),
                      branchName = context.TBL_BRANCH.Where(a => a.BRANCHID == x.BRANCHID).Select(a => a.BRANCHNAME).FirstOrDefault(),
                      email = x.EMAIL
                  }).FirstOrDefaultAsync();
        ...
        public StaffMISDetailsModel  StaffMIS(int staffId, string staffCode)
        {
            ...
            //    return null;
            //}
            return new StaffMISDetailsModel(); 
        }

        public async Task<IEnumerable<simpleStaffModel>> GetSearchedStaff(string search)
        {

            var branches =  await (from x in context.TBL_STAFF
                           ...
                           {
                               staffId = x.STAFFID,
                               firstName = x.FIRSTNAME + TranslateHelper.get(" ") + x.LASTNAME,
                               staffCode = x.STAFFCODE,
                               branchName =  context.TBL_BRANCH.Where(a=>a.BRANCHID==x.BRANCHID).Select(a=>a.BRANCHNAME + TranslateHelper.get(" - ") + a.BRANCHCODE).FirstOrDefault()
