...
namespace FintrakBanking.Repositories.CRMS
{
    public class CRMSRegulatories : ICRMSRegulatories
    {
        ...
        public async Task<string> GetCRMSCode(CRMSViewModel param)
        {

            String response = string.Empty ;
            if (param.isLms)
            {
                var loan = await context.TBL_LMSR_APPLICATION.Where(x => x.LOANAPPLICATIONID == param.loanId).Select(x => x).FirstOrDefaultAsync();
                if (loan != null)
                {
            ...
            {
                var loan = await context.TBL_LOAN_BOOKING_REQUEST.Where(x => x.LOAN_BOOKING_REQUESTID == param.loanId).Select(x => x).FirstOrDefaultAsync();
                if (loan != null)
                {
                    response = loan.CRMSCODE;
                }
        ...
        public async Task<bool> ResetCrmsCode(CRMSViewModel param)
        {
            bool response = false;
            if (param.isLms)
            {
        ...
        public async Task<string> AddCRMSCodeAsync(CRMSViewModel param)
        {
            ...
            if (param.isLms)
            {
                using (var trans = context.Database.BeginTransaction())
                {
                    ...
                    if (loan == null)
                        throw new ConditionNotMetException(TranslateHelper.get("This LMS Request does not exist"));

                    
                    param.crmsCode = param.crmsCode.Trim();
                    var codeExist = context.TBL_LMSR_APPLICATION.Where(x => x.CRMSCODE == param.crmsCode).Any();
                    if (codeExist == true)
                        throw new ConditionNotMetException(TranslateHelper.get($"This CRMS {param.crmsCode} code has aleady been Assigned, Kindly Provide Another Code.."));

                    if (!(string.IsNullOrEmpty(loan.CRMSCODE)) && (loan.CRMSVALIDATED ?? false))
                    {
                        throw new ConditionNotMetException(TranslateHelper.get($"CRMS code has aleady been captured for this request! on {loan.CRMSDATE}"));
                    }
                    ...
                    if (finishingJob.Any())
                    {
                        ...
                        response = await ForwardApplication(approvalModel);
                        ...
                        {
                            trans.Rollback();
                            throw new ConditionNotMetException(TranslateHelper.get("An error occured while trying to Capture CRMS CODE!"));
                        }
                    }
                    else
                    {
                        ...
                        {
                            trans.Commit();
                            return TranslateHelper.get("CRMS Code Captured");
                        }
                        trans.Rollback();
                        throw new ConditionNotMetException(TranslateHelper.get("An error occured while trying to Capture CRMS CODE!"));
                    }
            ...
            else
            {
                ...
                if (loan == null)
                    throw new ConditionNotMetException(TranslateHelper.get("This Booking Request does not exist"));
                ...
                if (finishingJob.Any())
                {
                    ...
                    response = await drawdown.GoForBookingRequestApproval(approvalModel, loan.LOAN_BOOKING_REQUESTID);
                    return response.responseMessage;
                }
                else
                {
                    ...
                    return TranslateHelper.get("Moved to Inputter");
        ...
        private async Task<int> GetFirstAvailmentLevelId(int operationId)
        {
        ...
        private async Task<List<CRMSTemplateViewModel>> GetFee(CRMSViewModel param)
        {
            var term = (from x in context.TBL_LOAN_APPLICATION_DETAIL
                       ...
                       }).OrderBy(a => a.LOANAPPLICATIONDETAILID).ToListAsync();
            ...
            return term.Result;//.Union(OD).Union(contingent).OrderBy(x => x.ACCOUNT).ToList();
        }
        private async Task<List<CRMSTemplateViewModel>> GetDirectors(CRMSViewModel param)
        {
            var term = (from x in context.TBL_LOAN_APPLICATION_DETAIL
                           //join c in context.TBL_CASA on x.CASAACCOUNTID equals c.CASAACCOUNTID
                           //join b in context.TBL_CUSTOMER on x.CUSTOMERID equals b.CUSTOMERID
        ...
        public async Task<List<CRMSRegulatoryViewModel>> GetAllLoansForCRMS(CRMSViewModel param)
        {
        ...
        public async Task<List<LoansCount>> LoanCountsByLegalStatus(List<CRMSRegulatoryViewModel> loans)
        {
        ...
        private async Task<CRMSRecord> GenerateCRMS300Template(List<CRMSTemplateViewModel> loanInput, List<CRMSViewModel> paramx, int forSingle = 0)
        {
            Byte[] fileBytes = null;
            ...
            if (loanInput != null)
            {
                using (ExcelPackage pck = new ExcelPackage())
                {
                    ...
                    foreach(var param in paramx)
                    {
                        ...
                        if (forSingle == 1)
                        {
                            output = await GetFee(param);
                            output = (List<CRMSTemplateViewModel>)output.Where(a => a.LOANAPPLICATIONDETAILID == param.loanId);
                        }
                        else
                        {
                            output = await GetFee(param);
                            output = (List<CRMSTemplateViewModel>)output.Where(x => DbFunctions.TruncateTime(x.DATETIMECREATED) >= DbFunctions.TruncateTime(param.startDate)
                        ...
                        if(rowCount <= 1)
                        {
                            ...
                            if (forSingle == 1 )
                            {
                                 directors = await GetDirectors(param);
                                directors = (List<CRMSTemplateViewModel>)directors.Where(a => a.LOANAPPLICATIONDETAILID == param.loanId).ToList();
                            }
                            else
                            {
                                directors = await GetDirectors(param);
                                directors = (List<CRMSTemplateViewModel>)directors.Where(x => DbFunctions.TruncateTime(x.DATETIMECREATED) >= DbFunctions.TruncateTime(param.startDate)
                             && DbFunctions.TruncateTime(x.DATETIMECREATED) <= DbFunctions.TruncateTime(param.endDate)).ToList();
                            }
        ...
        private async Task<CRMSRecord> GenerateCRMS400BTemplate(List<CRMSTemplateViewModel> loanInput, CRMSViewModel param)
        {
            Byte[] fileBytes = null;
            CRMSRecord excel = new CRMSRecord();
            if (loanInput != null)
            {
                using (ExcelPackage pck = new ExcelPackage())
                {
                    ...
                    var director = await GetDirectors400B(param);
                    ws2.Cells[1, 1].Value = "ACCOUNT";
                    ...
                    var fee = await GetFee400B(param);
        ...
        private async Task<List<CRMSTemplateViewModel>> GetFee400B(CRMSViewModel param)
        {
        ...
        private async Task<List<CRMSTemplateViewModel>> GetDirectors400B(CRMSViewModel param)
        {
            var operationRestructure = CommonHelpers.GetRestructureOperations();
        ...
        private async Task<IQueryable<CRMSTemplateViewModel>> GenerateCRMSReport400B(CRMSViewModel param)
        {
        ...
        private async Task<CRMSRecord> GenerateCRMS400CTemplate(List<CRMSTemplateViewModel> loanInput, CRMSViewModel param)
        {
            Byte[] fileBytes = null;
            CRMSRecord excel = new CRMSRecord();
            if (loanInput != null)
        ...
        private async Task<CRMSRecord> GenerateCRMS400CTemplate(CRMSViewModel param)
        {
            var result = await GenerateCRMSReport400C(param);
            ...
            if (result == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For T300"));

            return await GenerateCRMS400CTemplate(result.ToList(), param);
        }
        private async Task<IQueryable<CRMSTemplateViewModel>> GenerateCRMSReport400C(CRMSViewModel param)
        {
            // int[] crmsRegulatoryIds = { (int)CRMSRegulatory.Government, (int)CRMSRegulatory.Parastatals_MDA };
        ...
        private async Task<CRMSRecord> GenerateCRMS400A(CRMSViewModel param)
        {
            Byte[] fileBytes = null;
        ...
        private async Task<CRMSRecord> GenerateCRMS600Template(List<CRMSTemplateViewModel> loanInput)
        {
        ...
        private async Task<CRMSRecord> GenerateCRMS200Template(List<CRMSTemplateViewModel> loanInput)
        {
        ...
        private async Task<List<CRMSTemplateViewModel>> GenerateCRMSReport(CRMSViewModel param, int forSingle = 0)
        {
            // int[] crmsRegulatoryIds = { (int)CRMSRegulatory.Government, (int)CRMSRegulatory.Parastatals_MDA };
        ...
        private async Task<CRMSRecord> GenerateCRMS400BTemplate(CRMSViewModel param)
        {


            var result = await GenerateCRMSReport400B(param);
            result = result.Where(x => x.CRMSLEGALSTATUSID != (int)CRMSRegulatory.Government && x.CRMSLEGALSTATUSID != (int)CRMSRegulatory.Parastatals_MDA);
            if (result == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For T300"));

            return await GenerateCRMS400BTemplate(result.ToList(), param);

        }
        private async Task<CRMSRecord> GenerateCRMS300TemplateByLoanAppId(List<CRMSViewModel> paramx)
        {
            int forSingle = 1;
            List<CRMSTemplateViewModel> result = new List<CRMSTemplateViewModel>();
            foreach (var param in paramx)
            {
                var record = await GenerateCRMSReport (param, forSingle);
            ...
            return await GenerateCRMS300Template(result.ToList(), paramx, forSingle);
        }
        private async Task<CRMSRecord> GenerateCRMS300TemplateAsync(List<CRMSViewModel> paramx)
        {
            int forSingle = 0;
            List<CRMSTemplateViewModel> result = new List<CRMSTemplateViewModel>();
            foreach (var param in paramx)
            {
                var record = GenerateCRMSReport(param, forSingle).Result;
        ...
        private async Task<CRMSRecord> GenerateCRMS100TemplateByLoanAppId(CRMSViewModel param)
        {
            int forSingle = 1;
            var record = await GenerateCRMSReport(param, forSingle);
            ...
            result = result.Where(x => x.CRMSLEGALSTATUSID == (int)CRMSRegulatory.Government).ToList();
            if (result == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For T100"));

            return GenerateCRMS100Template(result.ToList(), param, forSingle);
        }
        private async Task<CRMSRecord> GenerateCRMS100Template(CRMSViewModel param)
        {
            int forSingle = 0;
            var record = await GenerateCRMSReport(param, forSingle);
        ...
        private async Task<CRMSRecord> GenerateCRMS200TemplateByLoanAppId(CRMSViewModel param)
        {
            int forSingle = 1;
            var record = await GenerateCRMSReport (param, forSingle);
            var result = record.Where(a => a.LOANAPPLICATIONDETAILID == param.loanId).ToList();
            ...
            if (result == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For T200"));

            return await GenerateCRMS200Template(result.ToList());
        }
        private async Task<CRMSRecord> GenerateCRMS200Template(CRMSViewModel param)
        {
            int forSingle = 0;
            var record = await GenerateCRMSReport(param, forSingle);
            ...
            return await GenerateCRMS200Template(result);
        }

        private async Task<CRMSRecord> GenerateCRMS600TemplateByLoanAppIdAsync(List<CRMSViewModel> paramx)
        {
            int forSingle = 1;
            List<CRMSTemplateViewModel> result = new List<CRMSTemplateViewModel>();
            foreach (var param in paramx)
            ...
            result = result.Where(x => x.CRMSLEGALSTATUSID != (int)CRMSRegulatory.Government && x.CRMSLEGALSTATUSID != (int)CRMSRegulatory.Parastatals_MDA).ToList();
            if (result == null)
                throw new ConditionNotMetException(TranslateHelper.get("Record Not Found For T600"));

            return await GenerateCRMS600Template(result.ToList());
        }
        private async Task<CRMSRecord> GenerateCRMS600Template(CRMSViewModel param)
        {
            int forSingle = 0;
            var record = await GenerateCRMSReport(param, forSingle);
        ...
        public async Task<CRMSRecord> GenerateCBNReport(List<CRMSViewModel> paramx)
        {
            var param = paramx.FirstOrDefault();
            if (param.templateTypeId == (int)CRMSTemplate.Template100)
            {
                return await GenerateCRMS100Template(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template200)
            {
                return await GenerateCRMS200Template(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template300)
            {
                return await GenerateCRMS300TemplateAsync(paramx);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template600)
            {
                return await GenerateCRMS600Template(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template400A)
            {
                return await GenerateCRMS400A(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template400B)
            {
                return await GenerateCRMS400BTemplate(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template400C)
            {
                return await GenerateCRMS400CTemplate(param);
            }
            return new CRMSRecord();
        }

        public async Task<CRMSRecord> GenerateCBNReportByLoanAppId(List<CRMSViewModel> paramx)
        {
            var param = paramx.FirstOrDefault();
            if (param.templateTypeId == (int)CRMSTemplate.Template100)
            {
                return await GenerateCRMS100TemplateByLoanAppId(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template200)
            {
                return await GenerateCRMS200TemplateByLoanAppId(param);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template300)
            {
                return await GenerateCRMS300TemplateByLoanAppId(paramx);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template600)
            {
                return await GenerateCRMS600TemplateByLoanAppIdAsync(paramx);
            }
            else if (param.templateTypeId == (int)CRMSTemplate.Template400A)
            {
                return await GenerateCRMS400A(param);
        ...
        public async Task<CRMSRecord> GenerateBatchPosting(DateRange model)
        {
            var loanInput = await finacleIntegration.GetBatchPostingDetailSearch(model.startDate, model.endDate, model.status);
        ...
        public async Task<bool> GenerateCRMSCodes(List<int> loanBookingRequestIds, UserViewModel model)
        {
            int ctr = 1;
            try
            {
                ...
                {
                    var crmsModels = await GenerateCRMSReport(requestId, model.companyId, ctr);
                    ctr = ctr + 1;
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Automatic CRMS Code Generation Failed." + "Core Banking API error: " + e.Message));
            }
        ...
        public async Task<bool> GenerateCRMSCode(int loanBookingRequestId, UserViewModel model)
        {
            int ctr = 1;
            try
            {
                var crmsModel =  GenerateCRMSReport(loanBookingRequestId, model.companyId, ctr);
                ...
                var loanSystemTypeId = crmsModel.Result.FirstOrDefault().loanSystemTypeId;

                var result =  integration.FetchCBNCRMSCode(crmsModel.Result.FirstOrDefault(), loanSystemTypeId);
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Automatic CRMS Code Generation Failed." + "Core Banking API error: "+ " Please Try Again or Choose to add the CRMS code Manually"));
            }
            return true;
        }

        private async Task<List<CRMSCodeGeneration>> GenerateCRMSReport(int loanBookingRequestId, int companyId, int serial )
        {
