...
namespace FintrakBanking.Repositories.Credit
{
    public class CreditCommonRepository
    {
        ...
        public async void LoadCustomerTurnover(int applicationId, List<int> customerIds, int staffId, CustomerTurnoverViewModel model, bool isLms = false) 
        {
            ...
            DateTime startDate = endDate.AddDays(-365);//DateTime.Now ;// currect this to be done.
            //DateTime.Now;
            ...
            foreach (var customer in customers)
            {
                if(customer.ISPROSPECT == false)
                {
                    ...
                    var accountNumber = casa.FirstOrDefault().PRODUCTACCOUNTNUMBER;
                    ...
                    apiTransactions = integration.GetCustomerAccountTurnover(accountNumber, startDate, endDate);

                    if (apiTransactions == null) {
                        throw new APIErrorException(TranslateHelper.get("Core Banking API Error, No Record Found!"));
                    }

                    if (apiTransactions.Count() == 0)
                    {
                        throw new APIErrorException(TranslateHelper.get("Core Banking API Info, No Record Found for the Customer!"));
                    }
                    //if (apiTransactions[0].min_Credit_Balance == 0 && apiTransactions[0].max_Debit_Balance == 0 && apiTransactions[0].debit_Turnover == 0)
                    //{
                    //    throw new APIErrorException(TranslateHelper.get("Core Banking API Info, No Record Found for the Customer!"));
                    //}
                    ...
                    foreach (var transaction in apiTransactions)
                    {
                        context.TBL_LOAN_APPLICATION_TRANS.Add(new TBL_LOAN_APPLICATION_TRANS
                        {
                            ...
                            PERIOD = transaction.month == null ? "" : new DateTime(2023, (int)transaction.month, 1).ToString("MMM", CultureInfo.InvariantCulture).ToUpper(),
        ...
        public async void LoadCustomerRatios(int applicationId, List<int> customerIds, int staffId) 
        {
            bool isGroup = false;
        ...
        public async void LoadCustomerGroupRatios(int applicationId, List<int> customerIds, int staffId)
        {
            bool isGroup = false;
        ...
        public async void GetCorporateCustomerRating(int applicationId, List<int> customerIds, int staffId)
        {
        ...
        public async void GetAutoLoansRetail(int loanApplicationDetailId, int customerId, int staffId)
        {
            //bool isGroup = false;
        ...
        public async void GetPersonalLoansRetail(int loanApplicationDetailId, int customerId, int staffId)
        {
            //bool isGroup = false;
        ...
        public async void GetCreditCardsRetail(int loanApplicationDetailId, int customerId, int staffId)
        {
            //bool isGroup = false;
