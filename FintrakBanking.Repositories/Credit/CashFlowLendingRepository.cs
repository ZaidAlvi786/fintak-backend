...
namespace FintrakBanking.Repositories.Credit
{
    public class CashFlowLendingRepository :ICashFlowLendingRepository
    {
        ...
        public async Task<APIResponse> AddCustomer(IncomingCustomerViewModels model)
        {
           ...
           {
                if (model.individualCustomerInformation.customerCode == string.Empty) { return fireResponse(TranslateHelper.get("Missing Customer Number"), "99", ""); }

                var businessunit = await context.TBL_PROFILE_BUSINESS_UNIT.Where(O => O.BUSINESSUNITSHORTCODE == model.individualCustomerInformation.businessUnit.ToUpper()).FirstOrDefaultAsync();
                if (businessunit == null) { return fireResponse(TranslateHelper.get("Customer's business unit does not exist in Fintrak!"), "99", ""); }

                var existingCustomer = await context.TBL_CUSTOMER.Where(O => O.CUSTOMERCODE == model.individualCustomerInformation.customerCode).FirstOrDefaultAsync();
                if (existingCustomer != null) { return fireResponse(TranslateHelper.get("Customer already exist in Fintrak!"), "77", ""); }

                model.individualCustomerInformation.businessUnitId = businessunit.BUSINESSUNITID;
                return await AddIndividualCustomer(model);
           }
           else if (model.customerType == "2")
           {
                if (model.corporateCustomerInformation.customerCode == string.Empty) { return fireResponse(TranslateHelper.get("Missing Customer Number"), "99", model.request_Id); }

                var businessunit = await context.TBL_PROFILE_BUSINESS_UNIT.Where(O => O.BUSINESSUNITSHORTCODE == model.corporateCustomerInformation.businessUnit.ToUpper()).FirstOrDefaultAsync();
                if (businessunit == null) { return fireResponse(TranslateHelper.get("Customer's business unit does not exist in Fintrak!"), "99", ""); }

                var existingCustomer = await context.TBL_CUSTOMER.Where(O => O.CUSTOMERCODE == model.corporateCustomerInformation.customerCode).FirstOrDefaultAsync();
                if (existingCustomer != null) { return fireResponse(TranslateHelper.get("Customer already exist in Fintrak!"), "77", ""); }

                model.corporateCustomerInformation.businessUnitId = businessunit.BUSINESSUNITID;
                return await AddCorporateCustomer(model);
           }
           else { return fireResponse(TranslateHelper.get("Unknown Customer Type"), "99", ""); }
        }

        private async Task<APIResponse> AddIndividualCustomer(IncomingCustomerViewModels model)
        {
            APIResponse response = new APIResponse();
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                List<string> mStatus = new List<string> { "single", "married", "divorced", "widowed", "1", "2", "3", "4"};
                ...
                if (model.individualCustomerInformation.maritalStatus == "0") { fireResponse("c", "99", ""); }
                ...
                var accountOfficer = await context.TBL_STAFF.Where(x => x.STAFFCODE == model.accountOfficerStaffCode).FirstOrDefaultAsync();
                if (accountOfficer == null)
                {
                    accountOfficer = await context.TBL_STAFF.Where(x => x.MISCODE == model.accountOfficerStaffCode).FirstOrDefaultAsync();
                    if (accountOfficer == null) return fireResponse(TranslateHelper.get("Account officer does not exist in Fintrak Credit360"), "99", "");
                }
                ...
                var subsector = await context.TBL_SUB_SECTOR.Where(x => x.CODE == model.individualCustomerInformation.subSector).FirstOrDefaultAsync();
                if (subsector == null) return fireResponse(TranslateHelper.get("Sub-Sector does not exist in Fintrak Credit360"), "99", "");
                ...
                if (await saveIndividualCustomerInformation(model))
                {
                    customer.UpdateCustomerCollateralId(model.individualCustomerInformation.customerCode);
                    //SaveLoanDocument();
                    trans.Commit();
                    return fireResponse(TranslateHelper.get("Success"), "00", model.request_Id);
                }
                else { return fireResponse(TranslateHelper.get("Unresolved error: could not save customer information"), "99", ""); }
            }
        }

        private async Task<APIResponse> AddCorporateCustomer(IncomingCustomerViewModels model)
        {
            APIResponse response = new APIResponse();
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                if (model.corporateCustomerInformation.subSector == null) return fireResponse(TranslateHelper.get("Missing Sub-Sector Code"), "99", "");

                var subsector = await context.TBL_SUB_SECTOR.Where(x => x.CODE == model.corporateCustomerInformation.subSector).FirstOrDefaultAsync();
                if (subsector == null) return fireResponse(TranslateHelper.get("Sub-Sector does not exist in Fintrak Credit360"), "99", "");

                var sector = await context.TBL_SECTOR.Where(x => x.SECTORID == subsector.SECTORID).FirstOrDefaultAsync();
                model.sectorId = sector.SECTORID;
                model.subsectorId = subsector.SUBSECTORID;
                ...
                if (await saveCorporateCustomerInformation(model))
                {
        ...
        private async Task<bool> saveIndividualCustomerInformation(IncomingCustomerViewModels entity)
        {
            try
            {
                ...
                if (entity.customerAddresses != null)
                {
                    ...
                    {
                        var state = await context.TBL_STATE.Where(O => O.STATENAME.ToLower() == address.state.ToLower()).FirstOrDefaultAsync();

                        customer.TBL_CUSTOMER_ADDRESS.Add(new TBL_CUSTOMER_ADDRESS()
                        {
                            ADDRESS = address.address,
                            CUSTOMERID = customer.CUSTOMERID,
                            STATEID = state != null ? state.STATEID : context.TBL_STATE.FirstOrDefault().STATEID,
                            CITYID = 1,
                            ADDRESSTYPEID = 1,
                            ACTIVE = true,
                ...
                if (entity.companyDirectors[0].firstName != null)
                {
                    foreach (var director in entity.companyDirectors)
                    {
                        if (model.customerType.ToLower() == "individual")
                        {
                        ...
                        DateTime.TryParseExact(director.dateOfBirth, "dd-MM-yyyy", CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime directorDateOfBirth);
        ...
        private async Task<bool> saveCorporateCustomerInformation(IncomingCustomerViewModels model)
        {
            ApiCustomerBusinessDetailsViewModel corporateDetails = model.corporateCustomerInformation;
            var crmsType = await context.TBL_CRMS_REGULATORY.Where(x => x.CODE == corporateDetails.crmsCompanySize).FirstOrDefaultAsync();
            ...
            if(corporateDetails.corporateName == null) { corporateDetails.corporateName = model.individualCustomerInformation.firstName; }
        ...
        public async Task<bool> ValidateCustomerCode(string customerCode)
        {
            ...
            var data = await (from a in context.TBL_CUSTOMER where a.CUSTOMERCODE == customerCode select a).ToListAsync();
            if (data.Count > 0)
            {
                itemExist = true;
            }
        ...
        public async Task<APIResponse> submitRequest(CflLoanApplication model)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                APIResponse response = new APIResponse();
                ...
                {
                    if (model.accountOfficerStaffCode == string.Empty) return fireResponse(TranslateHelper.get("Missing account officer code"), "99", "");

                    var accountOfficer1 = await context.TBL_STAFF.Where(x => x.STAFFCODE == model.accountOfficerStaffCode).FirstOrDefaultAsync();
                    if (accountOfficer1 == null) 
                    { 
                        accountOfficer1 = await context.TBL_STAFF.Where(x => x.MISCODE == model.accountOfficerStaffCode).FirstOrDefaultAsync();
                        if (accountOfficer1 == null) return fireResponse(TranslateHelper.get("Account officer does not exist in Fintrak Credit360"), "99", ""); 
                    }

                    var application = await context.TBL_LOAN_APPLICATION.Where(O => O.APPLICATIONREFERENCENUMBER == model.applicationReferenceNumber).FirstOrDefaultAsync();
                    if (application == null) { return fireResponse(TranslateHelper.get("Application does not exist in Fintrak Credit360"), "99", ""); }

                    if (model.creditBureauReport == null || model.creditBureauReport.Count <= 0) //{ return fireResponse(TranslateHelper.get("Missing Credit Bureau Report"), "99", ""); }

                        if (model.creditBureauReport == null || model.creditBureauReport.Count < 3) //{ return fireResponse(TranslateHelper.get("At least 3 Credit Reports are required"), "99", ""); }
                    ...
                    return fireResponse(TranslateHelper.get("Offer Letter successfully saved"), "00", application.APIREQUESTID);
                }

                var product = await context.TBL_PRODUCT.Where(x => x.PRODUCTCODE == model.productCode).FirstOrDefaultAsync();

                if (product == null) { return fireResponse(TranslateHelper.get("Product Code does not exist"), "99", ""); }
                if (model.requestId == null) { return fireResponse(TranslateHelper.get("Missing application unique indentifier"), "99", ""); }

                var subSector = await context.TBL_SUB_SECTOR.Where(x => x.CODE == model.subSectorCode).FirstOrDefaultAsync();
                if (subSector == null) { return fireResponse(TranslateHelper.get("Sub Sector Code does not exist in Fintrak Credit360"), "99", ""); }

                //var sector = context.TBL_SECTOR.Where(x => x.CODE == subSector.CODE).FirstOrDefault();
                var sector = await context.TBL_SECTOR.Where(x => x.CODE == model.sectorCode).FirstOrDefaultAsync();
                if (sector == null) { return fireResponse(TranslateHelper.get("Sector Code does not exist in Fintrak Credit360"), "99", ""); }

                var currency = await context.TBL_CURRENCY.Where(x => x.CURRENCYCODE == model.currencyCode || x.CURRENCYCODE == "NGN").FirstOrDefaultAsync();
                if (currency == null) return fireResponse(TranslateHelper.get("Missing currency code"), "99", "");
                ...
                var relationshipManager = await context.TBL_STAFF.Where(x => x.STAFFCODE == model.relationshipManagerStaffCode).FirstOrDefaultAsync();

                var customer = await context.TBL_CUSTOMER.Where(x => x.CUSTOMERCODE == model.customerCode).FirstOrDefaultAsync();
                if (customer == null) return fireResponse(TranslateHelper.get("This customer is not profiled on Fintrak Credit360 application"), "99", "");

                //var casa = context.TBL_CASA.Where(x => x.PRODUCTACCOUNTNUMBER == model.settlementAccount).FirstOrDefault();
                var casa = await context.TBL_CASA.Where(x => x.PRODUCTACCOUNTNUMBER == model.settlementAccount && x.CUSTOMERID == customer.CUSTOMERID).FirstOrDefaultAsync();

                LoanApplicationViewModel loanApp = new LoanApplicationViewModel();
                loanApp.customerId = customer.CUSTOMERID;
                loanApp.proposedTenor = Convert.ToInt32(model.tenor);

                var currentDate = genSetup.GetApplicationDate();
                var endDate = currentDate.AddDays(loanApp.proposedTenor);
                ...
                if (context.TBL_LOAN_APPLICATION.Any(x => x.APIREQUESTID == model.requestId && x.DELETED != true))
                {
                    if (model.callStatusCode == "01")
                    {
                        ...
                        if (await UpdateLoanApplicationDetail(loanApp))
                        {
                            SaveLoanDocument(model);
                    ...
                    else { return fireResponse(TranslateHelper.get("New application request Id already exist"), "99", ""); }
                }
                ...
                if (loanExist == true) return fireResponse(TranslateHelper.get("This loan application has already been saved"), "99", "");

                if (model.creditBureauReport == null || model.creditBureauReport.Count <= 0) { } //{ return fireResponse(TranslateHelper.get("Missing Credit Bureau Report"), "99", ""); }

                if (model.creditBureauReport == null || model.creditBureauReport.Count < 1) { } //{ return fireResponse(TranslateHelper.get("At least 3 Credit Reports are required"), "99", ""); }

                response.applicationReferenceNumber = await AddLoanApplication(loanApp, model.requestId);
                model.createdBy = accountOfficer.STAFFID;
                ...
                response.requestId = model.requestId;
                response.responseMessage = "loan application added successfully ";
                SaveCashflowRequestToApiLog(model, response);
                trans.Commit();
                return response;
            }
        ...
        private void SaveCashflowRequestToApiLog(CflLoanApplication request, APIResponse response)
        {
            ...
            context.TBL_CUSTOM_API_LOGS.Add(new TBL_CUSTOM_API_LOGS()
            {
                ...
                REQUESTMESSAGE = jsonRequest + ' ' + request.base64String,
        ...
        public int SaveCashflowRequestToApiLog2(CflLoanApplication request)
        {
            JavaScriptSerializer js = new JavaScriptSerializer();
            //request.creditBureauReport = null;
            //request.loanApplicationFiles = null;
            ...
            context.TBL_CUSTOM_API_LOGS.Add(new TBL_CUSTOM_API_LOGS()
            {
                ...
                REQUESTDATETIME = DateTime.Now,
                RESPONSEDATETIME = DateTime.Now,
                REQUESTMESSAGE = jsonRequest + ' ' + "this is base 64 " + request.base64String,
        ...
        private void SaveLoanDocument(CflLoanApplication model)
        {
            try
            {
                ...
                if (staffId == null)
                {
                    staffId = context.TBL_STAFF.Where(x => x.MISCODE == model.accountOfficerStaffCode).FirstOrDefault();
                    if (staffId == null) throw new SecureException(TranslateHelper.get("Account officer does not exist in Fintrak Credit360"));
                }
                ...
                model.base64String = "empty";
                if(model.isInsurance == true)
                {
                    model.operationId = (int)OperationsEnum.CashFlowDocumentReview;
                }
                else
                {
                    model.operationId = (int)OperationsEnum.CreditAppraisal;
                }
                ...
                foreach (var loanFile in model.loanApplicationFiles)
                {
                    ...
                    try
                    {
                        fileBytes = Convert.FromBase64String(loanFile.fileData);
                        var data = Newtonsoft.Json.JsonConvert.SerializeObject(fileBytes);
                        dats = Convert.FromBase64String(loanFile.fileData);//loanFile.fileData.ToByteArray();
                        //System.IO.File.WriteAllText(@"C:\inetpub\wwwroot\Logs\docLog.txt", data);
                        if (fileBytes.Length <= 0 ) { throw new SecureException(TranslateHelper.get("One or more file data is Invalid.")); }
                        if(dats.Length > 0 ) { model.base64String = loanFile.fileData; }
                    ...
                    var document = new TBL_DOCUMENT_UPLOAD()
                    {
                        ...
                        //FILESIZE = Convert.ToInt32(loanFile.fileData.Length),
                        //FILEDATA = Convert.FromBase64String(loanFile.fileData),
                        //FILESIZE = fileBytes.Length,
                        FILESIZE = dats.Length,
                        //FILEDATA = fileBytes,
                        FILEDATA = dats,
                ...
                if (loanApplication != null) loanApplication.ISINSURANCE = model.isInsurance;
                context.SaveChanges();
        ...
        public async Task<string> AddLoanApplication(LoanApplicationViewModel loan, string apiRequestId)
        {
           // ValidateLoanApplicationLimits(loan);
            var additionalAmount = loan.LoanApplicationDetail.Sum(x => x.exchangeAmount);
            var savedDetails = await context.TBL_LOAN_APPLICATION_DETAIL.Where(c => c.LOANAPPLICATIONID == loan.loanApplicationId && c.DELETED == false).ToListAsync();
            ...
            if (loan.relationshipOfficerId != 0)
            {
                ...
                if (validation.maximumAllowedLimit > 0) if ((cumulativeSum + additionalAmount) > (decimal)validation.limit) fireResponse($"{TranslateHelper.get("RM Limit Exceeded")}. {TranslateHelper.get("The limit of this RM is")} {validation.limit}", "99","");
            }
            ...
            {
                if (await UpdateLoanApplicationDetail(loan)) return loan.applicationReferenceNumber;
                else return null;
            }

            var loanData = await context.TBL_LOAN_APPLICATION.FirstOrDefaultAsync(l => l.APPLICATIONREFERENCENUMBER == loan.applicationReferenceNumber && l.DELETED == false);
            ...
            if (savedDetails.Count() == 0 || loan.isNewApplication)
            {
                if (loanData != null)
                {
                    ...
                    loanData.TOTALEXPOSUREAMOUNT = cumulativeSum + additionalAmount + await GetCustomerTotalOutstandingBalance((int)loan.customerId);
                ...
                if (loanData == null) 
                {
                    ...
                    var app = await AddloanApplicationSub (loan, apiRequestId);

                    if (await AddLoanApplicationDetail(loan, app))
                    {
                        await SubmitLoanApplicationForCam(app.LOANAPPLICATIONID, loan.createdBy, 0);
                        context.SaveChanges();      
                       
                        return app.APPLICATIONREFERENCENUMBER;
                    }
                }
                //return loan.applicationReferenceNumber;
            }
            else
            {
        ...
        private async Task<bool> AddLoanApplicationDetail(LoanApplicationViewModel loan, TBL_LOAN_APPLICATION app)//List<LoanApplicationDetailViewModel> entity, int createdBy)
        {
            try
            {
                ...
                var data = new TBL_LOAN_APPLICATION_DETAIL
                {
                    ...
                    APPROVEDPRODUCTID = (short)loan.productId,
                    ...
                    PROPOSEDINTERESTRATE = (double)app.INTERESTRATE,
                    PROPOSEDPRODUCTID = (short)loan.productId,
        ...
        private async Task<TBL_LOAN_APPLICATION> AddloanApplicationSub(LoanApplicationViewModel loan, string apiRequestId)
        {
            try
            {
                ...
                var dat = await context.TBL_PRODUCT_CLASS.Where(c => c.PRODUCTCLASSID == loan.productClassId).FirstOrDefaultAsync();
                if (dat != null)
                {
                ...
                decimal totalAmount = await GetCustomerTotalOutstandingBalance((int)loan.customerId) + (loan.LoanApplicationDetail.Sum(x => x.exchangeAmount));
        ...
        private async Task<bool> UpdateLoanApplicationDetail(LoanApplicationViewModel update)
        {
            UpdateLoanApplication(update); 

            var detail = await context.TBL_LOAN_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.LOANAPPLICATIONDETAILID == update.loanApplicationDetailId);
            ...
            if (detail == null) fireResponse(TranslateHelper.get("Sequence contain not single! " )+ 0, "99", "");
            ...
            var productClassId = context.TBL_PRODUCT.Where(p=>p.PRODUCTID == detail.PROPOSEDPRODUCTID).FirstOrDefault().PRODUCTCLASSID;
        ...
        private async void UpdateLoanApplication(LoanApplicationViewModel loan)
        {
        ...
        private void ValidateLoanApplicationLimits(LoanApplicationViewModel application)
        {
            ...
            try
            {
                ...
                {
                    fireResponse(TranslateHelper.get("Product Limit exceeded!"),"99","");
                }
            }
            catch (Exception ex) { throw ex; }
            ...
            {
                fireResponse(TranslateHelper.get("Company Limit Exceeded"),"99","");
            }
            ...
            {
                fireResponse(TranslateHelper.get("Insider Limit Exceeded"), "99","");
            }
            ...
            if (limitValidation.IsDirectorRelatedGroup(application.customerGroupId) || limitValidation.CustomerIsDirector(application.customerId))
            {
                ...
                {
                    fireResponse(TranslateHelper.get("Director Limit Exceeded"),"99","");
                }
            ...
            {
                fireResponse(TranslateHelper.get("Single Obligor Limit Exceeded"),"99","");
            }
        ...
        private async Task<short> SubmitLoanApplicationForCam(int applicationId, int staffId, int checkListIndex)
        {
        ...
        public async Task<int> AddLoanDocument(LoanDocumentViewModel model, byte[] file)
        {
            FinTrakBankingDocumentsContext docContext = new  FinTrakBankingDocumentsContext();
            ...
            return await docContext.SaveChangesAsync() == 0 ? 1 : 2;
        }
        ...
        public APIResponse SaveLoanDocuments(CflLoanApplication model)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                APIResponse response = new APIResponse();
                response.applicationReferenceNumber = model.applicationReferenceNumber;
                SaveLoanDocument(model);
                ...
                response.requestId = model.requestId;
                response.responseMessage = "Submitted successfully";
                SaveCashflowRequestToApiLog(model, response);
                trans.Commit();
                return response;
            }
...
<!-- Auto-push timestamp: 2026-05-17 12:50:06 -->