...
namespace FintrakBanking.Repositories.Credit
{
    public class LoanFeeChargeRepository : ILoanFeeChargeRepository
    {
        ...
        public async Task<string> SubmitTakeFee(LoanFeeChargesViewModel model)
        {
            ...
            foreach (var detail in model.feeDetails)
            {
                ...
                if (await context.SaveChangesAsync() == 0) throw new SecureException(TranslateHelper.get("An error occured while saving the data!")); // this save is necessary to grab targetid
                audit.AddAuditTrail(new TBL_AUDIT
                {
                    AUDITTYPEID = (short)AuditTypeEnum.LoanChargeFee,
                    STAFFID = model.createdBy,
                    BRANCHID = model.userBranchId,
                    DETAIL = $"Added to Loan fee '{ feeCharge.LOANCHARGEFEEID}' ",
            ...
            if (result)
            {
                return TranslateHelper.get("Successful!! Fee(s) Has Been Sent For Approval ");
            }
            else
            {
        ...
        public async Task<IEnumerable<LoanReviewOperationApprovalViewModel>> GetTakeFeeAwaitingApproval(int staffId, int companyId)
        {
            var applicationDate = general.GetApplicationDate();
            ...
            var dataLoan = await (from op in context.TBL_LOAN_FEE
            ...
            var dataRevolvingLoan = await (from op in context.TBL_LOAN_FEE 
            ...
            var dataContingentLoan = await (from op in context.TBL_LOAN_FEE
        ...
        public async Task<ApprovalStatusEnum> ApproveTakeFee(ApprovalViewModel userModel)
        {
