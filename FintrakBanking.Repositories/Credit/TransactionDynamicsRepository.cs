...
namespace FintrakBanking.Repositories.Credit
{
    public class TransactionDynamicsRepository : ITransactionDynamicsRepository
    {
        ...
        public async Task<bool> AddTransactionDynamics(TransactionDynamicsViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Transaction Dynamics")} '{ model.dynamicsId }' ",
        ...
        public async Task<bool> EditLoanTransactionDynamics(int id, TransactionDynamicsViewModel model)
        {
            var data = await this.context.TBL_LOAN_TRANSACTION_DYNAMICS.FindAsync(id);
            if (data == null)
            {
                return false;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Transaction Dynamics")} '{ model.dynamicsId }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        {
            var transactionDynamics = this.GetAllTransactionDynamics();
            return transactionDynamics.Where(x => x.loanApplicationDetailId == detailId).OrderBy(a=> a.position);
        }

        public async Task<bool> RemoveLoanTransactionDynamics(int id, UserInfo model)
        {
            var data =await this.context.TBL_LOAN_TRANSACTION_DYNAMICS.FindAsync(id);
            if (data == null)
            {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Remove Transaction Dynamics")} '{ data.DYNAMICS }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<List<TransactionDynamicsViewModel>> AddSelectedTransactionDynamics(SelectedIdsViewModel entity)
        {
            ...
            var conditions = await context.TBL_TRANSACTION_DYNAMICS.Where(x => entity.selectedIds.Contains(x.DYNAMICSID)).ToListAsync();
            ...
            return (List<TransactionDynamicsViewModel>)GetTransactionDynamicsByDetailId(entity.detailId);
        }

        #region CP Template

        public async Task<IEnumerable<TransactionDynamicsViewModel>> GetTransactionDynamicsTemplate()
        {
            return await this.context.TBL_TRANSACTION_DYNAMICS
        ...
        public async Task<bool> AddTransactionDynamicsTemplate(TransactionDynamicsViewModel model)
        {
            if (model.isExternal == null)
            {
                model.isExternal = false;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added Transaction Dynamics template")} '{ model.dynamicsId }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> UpdateTransactionDynamicsTemplate(TransactionDynamicsViewModel model, int dynamicsId)
        {
            var data = await this.context.TBL_TRANSACTION_DYNAMICS.FindAsync(dynamicsId);
            if (data == null)
            {
                return false;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.lastUpdatedBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Transaction Dynamics template")} '{ model.dynamicsId }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<List<TransactionDynamicsViewModel>> GetTransactionDynamicsDefaultByDetailIdLms(int detailId)
        {
            ...
            var dynamics = await this.context.TBL_TRANSACTION_DYNAMICS.Where(c => ids.Contains((short)c.PRODUCTID))
            .Select(c => new TransactionDynamicsViewModel
            {
                dynamicsId = c.DYNAMICSID,
                dynamics = c.DYNAMICS,
        ...
        public async Task<List<TransactionDynamicsViewModel>> GetTransactionDynamicsDefaultByApplicationIdAndOperationLms(int detailId, int? operationId)
        {
            var applicationDetail = await context.TBL_LMSR_APPLICATION_DETAIL.FindAsync(detailId);
            if (operationId != null)
            {
                var operation = await context.TBL_OPERATIONS.FindAsync(operationId);
                if (operation.ISCHECKLISTSPECIFIC != null && operation.ISCHECKLISTSPECIFIC.Value)
                {
                    var output =await GetTransactionDynamicsDefaultByDetailIdLms(applicationDetail.LOANREVIEWAPPLICATIONID);
                    return output.Where(x => x.operationId == operationId).ToList();

                }
            }
            return await GetTransactionDynamicsDefaultByDetailIdLms(applicationDetail.LOANREVIEWAPPLICATIONID);
        }
        ...
        public async Task<List<TransactionDynamicsViewModel>> AddSelectedTransactionDynamicsLms(SelectedIdsViewModel entity)
        {
            ...
            return (List<TransactionDynamicsViewModel>)await GetTransactionDynamicsByDetailIdLms(entity.detailId);
        }
        ...
        public async Task<bool> EditLoanTransactionDynamicsLms(int id, TransactionDynamicsViewModel model)
        {
            var data = await this.context.TBL_LMSR_TRANSACTION_DYNAMICS.FindAsync(id);
            if (data == null)
            {
                return false;
            }
        ...
        public async Task<bool> AddTransactionDynamicsLms(TransactionDynamicsViewModel model)
        {
            if (model.isExternal == null)
            {
                model.isExternal = false;
        ...
        public async Task<IEnumerable<TransactionDynamicsViewModel>> GetTransactionDynamicsByDetailIdLms(int detailId)
        {
            var transactionDynamicsLMS = await this.GetAllTransactionDynamicsLms();
            return transactionDynamicsLMS.Where(x => x.loanApplicationDetailId == detailId).OrderBy(a => a.position); // lms
        }

        public async Task<IEnumerable<TransactionDynamicsViewModel>> GetAllTransactionDynamicsLms()
        {
            return await this.context.TBL_LMSR_TRANSACTION_DYNAMICS
                .Join(
                    context.TBL_STAFF,
                    c => c.CREATEDBY,
                    s => s.STAFFID,
        ...
        public async Task<bool> AddSuggestedConditions(SuggestedConditionsViewModel entity)
        {
            if (entity == null)
            {
                return false;
        ...
        public async Task<List<SuggestedConditionsViewModel>> GetSuggestedConditions(int applicationDetailId)
        {
            return await context.TBL_SUGGESTED_CONDITIONS.Where(c => c.LOANAPPLICATIONDETAILID == applicationDetailId).Select(c => new SuggestedConditionsViewModel
            {
        ...
        public async Task<List<SuggestedConditionsViewModel>> GetSuggestedConditionsByApplicationId(int applicationId)
        {
            // get all detalids in the appplication
            var allDetailIds =await context.TBL_LOAN_APPLICATION_DETAIL.Where(l => l.LOANAPPLICATIONID == applicationId)
                .Select(l => l.LOANAPPLICATIONDETAILID).ToListAsync();
        ...
        public async Task<bool> UpdateSuggestedConditions(int id, SuggestedConditionsViewModel entity)
        {
            if (entity == null)
            {
                return false;
            ...
            var suggestedCondition =await context.TBL_SUGGESTED_CONDITIONS.FindAsync(entity.suggestionid);
        ...
        public async Task<bool> RemoveSuggestedConditions(int id, UserInfo user)
        {
