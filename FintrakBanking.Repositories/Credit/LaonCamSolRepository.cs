...
namespace FintrakBanking.Repositories.Credit
{
    public class LaonCamSolRepository : ILaonCamSolRepository
    {
        ...
        public async Task<List<LoanCAMSOLViewModel>> GetCamSol()
        {
            var data =  await (from camsol in context.TBL_LOAN_CAMSOL
        ...
        public async Task<List<LoanCAMSOLViewModel>> CamSolAwaitingApproval(int companyId, int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CamsolBackbookModification).ToList();

            var data =  await (from camsol in context.TBL_TEMP_LOAN_CAMSOL
        ...
        public async Task<List<LoanCAMSOLViewModel>> GetCamSolByCustomerCode(string customerCode)
        {
            var data = await (from camsol in context.TBL_LOAN_CAMSOL
                       join c in context.TBL_LOAN_CAMSOL_TYPE on camsol.CAMSOLTYPEID equals c.CAMSOLTYPEID
        ...
        public async Task<List<LoanCAMSOLViewModel>> GetCamSolByType(int camsolTyepId)
        {
            var data =  await (from camsol in context.TBL_LOAN_CAMSOL
                           //join a in context.TBL_LOAN_SYSTEM_TYPE on camsol.LOANSYSTEMTYPEID equals a.LOANSYSTEMTYPEID
                       join c in context.TBL_LOAN_CAMSOL_TYPE on camsol.CAMSOLTYPEID equals c.CAMSOLTYPEID
        ...
        public async Task<List<LoanCAMSOLViewModel>> GetCamSolType()
        {
            var camsolType = await (from x in context.TBL_LOAN_CAMSOL_TYPE
                              select new LoanCAMSOLViewModel
                              {
        ...
        public async Task<LoanCAMSOLViewModel> ViewCamSolByType(int id)
        {
            var data = await (from camsol in context.TBL_LOAN_CAMSOL
                           //join a in context.TBL_LOAN_SYSTEM_TYPE on camsol.LOANSYSTEMTYPEID equals a.LOANSYSTEMTYPEID
                       join c in context.TBL_LOAN_CAMSOL_TYPE on camsol.CAMSOLTYPEID equals c.CAMSOLTYPEID
        ...
        public async Task<LoanCAMSOLViewModel> CamSolAwaitingApprovalById(int id)
        {
            var data = await (from camsol in context.TBL_TEMP_LOAN_CAMSOL
                       join c in context.TBL_LOAN_CAMSOL_TYPE on camsol.CAMSOLTYPEID equals c.CAMSOLTYPEID
        ...
        public async Task<string> ApproveCamsol(LoanCAMSOLViewModel option)
        {

            var data = await (from camsol in context.TBL_LOAN_CAMSOL
                        where camsol.CUSTOMERCODE == option.customercode
                        select camsol).ToListAsync();
            ...
            if (data != null)
            {
                ...
                foreach (var x in data)
                {
                    ...
                    if (!iSCamsolExit)
                    {
                        ...
                        workflow.Comment = TranslateHelper.get("Request for CAMSOL/Blackbook approval");
                ...
                {
                    return TranslateHelper.get("The following Customer code is currently undergoing approval : ") + listOfExistingCamsols;
                }
                ...
                for (int i = range.FirstColumnIndex; i <= range.LastColumnIndex; i++)
                {
                    ...
                    switch (cellColumn)
                    {
                        case "A":
                            ...
                            {
                                rowSuccess = false;
                                camsolRowData.message = camsolRowData.message + $"{TranslateHelper.get("Customer Code cannot be null.")}";
                            }
                            break;
                        case "B":
                        ...
                        case "C":
                            ...
                            {
                                rowSuccess = false;
                                camsolRowData.message = camsolRowData.message + $"{TranslateHelper.get("customername cannot be null.")}";
                            }
                            break;
                        case "D":
                        ...
                        case "I":
                            ...
                            {
                                rowSuccess = false;
                                camsolRowData.message = camsolRowData.message + $"{TranslateHelper.get("Camsol Type Id Doesnt Exist in the System")}";
                            }
                            else
                            {
        ...
        public bool AddSimpleTempCamsol(LoanCAMSOLViewModel camsolModel)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Initiated Camsol for")} '{camsolModel?.customername}' {TranslateHelper.get("with code")}'{camsolModel?.customercode}'",
            ...
            workflow.Comment = TranslateHelper.get("New CAMSOL Creation");
            workflow.OperationId = (int)OperationsEnum.CamsolBackbookModification;
        ...
        public async Task<string> goForApproval(LoanCAMSOLViewModel data)
        {
           // bool response = false;
            bool action = false;
            using (var transaction = context.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if ( await context.SaveChangesAsync() > 0)
                    {
                        ...
                        if (action == true)
                            return TranslateHelper.get("Consession has been granted to access loan!");
                        else
                            return TranslateHelper.get("This customer has been blacklisted successfully!");
                    }

                    return TranslateHelper.get("Operation could not be completed!");
                }
                catch (Exception ex)
                {
        ...
        public async Task<bool> GoForBulkApproval(LoanCAMSOLViewModel data)
        {
        ...
        private bool ApproveLoanCamsol(int tempLoancamsolid, short approvalStatusId, LoanCAMSOLViewModel data)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = TranslateHelper.get("Approved Loan Camsol"),
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
...
<!-- Auto-push timestamp: 2026-05-12 22:51:01 -->