...
namespace FintrakBanking.Repositories.Credit
{
    public class CustomerCreditBureauRepository : ICustomerCreditBureauRepository
    {
        ...
        public void AddCustomerAccounts(IEnumerable<CustomerViewModels> customers)
        {
            ...
            if (setup.USE_THIRD_PARTY_INTEGRATION)
            {
                foreach (var customer in customers)
                {
                    ...
                    {
                        throw new ConditionNotMetException(TranslateHelper.get("An error occured while get customer account details"));
                    }
                }
            }
        }
        ...
        public int AddCustomerCreditBureauUpload(LoanCreditBureauViewModel entity, LoanDocumentViewModel docModel, byte[] file)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Kindly Upload Valid File with Accepted Extension ") + chanelArray);
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Credit Bureau Report Document with title")} : '{ docModel.documentTitle }' ",
        ...
        #region Integration 
        public async Task<XDSSearchResult> GetCustomerXDSCreditMatch(CreditBureauSearchViewModel searchInfoList)
        {
            ...
            var creditBureau = await context.TBL_CREDIT_BUREAU.FindAsync(searchInfoList.creditBureauId);
            if (creditBureau != null)
            {
                searchInfoList.userName = creditBureau.USERNAME;
                searchInfoList.password = creditBureau.PASSWORD;
            }
            else throw new ConditionNotMetException(TranslateHelper.get("Could not resolve the selected Credit Bureau item. Contact admin."));
        ...
        public async Task<CRCSearchResult> GetCustomerCRCCreditMatch(CRCRequestViewModel searchInfo)
        {
            ...
            var creditBureau = await context.TBL_CREDIT_BUREAU.FindAsync(searchInfo.creditBureauId);
            ...
            if (companyExternalServiceChargeInfo.creditBureauSearchTypeId != (short)ChargeTypeEnum.NoCharge)
            {
                ...
                {
                    if (casa == null) { throw new SecureException("Nominated Account Does not Exist"); }
                    if (creditBureauInputs.casaAccountId == 0) { throw new ConditionNotMetException(TranslateHelper.get("Missing Charge Account! Specify charge account or contact admin.")); }
                }
                ...
                if (chargeAmount > accountBalance)
                {
                    if (!searchInfo.debitBusiness)
                        throw new SecureException("The nominated customer account has insufficient fund to perform this transaction.");
                }
            }
            ...
            {
                var bizAccount = await context.TBL_OTHER_OPERATION_ACCOUNT.Where(x => x.OTHEROPERATIONID == (short)OtherOperationEnum.ChargeOnBank).FirstOrDefaultAsync();
                ...
                var customChartOfAccount = await context.TBL_CUSTOM_CHART_OF_ACCOUNT.FirstOrDefaultAsync(c => c.CUSTOMACCOUNTID == chargeModel.glAccountId);
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                else if (searchResponse.SearchCompleted == (int)SearchCompletedStatusEnum.SearchCompleted)
                {
                    ...
                    {
                        searchResponse.errorOccured = true;
                        throw new ConditionNotMetException(TranslateHelper.get("Search could not save the result file"));
                    }
        ...
        public async Task<string> GetCRCFullCreditMergeReport(MultiHitRequestViewModel request)
        {
            ...
            var casa = await context.TBL_CASA.FindAsync(creditBureauInputs.casaAccountId);
            var referenceNumber = CommonHelpers.GenerateRandomDigitCode(10);
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (task.Wait(TimeSpan.FromSeconds(3500)))
                    {
                        ...
                        if (dataResponse.Contains(DATA_PACKET))
                        {
                            if (!dataResponse.Contains(ERROR))
                            {
                                ...
                                else { throw new ConditionNotMetException(TranslateHelper.get("Fintrak Credit 360 Could not resolve the API response.")); }
                            }
                        }
                    ...
                    else throw new Exception(TranslateHelper.get("Application failed to fetch merge file"));
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (task.Wait(TimeSpan.FromSeconds(640)))
                    {
                        ...
                        {
                            {
                                ...
                                if (!SaveCreditBureauReportFile(customerCreditBureauId, fileArray, creditBureauInputs))
                                {
                                    throw new SecureException(TranslateHelper.get("Could not save file"));
                                }
                        ...
                        {
                            throw new SecureException(TranslateHelper.get("An error occured"));
                        }
                    }
                    else
                    {
                        throw new SecureException(TranslateHelper.get("Timed out"));
                    }
                }
                catch (APIErrorException ex)
                {
                    throw new APIErrorException(TranslateHelper.get(ex.Message));
                }
                catch (TimeoutException ex)
                {
                    throw new TimeoutException(TranslateHelper.get(ex.Message));
                }
                catch (ConditionNotMetException ex)
                {
                    throw new ConditionNotMetException(TranslateHelper.get(ex.Message));
                }
                catch (Exception ex)
                {
                    throw new SecureException(TranslateHelper.get(ex.Message));
                }
            }
        }

        public async Task<XDSSearchResult> GetXDSFullSearchResultInPDF(SearchInput searchInput)
        {
            var referenceNumber = CommonHelpers.GenerateRandomDigitCode(10);
            var companyExternalServiceChargeInfo = GetLoanThirdPartyServiceChargeStatusDetails(searchInput.companyId);

            var casa = await context.TBL_CASA.FindAsync(searchInput.casaAccountId);
            var accountBalance = casa != null ? financeTransaction.GetCASABalance(casa.CASAACCOUNTID).availableBalance : 0;

            var creditBureau = await context.TBL_CREDIT_BUREAU.FindAsync(searchInput.creditBureauId);
            ...
            if (companyExternalServiceChargeInfo.creditBureauSearchTypeId != (short)ChargeTypeEnum.ChargeBank)
            {
                if (companyExternalServiceChargeInfo.creditBureauSearchTypeId == (short)ChargeTypeEnum.ChargeCustomer || (companyExternalServiceChargeInfo.creditBureauSearchTypeId == (short)ChargeTypeEnum.ChargeCustomerORBank && !searchInput.debitBusiness))
                {
                    if (searchInput.casaAccountId == 0)
                        throw new ConditionNotMetException(TranslateHelper.get("Missing Charge Account! Specify charge account or contact admin."));

                    if (casa == null) throw new ConditionNotMetException(TranslateHelper.get("Nominated Account Does not Exist"));

                    if (chargeAmount > accountBalance) throw new ConditionNotMetException(TranslateHelper.get("The nominated customer account has insufficient fund to perform this transaction."));
                    if (casa != null) referenceNumber = casa.PRODUCTACCOUNTNUMBER;
            ...
            {
                if (binaryData == null)
                    throw new SecureException(TranslateHelper.get("File report not found. Please try again."));
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    ...
                    {
                        throw new Exception(TranslateHelper.get(ex.Message));
                    }
                }
            }
            else
            {
                throw new ConditionNotMetException(TranslateHelper.get("Search result Timed out"));
            }
        ...
        private bool SaveCreditBureauReportFile(int customerCreditBureauId, byte[] file, SearchInput model)
        {
            try
            {
                ...
                var data = new Entities.DocumentModels.TBL_CUSTOMER_CREDIT_BUREAU
                {
                    ...
                    DOCUMENT_TITLE = creditBureau.CREDITBUREAUNAME + TranslateHelper.get(" Report Document Upload"),
                    FILEEXTENSION = "pdf",
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Uploaded")} '{ creditBureauInfo.CREDITBUREAUNAME }' {TranslateHelper.get("Credit Bureau Report generated for merge ID list")} : '{ model.mergeList }' ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
