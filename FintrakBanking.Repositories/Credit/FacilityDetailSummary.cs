...
namespace FintrakBanking.Repositories.Credit
{
    public class FacilityDetailSummary : IFacilityDetailSummary
    {
        ...
        {
            var customerId = context.TBL_LOAN.Where(x => x.TERMLOANID == loanId).FirstOrDefault().CUSTOMERID;
            var data = (from c in context.TBL_COLLATERAL_CUSTOMER 
                        join ct in context.TBL_COLLATERAL_TYPE on c.COLLATERALTYPEID equals ct.COLLATERALTYPEID
                        join cs in context.TBL_COLLATERAL_TYPE_SUB on c.COLLATERALSUBTYPEID equals cs.COLLATERALSUBTYPEID
        ...
        public List<CollateralViewModel> CollateralByLoanIdODF(int loanId)
        {
            var customerId = context.TBL_LOAN_REVOLVING.Where(x => x.REVOLVINGLOANID == loanId).FirstOrDefault().CUSTOMERID;
            var data = (from c in context.TBL_COLLATERAL_CUSTOMER 
                        join ct in context.TBL_COLLATERAL_TYPE on c.COLLATERALTYPEID equals ct.COLLATERALTYPEID
                        join cs in context.TBL_COLLATERAL_TYPE_SUB on c.COLLATERALSUBTYPEID equals cs.COLLATERALSUBTYPEID
                        where c.CUSTOMERID == customerId
            ...
            foreach (var item in loanAppProdFee)
            {
                ...
                if (chargeFeeDetail != null)
                {
                    var prodFeeView = new ProductFeeViewModel()
                    {
                        ...
                        valueBase = chargeFeeDetail.TBL_CHARGE_FEE.TBL_FEE_TYPE.FEETYPENAME,
                        dealTypeId = (short)ChargeFeeDetailTypeEnum.Tax
                    };
        ...
        {
            List<LoanCovenantDetailViewModel> data = new List<LoanCovenantDetailViewModel>();
            var loanReviewApplicationIds = context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANID == loanId).Select(x => x.LOANREVIEWAPPLICATIONID).ToList();
            foreach(var id in loanReviewApplicationIds)
            {
                data = (from a in context.TBL_LMSR_APPLICATION_COVENANT
                        where a.LOANREVIEWAPPLICATIONID == id && a.DELETED == false
                        select new LoanCovenantDetailViewModel
                        {
                            loanCovenantDetailId = a.LOANCOVENANTDETAILID,
                            covenantDetail = a.COVENANTDETAIL,
                            ...
                            covenantTypeId = (short)a.COVENANTTYPEID,
                            frequencyTypeId = (short)a.FREQUENCYTYPEID,
                            frequencyTypeName = context.TBL_FREQUENCY_TYPE.Where(f => f.FREQUENCYTYPEID == a.FREQUENCYTYPEID).FirstOrDefault().MODE,
                            covenantAmount = a.COVENANTAMOUNT,
                            covenantDate = a.COVENANTDATE,
                            casaAccountId = a.CASAACCOUNTID
                        }).ToList();
        ...
        public List<LoanCovenantDetailViewModel> LMSLoanCovenantDetailODF(int loanId)
        {
            List<LoanCovenantDetailViewModel> data = new List<LoanCovenantDetailViewModel>();
            var loanReviewApplicationIds = context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANID == loanId).Select(x => x.LOANREVIEWAPPLICATIONID).ToList();
            foreach (var id in loanReviewApplicationIds)
