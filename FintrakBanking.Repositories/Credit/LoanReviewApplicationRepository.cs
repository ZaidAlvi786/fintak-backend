...
namespace FintrakBanking.Repositories.Credit
{
    public class LoanReviewApplicationRepository : ILoanReviewApplicationRepository
    {
        ...
        public async Task<IQueryable<LoanReviewApplicationViewModel>> GetLoanReviewForCRMS(UserInfo user, int operationId, int? classId)
        {
            // var declarations
            ...
            var query = await context.TBL_LMSR_APPLICATION.Where(x => x.BRANCHID == user.BranchId || ignoreBranch)
             ...
             .Select(x => new LoanReviewApplicationViewModel
             {
                 ...
                 facility = x.application.TBL_LMSR_APPLICATION_DETAIL.Where(t => t.DELETED == false).Count() > 1 ? TranslateHelper.get("Multilple(") + x.application.TBL_LMSR_APPLICATION_DETAIL.Where(t => t.DELETED == false).Count() + ")" : context.TBL_LMSR_APPLICATION_DETAIL
             ...
             }).GroupBy(d => d.loanReviewApplicationId).ToListAsync();
        ...
        public async Task<IQueryable<LoanReviewApplicationViewModel>> GetApplications(UserInfo user, int operationId, int? classId)
        {
            // var declarations
            int staffId = user.staffId;
            int branchId = user.BranchId;
            int companyId = user.companyId;
            var staff = await context.TBL_STAFF.FirstOrDefaultAsync(O => O.STAFFID == staffId);

            List<int> approvalOperations = await context.TBL_OPERATIONS.Where(x => x.OPERATIONTYPEID == (short)OperationTypeEnum.LoanReviewApplication
                ...
                .Select(x => x.OPERATIONID).ToListAsync();
            ...
            // .Join(context.TBL_BRANCH, a => a.BRANCHID, b => b.BRANCHID, (a, b) => new { a, b })
             ...
             .Select(x => new LoanReviewApplicationViewModel
             {
                 ...
                 relatedReferenceNumber = x.application.RELATEDREFERENCENUMBER,
                 branchId = x.branch.BRANCHID,
                 branchName = context.TBL_BRANCH.Where(b => b.BRANCHID == x.branch.BRANCHID).Select(b => b.BRANCHNAME).FirstOrDefault(),
                 customerId = x.customer.CUSTOMERID,
                 operationId = x.application.OPERATIONID,
                 customerName = x.customer.FIRSTNAME + " " + x.customer.MIDDLENAME + " " + x.customer.LASTNAME,
                 atInitiator = x.application.CREATEDBY == staffId,
        ...
        public async Task<SelectListViewModel> GetAllLMSApprovalOperationListByProductTypeId(int productTypeId)
        {
            var list = new SelectListViewModel();

            var frequency = await context.TBL_FREQUENCY_TYPE.Select(x => new DropDownSelect { id = x.FREQUENCYTYPEID, name = x.MODE }).ToListAsync();
            ...
            list.casaAccounts = context.TBL_PRODUCT_TYPE.Select(x => new DropDownSelect { id = x.PRODUCTTYPEID, name = x.PRODUCTTYPENAME }).ToList();
            list.productTypes = context.TBL_PRODUCT_TYPE.Select(x => new DropDownSelect { id = x.PRODUCTTYPEID, name = x.PRODUCTTYPENAME }).ToList();
            list.operationTypes = await context.TBL_OPERATIONS.Where(x =>
                // (
                x.OPERATIONTYPEID == (int)OperationTypeEnum.LoanReviewApplication
                //|| x.OPERATIONTYPEID == (int)OperationTypeEnum.LoanManagementOverdraft
                // || x.OPERATIONTYPEID == (int)OperationTypeEnum.Remedial)
                && x.ISDISABLED == false
            ).Select(x => new DropDownSelect { id = x.OPERATIONID, name = x.OPERATIONNAME, typeId = (int)x.OPERATIONTYPEID, productTypeId = x.PRODUCTTYPEID }).Where(t => t.productTypeId == productTypeId || t.productTypeId == null).OrderBy(o => o.name).ToListAsync();
            list.feeCharges = context.TBL_CHARGE_FEE.Select(x => new DropDownSelect { id = x.CHARGEFEEID, name = x.CHARGEFEENAME }).ToList();
        ...
        public async Task<SelectListViewModel> GetAllSelectList()
        {
            var list = new SelectListViewModel();
            ...
            list.operationTypes = await context.TBL_OPERATIONS.Where(x =>
                ...
                || x.OPERATIONTYPEID == (int)OperationTypeEnum.Remedial)
                && x.ISDISABLED == false
            ).Select(x => new DropDownSelect { id = x.OPERATIONID, name = x.OPERATIONNAME, typeId = (int)x.OPERATIONTYPEID }).OrderBy(o => o.name).ToListAsync();
            list.feeCharges = context.TBL_CHARGE_FEE.Select(x => new DropDownSelect { id = x.CHARGEFEEID, name = x.CHARGEFEENAME }).ToList();
            return list;
        }
        public async Task<LoanChargeFeeViewModel> GetChargeFeeDetails(int id)
        {
            var list = new LoanChargeFeeViewModel();
             list = await context.TBL_CHARGE_FEE.Where(a=>a.CHARGEFEEID == id).Select(x => new LoanChargeFeeViewModel {
        ...
        public async Task<string> SubmitLoanReviewApplication(LoanReviewApplicationViewModel model)
        {
            if (model.applicationDetails.Count() > 1 &&
                model.applicationDetails.Any(x => apsOperationIds.Contains(x.operationId)))
            {
                throw new SecureException(TranslateHelper.get("Only one operation request is allowed for APS release related applications!"));
            }
            ...
            var doesOperationExist = await (from a in context.TBL_LOAN_REVIEW_OPERATION
            ...
            {
                var thirdpatyLoan = await context.TBL_LOAN_EXTERNAL.Where(x => x.LOANREFERENCENUMBER == model.loanReferenceNumber).FirstOrDefaultAsync();
            ...
            foreach (var detail in model.applicationDetails)
            {
                ...
                {
                    result = await ValidateNewSubAllocationOperation(detail.detailId, model.customerId, detail.loanSystemTypeId);

                    if (result == false)
                        throw new ConditionNotMetException(TranslateHelper.get("Customer Must Have More Than One Tranche to Proceed With Sub Allocation"));
            ...
            foreach (var detail in model.applicationDetails)
            {
                ...
                loan = await GetLoanInformation(detail.loanSystemTypeId, loanId, applicationDate);
                ...
                context.TBL_LMSR_APPLICATION_DETAIL.Add(new TBL_LMSR_APPLICATION_DETAIL
                {
                    ...
                    //APPROVEDTENOR = tenor,
                    APPROVEDTENOR = detail.duration == 0 ? tenor : 0,
            ...
            if (context.SaveChanges() == 0) throw new SecureException(TranslateHelper.get("An error occured while saving the data!")); // this save is necessary to grab targetid
            ...
            if (assetManagement)
            {
                ...
                workflow.NextProcess(model.companyId, staffId, (short)model.operationId, null, application.LOANAPPLICATIONID, null, TranslateHelper.get("Initiation"), true, true, true);
            ...
            if (context.SaveChanges() > 0)
            {
               ...
               // if (setup.USE_THIRD_PARTY_INTEGRATION) creditCommon.LoadCustomerTurnover(application.LOANAPPLICATIONID,customerIds,staffId, true);
                ...
                return TranslateHelper.get("Application with reference number created") + "." + referenceNumber;
            }

            throw new SecureException(TranslateHelper.get("An error occured while saving the data!"));
        }
        ...
        public async Task<bool> ValidateSubAllocationOperation(int loanApplicationDetailId, int customerId)
        {

            if (loanApplicationDetailId != 0)
            {
                var loanData = await (from a in context.TBL_LOAN
        ...
        public async Task<bool> ValidateNewSubAllocationOperation(int loanApplicationDetailId, int customerId, int loanSystemTypeId)
        {
            ...
            else
            {
                ...
                var loanData = await (from a in context.TBL_LOAN_REVOLVING
        ...
        public async Task<WorkflowResponse> ForwardApplicationAppraisal (ForwardReviewViewModel model)
        {
            int nextProcessId = model.operationId + 1;
            int operationId = model.operationId; // beware of nplappraisal!
            var appl = await context.TBL_LMSR_APPLICATION.FindAsync(model.applicationId);
            int lastOperationId = (int)OperationsEnum.LoanReviewApprovalAvailment;

            var checklistValidation = await ChecklistCompleted(model.applicationId);
            ...
            {
                throw new SecureException(TranslateHelper.get("Checklist not completed!"));
            }
            ...
            {
                workflow.Amount = await GetMaximumApplicationOutstandingBalance(appl.LOANAPPLICATIONID);
            }
            ...
            if (model.forwardAction == 8 || model.forwardAction == 9)
            {
                ...
                workflow.NextLevelId = dictionary[TranslateHelper.get("levelId")];
                workflow.ToStaffId = dictionary[TranslateHelper.get("staffId")];
            ...
            if (model.recommendedChanges != null && model.recommendedChanges.Count() > 0) // only approving authority
            {
                ...
                items = await context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == appl.LOANAPPLICATIONID && x.DELETED == false).ToListAsync();
            ...
            if (workflow.NewState == (int)ApprovalState.Ended)
            {
                if (workflow.StatusId == (int)ApprovalStatusEnum.Approved && operationId != lastOperationId/* && model.operationId != 71*/) // jump process OR end flag
                {
                    ...
                    workflow.NextProcess(appl.COMPANYID, model.lastUpdatedBy, nextProcessId, null, appl.LOANAPPLICATIONID, null, TranslateHelper.get("New application"), true, true); // model.operationId must be used here!
                }
                ...
                await AddLoanCollateralMapping(model.applicationId);//, appl., (short)LoanSystemTypeEnum.OverdraftFacility);
            }
        ...
        public async Task<WorkflowResponse> ForwardApplication(ForwardReviewViewModel model)
        {
            var appl = await context.TBL_LMSR_APPLICATION.FindAsync(model.applicationId);
            int operationId = model.operationId; // beware of nplappraisal!
            if (appl == null)
            ...
            if (model.forwardAction != (int)ApprovalStatusEnum.Referred)
            {
                ...
                {
                    var classifiedTrail = await context.TBL_APPROVAL_TRAIL.FirstOrDefaultAsync(x =>
                ...
                var previousTrail = await context.TBL_APPROVAL_TRAIL.FirstOrDefaultAsync(x =>
                    x.OPERATIONID == (int)model.operationId
                    //&& x.RESPONSESTAFFID == null
                    && x.REFEREBACKSTATEID != (int)ApprovalState.Ended
                    ...
                    if (classifiedTrail != null && previousTrail == null)
                    {
                        ...
                        var previousTrail2 = await context.TBL_APPROVAL_TRAIL.FirstOrDefaultAsync(x =>
                         x.OPERATIONID == (int)model.operationId
                         && x.RESPONSESTAFFID == null
                         && x.DESTINATIONOPERATIONID == null
                        ...
                        if (model.operationId == (int)OperationsEnum.LoanReviewApprovalAvailment)
                        {
                            ...
                            workflow.Response.responseMessage = TranslateHelper.get("The Request has been Approved and Sent to Credit Inputter");
                        }
                        else
                        {
                            var lmsrDetail = context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == appl.LOANAPPLICATIONID);
            ...
            var product = await context.TBL_PRODUCT.FindAsync(appl.PRODUCTID);
            ...
            if (model.operationId == (int)OperationsEnum.LoanReviewApprovalAvailment)
            {
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    ...
                    //if (workflow.ThisLevelId != null && workflow.ThisLevelId != 0)
                    //{
                    //    //var currentApprovalGroup = context.TBL_APPROVAL_LEVEL.Where(x => x.APPROVALLEVELID == workflow.ThisLevelId).ToList();
                    //    //var allGroupLevels = context.TBL_APPROVAL_LEVEL.Where(x => x.GROUPID == currentApprovalGroup.FirstOrDefault().GROUPID).ToList();
                    //    //if (allGroupLevels.Count() == 1) isClosed = true;
                    //    //else
                    //    //{
                    //    //    var currentPosition = allGroupLevels.Where(c => c.APPROVALLEVELID == workflow.ThisLevelId).FirstOrDefault().POSITION;
                    //    //    if (!allGroupLevels.Where(d => d.POSITION > currentPosition).Any()) isClosed = true;
                    //    //}
                    //}
                    ...
                    if (workflow.NewState == (int)ApprovalState.Ended && model.isFlowTest == false && (isClosed == true || workflow.IgnorePostApprovalReviewer == true))
                    {
                        if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                        {
                            ...
                            var flowOrder = context.TBL_LMSR_FLOW_ORDER.Where(x => x.OPERATIONID == model.operationId && x.DELETED == false).FirstOrDefault();
                            var defaultFlowOrder = context.TBL_LMSR_FLOW_ORDER.Where(x => x.OPERATIONID == 0 && x.DELETED == false).FirstOrDefault();
                    ...
                    if (model.operationId == (short)OperationsEnum.LoanReviewApprovalAvailment)
                        {
                        if (model.forwardAction == (int)ApprovalStatusEnum.Approved)
                        {
                            appl.APPROVALSTATUSID = (short)ApprovalStatusEnum.Approved;
                        }
                        else
                        {
                            appl.APPROVALSTATUSID = (short)ApprovalStatusEnum.Disapproved;
                        }
                    }
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    ...
                    if (workflow.NewState == (int)ApprovalState.Ended && model.isFlowTest == false && (isClosed == true || workflow.IgnorePostApprovalReviewer == true))
                    {
                        if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                        {
                            ...
                            if(model.operationId == (short)OperationsEnum.TenorExtensionApproval || model.operationId == (short)OperationsEnum.OverdraftTenorExtensionApproval || model.operationId == (short)OperationsEnum.ContingentLiabilityTenorExtensionApproval)
                            {
                                var lmsrDetails = appl.TBL_LMSR_APPLICATION_DETAIL.FirstOrDefault();
                                if(lmsrDetails != null)
                                {
                                    lmsrDetails.APPROVEDTENOR = lmsrDetails.PROPOSEDTENOR;
                                }
                            }
        ...
        private void SendAppraisalDataToGroupOffice(ForwardReviewViewModel model)
        {
            var appl = context.TBL_LMSR_APPLICATION.Find(model.applicationId);
            var loanAppl = context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == appl.LOANAPPLICATIONID).FirstOrDefault();
            var product = context.TBL_PRODUCT.Where(x => x.PRODUCTID == loanAppl.PRODUCTID).FirstOrDefault();
            ...
            subToHqData.loanApplicationDetailId = loanAppl.LOANREVIEWAPPLICATIONID;
            ...
            subToHqData.applicationTenor = loanAppl.APPROVEDTENOR;
            ...
            subToHqData.loanApplicationId = appl.LOANAPPLICATIONID;
            subToHqData.loanApplicationTypeName = loanAppl.TBL_LOAN_SYSTEM_TYPE.LOANSYSTEMTYPENAME;
            subToHqData.operationName = loanAppl.TBL_OPERATIONS.OPERATIONNAME;
            subToHqData.productClassName = loanAppl.TBL_PRODUCT.TBL_PRODUCT_CLASS.PRODUCTCLASSNAME;
            subToHqData.productClassProcess = loanAppl.TBL_PRODUCT.TBL_PRODUCT_CLASS.TBL_PRODUCT_CLASS_PROCESS.PRODUCT_CLASS_PROCESS_NAME;
            ...
            subToHqData.toStaffId = model.staffId;
            ...
            subToHqData.operationId = model.operationId;
            ...
            subsidiaryRepo.PostLMSFacilityApprovalnputs(subToHqData);
        }
        ...
        public async Task<LoanViewModel> GetLoanInformation(int loanSystemTypeId, int loanId, DateTime startDate)
        {
            var result = new LoanViewModel();
            if (loanSystemTypeId == (int)LoanSystemTypeEnum.TermDisbursedFacility)
            {
                result = await context.TBL_LOAN.Where(x => x.TERMLOANID == loanId).Select(loan => new LoanViewModel
                {
                    customerId = loan.CUSTOMERID,
            ...
            {
                result = await context.TBL_LOAN_EXTERNAL.Where(x => x.EXTERNALLOANID == loanId).Select(loan => new LoanViewModel
                {
                    customerId = loan.CUSTOMERID,
                    currencyId = loan.CURRENCYID,
                    effectiveDate = startDate,
            ...
            {
                result = await context.TBL_LOAN_REVOLVING.Where(x => x.REVOLVINGLOANID == loanId).Select(loan => new LoanViewModel
                {
                    customerId = loan.CUSTOMERID,
                    currencyId = loan.CURRENCYID,
                    effectiveDate = startDate,
            ...
            {
                result = await context.TBL_LOAN_CONTINGENT.Where(x => x.CONTINGENTLOANID == loanId).Select(loan => new LoanViewModel
                {
                    customerId = loan.CUSTOMERID,
                    currencyId = loan.CURRENCYID,
                    effectiveDate = startDate,
            ...
            {
                result = await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONDETAILID == loanId).Select(loan => new LoanViewModel
                {
                    customerId = loan.CUSTOMERID,
                    currencyId = loan.CURRENCYID,
                    effectiveDate = startDate,
            ...
            {
                throw new SecureException(TranslateHelper.get("The Product type is Invalid"));
            }
            return result;
        }

        public async Task<LoanApplicationDetailViewModel> GetLoanApplicationDetail(int loanId, int loanTypeId)
        {
            ...
            var detail = await context.TBL_LOAN_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.LOANAPPLICATIONDETAILID == id);
            if (detail == null) throw new Exception(TranslateHelper.get("Could not find loan application detail with id of ") + id);
        ...
        public async Task<IQueryable<LoanReviewApplicationViewModel>> GetRegionalLoanApplications(int staffId)
        {
          ...
          var  levels = camLevels.Union(apsLevels).Distinct();
            ...
            var regions = await context.TBL_BRANCH_REGION_STAFF.Where(x => x.STAFFID == staffId)
                            .Join(context.TBL_BRANCH_REGION, s => s.REGIONID, r => r.REGIONID, (s, r) => new { s, r })
                            .Select(x => new
                            {
        ...
        #region
        public async Task<IEnumerable<LoanApplicationViewModel>> Search(string searchString)
        {
            ...
            var staffs = await context.TBL_STAFF.ToListAsync();
            ...
            int staffId = await context.TBL_STAFF.Where(o => o.STAFFCODE.ToLower().Contains(searchString)).Select(o => o.STAFFID).FirstOrDefaultAsync();

            var applications = await (from a in context.TBL_LMSR_APPLICATION
            ...
            var archivedloans = await (from a in context.TBL_LMSR_APPLICATION
                                join d in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals d.LOANAPPLICATIONID
                                join g in context.TBL_CUSTOMER on d.CUSTOMERID equals g.CUSTOMERID
                                join l in context.TBL_LOAN on d.LOANID equals l.TERMLOANID into dl
                                join r in context.TBL_LOAN_REVOLVING on d.LOANID equals r.REVOLVINGLOANID into lr
            ...
            var groupApplications = await (from a in context.TBL_LMSR_APPLICATION
                                    join d in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals d.LOANAPPLICATIONID
            ...
            foreach (var x in allRecord)
            {
                if (x.approvalStatusId == (int)ApprovalStatusEnum.Approved)
                {
                    ...
                    {
                        var appRecord2 = await context.TBL_APPROVAL_TRAIL.Where(o => o.TARGETID == operationRec.LOANREVIEWOPERATIONID && operations2.Contains(o.OPERATIONID)).OrderByDescending(r => r.APPROVALTRAILID).FirstOrDefaultAsync();
                        if (appRecord2 != null)
                        {
                        ...
                        if (appRecord2 != null && operationRec.LOANSYSTEMTYPEID == (short)LoanSystemTypeEnum.LineFacility && operationRec.APPROVALSTATUSID == (short)ApprovalStatusEnum.Approved && operationRec.OPERATIONCOMPLETED == true)
                        {
                ...
                {
                    var appRecord = await context.TBL_APPROVAL_TRAIL.Where(o => o.TARGETID == x.loanApplicationId && operations.Contains(o.OPERATIONID)).OrderByDescending(r => r.APPROVALTRAILID).FirstOrDefaultAsync();
                    if (appRecord != null)
                    {
        ...
        public async Task<IEnumerable<LoanReviewOperationViewModel>> ContingentSearch(string searchString)
        {
            searchString = searchString.Trim().ToLower();
            int staffId = await context.TBL_STAFF.Where(o => o.STAFFCODE.ToLower().Contains(searchString)).Select(o => o.STAFFID).FirstOrDefaultAsync();

            var ops = await (from op in context.TBL_LOAN_REVIEW_OPERATION
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> ExceptionalSearch(string searchString)
        {
            searchString = searchString.Trim().ToLower();
            int staffId = await context.TBL_STAFF.Where(o => o.STAFFCODE.ToLower().Contains(searchString)).Select(o => o.STAFFID).FirstOrDefaultAsync();

            var exceptionalLoansForApproval = await (from d in context.TBL_EXCEPTIONAL_LOAN_APPL_DETAIL
        ...
        public async Task<IEnumerable<LoanReviewOperationApprovalViewModel>> SearchLien(string searchString)
        {
            searchString = searchString.Trim().ToLower();
            var dataLoan = await (from ln in context.TBL_LOAN
            ...
            var dataRevolvingLoan = await (from ln in context.TBL_LOAN_REVOLVING
                                     join br in context.TBL_BRANCH on ln.BRANCHID equals br.BRANCHID
                                     join cu in context.TBL_CUSTOMER on ln.CUSTOMERID equals cu.CUSTOMERID
                                     join pr in context.TBL_PRODUCT on ln.PRODUCTID equals pr.PRODUCTID
                                     join st in context.TBL_STAFF on ln.RELATIONSHIPOFFICERID equals st.STAFFID
            ...
            var dataContingent = await (from ln in context.TBL_LOAN_CONTINGENT
                                     join br in context.TBL_BRANCH on ln.BRANCHID equals br.BRANCHID
                                     join cu in context.TBL_CUSTOMER on ln.CUSTOMERID equals cu.CUSTOMERID
                                     join pr in context.TBL_PRODUCT on ln.PRODUCTID equals pr.PRODUCTID
                                     join st in context.TBL_STAFF on ln.RELATIONSHIPOFFICERID equals st.STAFFID
        ...
        public async Task<IEnumerable<LoanReviewOperationApprovalViewModel>> GetAllLienRemovalApplications(int staffId, int companyId)
        {
        ...
        public async Task<bool> UpdateManagementPosition(ManagementPositionViewModel model)
        {
            var entity = await context.TBL_LMSR_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.LOANREVIEWAPPLICATIONID == model.applicationDetailId);
        ...
        public async Task<ManagementPositionViewModel> GetManagementPosition(int detailId)
        {
            var entity = await context.TBL_LMSR_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.LOANREVIEWAPPLICATIONID == detailId);
        ...
        public async Task<bool> ChecklistCompleted(int applicationId)
        {
        ...
        public async Task<List<LoanReviewOperationViewModel>> GetMaturityInstruction(int loanId, short loansystemTypeId)
        {
            var ops = await (from op in context.TBL_LOAN_MATURITY_INSTRUCTION
        ...
        public async Task<List<LoanReviewOperationViewModel>> GetLMSOperation(int loanId, short loansystemTypeId)
        {
            var ops = await (from op in context.TBL_LOAN_REVIEW_OPERATION
        ...
        public async Task<bool> AddLoanCollateralMapping(int loanApplicationId)
        {
            LoanApplicationViewModel appl;
            List<int> existingCollateralIds;
            List<TBL_LOAN_APPLICATION_COLLATERL> recommendedCollaterals;

            var details = await context.TBL_LMSR_APPLICATION_DETAIL.Where(d => d.DELETED == false).Where(x => x.LOANAPPLICATIONID == loanApplicationId).ToListAsync();
            
            foreach(var d in details)
            {
                appl = await GetLoanApplicationByLoanSystemType(d.LOANSYSTEMTYPEID, d.LOANID);

                existingCollateralIds = await context.TBL_LOAN_COLLATERAL_MAPPING
                    .Where(x => x.LOANID == d.LOANID && x.ISRELEASED == false)
                    .Select(x => x.COLLATERALCUSTOMERID)
                    .ToListAsync();

                recommendedCollaterals = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(x => x.LOANAPPLICATIONID == appl.loanApplicationId).ToListAsync();
        ...
        private async Task<LoanApplicationViewModel> GetLoanApplicationByLoanSystemType(int loanSystemTypeId, int loanId)
        {
            var result = new LoanApplicationViewModel();

            if (loanSystemTypeId == (int)LoanSystemTypeEnum.TermDisbursedFacility)
            {
                result = await context.TBL_LOAN.Where(x => x.TERMLOANID == loanId)
            ...
            {
                result = await context.TBL_LOAN_REVOLVING.Where(x => x.REVOLVINGLOANID == loanId)
                    .Join(context.TBL_LOAN_APPLICATION_DETAIL, l => l.LOANAPPLICATIONDETAILID, d => d.LOANAPPLICATIONDETAILID, (l, d) => new { l, d })
                    .Select(x => new LoanApplicationViewModel { loanApplicationId = x.d.LOANAPPLICATIONID })
                    .FirstOrDefaultAsync();
            }
            ...
            {
                result = await context.TBL_LOAN_CONTINGENT.Where(x => x.CONTINGENTLOANID == loanId)
                    .Join(context.TBL_LOAN_APPLICATION_DETAIL, l => l.LOANAPPLICATIONDETAILID, d => d.LOANAPPLICATIONDETAILID, (l, d) => new { l, d })
                    .Select(x => new LoanApplicationViewModel { loanApplicationId = x.d.LOANAPPLICATIONID })
                    .FirstOrDefaultAsync();
            }
            ...
            {
                throw new SecureException(TranslateHelper.get("Collateral Failed To Map. Loan System Type could not be resolved!"));
            }

            if (result.loanApplicationId < 1) throw new SecureException(TranslateHelper.get("Collateral Failed To Map. Error resolving Loan Application Information."));
        ...
        public async Task<decimal?> GetWrittenOffAccrualAmount(int loanId, short loanSystemTypeId)
        {
            decimal? amount = null;
            var camsol = await context.TBL_LOAN_CAMSOL.FirstOrDefaultAsync(x => x.LOANID == loanId && x.LOANSYSTEMTYPEID == loanSystemTypeId);
            if (camsol != null) amount = camsol.WRITTENOFFACCRUALAMOUNT;
            return amount;
        }

        public async Task<decimal> GetMaximumApplicationOutstandingBalance(int applicationId)
        {
            decimal amount = 0;
            var appl = await context.TBL_LMSR_APPLICATION_DETAIL.Where(x => 
        ...
        public async Task<ContingentLoansViewModel> GetContingentTotoalUsed(int contingetLoanId)
        {
            ContingentLoansViewModel data = new ContingentLoansViewModel();
            var appl = await (from a in context.TBL_LOAN_CONTINGENT
