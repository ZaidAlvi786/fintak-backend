...
namespace FintrakBanking.Repositories.Credit
{
    public class OfferLetterAndAvailmentRepository : IOfferLetterAndAvailmentRepository
    {
        ...
        public async Task<bool> AddCRMSCollateralType(int applicationId, ApprovedLoanDetailViewModel model)
        {
            
            bool output = false;
            var bookingRequest = await context.TBL_LOAN_BOOKING_REQUEST.FirstOrDefaultAsync(r => r.LOAN_BOOKING_REQUESTID == applicationId && r.DELETED == false);
        ...
        public IQueryable<CamProcessedLoanViewModel> GetApplicationsAtOfferLetter(int staffId, int branchId, int companyId) // RM Review
        {
            ...
            var acceptIds = (from a in context.TBL_LOAN_APPLICATION
                             ...
                             && ( (a.CREATEDBY == staffId && countryCode == "MZN") || (countryCode != "MZN"))
        ...
        public List<CamProcessedLoanViewModel> GetApplicationsDueForAvailmentRoute(int staffId, int companyId)
        {
            ...
            var dueForAvailmentDate = context.TBL_LOAN_APPLICATION.Where(x => x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationInProgress && x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationCompleted)
                ...
                .Select(x => new CamProcessedLoanViewModel
                {
                    ...
                    regionId = context.TBL_BRANCH_REGION.FirstOrDefault(c => c.REGIONID == x.c.a.CAPREGIONID).REGIONID ,
        ...
        public async Task<Form3800ViewModel> GenerateForm3800Template(string applicationRefNumber)
        {
            var applDate = context.TBL_FINANCECURRENTDATE.FirstOrDefault().CURRENTDATE;
            var currentDate = DateTime.Now;

            var targetAppl = await context.TBL_LOAN_APPLICATION.FirstOrDefaultAsync(x => x.APPLICATIONREFERENCENUMBER == applicationRefNumber && x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationInProgress && x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationCompleted);
            ...
            var productClassProcess = await context.TBL_PRODUCT_CLASS.FirstOrDefaultAsync(x => x.PRODUCTCLASSID == targetAppl.PRODUCTCLASSID);
            ...
            var conditionPrecedentsSub = await (from a in context.TBL_LOAN_APPLICATION
                                          join c in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals c.LOANAPPLICATIONID
                                         ...
                                         && c.STATUSID == (int)ApprovalStatusEnum.Approved
                                          ...
                                          select new OfferLetterConditionPrecidentViewModel()
                                          {
                                              ...
                                              isExternal = b.ISEXTERNAL,
                                              productName = c.TBL_PRODUCT.PRODUCTNAME
                                          }).GroupBy(x => x.conditionPrecident).Select(y => y.FirstOrDefault()).ToListAsync();

            var conditionPrecedents = conditionPrecedentsSub;

            var conditionSubsequents = await (from a in context.TBL_LOAN_APPLICATION
                                        join c in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals c.LOANAPPLICATIONID
                                        join b in context.TBL_LOAN_CONDITION_PRECEDENT on c.LOANAPPLICATIONDETAILID equals b.LOANAPPLICATIONDETAILID
                                        where a.APPLICATIONREFERENCENUMBER == applicationRefNumber
                                        && a.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationInProgress
            ...
            var products = await (from a in context.TBL_LOAN_APPLICATION
                            join c in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals c.LOANAPPLICATIONID
                            where a.APPLICATIONREFERENCENUMBER == applicationRefNumber
                            && a.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationInProgress
                            && a.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CancellationCompleted
            ...
            var fees = await (from a in context.TBL_LOAN_APPLICATION_DETL_FEE
                        join b in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONDETAILID equals b.LOANAPPLICATIONDETAILID
                        join c in context.TBL_CHARGE_FEE on a.CHARGEFEEID equals c.CHARGEFEEID
            ...
            var loanDetails = await (from a in context.TBL_LOAN_APPLICATION
                               join b in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
            ...
            var transactionDynamicsDetails = await (from a in context.TBL_LOAN_TRANSACTION_DYNAMICS
                                              join b in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONDETAILID equals b.LOANAPPLICATIONDETAILID
            ...
            var loanCollaterals = await (from x in context.TBL_LOAN_APPLICATION_COLLATRL2
            ...
            var loanMonitoringTriggers = await (from x in context.TBL_LOAN_APPLICATN_DETL_MTRIG
            ...
            var loanComments = await (from x in context.TBL_LOAN_APPLICATION_COMMENT
                                join y in context.TBL_LOAN_APPLICATION on x.LOANAPPLICATIONID equals y.LOANAPPLICATIONID
            ...
            foreach (var prod in products)
            {
                ...
                conditions = $"<p><strong> {TranslateHelper.get("Conditions Precedent(to be satisfied before drawdown)")} {prod.productName}</strong></p>";
                ...
                conditions = conditions +
                        ...
                        $"<strong> {TranslateHelper.get("Conditions Precedent")} </strong></td>" +

                        $"<td style='height:31.0pt; vertical-align:top; width:100.05pt'><p> &nbsp;</p>" +

                        $"<strong> {TranslateHelper.get("Applicable Facility")} </strong ></td>" +

                        $"<td style='height:31.0pt; vertical-align:top; width:1.0in'>" +

                        $"<strong> *{TranslateHelper.get("Credit Verification Officer's initial for compliance only")}</strong></td>" +

                        $"<td style='height:31.0pt; vertical-align:top; width:67.5pt'>" +

                        $"<strong> {TranslateHelper.get("Location of document")} </strong><strong><em> {TranslateHelper.get("(Corporate workflow)")}</em ></strong></td></tr>";
                ...
                conditions = conditions +
                    ...
                    $"<p><strong> {TranslateHelper.get("Other Conditions Precedent for Internal usage which does not have to be included in the offer letter")}" +
                    $"{TranslateHelper.get("The RM must ensure compliance with these conditions before drawdown")}.</strong></p></td></tr> ";
            ...
            foreach (var prod in products)
            {
                ...
                conditions = $"<p><strong>{TranslateHelper.get("Conditions Subsequent (to be satisfied after drawdown)")} {prod.productName}</strong></p>";
                ...
                conditions = conditions +
                        ...
                        $"<strong> {TranslateHelper.get("Conditions Subsequent")} </strong></td>" +

                        $"<td style='height:31.0pt; vertical-align:top; width:100.05pt'><p> &nbsp;</p>" +

                        $"<strong> {TranslateHelper.get("Timeline for compliance")} </strong ></td>" +

                        $"<td style='height:31.0pt; vertical-align:top; width:1.0in'><p> &nbsp;</p>" +

                        $"<strong> {TranslateHelper.get("Credit Monitoring Officer’s initial for compliance only")}</strong></td>" +
                ...
                conditions = conditions +
                    "<tr class='removeConditions_OL'><td colspan='5' style='height:18.4pt; vertical-align:top; width:490.5pt'>" +
                    $"<strong> {TranslateHelper.get("Other Conditions Subsequent for Internal usage which does not have to be included in the offer letter")} " +
                    $"{TranslateHelper.get("The RM must ensure compliance with these conditions after drawdown")}.</strong></td></tr> ";
            ...
            fee = fee +
                    ...
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                        $"<strong> S/No </strong></td>" +
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                    $"<strong> {TranslateHelper.get("Name")} </strong></p></td>" +

                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                    $"<strong> {TranslateHelper.get("Rate")} </strong></td>" +


                                        $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                    $"<strong> {TranslateHelper.get("Product")} </strong></td></tr>";
            ...
            loanDetail = loanDetail +
                    ...
                    $"<strong> {TranslateHelper.get("Facility Type")} </strong></p></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:130.5pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Purpose")} </strong></td>" +
                     $"<td style='height:29.65pt; vertical-align:top; width:119.8pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Limits")} N </strong></p></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:49.51n'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Tenor")} </strong></p></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:119.8pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Interest/Margin")} </strong></p></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:.75in'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Review Date")} </strong></td></tr>";
            ...
            loanCollateral = loanCollateral +
                    ...
                    $"<strong> S/No </strong></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:1.25in'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Type and description of security")} </strong></p></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:130.5pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Value")}(<s>N</s>) </strong></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:.75in'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Facility Amount")} (<s>N</s>) </strong></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:.75in'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Amount Stamped To Cover")} (<s>N</s>) </strong></td></tr>";
            ...
            loanComment = loanComment +
                    ...
                    $"<strong> S/No </strong></td>" +
                    $"<td style='height:29.65pt; vertical-align:top; width:1.25in'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Comments")} </strong></p></td></tr>";
            ...
            loanMonitoringTrigger = $"<p><strong> {TranslateHelper.get("Monitoring Triggers")}: </strong></p>";
            ...
            loanMonitoringTrigger = loanMonitoringTrigger +
                    ...
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                        $"<strong> S/No </strong></td>" +
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                    $"<strong> {TranslateHelper.get("Monitoring Trigger")} </strong></td>" +
                                       $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                        $"<strong> {TranslateHelper.get("Product Name")} </strong></td>" + 
            ...
            loanTransactionDynamics = $"<p><strong> {TranslateHelper.get("Transaction Dynamics")}: </strong></p>";
            ...
            loanTransactionDynamics = loanTransactionDynamics +
                    ...
                    $"<strong> S/No </strong></td>" +
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Dynamics")} </strong></p></td>" +
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'><p> &nbsp;</p>" +
                    $"<strong> {TranslateHelper.get("Credit Verification Officer’s initial for compliance only")} </strong></p></td>" +
                    $"<td style='height:31.0pt; vertical-align:top; width:40.45pt'>" +
                    $"<strong> {TranslateHelper.get("Product Name")} </strong></td></tr>";
        ...
        public async Task<Form3800ViewModel> GenerateForm3800TemplateLMS(string applicationRefNumber)
        {
            ...
            var targetAppl = await context.TBL_LMSR_APPLICATION.FirstOrDefaultAsync(x => x.APPLICATIONREFERENCENUMBER == applicationRefNumber);

            var templateLink = GetProductSpecificTemplate(1, 1);

          

            var conditionPrecedents = await (from a in context.TBL_LMSR_APPLICATION
            ...
            var conditionSubsequents = await (from a in context.TBL_LMSR_APPLICATION
                                        join c in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals c.LOANAPPLICATIONID
                                        join b in context.TBL_LMSR_CONDITION_PRECEDENT on c.LOANREVIEWAPPLICATIONID equals b.LOANREVIEWAPPLICATIONID
            ...
            var products = await (from a in context.TBL_LMSR_APPLICATION
                            join c in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals c.LOANAPPLICATIONID
                            where a.APPLICATIONREFERENCENUMBER == targetAppl.APPLICATIONREFERENCENUMBER
            ...
            var fees = await (from a in context.TBL_LMSR_APPLICATION_DETL_FEE
            ...
            var loanDetails = await (from a in context.TBL_LMSR_APPLICATION
            ...
            {
                 loanDetails = await (from a in context.TBL_LMSR_APPLICATION
                                 join b in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
            ...
            var transactionDynamicsDetails = await (from a in context.TBL_LMSR_TRANSACTION_DYNAMICS
                                              join b in context.TBL_LMSR_APPLICATION_DETAIL on a.LOANREVIEWAPPLICATIONID equals b.LOANREVIEWAPPLICATIONID
                                              //join c in context.TBL_CUSTOMER on a.CUSTOMERID equals c.CUSTOMERID into cc
                                              //from c in cc.DefaultIfEmpty()
                                              //join d in context.TBL_CUSTOMER_GROUP on a.CUSTOMERGROUPID equals d.CUSTOMERGROUPID into cg
            ...
            var loanCollaterals = await (from x in context.TBL_LMSR_APPLICATION_COLLATRL2
            ...
            var loanMonitoringTriggers = await (from x in context.TBL_LMSR_APPLICATN_DETL_MTRIG
                                          join y in context.TBL_LMSR_APPLICATION_DETAIL on x.LOANREVIEWAPPLICATIONID equals y.LOANREVIEWAPPLICATIONID
                                          where y.TBL_LMSR_APPLICATION.APPLICATIONREFERENCENUMBER == targetAppl.APPLICATIONREFERENCENUMBER
                                          select new MonitoringTriggersViewModel()
                                          {
            ...
            fee = $"<br/>" + $"<br/>" + $"<br/>" + $"<br/>" + $"<br/>" + $"<br/>" + $"<br/>" + $"<br/>" +
                $"<p><strong> {TranslateHelper.get("Fee Details")}: </strong></p>";
        ...
        public async Task<bool> SaveDraftOfferLetter(OfferLetterTemplateViewModel model)
        {
            if (model != null)
            {
                try
                {
                    var exisitingDocument = await context.TBL_TEMP_OFFERLETTER.Where(x => x.APPLICATIONREFERENCENUMBER == model.applicationReferenceNumber).FirstOrDefaultAsync();
        ...
        public async Task<bool> UpdateDraftOfferLetter(int documentId, OfferLetterTemplateViewModel model)
        {
            if (model != null)
            {
                try
                {
                    var exisitingDocument = await context.TBL_TEMP_OFFERLETTER.FindAsync(documentId);
        ...
        public async Task<int> ApproveLoanAvailmentDecision(LoanAvailmentApprovalViewModel entity)
        {
            int operationId = (int)OperationsEnum.LoanAvailment;// (int)OperationsEnum.LoanAvailment;
            var loanApplication = await context.TBL_LOAN_APPLICATION.FirstOrDefaultAsync(x => x.APPLICATIONREFERENCENUMBER == entity.applicationReferenceNumber);
            ...
            var initiated = await context.TBL_APPROVAL_TRAIL.Where(x => x.OPERATIONID == operationId && x.TARGETID == loanApplication.LOANAPPLICATIONID).AnyAsync();
        ...
        private int BeforeAvailmentValidationChecks(int applicationId)
        {
            ...
            {
                result = 1;
                throw new ConditionNotMetException(TranslateHelper.get("Kindly Ensure All Loan Details Have CRMS Collateral Type Attached"));
            }
        ...
        private void PendingJobRequestCheck(IQueryable<TBL_LOAN_APPLICATION_DETAIL> loanApplicationDetails)
        {
            foreach (var item in loanApplicationDetails)
            {
                if (context.TBL_JOB_REQUEST.Where(x => x.TARGETID == item.LOANAPPLICATIONDETAILID && x.JOBTYPEID != (short)JobTypeEnum.legal && (x.REQUESTSTATUSID == (short)JobRequestStatusEnum.pending || x.REQUESTSTATUSID == (short)JobRequestStatusEnum.processing)).Any())
                    throw new ConditionNotMetException(TranslateHelper.get("There are pending job request for this application."));
                ...
                {
                    if (context.TBL_JOB_REQUEST.Where(x => x.TARGETID == item.LOANAPPLICATIONDETAILID && x.JOBTYPEID == (short)JobTypeEnum.legal && (x.REQUESTSTATUSID == (short)JobRequestStatusEnum.pending || x.REQUESTSTATUSID == (short)JobRequestStatusEnum.processing)).Any())
                        throw new ConditionNotMetException(TranslateHelper.get("There are unattended Legal job request which must be attended to."));
                }
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public async Task<WorkflowResponse> ApproveOfferLetterGeneration(LoanAvailmentApprovalViewModel model)
        {
            var operationId = (int)OperationsEnum.OfferLetterApproval;
            var appl = await context.TBL_LOAN_APPLICATION.FirstOrDefaultAsync(x => x.APPLICATIONREFERENCENUMBER == model.applicationReferenceNumber
            ...
            if (appl == null) throw new SecureException(TranslateHelper.get("Loan application with the given reference number not found!"));
            ...
            if (workflow.NewState == (int)ApprovalState.Ended)
            {
                ...
                if (appl.PRODUCTCLASSID == (short)ProductClassEnum.BondAndGuarantees) // Bonds and Guarantees adapter
                {
                    ...
                    if (bondAndGauranteeSent == false)
                        throw new ConditionNotMetException(TranslateHelper.get("There is no Job Request sent to Legal for the B&G document. Please send one to proceed to availment!."));
                }
                ...
                var loanApplicationDetails = await context.TBL_LOAN_APPLICATION_DETAIL
                ...
                var staffName = await context.TBL_STAFF.Where(s => s.STAFFID == model.staffId).FirstOrDefaultAsync();
        ...
        public async Task<bool> OfferLetterRejection(ForwardViewModel model)
        {
            var operationId = model.operationId;
            var o = await context.TBL_APPROVAL_TRAIL.FindAsync(model.trailId); // here we try to get the staffid on the trail row
            var appl = await context.TBL_LOAN_APPLICATION.FindAsync(model.applicationId);

            var trail = await context.TBL_APPROVAL_TRAIL.FirstOrDefaultAsync(x =>
                x.OPERATIONID == operationId
                && x.TARGETID == appl.LOANAPPLICATIONID
                && x.REQUESTSTAFFID == o.REQUESTSTAFFID
            );
        ...
        public async Task<IEnumerable<CommentOnLoanAvailmentViewModel>> GetCommentOnLoanAvailment(string applicationRefNumber)
        {
            var data = await (from a in context.TBL_APPROVAL_TRAIL
        ...
        public async Task<bool> SendBackToBusinessAvailment(LoanAvailmentApprovalViewModel model)
        {
            int? productClassId = 0;
            int staffId = 0;
            if (model.operationId == (int)OperationsEnum.LoanAvailment)
            {
                var appla = await context.TBL_LOAN_APPLICATION.FindAsync(model.targetId);
            ...
            {
                var applb = await context.TBL_LMSR_APPLICATION.FindAsync(model.targetId);
                productClassId = null;
                staffId = applb.CREATEDBY;
            }

            var staff = await context.TBL_STAFF.Where(x => x.STAFFID == staffId).FirstOrDefaultAsync();

            var levels = await context.TBL_APPROVAL_GROUP_MAPPING.Where(x => x.OPERATIONID == model.operationId && x.PRODUCTCLASSID == productClassId)
                 .Join(context.TBL_APPROVAL_GROUP, m => m.GROUPID, g => g.GROUPID, (m, g) => new { m, g })
                 .Join(context.TBL_APPROVAL_LEVEL.Where(x => x.ISACTIVE == true),
                     mg => mg.g.GROUPID, l => l.GROUPID, (mg, l) => new
                     {
        ...
        public async Task<bool> ReferBackOneStep(LoanAvailmentApprovalViewModel model)
        {
            int? productClassId = 0;
            ...
            var from = await context.TBL_STAFF.Where(x => x.STAFFID == model.staffId).FirstOrDefaultAsync();
            ...
            workflow.ToStaffId = staffId;
            workflow.StatusId = (int)ApprovalStatusEnum.Referred;
            workflow.Comment = TranslateHelper.get("Referred back from ") + from.FIRSTNAME + " " + from.MIDDLENAME + " " + from.LASTNAME;
            workflow.DeferredExecution = true;
        ...
        private void ChecklistValidation(int applicationId)
        {
            ...
            if (conditionItems.Any()) throw new SecureException(TranslateHelper.get("One or more condition(s) is not validated") + "." + Environment.NewLine + " " + TranslateHelper.get("Please check your response to confirm") + "." + Environment.NewLine);
            ...
            foreach (var d in applicationDetails)
            {
                foreach (var item in types)
                {
                    ...
                    if (omission.Any()) throw new SecureException($"{TranslateHelper.get("One or more item(s) is not validated")} : { item.CHECKLIST_TYPE_NAME } {Environment.NewLine} {TranslateHelper.get(" Please check your response to confirm. ")} {Environment.NewLine}");
        ...
        public void AddOfferLetterClauses(int applicationId, int staffId,bool isLMS, bool callSaveChanges)
        {
            ...
            {
                offerLetterDoc.OFFERLETTERSALUTATION = TranslateHelper.get("The Managing Director") + ", <br /><br /> " + detail.customerName + "<br /><br />" + detail.customerAddress + "<br /><br />" + TranslateHelper.get("Attention") + ": " + detail.title + " " + detail.customerName ;
        ...
        public async Task<bool> EditOfferLetterTitle(int custimerId, string data, int staffId, int branchId)
        {
            var clause = await context.TBL_CUSTOMER.Where(o => o.CUSTOMERID == custimerId).Select(o => o).FirstOrDefaultAsync();
            if (clause != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL =
                   $"{TranslateHelper.get("Change offer letter title for a customer with customerId")} " + custimerId,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = "",
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> EditOfferLetterSalutation(int custimerId, string data, int staffId, int branchId)
        {
            var clause = await context.TBL_CUSTOMER.Where(o => o.CUSTOMERID == custimerId).Select(o => o).FirstOrDefaultAsync();
            if (clause != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    BRANCHID = (short)branchId,
                    DETAIL =
                  $"{TranslateHelper.get("Change offer letter saluatation for a customer with customerId")} " + custimerId,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = "",
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> EditOfferLetterAcceptance(int applicationId, string data, bool isLMS, int staffId, int branchId)
        {
            var clause = await context.TBL_LOAN_OFFER_LETTER.Where(o => o.LOANAPPLICATIONID == applicationId && o.ISLMS == isLMS).Select(o => o).FirstOrDefaultAsync();
            if (clause != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    BRANCHID = (short)branchId,
                    DETAIL =
                   $"{TranslateHelper.get("Change offer letter acceptance description for customer with loan application Id")} " + applicationId,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = "",
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> EditOfferLetterClause(int applicationId, string data, bool isLMS, int staffId, int branchId)
        {
            var clause = await context.TBL_LOAN_OFFER_LETTER.Where(o => o.LOANAPPLICATIONID == applicationId && o.ISLMS==isLMS).Select(o => o).FirstOrDefaultAsync();
            if (clause != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    BRANCHID = (short)branchId,
                    DETAIL =
                   $"{TranslateHelper.get("Change offer letter clause for customer with loan application Id")} " + applicationId,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),                
                    DEVICENAME = CommonHelpers.GetDeviceName(),
                    OSNAME = CommonHelpers.FriendlyName(),
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
        ...
        public async Task<OfferLetterViewModel> GetOfferLetterTitle(int custimerId)
        {
            return await context.TBL_CUSTOMER.Where(o => o.CUSTOMERID == custimerId).Select( o => new OfferLetterViewModel {
        ...
        public async Task<OfferLetterViewModel> GetOfferLetterAcceptance(int applicationId)
        {
            return await context.TBL_LOAN_OFFER_LETTER.Where(o => o.LOANAPPLICATIONID == applicationId).Select(o => new OfferLetterViewModel
            {
        ...
        public async Task<OfferLetterViewModel> GetOfferLetterClause(int applicationId)
        {
            return await context.TBL_LOAN_OFFER_LETTER.Where(o => o.LOANAPPLICATIONID == applicationId).Select(o => new OfferLetterViewModel
            {
        ...
        public async Task<bool> ApplyTemplateToOfferLetter(int templateId, int loanApplicationId, int staffId, int branchId)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)branchId,
                DETAIL =
               $"{TranslateHelper.get("Offer Letter Template Has Been Applied For Loan Application ID")}  " + loanApplicationId,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
                APPLICATIONDATE = genSetup.GetApplicationDate(),
        ...
        public void AddOfferLetterClausesWithTemplate(int applicationId, int templateId, int staffId, bool isLMS, bool callSaveChanges)
        {
            ...
            if (offerLetterDoc != null)
            {
                offerLetterDoc.OFFERLETTERSALUTATION = TranslateHelper.get("The Managing Director")+ ", <br /><br /> " + detail.customerName + "<br /><br />" + detail.customerAddress + "<br /><br />" + TranslateHelper.get("Attention") + ": " + detail.title + " " + detail.customerName2;
