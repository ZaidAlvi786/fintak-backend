...
namespace FintrakBanking.Repositories.Credit
{
    ...
    public class LoanPreliminaryEvaluationRepository : ILoanPreliminaryEvaluationRepository
    {
        ...
        public async Task<LoanPreliminaryEvaluationViewModel> AddPreliminaryEvaluation(LoanPreliminaryEvaluationViewModel model)
        {
            if (model == null)
            {
                throw new SecureException(TranslateHelper.get("The data submitted in the form is invalid. Please try again"));
            }
            ...
            {
                detail = $"{TranslateHelper.get("Created Preliminary Evaluation with code")} ({model.preliminaryEvaluationCode}) {TranslateHelper.get("for customer with code")} {customerRecord.CUSTOMERCODE}";
            }
            else
            {
                detail = $"{TranslateHelper.get("Created Preliminary Evaluation with code")} ({model.preliminaryEvaluationCode})";
            }
            ...
            {
                auditRecord.DETAIL =
                    $"{TranslateHelper.get("Created Preliminary Evaluation Note with code")} ({model.preliminaryEvaluationCode}) {TranslateHelper.get("for customer")} {customerRecord?.FIRSTNAME} {customerRecord?.LASTNAME}";
            }
            else
            {
                auditRecord.DETAIL = $"{TranslateHelper.get("Created Preliminary Evaluation with code")} ({model.preliminaryEvaluationCode}) {TranslateHelper.get("for customer group")} {groupRecord?.GROUPNAME}";
            }
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (model.sendForEvaluation)
                    {
                        ...
                        workFlow.Comment = TranslateHelper.get("Request Preliminary Evaluation");
        ...
        public async Task<LoanPreliminaryEvaluationViewModel> AddMultiplePreliminaryEvaluation(List<LoanPreliminaryEvaluationViewModel> model)
        {
            ...
            foreach (var item in model)
            {
                ...
                var auditRecord = new TBL_AUDIT()
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Created Preliminary Evaluation with code")} ({penRecord.PRELIMINARYEVALUATIONCODE}) {TranslateHelper.get("for customer")} {customerRecord.FIRSTNAME} {customerRecord.LASTNAME}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetLoanPreliminaryEvaluationsAwaitingApprovalByLoanTypeId(int staffId, int companyId, int loanTypeId)
        {
            int[] regionIds = GetStaffRegions(staffId);

            if (regionIds.Length > 0)
                return await GetCustomerPreliminaryEvaluationsByAtaffRegionAwaitingApproval(staffId, companyId, regionIds);


            switch (loanTypeId)
            {
                case (int)LoanTypeEnum.Single:
                    return await GetSingleCustomerPreliminaryEvaluationsAwaitingApproval(staffId, companyId);

                case (int)LoanTypeEnum.CustomerGroup:
                    return await GetGroupCustomerPreliminaryEvaluationsAwaitingApproval(staffId, companyId);
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetSingleCustomerPreliminaryEvaluationsAwaitingApproval(int staffId, int companyId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.LoanPreliminaryEvaluation).ToList();

            var data = await (from pen in context.TBL_LOAN_PRELIMINARY_EVALUATN
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetGroupCustomerPreliminaryEvaluationsAwaitingApproval(int staffId, int companyId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.LoanPreliminaryEvaluation).ToList();
        ...
        private bool ApprovePreliminaryEvaluation(int loanPenId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved Prelimenary Evaluation with code")} ({penRecord.PRELIMINARYEVALUATIONCODE})",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetLoanPreliminaryEvaluationsByLoanTypeId(int loanTypeId)
        {
           
            switch (loanTypeId)
            {
                case (int)LoanTypeEnum.Single:
                    return await GetAllSingleCustomerLoanPreliminaryEvaluations();

                case (int)LoanTypeEnum.CustomerGroup:
                    return await GetAllGroupCustomerLoanPreliminaryEvaluations();
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetLoanApplicationPreliminaryEvaluations(int applicationId)
        {
        ...
        public async Task<IEnumerable<LookupViewModel>> GetCustomerLoanPreliminaryEvaluations(int customerId, int loanTypeId, int customerGroupId = 0)
        {
            if ((int)LoanTypeEnum.Single == loanTypeId)
            {
                var data = await (from p in context.TBL_LOAN_PRELIMINARY_EVALUATN
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetLoanPreliminaryEvaluationMappedToApplication()
        {
            var data = await (from p in context.TBL_LOAN_PRELIMINARY_EVALUATN
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetAllSingleCustomerLoanPreliminaryEvaluations()
        {
            var data = await (from p in context.TBL_LOAN_PRELIMINARY_EVALUATN
                        join coy in context.TBL_COMPANY on p.COMPANYID equals coy.COMPANYID
                        join br in context.TBL_BRANCH on p.BRANCHID equals br.BRANCHID
        ...
        public async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetAllGroupCustomerLoanPreliminaryEvaluations()
        {
            var data = await (from p in context.TBL_LOAN_PRELIMINARY_EVALUATN
                        join coy in context.TBL_COMPANY on p.COMPANYID equals coy.COMPANYID
                        join br in context.TBL_BRANCH on p.BRANCHID equals br.BRANCHID
        ...
        public async Task<bool> UpdatePreliminaryEvaluation(int loanPenId, LoanPreliminaryEvaluationViewModel model)
        {
            ...
            var auditRecord = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Preliminary Evaluation with code")} ({penRecord.PRELIMINARYEVALUATIONCODE}) {TranslateHelper.get("updated")}",
        ...
        private async Task<IEnumerable<LoanPreliminaryEvaluationViewModel>> GetCustomerPreliminaryEvaluationsByAtaffRegionAwaitingApproval(int staffId, int companyId, int[] regionIds)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.LoanPreliminaryEvaluation).ToList();
            ...
            var data2 = await (from pen in context.TBL_LOAN_PRELIMINARY_EVALUATN
                        join coy in context.TBL_COMPANY on pen.COMPANYID equals coy.COMPANYID
                        join br in context.TBL_BRANCH on pen.BRANCHID equals br.BRANCHID
                        join atrail in context.TBL_APPROVAL_TRAIL on pen.LOANPRELIMINARYEVALUATIONID equals atrail.TARGETID
                        where (atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Pending || atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing)
        ...
        public async Task<bool> SendPreliminaryEvaluationForLoanApplication(int loanPenId, LoanPreliminaryEvaluationViewModel model)
        {
            var penRecord = await context.TBL_LOAN_PRELIMINARY_EVALUATN.FindAsync(loanPenId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = model.branchId,
                DETAIL = $"{TranslateHelper.get("Prelimenary Evaluation with code")} ({penRecord.PRELIMINARYEVALUATIONCODE}) {TranslateHelper.get("has been sent for loan application")}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
