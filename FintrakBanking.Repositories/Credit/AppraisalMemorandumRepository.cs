...
namespace FintrakBanking.Repositories.Credit
{
    public class AppraisalMemorandumRepository : IAppraisalMemorandumRepository
    {
        ...
        private FinTrakBankingStagingContext stgContext;
            ...
            )
        {
            ...
            this.subsidiaryRepo = _subsidiaryRepo;
        }
        ...
        public async Task<AppraisalMemorandumViewModel> AddAppraisalMemorandum(AppraisalMemorandumViewModel model)
        {
            var appl = context.TBL_LOAN_APPLICATION.Find(model.loanApplicationId);

            List<int> ExclusiveOperations = await (from flow in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
                                             where flow.FLOWCHANGEID == appl.FLOWCHANGEID
                                             select flow.OPERATIONID).ToListAsync();
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added AppraisalMemorandum")} '{ model.camRef }' ",
        ...
        public bool UpdateAppraisalMemorandum(AppraisalMemorandumViewModel model, int documentId)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Appraisal Memorandum Document")}'{ model.camRef }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(), //model.userIPAddress,
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public WorkflowResponse ForwardAppraisalMemorandum(ForwardViewModel model)
        {
            //   Task.Run(() => CreateOutPutDocument(model.applicationId));
                ...
                var countryCode = context.TBL_COMPANY.FirstOrDefault().COUNTRYCODE;
            ...
            items = context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == appl.LOANAPPLICATIONID && x.DELETED == false).ToList();
                ...
                {
                    throw new SecureException(TranslateHelper.get("Please Kindly refresh your browser and try again, Thanks"));
                }
                ...
                if (appl.APPROVALSTATUSID == (int)ApprovalStatusEnum.Referred)
                {
                    ...
                    {
                        var resStaffId = 0;
                        if (model.isRemoteCall == true) 
                        {
                            var staf = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == model.staffRoleCode).FirstOrDefault();
                            if (staf != null)
                            {
                                resStaffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == model.staffRoleCode).FirstOrDefault().REMOTENETWORKID;
                            }
                            else
                            {
                                resStaffId = context.TBL_STAFF.FirstOrDefault().STAFFID;
                            ...
                            currentTrail.RESPONSESTAFFID = resStaffId;
                        ...
                        workflow.IsExternalSystemApprover = appl.TBL_APPROVAL_LEVEL.ISEXTERNALSYSTEMAPPROVER;
                        workflow.IsGlobalCustomer = appl.TBL_CUSTOMER.ISGLOBALCUSTOMER;
                        workflow.StaffRoleCode = model.staffRoleCode;
                        workflow.IsRemoteCall = model.isRemoteCall;
                        // if (true) { workflow.IsRemoteCall = true; }
                        ...
                        if (workflow.Response.isExternalSystemApprover)
                        {
                            var roleId = context.TBL_APPROVAL_LEVEL.Where(x => x.APPROVALLEVELID == workflow.NextLevelId).FirstOrDefault()?.STAFFROLEID;
                            model.staffRoleCode = context.TBL_STAFF_ROLE.FirstOrDefault(x => x.STAFFROLEID == roleId)?.STAFFROLECODE;
                            model.staffRoleId = context.TBL_APPROVAL_LEVEL.Where(c => c.APPROVALLEVELID == workflow.NextLevelId).FirstOrDefault().STAFFROLEID;
                            model.nextApprovalLevelId = (int)workflow?.NextLevelId;
                            SendAppraisalDataToGroupOffice(model);
                            trans.Commit();
                        }
                ...
                // WORKFLOW
                if(model.vote > 5) { model.vote = (short)ApprovalStatusEnum.Referred; };
                    ...
                    workflow.StaffRoleCode = model.staffRoleCode;
                    workflow.IsRemoteCall = model.isRemoteCall;
                    if(appl.CUSTOMERID != null && appl.CUSTOMERGROUPID == null){ workflow.IsGlobalCustomer = appl.TBL_CUSTOMER.ISGLOBALCUSTOMER; };
                    if (appl.CUSTOMERID == null && appl.CUSTOMERGROUPID != null) { workflow.IsGlobalCustomer = (bool)context.TBL_CUSTOMER_GROUP.Where(x => x.CUSTOMERGROUPID == appl.CUSTOMERGROUPID).FirstOrDefault().ISGLOBALCUSTOMER; };
                    ...
                    if (appl.CUSTOMERGROUPID == null)
                    {
                        ...
                        placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c?.MIDDLENAME + " " + c.LASTNAME;
                    }
                    else
                    {
                        placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + appl.TBL_CUSTOMER_GROUP.GROUPNAME;
                    }
                    placeholders.referenceNumber = TranslateHelper.get("<br />APPLICATION REFERENCENUMBER: ") + appl.APPLICATIONREFERENCENUMBER;
                    placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: ") + facilityInformationMarkup;
                    placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: Loan Origination");
                    placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + appl.TBL_BRANCH.BRANCHNAME;
                ...
                // UPDATE APPLICATION
                    ...
                    if (staf != null)
                    {
                        staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == model.staffRoleCode).FirstOrDefault().REMOTENETWORKID;
                    }
                    else
                    {
                ...
                if (appl.APIREQUESTID != null )
                {
                    staffId = model.staffId;
                    var remoteRoleId = context.TBL_STAFF.Where(x => x.STAFFID == staffId).FirstOrDefault().STAFFROLEID;
                    ///////////////////// Call Refer Back API /////////////////////
                    var statusCode = ""; // Approved = "90", Rejected = "99"
                    statusCode = workflow.StatusId == (short)ApprovalStatusEnum.Disapproved ? "05" : "00";
                    var status = "";
                    status = workflow.NewState == (int)ApprovalState.Ended ? "Completed" : "In Progress";
                    if (model.isFlowTest == false)
                    {
                        if (appl.APIREQUESTID != null)
                        {
                            if (staffId == 0) { throw new SecureException("Staff does not exist in current context"); }
                            LoanStatusChangeThroughAPI(appl, model.comment, staffId, statusCode, status);
                        }
                        //else { LoanStatusChangeThroughAPI(appl, model.comment, staff.STAFFID, statusCode); } Cash Flow only.... Repetition
                    }
                ...
                ////////////////////// Call Status Change API /////////////////
                    ...
                    if (model.isRemoteCall == true) { staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == model.staffRoleCode).FirstOrDefault().REMOTENETWORKID; };
                    var statusCode = ""; // Approved = "90", Rejected = "99"
                        statusCode = workflow.StatusId == (short)ApprovalStatusEnum.Disapproved ? "05" : "00";
                    var status = "";
                    status = workflow.NewState == (int)ApprovalState.Ended ? "Completed" : "In Progress";
                   ...
                   if (!model.isFlowTest) { 
                    ...
                    if (workflow.NewState == (int)ApprovalState.Ended) // cam status
                    {
                        ...
                        if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                        {
                            ...
                            if (appl.APIREQUESTID != null)
                            {
                                appl.APPLICATIONSTATUSID = (short)LoanApplicationStatusEnum.BookingRequestCompleted;
                                appl.APPROVALSTATUSID = (short)ApprovalStatusEnum.Approved;
                            }
                        ...
                        if (contextControl != null)
                        {
                            ...
                            if (model.isFlowTest == true) { trans.Rollback(); }
                    ...
                    if (model.isRemoteCall == true)
                    {
                        var staf = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.LOCALNETWORKID == model.createdBy).FirstOrDefault();
                        if(staf != null)
                        {
                            staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.LOCALNETWORKID == model.createdBy).FirstOrDefault().REMOTENETWORKID;
                        }
                        else
                        {
                            staffId = context.TBL_STAFF.FirstOrDefault().STAFFID;
                        ...
                        var audit = new TBL_AUDIT
                        {
                            ...
                            DETAIL = $"{TranslateHelper.get("Loan Application Reference Number:")} '{ appl.APPLICATIONREFERENCENUMBER }', " +
                                        $"{TranslateHelper.get("StaffId:")} '{ staffId }', " +
                                        $"{TranslateHelper.get("TargetId:")} '{ model.applicationId }', " +
                                        $"{TranslateHelper.get("Vote:")} '{ model.vote }', " +
                                        $"{TranslateHelper.get("NextLevelId:")} '{ model.receiverLevelId }', " +
                                        $"{TranslateHelper.get("ToStaffId:")} '{ model.receiverStaffId }', " +
                                        $"{TranslateHelper.get("StatusId:")} '{ model.forwardAction }', " +
                                        $"{TranslateHelper.get("Comment:")} '{ model.comment }', " +
                                        $"{TranslateHelper.get("LINE CHANGES:")}" +
                       ...
                       var audit = new TBL_AUDIT
                        {
                            ...
                            BRANCHID = (short)model.userBranchId,
                            DETAIL = $"{TranslateHelper.get("Loan Application Reference Number:")} '{ appl.APPLICATIONREFERENCENUMBER }', " +
                                           $"{TranslateHelper.get("StaffId:")} '{ model.createdBy }', " +
                                           $"{TranslateHelper.get("TargetId:")} '{ model.applicationId }', " +
                                           $"{TranslateHelper.get("Vote:")} '{ model.vote }', " +
                                           $"{TranslateHelper.get("NextLevelId:")} '{ model.receiverLevelId }', " +
                                           $"{TranslateHelper.get("ToStaffId:")} '{ model.receiverStaffId }', " +
                    ...
                    if (workflow.NewState == (int)ApprovalState.Ended && workflow.StatusId != (int)ApprovalStatusEnum.Disapproved && appl.PRODUCTCLASSID != (short)ProductClassEnum.Creditcards)
                    {
                        ...
                        if (appl.APIREQUESTID !=null && countryCode == "MZN" && appl.OPERATIONID != (int)OperationsEnum.CashFlowDocumentReview )
                        {
                            appl.APPLICATIONSTATUSID = (int)LoanApplicationStatusEnum.CashFlowDocumentReviewInProgress;
                            appl.OPERATIONID = (int)OperationsEnum.CashFlowDocumentReview;
                            workflow.NextProcess(appl.COMPANYID, model.createdBy, (int)OperationsEnum.CashFlowDocumentReview, null,
        ...
        private void SendAppraisalDataToGroupOffice(ForwardViewModel model)
        {
            var appl = context.TBL_LOAN_APPLICATION.Find(model.applicationId);
            var loanAppl = context.TBL_LOAN_APPLICATION_DETAIL.Where(x=>x.LOANAPPLICATIONID == appl.LOANAPPLICATIONID).FirstOrDefault();
            var product = context.TBL_PRODUCT.Where(x => x.PRODUCTID == loanAppl.PROPOSEDPRODUCTID).FirstOrDefault();
            var customer = context.TBL_CUSTOMER.Where(x => x.CUSTOMERID == loanAppl.CUSTOMERID).FirstOrDefault();
            var bunit = context.TBL_PROFILE_BUSINESS_UNIT.Where(x => x.BUSINESSUNITID == customer.BUSINESSUNTID).FirstOrDefault();

            var approvalLevelId = context.TBL_APPROVAL_TRAIL.Where(x => x.TARGETID == appl.LOANAPPLICATIONID).FirstOrDefault();
            HeadOfficeFacilityApprovalViewModel subToHqData = new HeadOfficeFacilityApprovalViewModel();
            subToHqData.loanApplicationDetailId = loanAppl.LOANAPPLICATIONDETAILID;
            subToHqData.loanApplicationId = appl.LOANAPPLICATIONID;
            subToHqData.applicationAmount = loanAppl.PROPOSEDAMOUNT;
            subToHqData.interestRate = loanAppl.PROPOSEDINTERESTRATE;
            subToHqData.applicationDate = appl.APPLICATIONDATE;
            subToHqData.applicationReferenceNumber = appl.APPLICATIONREFERENCENUMBER;
            subToHqData.applicationStatusId = appl.APPLICATIONSTATUSID;
            subToHqData.applicationTenor = appl.APPLICATIONTENOR;
            subToHqData.approvalLevelId = model.nextApprovalLevelId;
            subToHqData.approvalStatusId = (short)workflow.StatusId;
            subToHqData.businessUnitShortCode = bunit?.BUSINESSUNITSHORTCODE;
            subToHqData.createdBy = appl.CREATEDBY;
            subToHqData.customerGlobalId = customer.CUSTOMERGLOBALNUMBER;
            subToHqData.customerId = appl.CUSTOMERID;
            subToHqData.dateTimeCreated = appl.DATETIMECREATED;
            subToHqData.dateTimeDeleted = appl.DATETIMEDELETED;
            subToHqData.dateTimeUpdated = appl.DATETIMEUPDATED;
            subToHqData.deleted = appl.DELETED;
            subToHqData.deletedBy = appl.DELETEDBY;
            if (appl.CUSTOMERID != null && appl.CUSTOMERGROUPID == null) { 
                subToHqData.firstName = appl.TBL_CUSTOMER.FIRSTNAME;
                subToHqData.lastName = appl.TBL_CUSTOMER.LASTNAME;
                subToHqData.middleName = appl.TBL_CUSTOMER.MIDDLENAME;
            };
            if (appl.CUSTOMERID == null && appl.CUSTOMERGROUPID != null) {
                subToHqData.firstName = context.TBL_CUSTOMER_GROUP.Where(x => x.CUSTOMERGROUPID == appl.CUSTOMERGROUPID).FirstOrDefault().GROUPNAME;
                subToHqData.lastName = context.TBL_CUSTOMER_GROUP.Where(x => x.CUSTOMERGROUPID == appl.CUSTOMERGROUPID).FirstOrDefault().GROUPDESCRIPTION;
            };
            
            subToHqData.lastUpdatedBy = appl.LASTUPDATEDBY;
            subToHqData.loanApplicationId = appl.LOANAPPLICATIONID;
            subToHqData.loanApplicationTypeName = appl.TBL_LOAN_APPLICATION_TYPE.LOANAPPLICATIONTYPENAME;
            subToHqData.operationName = appl.TBL_OPERATIONS.OPERATIONNAME;
            subToHqData.productClassName = appl.TBL_PRODUCT_CLASS.PRODUCTCLASSNAME;
            subToHqData.productClassProcess = appl.TBL_PRODUCT_CLASS_PROCESS.PRODUCT_CLASS_PROCESS_NAME;
            subToHqData.productName = product?.PRODUCTNAME;
            subToHqData.relatedReferenceNumber = appl.RELATEDREFERENCENUMBER;
            subToHqData.subsidiaryId = 1;
            subToHqData.toStaffId = appl.TBL_STAFF.STAFFID;
            subToHqData.totalExposureAmount = appl.TOTALEXPOSUREAMOUNT;
            subToHqData.systemDateTime = appl.SYSTEMDATETIME;
            subToHqData.approvalLevelGlobalCode = null;
            subToHqData.countryCode = context.TBL_COMPANY.Where(x=>x.COMPANYID == 1).FirstOrDefault().COUNTRYCODE;
            subToHqData.systemArrivalDateTime = DateTime.Now;
            subToHqData.staffRoleId = model.staffRoleId;
            subToHqData.trailId = model.trailId;
            subToHqData.staffRoleCode = model.staffRoleCode;
            subToHqData.actedOn = false;
            subToHqData.targetId = appl.LOANAPPLICATIONID;
            subToHqData.operationId = appl.TBL_OPERATIONS.OPERATIONID;
            var data = Newtonsoft.Json.JsonConvert.SerializeObject(subToHqData);
            //System.IO.File.WriteAllText(@"C:\Access Bank Projects\substohq.txt", data);
            subsidiaryRepo.PostFacilityApprovalnputs(subToHqData);
        }
        ...
        public async void LoanStatusChangeThroughAPI(TBL_LOAN_APPLICATION loanApplication, string comment, int staffId, string statusCode, string status)
        {
            //string cflReport = builder.ToString();
            string WorkflowStageName = status;
            ...
            CFLNotifyResponse offerLetters = new CFLNotifyResponse();
            ...
            offerLetters.isInsurance = loanApplication.ISINSURANCE;
        ...
        private async void ReferBackThroughAPI(TBL_LOAN_APPLICATION loanApplication, ForwardViewModel model, int staffRoleId)
        {
            if (loanApplication != null && loanApplication.APIREQUESTID != null)
            {
                ...
                if (product?.PRODUCTCODE == "EBFC")
                {
                    ...
                    if (WorkflowStageName != "" && loanApplication.APIREQUESTID != null) {
                       await transaction.ReferBackThroughAPI(offerLetters, loanApplication.APPLICATIONREFERENCENUMBER);
                    }
                }
            }
        }
        ...
        public async Task<IQueryable<LoanApplicationViewModel>> GetPendingAdhocApplications(int operationId, int companyId, int branchId, int staffId, int? classId)
        {
            // var declarations
            ...
            var querytest1 = await (from a in context.TBL_LOAN_APPLICATION
                             where
            ...
            var querytest2 = await (from b in context.TBL_APPROVAL_TRAIL where
        ...
        public async Task<WorkflowResponse> AdhocAppraisalMemorandum(ForwardViewModel model)
        {
            //   Task.Run(() => CreateOutPutDocument(model.applicationId));
            ...
            var nextStaff = await loanApp.GetFirstLevelStaffId((int)nextLevel, model.userBranchId);
            workflow.NextLevelId = null;
            //workflow.ToStaffId = nextStaff;PR
            workflow.StatusId = model.forwardAction;
            workflow.Comment = model.comment;
            string facilityInformationMarkup = GetFacilityInformationMarkup(appl.LOANAPPLICATIONID);
      ...
      */
        ...
        private bool PopulateLcCashBuildUp(int lcIssuanceId)
        {
            ...
            if (lc == null)
            {
                new SecureException(TranslateHelper.get("LC for cash buildup plan date population cannot be null!"));
            }
        ...
        public async Task<WorkflowResponse> LcAppraisalMemorandum(LcForwardViewModel model)
        {
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                var placeholders = new AlertPlaceholders();
                placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME;
                placeholders.referenceNumber = TranslateHelper.get("<br />LC REFERENCENUMBER: ") + lc.LCREFERENCENUMBER;
                placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER OF CREDIT");
                placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: LC ISSUANCE");
                placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
                workflow.Placeholders = placeholders;
        ...
        public async Task<WorkflowResponse> LcReleaseMemorandum(LcForwardViewModel model)
        {
            ...
            {
                var nextLevel = loanApp.GetFirstReceiverLevel(model.createdBy, operationId, null, null,null, true);
                var nextLvlStaff =await loanApp.GetFirstLevelStaffId((int)nextLevel, model.userBranchId);
                ...
                var testStaff =await  loanApp.GetFirstLevelStaffId((int)secondLvl, model.userBranchId);
  ...
  */
        ...
        public async Task<WorkflowResponse> LcCancelationMemorandum(LcForwardViewModel model)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("LC Issuance Cancelation Approval Already Ongoing or Completed"));
            }
            if (model.forwardAction != (int)ApprovalStatusEnum.Disapproved) { model.forwardAction = (int)ApprovalStatusEnum.Processing; }
            // WORKFLOW
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME;
                placeholders.referenceNumber = TranslateHelper.get("<br />LC REFERENCENUMBER: ") + lc.LCREFERENCENUMBER;
                placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER OF CREDIT: CANCELATION");
                placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: LC CANCELATION");
                placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
                workflow.Placeholders = placeholders;
        ...
        public async Task<bool> AddLcArchive(int LcIssuanceId, int operationId)
        {
            var lc = context.TBL_LC_ISSUANCE.Find(LcIssuanceId);
            if (lc == null)
            {
                throw new SecureException(TranslateHelper.get("LC NOT FOUND TO BE ARCHIVED!"));
            }
        ...
        private async Task<bool> UpdateLcWithEnhancement(int tempLcIssuanceId)
        {
            var newLc = context.TBL_TEMP_LC_ISSUANCE.Find(tempLcIssuanceId);
            var oldLc = context.TBL_LC_ISSUANCE.Find(newLc.LCISSUANCEID);
            if (newLc == null)
            {
                throw new SecureException(TranslateHelper.get("LC Enhancement Data Not Found!"));
            }
            if (newLc == null)
            {
                throw new SecureException(TranslateHelper.get("LC Issuance Data Not Found!"));
            }
        ...
        public async Task<WorkflowResponse> LcEnhancementMemorandum(LcForwardViewModel model)
        {
            var lc = context.TBL_LC_ISSUANCE.Find(model.LcIssuanceId);
            ...
            {
                throw new SecureException(TranslateHelper.get("This LC is already undergoing Termination Or has been Terminated"));
            }
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                var placeholders = new AlertPlaceholders();
                placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME;
                placeholders.referenceNumber = TranslateHelper.get("<br />LC REFERENCENUMBER: ") + tempLc.LCREFERENCENUMBER;
                placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER OF CREDIT");
                placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: LC AMOUNT MODIFICATION");
                placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
                workflow.Placeholders = placeholders;
                ...
                if (workflow.NewState == (int)ApprovalState.Ended) // cam status
                {
                    if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                    {
                        ...
                        workflow.SetResponse = false;
                        var archived = await AddLcArchive(tempLc.LCISSUANCEID, operationId);
                        if (archived)
                        {
                           await UpdateLcWithEnhancement(tempLc.TEMPLCISSUANCEID);
                        }
                        else
                        {
                            throw new SecureException(TranslateHelper.get("There was an Error Archiving this LC!"));
                        }
                    }
                    else if (workflow.StatusId == (int)ApprovalStatusEnum.Disapproved)
                    {
        ...
        public async Task<WorkflowResponse> LcIssuanceExtensionMemorandum(LcForwardViewModel model)
        {
            var lc = context.TBL_LC_ISSUANCE.Find(model.LcIssuanceId);
            if (lc.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CancellationInProgress || lc.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CancellationInProgress)
            {
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                if (workflow.NewState == (int)ApprovalState.Ended) // cam status
                {
                    if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                    {
                        ...
                        {
                            trans.Rollback();
                            throw new SecureException(TranslateHelper.get("There was an Error Archiving this LC Issuance!"));
                        }
                    }
                    else if (workflow.StatusId == (int)ApprovalStatusEnum.Disapproved)
                    {
        ...
        private async Task<bool> ArchiveLCUsance(int lcUssanceId, int operationId)
        {
            var lcUsance = context.TBL_LC_USSANCE.Find(lcUssanceId);
            if (lcUsance == null)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Lc Usance record not found!"));
            }
        ...
        private void UpdateLcUsance(int tempLcUsanceId)
        {
            if (tempLcUsanceId <= 0)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Temp Usance Id must be greaterthan 1!"));
            }

            var tempUs = context.TBL_TEMP_LC_USSANCE.Find(tempLcUsanceId);
            if (tempUs == null)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Temp Lc Usance record not found!"));
            }
        ...
        public async Task<WorkflowResponse> LcUsanceExtensionMemorandum(LcForwardViewModel model)
        {
            int operationId = (int)OperationsEnum.LCUsanceExtensionApproval; // CHANGE
            var applicationDate = general.GetApplicationDate();
            if (model.LcIssuanceId <= 0)
            {
                throw new SecureException(TranslateHelper.get("LC Issuance Id must be greater than zero!"));
            }
            if (model.tempLcUsanceId <= 0)
            {
                throw new SecureException(TranslateHelper.get("TempLcUsanceId must be greater than zero!"));
            }
            if (model.lcUssanceId <= 0)
            {
                throw new SecureException(TranslateHelper.get("LcUssanceId must be greater than zero!"));
            }
            var lc = context.TBL_LC_ISSUANCE.Find(model.LcIssuanceId);
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                placeholders.referenceNumber = TranslateHelper.get("<br />LC REFERENCENUMBER: ")+ lc.LCREFERENCENUMBER;
                placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER OF CREDIT");
                placeholders.operationName =TranslateHelper.get( "<br />OPERATION NAME: USANCE EXTENSION");
                placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
                workflow.Placeholders = placeholders;
                ...
                if (workflow.NewState == (int)ApprovalState.Ended) // cam status
                {
                    if (workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                    {
                        ...
                        var archived = await ArchiveLCUsance(us.LCUSSANCEID, operationId);
                        if (archived)
                        {
                        ...
                        {
                            trans.Rollback();
                            throw new SecureException(TranslateHelper.get("There was an Error Archiving this LC Usance!"));
                        }
                        workflow.SetResponse = false;
                    }
                    else if (workflow.StatusId == (int)ApprovalStatusEnum.Disapproved)
        ...
        public async Task<WorkflowResponse> LcUssanceMemorandum(LcForwardViewModel model)
        {
            ...
            placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME;
            placeholders.referenceNumber = TranslateHelper.get("<br />LC REFERENCENUMBER: ") + lc.LCREFERENCENUMBER;
            placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER OF CREDIT: USANCE");
            placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: LC USANCE");
            placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
            workflow.Placeholders = placeholders;
  ...
  */
        ...
        public WorkflowResponse LetterGenerationRequestMemorandum(LetterGenerationRequestViewModel model)
        {
            ...
            var placeholders = new AlertPlaceholders();
            placeholders.customerName = TranslateHelper.get("<br />CUSTOMER NAME: ") + c.FIRSTNAME + " " + c.MIDDLENAME + " " + c.LASTNAME;
            placeholders.referenceNumber = TranslateHelper.get("<br />LETTER GEN REQ REFERENCENUMBER: ") + lgr.REQUESTREF;
            placeholders.facilityType = TranslateHelper.get("<br />FACILITY INFORMATION: LETTER LETTER GEN REQ");
            placeholders.operationName = TranslateHelper.get("<br />OPERATION NAME: LETTER GEN REQ");
            placeholders.branchName = TranslateHelper.get("<br />BRANCH NAME: ") + c.TBL_BRANCH.BRANCHNAME;
            workflow.Placeholders = placeholders;
        ...
        private bool ValidateReleaseAmount(LcReleaseAmountViewModel model)
        {
           var lc = context.TBL_LC_ISSUANCE.Find(model.lcIssuanceId);
            ...
            {
                throw new SecureException(TranslateHelper.get("Sorry, available Amount for release is now " + currCode + " " + availableAmount));
            }
        ...
        public async Task<bool> ValidateAllFromReceiverLevels(int staffId, int operationId)
        {
            ...
            if (trail.Count() > 0)
            {
                foreach(var t in trail)
                {
                    ...
                    if (level == null && t.REQUESTSTAFFID == staffId) { throw new SecureException(TranslateHelper.get("Please make sure you are assigned an approval Level for the current operation")); }
        ...
        private Dictionary<string, int> GetRepresentStepdownItems(int applicationId, int action, int operationId)
        {
            ...
            if (levelId < 1 || staffId < 1) throw new SecureException(TranslateHelper.get("Error while resolving receiving staff."));
        ...
        public async Task<IEnumerable<ApprovalTrailViewModel>> GetAppraisalMemorandumTrail(int applicationId, int operationId, bool getAll = false)
        {
        ...
        public async Task<IEnumerable<FailedTransactionViewModel>> GetFailedGroupOfficeTransactions()
        {
            var transactions = await (from f in context.TBL_FAILED_TRANSACTIONS
                                      where f.STATUS == false
                                      select new FailedTransactionViewModel
                                      {
                                          transactionId = f.TRANSACTIONID,
                                          loanApplicationId = f.LOANAPPLICATIONID,
                                          status = f.STATUS,
        ...
        public async Task<IEnumerable<ApprovalTrailViewModel>> GetGlobalInterestRateChangeTrail(int applicationId, int operationid)
        {
            var staffRoles = context.TBL_STAFF_ROLE.ToList();
            var staffs = from s in context.TBL_STAFF select s;
        ...
        private async Task<bool> RunningProcess(int operationId, int targetId)
        {
        ...
        //}
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSAllCollateralType(int companyid)
        {
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSSecuredCollateralType(int companyid)
        {
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSUnsecuredCollateralType(int companyid)
        {
        ...
        public async Task<LoanApplicationDetailsViewModel> GetLoanApplicationDetail(int applicationId)
        {
            var details = new LoanApplicationDetailsViewModel();
                var facilities = await context.TBL_LOAN_APPLICATION.Where(x => x.LOANAPPLICATIONID == applicationId)
            ...
            details.application = await GetLoanApplicationInformation(applicationId);

            return details;
        }

        public async Task<LoanApplicationDetailsViewModel> GetApprovedTrancheDetail(int bookingRequestId)
        {
            var details = new LoanApplicationDetailsViewModel();
            ...
            details.facilities = facilities;
            details.application = await GetLoanApplicationInformation(facilities.FirstOrDefault().applicationId);

            return details;
        }

        public async Task<LoanApplicationDetailsViewModel> GetLoanApplicationDetailByRefNo(string applicationReferenceNumber)
        {
            var details = new LoanApplicationDetailsViewModel();
        ...
        public async Task<LoanApplicationDetailsViewModel> GetSingleLoanApplicationDetail(int detailId)
        {
            var details = new LoanApplicationDetailsViewModel();
            ...
            //    }).ToList();
            details.facilities = facilities;
            details.application = await GetLoanApplicationInformation(detailId,true);
           // details.syndicated = syndicated;
            return details;
        }

        private async Task<LoanApplicationViewModel> GetLoanApplicationInformation(int id, bool fromdetail = false)
        {
        ...
        public async Task<IEnumerable<LoanDetailsFeeViewModel>> GetLoanDetailsFee(int applicationId)
        {
        ...
        public async Task<IEnumerable<LoanApplicationDetailLogViewModel>> GetLoanDetailChangeLog(int applicationId)
        {
        ...
        public async Task<IEnumerable<DocumentationViewModel>> GetAllDocumentation(int applicationId)
        {
        ...
        public IQueryable<LoanApplicationViewModel> GetPendingLoanApplications(int operationId, int companyId, int branchId, int staffId, int? classId, bool isSpecific)
        {
            ...
            query =  context.TBL_LOAN_APPLICATION.Where(x =>
                ...
                x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.ApplicationRejected && x.APPLICATIONSTATUSID != (int)LoanApplicationStatusEnum.CashFlowDocumentReviewInProgress
                && x.APPROVALSTATUSID != (int)ApprovalStatusEnum.Approved
        ...
        .Select(x => new LoanApplicationViewModel
        {
            ...
            loanApplicationDetailId = context.TBL_LOAN_APPLICATION_DETAIL
                                        .Where(s => s.LOANAPPLICATIONID == x.a.LOANAPPLICATIONID && s.DELETED == false)
                                        .Select(s => s.LOANAPPLICATIONDETAILID)
                                        .FirstOrDefault(),
            applicationReferenceNumber = x.a.APPLICATIONREFERENCENUMBER,
            ...
            custTypeId = context.TBL_CUSTOMER.Where(c => c.CUSTOMERID == x.a.CUSTOMERID).Select(c => c.CUSTOMERTYPEID).FirstOrDefault()
        }).ToList();
        ...
        public IQueryable<LoanApplicationViewModel> GetPendingCashFlowDocumentApplication(int operationId, int companyId, int branchId, int staffId, int? classId, bool isSpecific)
        {
            // var declarations
            List<int> ExclusiveOperations = (from flow in context.TBL_LOAN_APPLICATN_FLOW_CHANGE select flow.OPERATIONID).ToList();
            List<int> levelIds = new List<int>();
            ...
            query = context.TBL_LOAN_APPLICATION.Where(x =>
                ...
                && (x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved || x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing)
                && x.COMPANYID == companyId
                && (x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CashFlowDocumentReviewInProgress || x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CAMInProgress)
                && x.ISINSURANCE == true
                && (classId == null) ? true : (x.PRODUCTCLASSID == (short?)classId)
                && x.ISADHOCAPPLICATION != true
            )
        ...
        public IEnumerable<DateTime> GetDateRange(DateTime startDate, DateTime endDate)
        {
            ...
            {
                throw new ArgumentException(TranslateHelper.get("endDate must be greater than or equal to startDate"));
            }
        ...
        public async Task<bool> IsInHolidays(DateTime date, int countryId)
        {
        ...
        public async Task<IEnumerable<DateTime>> FilterHolidaysFromDateIntervals(IEnumerable<DateTime> dateTimes)
        {
            var list = dateTimes.ToList();
            var countryId = context.TBL_COUNTRY.FirstOrDefault().COUNTRYID;
            list = list.FindAll(l => !IsInHolidays(l, countryId).Result);
            return list;
        }
        ...
        public double GetTimeIntervalHours(DateTime startDate, DateTime endDate)
        {
            ...
            range =  FilterHolidaysFromDateIntervals(range).Result;
        ...
        public async Task<bool> ChangeApplicationOwner(int loanApplicationId, int staffId, GeneralEntity model)
        {
            bool saved = false;
            using (var trans = context.Database.BeginTransaction())
            {
        ...
        public async Task<bool> SelfAssignMultpleApplication(List<ForwardViewModel> models, GeneralEntity userEntity)
        {
            bool response = false;
            foreach(var model in models)
            {
                if (model.trailId != null) { response = await AssignApplication(model.trailId.Value, userEntity.createdBy, userEntity); }
            }
            return response;
        }

        public async Task<bool> ReassignMultipleRequests(List<int> models, GeneralEntity userEntity, int staffId)
        {
            bool response = false;
            foreach (var model in models)
            {
                if (model > 0) { response = await AssignApplication(model, staffId, userEntity); }
            }
            return response;
        }

        public async Task<bool> AssignApplication(int approvalTrailId, int staffId, GeneralEntity model)
        {
            //bool saved = false;
            using (var trans = context.Database.BeginTransaction())
            {
        ...
        public async Task<bool> ReturnAssignApplicationToPool(int approvalTrailId, GeneralEntity model)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                if (approvalTrailId > 0)
        ...
        public async Task<IEnumerable<CurrentCommitteeViewModel>> GetCurrentCommittee(int loanApplicationId)
        {
            int operationId = (int)OperationsEnum.CreditAppraisal;
        ...
        public async Task<bool> SecretariatForwardAppraisalMemorandum(ForwardCommitteeCamViewModel model)
        {
            var appl = context.TBL_LOAN_APPLICATION.Find(model.applicationId);
            if (appl.CREDITGRADEID == (int)CreditGradeEnum.InvestmentGrade) model.investmentGrade = true;
            if (appl.CREDITGRADEID == (int)CreditGradeEnum.StandardGrade) model.standardGrade = true;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Committee Vote on Loan Application Reference Number:")} '{ appl.APPLICATIONREFERENCENUMBER }', ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(), // model.userIPAddress,
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<MonitoringTriggersViewModel>> GetApplicationMonitoringTriggers(int applicationId)
        {
        ...
        public async Task<IEnumerable<MonitoringTriggersViewModel>> GetApplicationMonitoringTriggersByOperationId(int operationId,int applicationDetailId)
        {
        ...
        public async Task<IEnumerable<MonitoringTriggersViewModel>> SaveApplicationMonitoringTriggers(int applicationId, List<MonitoringTriggersViewModel> items, int staffId)
        {
            ...
            return await GetApplicationMonitoringTriggers(applicationId);
        }

        public async Task<bool> WorkflowTest()
        {
        ...
        public async Task<ApprovalTrailViewModel> GetapprovalTrailByTrailId(int approvalTrailId)
        {
            if (approvalTrailId > 0)
            {
                var trail = context.TBL_APPROVAL_TRAIL.Find(approvalTrailId);
                ...
                if (data.fromApprovalLevelId == data.toApprovalLevelId && data.loopedStaffId > 0)
                {
                    ...
                    if(loopedStaff == null)
                    {
                        throw new SecureException(TranslateHelper.get("Looped Staff Can't be null!"));
                    }
        ...
        public async Task<IEnumerable<RepaymentScheduleTermsViewModel>> SaveRepaymentScheduleAndTerms(RepaymentScheduleTermsViewModel model)
        {
        ...
        public async Task<IEnumerable<RepaymentScheduleTermsViewModel>> GetRepaymentScheduleAndTerms(int applicationId)
        {
            var repaymentTerms = await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == applicationId && x.DELETED == false)
                .Select(x => new RepaymentScheduleTermsViewModel
                {
                    applicationDetailId = x.LOANAPPLICATIONDETAILID,
                    terms = x.REPAYMENTTERMS,
        ...
        public async Task<string> GetAllOldApplicationReference(string data)
        {
        ...
        public async Task<List<ProductLimitValidationViewModel>> SaveProductLimitValidation(ProductLimitValidationViewModel entity)
        {
            ...
            context.SaveChanges();
            return await GetProductLimitValidation(detail.LOANAPPLICATIONID, entity.productClassId);
        }

        public async Task<List<ProductLimitValidationViewModel>> GetProductLimitValidation(int applicationId, int classId)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> GetRecommendedCollateral(int applicationId, int staffId)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> AddRecommendedCollateral(RecommendedCollateralViewModel entity)
        {
            ...
            context.SaveChanges();
            return await GetRecommendedCollateral(entity.applicationId,entity.createdBy);
        }


        public async Task<bool> AddContractorTiering(ContractorTieringViewModel contractorCriteria)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> UpdateRecommendedCollateral(RecommendedCollateralViewModel entity)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> GetRecommendedCollateralHistory(int applicationId)
        {
        ...
        public async Task<IEnumerable<MonitoringTriggersViewModel>> GetApplicationMonitoringTriggersLms(int applicationId)
        {
        ...
        public async Task<IEnumerable<MonitoringTriggersViewModel>> SaveApplicationMonitoringTriggersLms(int applicationId, List<MonitoringTriggersViewModel> items, int staffId)
        {
        ...
        public async Task<List<RepaymentScheduleTermsViewModel>> SaveRepaymentScheduleAndTermsLms(RepaymentScheduleTermsViewModel entity)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> UpdateRecommendedCollateralLms(RecommendedCollateralViewModel entity)
        {
            ...
            recommendation.STAMPEDTOCOVERAMOUNT = entity.stampedToCoverAmount;
            context.SaveChanges();
            return await GetRecommendedCollateralLms (entity.applicationId);
        }

        public async Task<List<RecommendedCollateralViewModel>> AddRecommendedCollateralLms(RecommendedCollateralViewModel entity)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> GetRecommendedCollateralLms(int applicationId)
        {
        ...
        public async Task<bool> saveTranchDisbursmentApprovalLevel(TranchDisbursmentViewModel entity)
        {
        ...
        public async Task<List<RecommendedCollateralViewModel>> GetRecommendedCollateralHistoryLms(int applicationId)
        {
        ...
        public async Task<LoanApplicationDetailsViewModel> GetLMSLoanApplicationDetail(int applicationId)
        {
            var details = new LoanApplicationDetailsViewModel();
        ...
        private void SendEmailToCustomerForLoanApproval(int loanApplicationId, int companyId)
        {
            ...
            foreach(var customer in data)
            {
                ...
                string messageSubject = TranslateHelper.get("APPROVAL FOR LOAN APPLICATION");
        ...
        private void SendEmailToCustomerForLoanDisapproval(int loanApplicationId, int companyId)
        {
            ...
            foreach (var customer in data)
            {
                ...
                string messageSubject = TranslateHelper.get("DISAPPROVAL FOR LOAN APPLICATION");
        ...
        private string GetFacilityInformationMarkup(int applicationId)
        {
            ...
            result = result + $@"
                <table border=1>
                    <tr>
                        <th>S/N</th>
                        <th>{TranslateHelper.get("Facility Type")}</th>
                        <th>{TranslateHelper.get("Amount")}</th>
                        <th>{TranslateHelper.get("Rate")}</th>
                        <th>{TranslateHelper.get("Tenor")}</th>
                    </tr>
                 ";
            foreach (var f in facilities)
            {
        ...
        public async Task<WorkflowResponse> GetWorkflowNextStatus(ForwardViewModel model)
        {
            int operationId = (int)OperationsEnum.CreditAppraisal;
            var applicationDate = general.GetApplicationDate();
            var appl = context.TBL_LOAN_APPLICATION.Find(model.applicationId);
        ...
        public async Task<WorkflowResponse> GetWorkflowNextStatusLms(ForwardReviewViewModel model)
        {
            var applicationDate = general.GetApplicationDate();
        ...
        public async Task<ProjectRiskRatingViewModel> AddProjectRiskRating(ProjectRiskRatingViewModel projectRiskRating)
        {
            ...
            if (validateExisting != null && validateExisting.Count() > 0)
            {
                throw new SecureException(TranslateHelper.get("Sorry project risk rating already captured"));
            }
            try
            {
