...
namespace FintrakBanking.Repositories.Credit
{
    public class LoanPerformanceRepository : ILoanPerformanceRepository
    {
        ...
        public async Task<IEnumerable<PrudGuildlineTypeViewModel>> GetPrudGuildlineType()
        {
            var allFilteredLoan = await (from a in context.TBL_LOAN_PRUDENT_GUIDE_TYPE
        ...
        public async Task<IEnumerable<PrudentialGuidelineViewModel>> GetPrudGuildlineStatus()
        {
            var status =  await (from a in context.TBL_LOAN_PRUDENTIALGUIDELINE
       ...
       var bbw =     allFilteredLoan.ToList();
        ...
        public List<FinanceTransactionViewModel> BuildPerformingToNonPerformingLoanPosting(LoanViewModel model)
        {
            ...
            debit.description = TranslateHelper.get("Performing Loan to Non-Performing Loan");
            ...
            credit.description = TranslateHelper.get("Performing Loan to Non-Performing Loan");
        ...
        public List<FinanceTransactionViewModel> BuildNonPerformingToPerformingLoanPosting(LoanViewModel model)
        {
            ...
            FinanceTransactionViewModel debit = new FinanceTransactionViewModel();
            debit.operationId = (int)OperationsEnum.LoanPerformance;
            debit.description = TranslateHelper.get("Non-Performing Loan To Performing Loan ");
            debit.valueDate = generalSetup.GetApplicationDate();
            debit.transactionDate = debit.valueDate;
            debit.currencyId = (short)model.currencyId;
            debit.currencyRate = financeTransaction.GetExchangeRate(debit.valueDate, debit.currencyId, model.companyId).sellingRate;
            ...
            FinanceTransactionViewModel credit = new FinanceTransactionViewModel();
            credit.operationId = (int)OperationsEnum.LoanPerformance;
            credit.description = TranslateHelper.get("Non-Performing Loan To Performing Loan");
            credit.valueDate = debit.valueDate;
            credit.transactionDate = debit.valueDate;
            credit.currencyId = (short)model.currencyId;
            credit.currencyRate = financeTransaction.GetExchangeRate(debit.valueDate, debit.currencyId, model.companyId).sellingRate;
        ...
        public async Task<bool> LoanPerformanceStatusChange(PrudGuidelineStatusChangeViewModel entity)
        {
            string emailBody = "";
            string emailSubject = "";
            ...
            {
                var termLoan = await GetTermLoan().Where(x => x.loanId == entity.loanId).FirstOrDefaultAsync();
                ...
                emailBody = TranslateHelper.get("Dear Valuable Customer, <br /><br />") + TranslateHelper.get("Your facility with Reference Number: ") + termLoan.loanReferenceNumber + TranslateHelper.get("has been classified as ") + loanClasssification + ".,<br />" +  TranslateHelper.get("Kindly contact your Relationship Manager for more information.");
                emailSubject = TranslateHelper.get("LOAN PERFORMANCE STATUS REPORT");
            ...
            else if (entity.loanSystemTypeId == (int)LoanSystemTypeEnum.OverdraftFacility)
            {
                ...
                //Send Email to Customer
                string loanClasssification = context.TBL_LOAN_PRUDENT_GUIDE_TYPE.Where(o => o.PRUDENTIALGUIDELINETYPEID == entity.prudentialGuidelineStatusId).Select(o => o.PRUDENTIALGUIDELINETYPENAME).FirstOrDefault();
                emailBody = TranslateHelper.get("Dear Valuable Customer, <br /><br />") + TranslateHelper.get("Your facility with Reference Number: ") + revolvingLoan.loanReferenceNumber + TranslateHelper.get("has been classified as ") + loanClasssification + ".,<br />" + TranslateHelper.get("Kindly contact your Relationship Manager for more information.");
                emailSubject = TranslateHelper.get("LOAN PERFORMANCE STATUS REPORT");
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Loan Performance Status Change for Loan with LoanId")} ({entity.loanId})",
