    ...
    public class LoanCovenantRepository : ILoanCovenantRepository
    {
        ...
        public async Task<bool> DeleteLoanCovenantDetail(int loanCovenantDetailId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Delete loan convent to loan ref")}: { loanRef } ",
        ...
        private async Task<IEnumerable<LoanCovenantDetailViewModel>> LoanCovenantDetail(int companyId)
        {
        ...
        public async Task<IEnumerable<LoanCovenantDetailViewModel>> GetLoanCovenantDetailByCovenantType(int covenantTypeId, int companyId)
        {
            var loanCovenantDetail = await LoanCovenantDetail(companyId);
            return loanCovenantDetail.Where(c => c.covenantTypeId == covenantTypeId);
        }

        public async Task<IEnumerable<LoanCovenantDetailViewModel>> GetLoanCovenantDetailByloanId(int loanId, int companyId)
        {
            var loanCovenant = await LoanCovenantDetail(companyId);
            return loanCovenant.Where(c => c.loanId == loanId);
        }

        public async Task<bool> UpdateLoanCovenantDetail(int id, LoanCovenantDetailViewModel entity)
        {
            var convenant = await context.TBL_LOAN_COVENANT_DETAIL.FindAsync(id);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated loan convent to loan ref:")} { loanRef } ",
                IPADDRESS = entity.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        {
            var loanCovenantType =  LoanCovenantType(companyId);
            return loanCovenantType.Where(c => c.covenantTypeId == covenantDetailId);
        }
        ...
        public async Task<bool> AddLoanCovenantType(LoanCovenantTypeViewModel entity)
        {
            var convenant = new TBL_LOAN_COVENANT_TYPE
            {
                ...
                ISCLEANUPCYCLE = entity.isCleanupCycle
            };
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Defined loan convent type")}: { entity.covenantTypeName } ",
                IPADDRESS = entity.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateLoanCovenantType(short id, LoanCovenantTypeViewModel entity)
        {
            ...
            convenant.ISCLEANUPCYCLE = entity.isCleanupCycle;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.lastUpdatedBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated loan convent type")}: { entity.covenantTypeName } ",
                IPADDRESS = entity.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public IEnumerable<LoanCovenantTypeViewModel> LoanCovenantType(int companyId)
        {
            return  context.TBL_LOAN_COVENANT_TYPE.Where(c => c.COMPANYID == companyId).Select(c => new LoanCovenantTypeViewModel
            {
                ...
                isCleanupCycle = c.ISCLEANUPCYCLE,
            }).ToList();
        }
        ...
        public async Task<IEnumerable<LoanCovenantDetailViewModel>> GetLoanApplicationCovenant(int applicationId)
        {
            var ids = context.TBL_LOAN_APPLICATION_DETAIL
                .Where(x => x.LOANAPPLICATIONID == applicationId)
                .Select(x => x.LOANAPPLICATIONDETAILID);

            var records =await context.TBL_LOAN_APPLICATION_COVENANT.Where(x =>
        ...
        public async Task<IEnumerable<LoanCovenantDetailViewModel>> GetLoanApplicationDetailCovenant(int applicationDetailId)
        {
            var records =await context.TBL_LOAN_APPLICATION_COVENANT.Where(x =>
                                ...
                                if ((DateTime)covenant.PREVIOUSCOVENANTDATE.Value.Date == DateTime.Now.Date && covenant.COVENANTTYPEID == (short)LoanCovenantTypeEnum.Cleanup)
                                {
                                    if (data != null)
                                    {
                                        ...
                                        if (covenant.COVENANTAMOUNT > availableBalance)
                                        {
                                            ...
                                            var subject = TranslateHelper.get("OD CLEAN-UP VIOLATION NOTIFICATION");
                                            var message = TranslateHelper.get("This is to inform you that an OD clean-up with reference number: ") + appDetails.APPLICATIONREFERENCENUMBER + TranslateHelper.get(" with customer detail: ( ") + customerDetail.CUSTOMERCODE + "," + customerDetail.FIRSTNAME + " " + customerDetail.MIDDLENAME + " " + customerDetail.LASTNAME + TranslateHelper.get(") condition has been violated by the customer.");
                                            ...
                                            var casaLienViewModel = new CasaLienViewModel
                                            {
                                                ...
                                                description = TranslateHelper.get("Place lien on the account ") + casaAccount.PRODUCTACCOUNTNUMBER + TranslateHelper.get("with amount ") + (loanDetails.APPROVEDAMOUNT - (decimal)covenant.COVENANTAMOUNT),
                                ...
                                if ((DateTime)covenant.PREVIOUSCOVENANTDATE.Value.Date == DateTime.Now.Date && covenant.COVENANTTYPEID != (short)LoanCovenantTypeEnum.Cleanup)
                                {
                                    if (data != null)
                                    {
                                        ...
                                        if (covenant.COVENANTAMOUNT > availableBalance)
                                        {
                                            ...
                                            emailList = GetBusinessTeamsEmails(staffMisCode);
                                            alerts.receiverEmailList.Add(emailList);
                                            var subject = TranslateHelper.get("COVENANT VIOLATION NOTIFICATION");
                                            var message = TranslateHelper.get("This is to inform you that a loan with reference number: ") + appDetails.APPLICATIONREFERENCENUMBER + TranslateHelper.get(" with customer detail:") + " ( " + customerDetail.CUSTOMERCODE + "," + customerDetail.FIRSTNAME + " " + customerDetail.MIDDLENAME + " " + customerDetail.LASTNAME + ") " +  TranslateHelper.get("condition has been violated by the customer.");
                    ...
                    if (interestRateChanges.Count() > 0)
                    {
                        ...
                        foreach (var interestRateChange in interestRateChanges)
                        {
                            if (interestRateChange.EFFECTIVEDATE.Date == DateTime.Now.Date)
                            {
                                ...
                                emailList = GetBusinessTeamsEmails(staffMisCode);
                                alerts.receiverEmailList.Add(emailList);
                                var subject = TranslateHelper.get("POSTDATED PERIOD OF RATE CHANGE NOTIFICATION");
                                var message = TranslateHelper.get("This is to inform you that Postdated Period of Rate Change of effective date ") + interestRateChange.EFFECTIVEDATE + TranslateHelper.get("  on a loan with reference number: ") + appDetails.APPLICATIONREFERENCENUMBER + TranslateHelper.get(" with customer detail:") +"( " + customerDetail.CUSTOMERCODE + "," + customerDetail.FIRSTNAME + " " + customerDetail.MIDDLENAME + " " + customerDetail.LASTNAME + ")" +  TranslateHelper.get("is due today.");
        ...
        public async Task<bool> AddLoanApplicationCovenant(LoanCovenantDetailViewModel entity)
        {
            if (entity.loanCovenantDetailId > 0)
            {
                var convenant =await context.TBL_LOAN_APPLICATION_COVENANT.FindAsync(entity.loanCovenantDetailId);
                convenant.COVENANTAMOUNT = entity.covenantAmount;
                convenant.COVENANTDATE = entity.covenantDate;
                convenant.COVENANTDETAIL = entity.covenantDetail;
                convenant.COVENANTTYPEID = entity.covenantTypeId;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added loan application covenant on application:")} { appl.TBL_LOAN_APPLICATION.APPLICATIONREFERENCENUMBER } ",
                IPADDRESS = entity.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteLoanApplicationCovenant(int covenantId, UserInfo user)
        {
            var covenant =await context.TBL_LOAN_APPLICATION_COVENANT.FindAsync(covenantId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Delete loan application covenant:")} { covenant.COVENANTDETAIL } ",
                IPADDRESS = user.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<LoanCovenantDetailViewModel>> GetLoanApplicationCovenantLms(int applicationId)
        {
            var ids = context.TBL_LMSR_APPLICATION_DETAIL
                 .Where(x => x.LOANAPPLICATIONID == applicationId)
                 .Select(x => x.LOANREVIEWAPPLICATIONID);

            return await context.TBL_LMSR_APPLICATION_COVENANT.Where(x =>
        ...
        public async Task<bool> AddLoanApplicationCovenantLms(LoanCovenantDetailViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added loan REVIEW application covenant on application:")} { appl.TBL_LMSR_APPLICATION.APPLICATIONREFERENCENUMBER } ",
                IPADDRESS = entity.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateLoanApplicationCovenantLms(int covenantId, LoanCovenantDetailViewModel entity)
        {
            try
            {
                var covenant = await context.TBL_LMSR_APPLICATION_COVENANT.FindAsync(covenantId);
                {
                    //covenant.LOANCOVENANTDETAILID = entity.loanCovenantDetailId;
                    covenant.COVENANTAMOUNT = entity.covenantAmount;
                    covenant.COVENANTDATE = entity.covenantDate;
                    covenant.COVENANTDETAIL = entity.covenantDetail;
                    covenant.COVENANTTYPEID = entity.covenantTypeId;
                    covenant.FREQUENCYTYPEID = entity.frequencyTypeId;
                    covenant.LOANREVIEWAPPLICATIONID = entity.loanApplicationDetailId;
                    covenant.ISPERCENTAGE = entity.isPercentage;
                    covenant.NEXTCOVENANTDATE = entity.nextCovenantDate;
                    covenant.CASAACCOUNTID = entity.casaAccountId;
                    covenant.CREATEDBY = entity.createdBy;
                    covenant.DATETIMECREATED = this.genSetup.GetApplicationDate().Date;
                    covenant.COMPANYID = entity.companyId;
                };
                context.SaveChanges();
        ...
        public async Task<bool> DeleteLoanApplicationCovenantLms(int covenantId, UserInfo user)
        {
            var covenant =await context.TBL_LMSR_APPLICATION_COVENANT.FindAsync(covenantId);
            covenant.DELETED = true;
            covenant.DELETEDBY = user.staffId;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Delete loan REVIEW application covenant:")} { covenant.COVENANTDETAIL } ",
                IPADDRESS = user.userIPAddress,
                URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
