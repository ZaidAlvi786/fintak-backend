...
namespace FintrakBanking.Repositories.credit
{
    public class LcCashBuildUpPlanRepository : ILcCashBuildUpPlanRepository
    {
        ...
        public async Task<IEnumerable<LcCashBuildUpPlanViewModel>> GetLcCashBuildUpPlansByLcIssuanceId(int id)
        {
            return await context.TBL_LC_CASHBUILDUPPLAN.Where(x => x.DELETED == false && x.LCISSUANCEID == id)
        ...
        public async Task<IEnumerable<LcCashBuildUpPlanViewModel>> GetLcCashBuildUpReferenceTypes()
        {
            var cashBuildUp = context.TBL_CASHBUILDUPREFERENCETYPE.ToList();
            return await context.TBL_CASHBUILDUPREFERENCETYPE
                .Select(x => new LcCashBuildUpPlanViewModel
                {
        ...
        public async Task<bool> AddLcCashBuildUpPlan(LcCashBuildUpPlanViewModel model)
        {
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                DETAIL = $"TBL_Lc CashBuildUpPlan '{entity.ToString()}' {TranslateHelper.get("created by")} {auditStaff}",
        ...
        public async Task<bool> UpdateLcCashBuildUpPlan(LcCashBuildUpPlanViewModel model, int id, UserInfo user)
        {
            var entity = await this.context.TBL_LC_CASHBUILDUPPLAN.FindAsync(id);
        ...
        public async Task<bool> DeleteLcCashBuildUpPlan(int id, UserInfo user)
        {
            var entity = await this.context.TBL_LC_CASHBUILDUPPLAN.FindAsync(id);
            entity.DELETED = true;
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"TBL_Lc CashBuildUpPlan '{entity.ToString()}' {TranslateHelper.get("was deleted by")} {auditStaff}",
                IPADDRESS = user.userIPAddress,
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
