...
namespace FintrakBanking.Repositories.Credit
{
    public class FeeConcessionRepository : IFeeConcessionRepository
    {
        ...
        public async Task<IEnumerable<FeeConcessionTypeViewModel>> GetConcessionFeeType()
        {
            var type = await (from a in context.TBL_LOAN_CONCESSION_TYPE
        ...
        public async Task<IEnumerable<LoanFeeChargesViewModel>> GetAllLoanFeeChargeByDetailId(int loanApplicationDetailId)
        {
            var type = await (from a in context.TBL_LOAN_APPLICATION_DETL_FEE
        ...
        public async Task<IEnumerable<FeeConcessionViewModel>> GetAllConcessionFee(int loanApplicationDetailId)
        {
            var feeConcession = await (from a in context.TBL_LOAN_RATE_FEE_CONCESSION
                                 where a.LOANAPPLICATIONDETAILID == loanApplicationDetailId
        ...
        public async Task<IEnumerable<FeeConcessionViewModel>> GetAllConcessionFeeAwaitingApproval(int staffId, int companyId)
        {
        ...
        public async Task<Tuple<int, string>> AddUpdateFeeConcession(FeeConcessionViewModel model)
        {
            if (model == null) return new Tuple<int, string>(0, string.Empty);
            TBL_LOAN_RATE_FEE_CONCESSION data;
            if (model.concessionId > 0)
            {
                data = await context.TBL_LOAN_RATE_FEE_CONCESSION.FindAsync(model.concessionId);
                if (data != null)
                {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = TranslateHelper.get("Added new Fee Concession"),
            ...
            if (model.concessionId == 0)
            {
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            trans.Commit();
                            return new Tuple<int, string>(data.CONCESSIONID, "success");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private bool ApproveFeeConcession(int targetId, ApprovalViewModel user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approve fee concession with concessionId:")} {targetId}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> ValidateFeeConcession(int loanApplicationDetailId, int? loanChargeFeeId)
        {
            bool returnVal = false;
            var exist = await (from a in context.TBL_LOAN_RATE_FEE_CONCESSION
                         where a.LOANAPPLICATIONDETAILID == loanApplicationDetailId &&
        ...
        public async Task<bool> ValidateApprovedFeeConcession(int concessionId)
        {
            bool returnVal = false;
            var isApproved = await (from a in context.TBL_LOAN_RATE_FEE_CONCESSION
