...
namespace FintrakBanking.Repositories.Credit
{
    public partial class LoanApplicationRepository : ILoanApplicationRepository
    {
        ...
        public async Task<IEnumerable<ExistingLoanApplicationViewModel>> ExistingLoanApplication(int customerId, int companyId)
        {
            var data = await context.TBL_LOAN_APPLICATION.Where(c => c.CUSTOMERID == customerId && c.COMPANYID == companyId)
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> GetLoanApplicationDedubeCheck(int customerId, int companyId)
        {
            var data = await GetLoanApplications(companyId);
            data=(IQueryable < LoanApplicationViewModel > ) data.Where(c => c.customerId == customerId
           && c.approvalStatusId != (int)ApprovalStatusEnum.Approved && c.approvalStatusId != (int)ApprovalStatusEnum.Disapproved).ToList();
            return data.ToList();
        }

        private async Task<IQueryable<LoanApplicationViewModel>> GetLoanApplications(int companyId)
        {
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> GetAllLoanApplications(int companyId)
        {
            return await GetLoanApplications(companyId);
        }

        public async Task<IEnumerable<LoanApplicationViewModel>> GetLoanApplicationJobs(int companyId, int levelId, int scope)
        {
            var applications = await GetLoanApplications(companyId);
                applications=applications.Where(x => x.approvalStatusId == (int)ApprovalStatusEnum.Pending); // scope 3 entire process

            if (scope == (int)ProcessViewScopeEnum.Group)
            {
                int levelGroupId;
                var level = await context.TBL_APPROVAL_LEVEL.FindAsync(levelId);
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> GetLoanApplicationById(int loanApplicationId, int companyId)
        {
            var loanApplications = await GetLoanApplications(companyId);
            return loanApplications.Where(c => c.loanApplicationId == loanApplicationId).ToList();
        }

        public async Task<LoanApplicationViewModel> GetSingleLoanApplicationById(int loanApplicationId, int companyId)
        {
            var loanApplications = await GetLoanApplications(companyId);
            return loanApplications.Where(c => c.loanApplicationId == loanApplicationId).FirstOrDefault();

        }

        public async Task<dynamic> GetLoanAppById(int loanApplicationDetailId, int companyId)
        {
            var data =await (from a in context.TBL_LOAN_APPLICATION
        ...
        public async Task<IEnumerable<jobLoanApplicationDetailViewModel>> GetLoanApplicationDetailById(int loanApplicationDetailId, int companyId)
        {
            ...
            try
            {
                ...
                if (casaAcct != null)
                {
                    casa = await context.TBL_CASA.Where(x => x.CASAACCOUNTID == casaAcct).FirstOrDefaultAsync();
                ...
                data = await (from a in context.TBL_LOAN_APPLICATION_DETAIL
            ...
            {
                var relationshipOfficer = await context.TBL_STAFF.Where(s => s.STAFFID == i.relationshipOfficerId).FirstOrDefaultAsync();
                var relationshipManager = await context.TBL_STAFF.Where(s => s.STAFFID == i.relationshipManagerId).FirstOrDefaultAsync();
        ...
        public async Task<IEnumerable<jobLoanApplicationDetailViewModel>> GetLoanApplicationDetailByLoanApplicationId(int loanApplicationId, int companyId)
        {
            var data = await (from a in context.TBL_LOAN_APPLICATION_DETAIL
                        where a.TBL_LOAN_APPLICATION.COMPANYID == companyId && a.DELETED == false
        ...
        public bool SendApplicationToEdit(int loanApplicationId, int operationId, int accountOfficerId)
        {
            ...
            if (loan == null)
            {
                throw new SecureException(TranslateHelper.get("This Loan doesn't exist on the System"));
            }

            if (!(staffs.Contains(loan.OWNEDBY)))
            {
                throw new SecureException(TranslateHelper.get("You cannot modify a Loan you didn't initiate!"));
            }
            ...
            if (trail != null)
            {
                ...
                trail.COMMENT += TranslateHelper.get(" Sent to Applications to edit Application with loan Reference Id: ") + loan.APPLICATIONREFERENCENUMBER;
        ...
        public async Task<IEnumerable<dynamic>> GetLoanApplicationByRelationshipOfficerId(int relationshipOfficerId, int companyId)
        {
        ...
        {
            var data = await this.context.TBL_LOAN_APPLICATION.FindAsync(entity.targetId);
            {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL =
                    $"{TranslateHelper.get("Change Loan Application Status with reference number")} '{data.APPLICATIONREFERENCENUMBER}' to {GetLoanStatus((short)entity.approvalStatusId)}",
        ...
        public async Task<IEnumerable<ProductClassViewModel>> GetProductClass()
        {
            return await (from data in context.TBL_PRODUCT_CLASS
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> FindLoanApplication(string referenceNumberOrName, int companyId)
        {
            var data = await (from a in context.TBL_LOAN_APPLICATION
                        where a.COMPANYID == companyId && a.DELETED == false
        ...
        private async Task<string> GetLoanStatus(short loanStatusId)
        {
            var loanStatus = await this.context.TBL_LOAN_STATUS.Where(x => x.LOANSTATUSID == loanStatusId).SingleOrDefaultAsync();
                return loanStatus.ACCOUNTSTATUS;
        }

        public async Task<IEnumerable<CustomerViewModels>> GetCustomerByApplicationId(int applicationId, string processtype)
        {
            List<CustomerViewModels> customers = new List<CustomerViewModels>();
            if (processtype == "LOS")
            {
                customers =await  (from a in context.TBL_LOAN_APPLICATION_DETAIL
            ...
            {
                customers = await(from a in context.TBL_LMSR_APPLICATION_DETAIL
                             join b in context.TBL_CUSTOMER on a.CUSTOMERID equals b.CUSTOMERID
                             where a.LOANAPPLICATIONID == applicationId
                             select new CustomerViewModels
                             {
        ...
        public async Task<List<accountsViewModels>> GetCustomerTransactionsAccounts(int customerId, int applicationId, bool isLms = false)
        {
            var first =await (from a in context.TBL_LOAN_APPLICATION_TRANS
            ...
            var second = await(from a in context.TBL_LOAN_APPLICATION_TRANS2
                          where a.CUSTOMERID == customerId
                          select new accountsViewModels
                          {
                              accountNumber = a.ACCOUNTNUMBER,
        ...
        public async Task<CustomerApplicationTransactionsViewModels> GetCustomerTransactions(int customerId, int applicationId, bool isLms = false)
        {
            ...
            var first = await(from a in context.TBL_LOAN_APPLICATION_TRANS
                         ...
                         }).GroupBy(O => new { O.accountNumber, O.credit_Turnover, O.debit_Turnover, O.min_Credit_Balance, O.min_Debit_Balance }).Select(O => O.FirstOrDefault()).OrderByDescending(m => m.year).ThenByDescending(b => b.month).ToListAsync();
            ...
            var second = await(from a in context.TBL_LOAN_APPLICATION_TRANS2
                          ...
                          select new CustomerTransactionsViewModels
                          {
                              ...
                              year = a.YEAR,
                              productAccountName = context.TBL_CASA.Where(o => o.CUSTOMERID == customerId).Select(o => o.PRODUCTACCOUNTNAME).FirstOrDefault(),
                          }).OrderByDescending(m => m.year).ThenByDescending(b => b.month).ToListAsync();
        ...
        public async Task<CustomerApplicationTransactionsViewModels> GetCustomerTransactionsFiltered(int customerId, int applicationId, string accountnumber, int? fromYear, int fromMonth, int? toYear, int toMonth, bool isLms = false)
        {
            ...
            if (fromYear.HasValue)
            {
                ...
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Specify all date values to filter by date"));
                }
            }
            ...
            firstRecord = await (from a in context.TBL_LOAN_APPLICATION_TRANS
                           where a.CUSTOMERID == customerId && a.LOANAPPLICATIONID == applicationId && a.ISLMS == isLms
                           ...
                           select new CustomerTransactionsViewModels
                           {
                               ...
                               month = a.MONTH,
                               year = a.YEAR,
                           }).OrderBy(p => p.accountNumber).OrderBy(l => l.year).OrderBy(q => q.month).ToListAsync();
            ...
            secondRecord = await (from a in context.TBL_LOAN_APPLICATION_TRANS2
                            where a.CUSTOMERID == customerId && a.LOANAPPLICATIONID == applicationId && a.ISLMS == isLms
                            //&& a.ACCOUNTNUMBER == accountnumber
                            //&& (a.YEAR >= fromYear && a.MONTH >= fromMonth)
                            //&& (a.YEAR <= toYear && a.MONTH <= toMonth)
        ...
        public async Task<CustomerApplicationTransactionsViewModels> GetCustomerTransactionsByFilterLogic(int customerId, int applicationId, int froma, int to, int fYear, int tYear, bool isLms = false)
        {
            var fields = new CustomerApplicationTransactionsViewModels();
        ...
        public async Task<List<RatingAndRatioViewModel>> GetCustomerRatios(int customerId, int applicationId, bool isLms = false)
        {
            ...
            var fields = await(from a in context.TBL_CUSTOMER_RATIOS
        ...
        public async Task<LoanApplicationUpdateMessage> UpdateApprovalStatusForApplication(int applicationId, int staffId)//, object entity)
        {
            int targetId = 0;
            ...
            try
            {
                if (loanApplicationDetails.Any())
                {
                    ...
                    foreach (var detail in loanApplicationDetails)
                    {
                        ...
                        var product =await context.TBL_PRODUCT.FirstOrDefaultAsync(x => x.PRODUCTID == detail.APPROVEDPRODUCTID);
                        ...
                        var checklistTypes = await(from a in context.TBL_CHECKLIST_TYPE select a).ToListAsync();
                        foreach (var checklistType in checklistTypes) // through checklist types
                        {
                            ...
                            if (negativeChecklistDetails.Any())
                            {
                                ...
                                str = str + TranslateHelper.get("One or more item(s) did not meet up with the condition.") + Environment.NewLine
                                            + TranslateHelper.get(" Please check your response to confirm.") + Environment.NewLine;
                            }
                ...
                if (await PushApplicationToDrawdown(application.APPLICATIONREFERENCENUMBER)) { jumpToDrawdown = true; }

                if (isCheckListDone && await SubmitLoanApplicationForCam(applicationId, staffId, checkListIndex) == 1)
                {
        ...
        public void LoadCustomerTurnover(int applicationId, int staffId, CustomerTurnoverViewModel model)
        {
            var loanApplicationDetails = context.TBL_LOAN_APPLICATION_DETAIL.Where(c => c.LOANAPPLICATIONID == applicationId);

            if (loanApplicationDetails != null) {
                creditCommon.LoadCustomerTurnover(applicationId, loanApplicationDetails.Select(x => x.CUSTOMERID).Distinct().ToList(), staffId, model);
            }
        }

        public void LoadCustomerTurnoverLms(int applicationId, int staffId, CustomerTurnoverViewModel model)
        {
        ...
        public async Task<List<FacilityRatingViewModel>> GetFacilityRating(int loanApplicationDetailId)
        {
            var facilityRating =await context.TBL_FACILITY_RATING.Where(O => O.LOANAPPLICATIONDETAILID == loanApplicationDetailId)
        ...
        public async Task<short> SubmitLoanApplicationForCam(int applicationId, int staffId, int checkListIndex)
        {
            ...
            * 2 = successfuly moved to drawdown **/
            ...
            var appl =await context.TBL_LOAN_APPLICATION.FindAsync(applicationId);
            appl.TOTALEXPOSUREAMOUNT = 0;
            var detail =await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == applicationId).FirstOrDefaultAsync();
            ...
            int operationId;
            var product =await context.TBL_PRODUCT.FindAsync(detail.PROPOSEDPRODUCTID);
            var productBahaviour = await context.TBL_PRODUCT_BEHAVIOUR.Where(x => x.PRODUCTID == detail.PROPOSEDPRODUCTID).FirstOrDefaultAsync();
            ...
            else if (await PushApplicationToDrawdown(appl.APPLICATIONREFERENCENUMBER))
            {
                return 2;
            }
            else
            ...
            workflow.Comment = TranslateHelper.get("New loan application");
        ...
        private async Task<bool> ValidateCollateralSearchJobRequests(int loanApplicationDetailId, int? requireCollateralTypeId = null, bool throwErrorMessage = false)
        {
            ...
            {
                requests =await context.TBL_JOB_REQUEST
        ...
        public int? GetFirstReceiverLevel(int staffId, int operationId, short? productClassId, int? productId, int? exclusiveFlowChangeId, bool next = false)
        {
            ...
            if (mappingsOnProducts.Any() == false && mappingsOnProductClass.Any() == false && mappingsOnOperations.Any() == false && mappingsOnExclusiveOperations.Any() == false)
            {
                ...
                if (operation == null) throw new SecureException(TranslateHelper.get("Operation ID didn't match"));
                if (productClassId != null)
                {
                    var productClass = context.TBL_PRODUCT_CLASS.Find(productClassId);
                    throw new SecureException(TranslateHelper.get("There is no approval workflow setup for the OPERATION: ") + operation.OPERATIONNAME + TranslateHelper.get(", PRODUCT CLASS: ") + productClass.PRODUCTCLASSNAME);
                }
                throw new SecureException(TranslateHelper.get("There is no approval workflow setup for the OPERATION: ") + operation.OPERATIONNAME);
            }
        ...
        {
            var staff = context.TBL_STAFF.FindAsync(staffId);
        ...
        public async Task<int?> GetFirstLevelStaffId(int levelId, int userBranch)
        {
            ...
            {
                var staffLevel =await context.TBL_APPROVAL_LEVEL.FindAsync(levelId);
            ...
            if (model.LoanApplicationDetail.Count() > 0)
            {
                ...
                if (newlineRecord.proposedProductId != 12)
                {
                    ...
                    if (productIds.Contains(12))
                    {
                        ...
                        throw new ConditionNotMetException(TranslateHelper.get("Application already has product ") + productRecord.PRODUCTNAME + TranslateHelper.get(" with unique behaviour"));
                    }
        ...
        private async Task<bool> PushApplicationToDrawdown(string applicationReferenceNumber)
        {
            //var newLineRecord = model.LoanApplicationDetail.FirstOrDefault();
            ...
            var productBehaviour = await context.TBL_LOAN_APPLICATN_FLOW_CHANGE.FindAsync(headerrecord.FLOWCHANGEID);
            ...
            foreach (var item in customer)
            {
                ...
                exposure = (from a in context.TBL_GLOBAL_EXPOSURE
                            ...
                            select new CurrentCustomerExposure
                            {
                                ...
                                approvedAmountLcy = a.LOANAMOUNTLCY ?? 0,
        ...
        public LoanApplicationViewModel AddLoanApplication(LoanApplicationViewModel loan, bool isExceptionalLoan = false)
        {
            if (!isExceptionalLoan) {
                ...
                catch (SecureException ex)
                {
                    ...
                    throw new SecureException($"{ex.Message} - {TranslateHelper.get("Loan has been saved for Exceptional Workflow!")}");
                }
            }
            ...
            if (savedDetails.Count() == 0 || loan.isNewApplication)
            {
                ...
                if (loan.LoanApplicationDetail.Count > 0)
                {
                    ...
                    {
                        var racReponse = AddExceptionalLoanApplicationDetail(loan);
                        if (racReponse != null)
                        {
                            LoanApplicationViewModel model = new LoanApplicationViewModel();
                            model.loanApplicationId = (int)racReponse.loanApplicationId;
        ...
        public RacReturnInfoViewModel SaveRac(RacInformationViewModel rac, int? operationId, int productId, int? productClassId, int targetId, int staffId, int applicationId)
        {
            //try { 
                ...
                if (isRacRelated == true)
                {
                    ...
                    if (defaultTierItems.Count() <= 0) { throw new ConditionNotMetException(TranslateHelper.get("Control keys have not been setup for the RAC Tiers")); }
                ...
                for (int i = 0; i < definitions.Count; i++)
                {
                    ...
                    throw new SecureException($"{TranslateHelper.get("Cannot Proceed.")} {TranslateHelper.get("RAC not met for")}  {definition.TBL_RAC_ITEM.CRITERIA}");
                    /* if (racTiers.Count() <= 0)
                     {
        ...
        public async Task<RacReturnInfoViewModel> SaveExceptionalRac(RacInformationViewModel rac, int? operationId, int productId, int? productClassId, int targetId, int staffId, int applicationId)
        {
            try
            {
                List<TBL_RAC_DEFINITION> definitions = new List<TBL_RAC_DEFINITION>();
                ...
                definitions = await context.TBL_RAC_DEFINITION.Where(x => x.ISACTIVE == true && x.DELETED == false
                   && ids.Contains(x.RACDEFINITIONID)
               ).ToListAsync();
                ...
                var b = definitions.FirstOrDefault();
                allTierRacs = await context.TBL_RAC_DEFINITION.Where(x => x.ISACTIVE == true && x.DELETED == false && x.RACCATEGORYID == b.RACCATEGORYID).ToListAsync();
                int lastRacIndex = 0;
                //List<int?> racCategoryTypeIds = allTierRacs.Select(x => x.RACCATEGORYTYPEID).ToList() ;
                ...
                for (int i = 0; i < definitions.Count; i++)
                {
                    ...
                    if (validation == false && ctr == 0)
                    {
                        ...
                        {
                            var lastRacIndexTierItems = defaultTierItems[lastRacIndex + 1];
                            definitions = racTiers =await context.TBL_RAC_DEFINITION.Where(x => x.ISACTIVE == true && x.DELETED == false
                            && ids.Contains(x.RACDEFINITIONID) && x.RACCATEGORYTYPEID == lastRacIndexTierItems.RACCATEGORYTYPEID && x.RACCATEGORYTYPEID != definition.RACCATEGORYTYPEID
                            ).Select(x => x).OrderByDescending(a => a.RACCATEGORYTYPEID).ThenByDescending(a => a.RACITEMID).ToListAsync();
        ...
        private async Task<List<TBL_RAC_DETAIL>> saveRacoptions(List<TBL_RAC_DEFINITION> definitions, RacInformationViewModel rac, int operationId, int targetId, int staffId)
        {
        ...
        private void AddloanApplicationSub(LoanApplicationViewModel loan)
        {
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = loan.createdBy,
                    BRANCHID = (short)loan.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Applied for loan with reference number:")} {loan.applicationReferenceNumber}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = loan.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        private bool UpdateLoanApplicationDetail(LoanApplicationViewModel loan)
        {
            ...
            if (update == null) throw new SecureException($"{TranslateHelper.get("Sequence contain not single!")}  { loan.LoanApplicationDetail.Count() }");
            ...
            detail.ISLINEFACILITY = update.isLineFacility;
            ...
            {
                throw new SecureException(TranslateHelper.get("Please select a repayment pattern for the product " )+ update.productName);
            }
            ...
            {
                throw new SecureException(TranslateHelper.get("Nothing was updated!"));
            }
            return true;
        }
        ...
        public async Task<bool> ValidateDuplicateLoanApplication(LoanApplicationViewModel loan)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("Please select a repayment pattern"));
            }

            if (a.proposedTenor == 0)
            {
                throw new SecureException(TranslateHelper.get("Tenor can not be ZERO (0)"));
            }
            ...
            var data = new TBL_LOAN_APPLICATION_DETAIL
            {
                ...
                PROPOSEDAMOUNT = a.proposedAmount,
                PROPOSEDINTERESTRATE = (double)a.proposedInterestRate,
                PROPOSEDPRODUCTID = a.proposedProductId,
        ...
        private void AddExceptionalLoanApplicationDetail(LoanApplicationViewModel loan, string breachedLimitName)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("No fee is defined for this product(s)"));
            }
            ...
            try
            {
                ...
                {
                    comment = TranslateHelper.get("Saved Exceptional Loan Application"),
        ...
        private RacReturnInfoViewModel AddLoanApplicationDetail(LoanApplicationViewModel loan)//List<LoanApplicationDetailViewModel> entity, int createdBy)
        {
            ...
            try
            {
                ...
                if (response > 0)
                {
                    if (loan.rac != null)
                    {
                        if( loan.rac.form.Count() > 0)
                        {
                            if (loan.rac != null && loan.rac.form[0].value != null)
                            {
        ...
        private RacReturnInfoViewModel AddExceptionalLoanApplicationDetail(LoanApplicationViewModel loan)//List<LoanApplicationDetailViewModel> entity, int createdBy)
        {
            var createdBy = loan.createdBy;
            //foreach (var a in entity)
            //{
            ...
            int applicationId = this.loanData == null ? 0 : this.loanData.LOANAPPLICATIONID; // ?
            int tenor = ConvertTenorToDays(a.proposedTenorConverted, a.tenorModeId);
        ...
        public IEnumerable<LoanApplicationDetailViewModel> GetExceptionalLoansForApproval(int staffId)
        {
            ...
            var staffrecord = context.TBL_STAFF.Where(s => s.STAFFID == staffId).FirstOrDefault();
            var staffroleCode = context.TBL_STAFF_ROLE.Where(r => r.STAFFROLEID == staffrecord.STAFFROLEID).FirstOrDefault().STAFFROLECODE;
            if(staffroleCode == "AO")
            {
                var exceptionalLoansForApproval = (from d in context.TBL_EXCEPTIONAL_LOAN_APPL_DETAIL
                                                   ...
                                                   where d.DELETED == false && staffId == d.CREATEDBY  && t.OPERATIONID == (int)OperationsEnum.ExceptionalLoan
                                                    &&
                                                    ((
                                                   ...
                                                   select new LoanApplicationDetailViewModel
                                                   {
                                                       ...
                                                       approvalStatus = t.TBL_APPROVAL_STATUS.APPROVALSTATUSNAME.ToUpper()
        ...
        public async Task<WorkflowResponse> GoForApprovalExceptionalLoan(ExceptionalLoanViewModel model)
        {
            int operationId = (int)OperationsEnum.ExceptionalLoan; 
            var cs = await context.TBL_EXCEPTIONAL_LOAN_APPL_DETAIL.FindAsync(model.loanApplicationDetailId);
        ...
        public String ResponseMessage(WorkflowResponse response, string itemHeading)
        {
            if (response.stateId != (int)ApprovalState.Ended)
            {
                ...
                else
                {
                    if (response.nextPersonId > 0)
                    {
                        return $"{TranslateHelper.get("The")} {itemHeading} {TranslateHelper.get("request has been SENT to")} {response.nextPersonName}";
                    }
                    else
                    {
                        return $"{TranslateHelper.get("The")} {itemHeading} {TranslateHelper.get("request has been REFERRED to")} {response.nextLevelName}";
        ...
        public async Task<LoanApplicationDetailViewModel> GetLoanApplicationDetailFields(int detailId)
        {
            var fields = new LoanApplicationDetailViewModel();

            var d = await context.TBL_LOAN_APPLICATION_DETAIL.FirstOrDefaultAsync(x => x.LOANAPPLICATIONDETAILID == detailId);
            //var inv = context.TBL_LOAN_APPLICATION_DETL_INV.FirstOrDefault(x => x.LOANAPPLICATIONDETAILID == detailId);
            var product = await context.TBL_PRODUCT.FindAsync(d.APPROVEDPRODUCTID);
            ...
            var invoiceDetails = await (from a in context.TBL_LOAN_APPLICATION_DETL_INV
            ...
            var bondDetails = await (from a in context.TBL_LOAN_APPLICATION_DETL_BG
                               where a.LOANAPPLICATIONDETAILID == detailId
                               select new BondsAndGuranty
                               {
                                   loanApplicationDetailId = a.LOANAPPLICATIONDETAILID,
            ...
            var educationLoan = await (from a in context.TBL_LOAN_APPLICATION_DETL_EDU
                                 where a.LOANAPPLICATIONDETAILID == detailId
                                 select new EducationLoanViewModel
                                 {
            ...
            var traderLoan =await (from a in context.TBL_LOAN_APPLICATION_DETL_TRA
                              where a.LOANAPPLICATIONDETAILID == detailId
                              select new TraderLoanViewModel
                              {
                                  loanApplicationDetailId = a.LOANAPPLICATIONDETAILID,
        ...
        public bool ArchiveLoanApplication(int loanAppliactionId, int operationId, short applicationStatus, int archivedBy)
        {
            ...
            if (app == null)
            {
                throw new SecureException(TranslateHelper.get("Loan Application doesn't exist!"));
            }
        ...
        public void ArchiveLoanApplicationDetails(int loanApplicationDetailId)
        {
            ...
            if (detailRow == null)
            {
                throw new SecureException(TranslateHelper.get("Facility doesn't exist!"));
            }
        ...
        //lms operation ======================================
        public async Task<bool> ArchiveLmsLoanApplication(int loanAppliactionId, int operationId, short applicationStatus, int archivedBy)
        {
            short applicationStatusId = 0;
            var app = await context.TBL_LMSR_APPLICATION.FirstOrDefaultAsync(l => l.LOANAPPLICATIONID == loanAppliactionId);
            if (app == null)
            {
                throw new SecureException(TranslateHelper.get("Loan Application doesn't exist!"));
            }
        ...
        {
            var dataapplication = await context.TBL_LOAN_APPLICATION.Where(d => d.LOANAPPLICATIONID == loanApplicationId).FirstOrDefaultAsync();
            var datadetail = await context.TBL_LOAN_APPLICATION_DETAIL.Where(d => d.LOANAPPLICATIONID == dataapplication.LOANAPPLICATIONID).ToListAsync();

            if (datadetail != null)
            {
                foreach (var data in datadetail)
                {
                   await  DeleteLoanApplicationDetail(data.LOANAPPLICATIONDETAILID);
        ...
        public async Task<bool> DeleteLoanApplicationDetail(int loanApplicationDetailId)
        {
            var data = await context.TBL_LOAN_APPLICATION_DETAIL.Where(d => d.LOANAPPLICATIONDETAILID == loanApplicationDetailId).FirstOrDefaultAsync();
        ...
        public async Task<bool> UpdateCreditBureauCustomerReportStatus(bool status, LoanCreditBureauViewModel model)
        {
            var data =await  context.TBL_CUSTOMER_CREDIT_BUREAU.Where(c => c.CREDITBUREAUID == model.creditBureauId && c.CUSTOMERID == model.customerId).FirstOrDefaultAsync();
        ...
        public async Task<bool> UpdateMultipleCreditBureauCustomerReportStatus(bool status, List<LoanCreditBureauViewModel> model)
        {
            foreach (var item in model)
            {
                if (await UpdateCreditBureauCustomerReportStatus(status, item) == false) return false;
            }
        ...
        public async Task<IEnumerable<LoanApplicationCollateralViewModel>> GetLoanApplicationCollateral(int loanApplicatioinCollateralId)
        {
            var data =await context.TBL_LOAN_APPLICATION_COLLATERL.Where(c => c.LOANAPPLICATIONID == loanApplicatioinCollateralId && c.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved).Select(c => new LoanApplicationCollateralViewModel
            {
            ...
            }).OrderByDescending(x => x.loanAppCollateralId).ToListAsync();
            return data.ToList();
        }

        public async Task<bool> AddLoanApplicationCollateral(List<LoanApplicationCollateralViewModel> entity)
        {
            var unmapped = new List<TBL_LOAN_APPLICATION_COLLATERL>();
            foreach (var ent in entity)
            {
                var dat = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(c =>
        ...
        public async Task<bool> CheckExistingCertificateOfOwnership(string certificateOfOwnership, int companyId)
        {
        ...
        public async Task<IQueryable<LoanApplicationDetailViewModel>> GetLoanApplicationsAwaitingCheckList(int companyId)
        {
            var data =await (from a in context.TBL_LOAN_APPLICATION
        ...
        public async Task<LoanApplicationDetailViewModel> GetSingleLoanApplicationsDetails(int loanApplicationDetailId, int companyId)
        {
            var data = await (from a in context.TBL_LOAN_APPLICATION
                        join b in context.TBL_LOAN_APPLICATION_DETAIL
                        on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> GetLoanApplicationsDetails(int loanApplicationId, int companyId)
        {
            var data =await (from a in context.TBL_LOAN_APPLICATION
                        join b in context.TBL_LOAN_APPLICATION_DETAIL
                        on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> GetAllLoanApplicationsDetailsById(int loanApplicationId, int companyId)
        {
        ...
        private async Task<IQueryable<LoanApplicationDetailViewModel>> GetLoanApplicationsDetails(int companyId)
        {
            var data = await (from b in context.TBL_LOAN_APPLICATION_DETAIL
                        ...
                        select new LoanApplicationDetailViewModel()
                        {
                            ...
                            branchName = a.TBL_BRANCH.BRANCHNAME,
                            customerGroupName = a.CUSTOMERGROUPID.HasValue ? a.TBL_CUSTOMER_GROUP.GROUPNAME : "",
                            //customerAccountNumber = string.IsNullOrEmpty(a.TBL_CASA.PRODUCTACCOUNTNUMBER) ? a.TBL_CASA.PRODUCTACCOUNTNUMBER : ""

                        }).ToListAsync();
            return data.AsQueryable();
        }
        private async Task<IQueryable<LoanApplicationDetailViewModel>> GetLmsLoanApplicationsDetails(int companyId)
        {
            var data = await (from b in context.TBL_LMSR_APPLICATION_DETAIL
        ...
        public async Task<List<LoanApplicationDetailViewModel>> GetLmsLoanApplicationDetailsById(int loanApplicationId, int companyId)
        {
            var loanApplicationDetails = await GetLmsLoanApplicationsDetails(companyId);
                return loanApplicationDetails.Where(x => x.loanApplicationId == loanApplicationId).ToList();
        }

        public async Task<List<LoanApplicationDetailViewModel>> GetLoanApplicationDetailsById(int loanApplicationId, int companyId)
        {
            var loanApplicationDetails = await GetLoanApplicationsDetails(companyId);
            return loanApplicationDetails.Where(x => x.loanApplicationId == loanApplicationId).ToList();
        }
        ...
        public async Task<List<LoanApplicationViewModel>> Search(string searchString, int staffId = 0)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                ...
                var applications = await (from x in context.TBL_LOAN_APPLICATION
                                    join a in context.TBL_LOAN_APPLICATION_DETAIL on x.LOANAPPLICATIONID equals a.LOANAPPLICATIONID
                ...
                var groupApplications = await(from x in context.TBL_LOAN_APPLICATION
                                         join a in context.TBL_LOAN_APPLICATION_DETAIL on x.LOANAPPLICATIONID equals a.LOANAPPLICATIONID
                                         join p in context.TBL_PRODUCT on a.APPROVEDPRODUCTID equals p.PRODUCTID
                ...
                foreach (var x in allRecord)
                {
                    ...
                    else
                    {
                        ...
                        {
                            x.currentApprovalLevel = TranslateHelper.get("CLICK VIEW FOR DRAWDOWN DETAILS");
                            x.responsiblePerson = TranslateHelper.get("CLICK VIEW FOR DRAWDOWN DETAILS");
                        }
                        else
                        {
                            x.currentApprovalLevel = x.isFacilityCreated == true ? TranslateHelper.get("FIRST TRANCHE DISBURSEMENT HAS OCCURRED") : TranslateHelper.get("NOT YET IN APPRAISAL");
                            x.responsiblePerson = x.isFacilityCreated == true ? TranslateHelper.get("FIRST TRANCHE DISBURSEMENT HAS OCCURRED") : TranslateHelper.get("WITH INITIATING OFFICER");
                        }
                    }
        ...
        public async Task<List<LoanReviewApplicationViewModel>> LmsSearch(string searchString, int staffId = 0)
        {
                ...
                using (FinTrakBankingContext context = new FinTrakBankingContext())
                {
                    ...
                    if (staffId != 0)
                    {
                        relieverStaff = await context.TBL_STAFF_RELIEF
                   ...
                   (int)OperationsEnum.ContingentLiabilityRenewal,(int)OperationsEnum.ContingentLiabilityUsage,(int)OperationsEnum.ContingentRequestBooking,
                    ...
                    var applications =await (from x in context.TBL_LMSR_APPLICATION
                    ...
                    var groupApplications =await (from x in context.TBL_LMSR_APPLICATION
                                             join a in context.TBL_LMSR_APPLICATION_DETAIL on x.LOANAPPLICATIONID equals a.LOANAPPLICATIONID
                                             join p in context.TBL_PRODUCT on a.PRODUCTID equals p.PRODUCTID
                                             join c in context.TBL_CUSTOMER_GROUP on x.CUSTOMERGROUPID equals c.CUSTOMERGROUPID
                                             let creatorStaff = context.TBL_STAFF.FirstOrDefault(f => f.STAFFID == x.CREATEDBY)
                    ...
                    foreach (var x in allRecord)
                    {
                        ...
                        if (appRecord != null)
                        {
                            ...
                            if (singleRec.FROMAPPROVALLEVELID == singleRec.TOAPPROVALLEVELID)
                            {
                                if (singleRec.LOOPEDSTAFFID > 0)
                                {
                                    var staff =await  context.TBL_STAFF.FirstOrDefaultAsync(s => s.STAFFID == singleRec.LOOPEDSTAFFID);
                                    x.currentApprovalLevel = context.TBL_STAFF_ROLE.FirstOrDefault(r => r.STAFFROLEID == staff.STAFFROLEID).STAFFROLENAME;
                                    x.responsiblePerson = staff.FIRSTNAME + " " + staff.MIDDLENAME + " " + staff.LASTNAME;
        ...
        public async Task<List<LoanApplicationViewModel>> SearchDrawDown(string searchString, int staffId = 0)
        {
        ...
        public async Task<List<WorkflowTrackerViewModel>> SearchBookedLoans(int loanApplicationDetailId)
        {
            using (FinTrakBankingContext context = new FinTrakBankingContext())
            {
                ...
                var drawdownsNotYetInitiated = await (
                           ...
                           ).OrderByDescending(o => o.approvalTrailId).ToListAsync();
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> LoanSearch(string searchString)
        {
        ...
        public async Task<IEnumerable<CreditApplicationViewModel>> CommitteeCreditApplications(int applicationTypeId, int staffId)
        {
            List<int> ids = new List<int>();
            string applicationType;
            IQueryable<CreditApplicationViewModel> applications = null;

            List<int> ExclusiveOperations = await (from flow in context.TBL_LOAN_APPLICATN_FLOW_CHANGE select flow.OPERATIONID).ToListAsync();
        ...
        public async Task<dynamic> GetLoanApplicationDetailsProductProgram(int loanApplicationDetailId)
        {
            var details = await (from a in context.TBL_LOAN_APPLICATION_DETAIL
                           join b in context.TBL_LOAN_APPLICATION
                           on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
                           where a.LOANAPPLICATIONDETAILID == loanApplicationDetailId
                           select a.TBL_PRODUCT1.PRODUCTCLASSID).FirstOrDefaultAsync();

            var typeId = await (from a in context.TBL_LOAN_APPLICATION_DETAIL
                          join b in context.TBL_LOAN_APPLICATION
                          on a.LOANAPPLICATIONID equals b.LOANAPPLICATIONID
                          where a.LOANAPPLICATIONDETAILID == loanApplicationDetailId
                          select a.TBL_PRODUCT.PRODUCTTYPEID).FirstOrDefaultAsync();


            if (typeId == (short)LoanProductTypeEnum.SyndicatedTermLoan)
            {
                var syndication =await  (from j in context.TBL_LOAN_APPLICATION_DETL_SYN
            ...
            {
                var inv = await (from a in context.TBL_LOAN_APPLICATION_DETL_INV
                           where a.LOANAPPLICATIONDETAILID == loanApplicationDetailId
                           select new InvoiceDetailViewModel()
                           {
                               invoiceId = a.INVOICEID,
            ...
            else if (details == (short)ProductClassEnum.BondAndGuarantees)
            {
                ...
                var bg =await (from b in context.TBL_LOAN_APPLICATION_DETL_BG
        ...
        public async Task<ValidateDataViewModel> ValidateDocumentDate(ValidateDataViewModel data)
        {
            var dat = await context.TBL_PRODUCT.Where(c => c.PRODUCTID == data.productId).FirstOrDefaultAsync();
        ...
        public async Task<ValidateNumberViewModel> ValidateDocumentNumber(ValidateNumberViewModel data)
        {
            var dat =await  context.TBL_LOAN_APPLICATION_DETL_INV
        ...
        public async Task<bool> ValidateInvoiceDetails(ValidateNumberViewModel data)
        {
            ...
            //           select a).ToList();
            var invoice =await (from a in context.TBL_LOAN_APPLICATION_DETL_INV
                           join b in context.TBL_LOAN_APPLICATION_DETAIL on a.LOANAPPLICATIONDETAILID equals b.LOANAPPLICATIONDETAILID
                           where b.CUSTOMERID == data.customerId && a.INVOICENO == data.documentNo
        ...
        public async Task<IQueryable<LoanApplicationViewModel>> GetLoanApplicationsByOperation(int operationId, int? classId, int branchId, int staffId)
        {
            bool isHeadOffice = (branchId == 1) ? true : false;

            var ids = genSetup.GetStaffApprovalLevelIds(staffId, operationId).ToList();

            var applications =await  (context.TBL_LOAN_APPLICATION
                .Where(x =>
            ...
            .OrderByDescending(x => x.applicationDate)
            .ThenByDescending(x => x.loanApplicationId)).ToListAsync()
            ;
        ...
        public async Task<IQueryable<LoanApplicationViewModel>> GetRejectedLoanApplications(UserInfo user)
        {
        ...
        public async Task<IQueryable<LoanReviewApplicationViewModel>> GetRejectedReviewLoanApplications(UserInfo user)
        {

            bool isHeadOffice = (user.BranchId == 1) ? true : false;

            var applications = await (context.TBL_LMSR_APPLICATION
                .Where(x => (x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.OfferLetterRejected || x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.ApplicationRejected
                || x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CancellationCompleted)
                )
            ...
            }).OrderByDescending(d => d.loanReviewApplicationId)).ToListAsync();

            return applications.AsQueryable();
        }

        public async Task<IQueryable<LoanApplicationViewModel>> GetRejectedLoanApplicationsArch(UserInfo user)
        {
            bool isHeadOffice = (user.BranchId == 1) ? true : false;
            var reliefStaffIds = genSetup.GetStaffRlieved(user.staffId);

            var applications =await ( context.TBL_LOAN_APPLICATION_ARCHIVE
        ...
        public async Task<string> ReviewRequest(ForwardViewModel model)
        {
            var applArchive =await context.TBL_LOAN_APPLICATION_ARCHIVE.Where(ar=>ar.LOANAPPLICATIONID == model.applicationId).FirstOrDefaultAsync();
            var appl = await context.TBL_LOAN_APPLICATION.Where(a => a.LOANAPPLICATIONID == model.applicationId).Select(a=>a).FirstOrDefaultAsync();

            if (appl.CREDITGRADEID == (int)CreditGradeEnum.InvestmentGrade) model.investmentGrade = true;
            if (appl.CREDITGRADEID == (int)CreditGradeEnum.StandardGrade) model.standardGrade = true;
            if (appl.CREDITGRADEID == (int)CreditGradeEnum.RenewalLimit) model.renewalLimit = true;
            ...
            {
                return TranslateHelper.get("This application is already re-initiated!");
            }
            ...
            if (await context.SaveChangesAsync() > 0) // <------------------------- skip to test
            {
                /*int i;
                ...
                workflow.StandardGrade = model.standardGrade;
                workflow.RenewalLimit = model.renewalLimit;
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Re-applied for loan with reference number:")} { appl.APPLICATIONREFERENCENUMBER }",
                    IPADDRESS =CommonHelpers.GetLocalIpAddress(),
                ...
                return await context.SaveChangesAsync() > 0 ? TranslateHelper.get("New Loan Application Reference Number ") + appl.APPLICATIONREFERENCENUMBER : string.Empty;
            }
        ...
        public async Task<bool> UpdateLoanApplicationDetails(LoanApplicationDatailViewModel entity, UserInfo user)
        {
            decimal newAmount;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Updated loan application with reference Number:")} {loan.APPLICATIONREFERENCENUMBER}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<decimal> GetCustomerTotalOutstandingBalance(int customerId)
        {
            var loanData =await context.TBL_LOAN.FirstOrDefaultAsync(x => x.CUSTOMERID == customerId);
            var overdraftData = await context.TBL_LOAN_REVOLVING.FirstOrDefaultAsync(x => x.CUSTOMERID == customerId);
        ...
        public async Task<bool> ProductFeesConcession(ProductFeesViewModel fees, UserInfo user)
        {
            var entity = await context.TBL_LOAN_APPLICATION_DETL_FEE.Where(c => c.CHARGEFEEID == fees.feeId && c.LOANAPPLICATIONDETAILID == fees.loanApplicationDetailId).FirstOrDefaultAsync();
        ...
        public async Task<List<ProductFeeViewModel>> GetLoanApplicationProductFees(int loanApplicationDeatilId)
        {
            var loanAppProdFee = await (from fa in context.TBL_LOAN_APPLICATION_DETL_FEE
        ...
        public async Task<IEnumerable<ProductFeesViewModel>> GetLoanApplicationFees(int loanDetailId)
        {
            var data = await (context.TBL_LOAN_APPLICATION_DETL_FEE.Where(c => c.LOANAPPLICATIONDETAILID == loanDetailId).Select(c => new ProductFeesViewModel
            {
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> SearchForLoan(string searchString)
        {
            var applications =await (from a in context.TBL_LOAN_APPLICATION
        ...
        public async Task<List<LoanApplicationViewModel>> GetLoanApplication(string searchQuery)
        {
            var allFilteredLoan =await (from a in context.TBL_LOAN_APPLICATION
        ...
        private async Task<IQueryable<WorkflowTrackerViewModel>> GetApprovalTrail(int companyId, int staffId, int targetId, int operationId)
        {
            var loggedsStaff =await context.TBL_STAFF.FindAsync(staffId);
            var result = await (from a in context.TBL_APPROVAL_TRAIL
        ...
        {
            var result = await GetApprovalTrail(companyId, staffId, targetId, operationId);
            return result.Distinct();
        }

        public async Task<int> SaveCancelledApplcation(LoanApplicationViewModel data)
        {

            var appl = await context.TBL_LOAN_APPLICATION.FindAsync(data.loanApplicationId);
            var ApprovalTrail = await GetApprovalTrailByOperationIdAndTargetId(appl.OPERATIONID, data.loanApplicationId, data.companyId, data.createdBy);
            var ApprovalStaffCount = ApprovalTrail.Where(a => a.requestStaffId != data.createdBy).Count();
            var staffRecord =await context.TBL_STAFF.FindAsync(data.createdBy);
            var userAdminRole =await context.TBL_STAFF_ROLE.Where(x => x.STAFFROLENAME.ToUpper().Contains("USER ADMIN")).FirstOrDefaultAsync();
            ...
            var isCancellatuionInProgress = context.TBL_LOAN_APPLICATION.Where(x => x.LOANAPPLICATIONID == data.loanApplicationId && x.APPLICATIONSTATUSID == (int)LoanApplicationStatusEnum.CancellationInProgress).Any();
                if (isCancellatuionInProgress == true)
                    throw new ConditionNotMetException(TranslateHelper.get("This Loan is currently under going cancellation process"));
                ...
                if (isCancellatuionCompleted == true)
                    throw new ConditionNotMetException(TranslateHelper.get("This Loan has already been cancelled"));

                var application = await context.TBL_LOAN_APPLICATION.Where(x => x.LOANAPPLICATIONID == data.loanApplicationId).Select(x => x).FirstOrDefaultAsync();
                if (application != null)
                {
                    ...
                    var staffName = await this.context.TBL_STAFF.Where(x => x.STAFFID == data.createdBy).Select(x => x.FIRSTNAME + " " + x.MIDDLENAME + " " + x.LASTNAME).FirstOrDefaultAsync();
                    string messageBoby = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring your attention a request for loan application cancellation has been initiated by {staffName} on {data.applicationReferenceNumber} application reference number")}. {TranslateHelper.get("The Loan application is currently under going approval.")} <br /><br />";
                    string alertSubject = $"{TranslateHelper.get("Loan Application Cancellation Approval Notification")}";
                    LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject, await GetLoanApplicationEmailRecipients(data.loanApplicationId));
                    ...
                    workflow.Comment = $"{TranslateHelper.get("Cancellation request for Loan Application ID with '{data.loanApplicationId}' has been initiated")}. {TranslateHelper.get("Reason being : {data.cancellationReason}")}  ";
        ...
        public async Task<int> SaveLMSCancelledApplcation(LoanReviewApplicationViewModel data)
        {

            var appl = await context.TBL_LMSR_APPLICATION.FindAsync(data.loanReviewApplicationId);
            var ApprovalTrail =await GetApprovalTrailByOperationIdAndTargetId(appl.OPERATIONID, data.loanReviewApplicationId, data.companyId, data.createdBy);
            var ApprovalStaffCount = ApprovalTrail.Where(a => a.requestStaffId != data.createdBy).Count();
            var staffRecord =await context.TBL_STAFF.FindAsync(data.createdBy);
            var userAdminRole =await context.TBL_STAFF_ROLE.Where(x => x.STAFFROLENAME.ToUpper().Contains("USER ADMIN")).FirstOrDefaultAsync();
            if (ApprovalStaffCount == 0 || (staffRecord != null && staffRecord.STAFFROLEID == userAdminRole?.STAFFROLEID))
            ...
            if (application != null)
            {
                ...
                string messageBoby = $"{TranslateHelper.get("Dear Team,")} <br /><br />{TranslateHelper.get("This is to bring to your attention that a request for LMS application cancellation has been initiated by {staffName} on {appl.APPLICATIONREFERENCENUMBER} application reference number.")} {TranslateHelper.get("The Loan application is currently under going approval.")} <br /><br />";
                string alertSubject = $"{TranslateHelper.get("Loan Application Cancellation Approval Notification")}";
                LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject, await GetLoanApplicationEmailRecipients(data.loanReviewApplicationId));

                workflow.StaffId = data.createdBy;
                workflow.CompanyId = data.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = data.tempApplicationCancellationId;
                workflow.Comment = $"{TranslateHelper.get("Cancellation request for Loan Application ID with")} '{data.loanReviewApplicationId}' {TranslateHelper.get("has been initiated.")} {TranslateHelper.get("Reason being")} : {data.cancellationReason}  ";
        ...
        public async Task<List<LoanApplicationViewModel>> GetAllRequestsForLoanCancellation(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.LoanApplicationCancellation).ToList();


            var applications =await (from t in context.TBL_TEMP_LOAN_APPLTN_CANCELTN
        ...
        public async Task<List<LoanReviewApplicationViewModel>> GetAllLmsRequestsForLoanCancellation(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.LmsLoanApplicationCancellation).ToList();


            var applications = await (from t in context.TBL_TEMP_LMSR_APPLTN_CANCELTN
        ...
        public async Task<LoanApplicationViewModel> ViewLaonApplicationCancellationDetails(LoanApplicationViewModel values)
        {
            var data = await Search(values.applicationReferenceNumber);
            if (data != null)
            {
        ...
        public async Task<LoanReviewApplicationViewModel> ViewLmsLaonApplicationCancellationDetails(LoanReviewApplicationViewModel values)
        {
            var data =await LmsSearch(values.applicationReferenceNumber);
            if (data != null)
            {
                var newData = data.FirstOrDefault(x => x.applicationReferenceNumber == values.applicationReferenceNumber);
                newData.operationId = values.operationId;
        ...
        public async Task<bool> GoForLoanApplicationCancellationApproval(LoanApplicationViewModel data)
        {
            int responce = 0;
                ...
                try
                {
                    if (workflow.NewState == (int)ApprovalState.Ended)
                    {
                        if (data.approvalStatusId == (int)ApprovalStatusEnum.Disapproved)
                        {
                            ...
                            var staffName = this.context.TBL_STAFF.Where(x => x.STAFFID == data.createdBy).Select(x => x.FIRSTNAME + " " + x.MIDDLENAME + " " + x.LASTNAME).FirstOrDefault();
                            string messageBoby = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring your attention the loan with {data.applicationReferenceNumber} application reference number which was going through approval for cancellation has been successfully disapproved by {staffName}.")} <br /><br />";
                            string alertSubject = $"{TranslateHelper.get("Loan Application Cancellation Approval Notification")}";
                            LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject, await GetLoanApplicationEmailRecipients(data.loanApplicationId));
                        }
                        else
                        {
                            ...
                            //NOTIFY STAKE HOLDER OF THE TOTAL CANCELLATION
                            var staffName = this.context.TBL_STAFF.Where(x => x.STAFFID == data.createdBy).Select(x => x.FIRSTNAME + " " + x.MIDDLENAME + " " + x.LASTNAME).FirstOrDefault();
                            string messageBoby = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring your attention the loan with {data.applicationReferenceNumber} application reference number which was going through approval for cancellation has been successfully approved by {staffName}")}. <br /><br />";
                            string alertSubject = $"{TranslateHelper.get("Loan Application Cancellation Approval Notification")}";
                            LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject, await GetLoanApplicationEmailRecipients(data.loanApplicationId));
                        }
                    }
        ...
        public async Task<bool> GoForLmsLoanApplicationCancellationApproval(LoanReviewApplicationViewModel data)
        {
            int responce = 0;
            using (var transaction = context.Database.BeginTransaction())
            {
                ...
                try
                {
                    if (workflow.NewState == (int)ApprovalState.Ended)
                    {
                        if (data.approvalStatusId == (int)ApprovalStatusEnum.Disapproved)
                        {
                            ...
                            //NOTIFY STAKE HOLDER OF THE TOTAL CANCELLATION
                            var staffName = this.context.TBL_STAFF.Where(x => x.STAFFID == data.createdBy).Select(x => x.FIRSTNAME + " " + x.MIDDLENAME + " " + x.LASTNAME).FirstOrDefault();
                            string messageBoby = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring to your attention that the loan review with {data.referenceNumber} application reference number which was going through approval for cancellation has been successfully disapproved by {staffName}.")} <br /><br />";
                            string alertSubject = $"{TranslateHelper.get("Loan Review Application Cancellation Approval Notification")}";
                            LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject, await GetLoanApplicationEmailRecipients(data.loanApplicationId));
                        }
                        else
                        {
                            ...
                            //NOTIFY STAKE HOLDER OF THE TOTAL CANCELLATION
                            var staffName = this.context.TBL_STAFF.Where(x => x.STAFFID == data.createdBy).Select(x => x.FIRSTNAME + " " + x.MIDDLENAME + " " + x.LASTNAME).FirstOrDefault();
                            string messageBoby = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring your attention the loan with {data.referenceNumber} application reference number which was going through approval for cancellation has been successfully approved by {staffName}.")} <br /><br />";
                            string alertSubject = $"{TranslateHelper.get("Loan Application Cancellation Approval Notification")}";
                            LogEmailAlertForLoanApplicationCancellation(messageBoby, alertSubject,await GetLoanApplicationEmailRecipients(data.loanApplicationId));
                        }
                    }
        ...
        private void LaonApplcationCancelllationCompelted(LoanApplicationViewModel data)
        {
            ...
            if (lastTrail != null)
            {
                ...
                lastTrail.COMMENT += TranslateHelper.get(" Loan was Cancelled by ") + staff.STAFFCODE + TranslateHelper.get(" Reason being : ") + data.cancellationReason;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Cancellation request for Loan Application ID with '{data.loanApplicationId}' has been completed.")} {TranslateHelper.get("Reason being")} : {data.cancellationReason} ",
        ...
        private void LmsLaonApplcationCancelllationCompelted(LoanReviewApplicationViewModel data)
        {
            ...
            if (lastTrail != null)
            {
                ...
                lastTrail.APPROVALSTATEID = (int)ApprovalState.Ended;
                lastTrail.APPROVALSTATUSID = (int)ApprovalStatusEnum.Disapproved;
                lastTrail.COMMENT += TranslateHelper.get(" Lms Loan was Cancelled by ") + staff.STAFFCODE + TranslateHelper.get(" Reason being : ") + data.cancellationReason;
            }
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = data.createdBy,
                BRANCHID = (short)data.userBranchId,
                DETAIL = $"{TranslateHelper.get("Cancellation request for Lms Application ID with '{data.loanApplicationId}' has been completed.")} {TranslateHelper.get("Reason being")} : {data.cancellationReason} ",
                IPADDRESS = data.userIPAddress,
                URL = data.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        private void LogEmailAlertForLoanApplicationCancellation(string messageBody, string alertSubject, string recipients)
        {
            try
            {
                ...
                string messageContent = $"{TranslateHelper.get("Dear Team")}, <br /><br />{TranslateHelper.get("This is to bring your attention the following loan covenants which are approaching their due date.")} <br /><br />";
        ...
        private async Task<string> GetLoanApplicationEmailRecipients(int targetId)
        {
            ...
            if (staffId != null)
            {
                return await context.TBL_STAFF.Where(x => x.STAFFID == staffId).Select(x => x.EMAIL).FirstOrDefaultAsync();

            }
            else
            {
                int? approvalLevelId = await context.TBL_APPROVAL_TRAIL.Where(x => x.TARGETID == targetId).OrderByDescending(x => x.SYSTEMARRIVALDATETIME).Select(x => x.FROMAPPROVALLEVELID).FirstOrDefaultAsync();
                ...
                foreach (var a in staffIds)
                    recipientEmailAddresses = await context.TBL_STAFF.Where(x => x.STAFFID == a.staffId).Select(x => x.EMAIL).FirstOrDefaultAsync() + ";";
        ...
        public async Task<List<TransactionDynamicsViewModel>> GetTrnasactionDynamics(int loanApplicationId)
        {
            int detailId = await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == loanApplicationId).Select(x => x.LOANAPPLICATIONDETAILID).FirstOrDefaultAsync();
            return await (from x in context.TBL_LOAN_TRANSACTION_DYNAMICS
        ...
        public async Task<List<ConditionPrecedentViewModel>> GetConditionPrecidents(int loanApplicationId)
        {
            int detailId = await context.TBL_LOAN_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == loanApplicationId).Select(x => x.LOANAPPLICATIONDETAILID).FirstOrDefaultAsync();
            return await (from x in context.TBL_LOAN_CONDITION_PRECEDENT
                    where x.LOANAPPLICATIONDETAILID == detailId
        ...
        public async Task<List<ConditionPrecedentViewModel>> GetLMSConditionPrecidents(int loanApplicationId)
        {
            int detailId = await context.TBL_LMSR_APPLICATION_DETAIL.Where(x => x.LOANAPPLICATIONID == loanApplicationId).Select(x => x.LOANREVIEWAPPLICATIONID).FirstOrDefaultAsync();
            return await (from x in context.TBL_LMSR_CONDITION_PRECEDENT
        ...
        public async Task<bool> updateSuggestionsLoanApplicationdetail(LoanApplicationDetailViewModel model)
        {
            if (model == null) return false;

            var loanDetail =await context.TBL_LOAN_APPLICATION_DETAIL.FindAsync(model.loanApplicationDetailId);
            if (loanDetail != null)
            {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated loan application detail with applicationdetailId")} : {model.loanApplicationDetailId}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSFundingSource()
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.FundingSource).Select(x => new LookupViewModel()
            {
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSRepaymentSource()
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.RepaymentSourceType).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
            }).ToListAsync();
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSRepaymentAgreementType()
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.RepaymentAgreementType).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
            }).ToListAsync();
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllSyndicationType()
        {
            return await context.TBL_LOAN_SYNDICATION_PARTY_TYP.Select(x => new LookupViewModel()
            {
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> GetLoanApplicationDetailsByReference(string reference, int companyId)
        {
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> SearchApprovedLoanApplicationDetails(string reference, int companyId)
        {
            var data = await GetLoanApplicationDetailsByReference(reference, companyId);
            data= data.Where(a => a.approvalStatusId == (int)ApprovalStatusEnum.Approved).ToList();
            var mappings =await  context.TBL_LOAN_APPLICATION_COLLATERL.Where(m => m.DELETED == false).ToListAsync();
        ...
        public void ValidateLoanApplicationLimits(LoanApplicationViewModel application)
        {
                    ...
                    if (facility != null && (facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.Renewal && facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.RenewalWithDecrease))
                    {
                        ...
                        if (sectorValidation != null)
                        {
                            ...
                            if (percentageTotalExposure > 0 && percentageTotalExposure >= sectorValidation.maximumAllowedLimit)
                                throw new SecureException(TranslateHelper.get("Sector Limit for sector, ") + sectorName + TranslateHelper.get(" exceeded!"));
            ...
            {
                throw new SecureException(TranslateHelper.get("Product Limit exceeded!"));
            }
            ...
            {
                throw new SecureException(TranslateHelper.get("Company Limit Exceeded"));
            }
            ...
            if (limitValidation.ValidateIsInsiderCustomer(customerId))
            {
                foreach (var facility in details)
                {
                    if (facility != null && (facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.Renewal && facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.RenewalWithDecrease))
                    {
                        ...
                        {
                            throw new SecureException(TranslateHelper.get("Insider Limit Exceeded"));
                        }
                    }
                }
            }
            ...
            if(limitValidation.IsDirectorRelatedGroup(application.customerGroupId) || limitValidation.CustomerIsDirector(application.customerId))
            {
                foreach (var facility in details)
                {
                    if (facility != null && (facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.Renewal && facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.RenewalWithDecrease))
                    {
                        ...
                        {
                            throw new SecureException(TranslateHelper.get("Director Limit Exceeded"));
                        }
                    }
                }
            ...
            else
            {
                foreach (var facility in details)
                {
                    if (facility != null && (facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.Renewal && facility.loanDetailReviewTypeId != (int)LoanDetailReviewTypeEnum.RenewalWithDecrease))
                    {
                        ...
                        {
                            throw new SecureException(TranslateHelper.get("Single Obligor Limit Exceeded"));
                        }
                    }
                }
            ...
            if (applications != null)
            {
                ...
                if (currency.CURRENCYCODE.Trim() != "NGN")
                {
                    ...
                    if (currencyLimits != null)
                    {
                        ...
                        {
                            throw new SecureException(TranslateHelper.get("Currency Limit Exceeded"));
                        }
                    }
                }
            }
            ...
            if (application.loanTypeId == (int)LoanTypeEnum.CustomerGroup)
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Group Limit for the first 20 Group Customers exposures Exceeded"));
                }
            }
            ...
            if (application.loanTypeId == (int)LoanTypeEnum.CustomerGroup)
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Group Limit for the first 100 Group Customers exposures Exceeded"));
                }
            }
        ...
        private void ValidateFacilityModification(TBL_LOAN_APPLICATION_DETAIL facility, FacilityModificationViewModel model)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Amount Cannot be greater than Approved amount"));
            }
            if (model.approvedAmount > facility.APPROVEDAMOUNT)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Tenor Cannot be greater than Approved Tenor"));
            }
        }
        ...
        public async Task<LoanApplicationTagsLMSViewModel> GetLoanApplicationTagsLMS(int id)
        {
            var entity = await context.TBL_LMSR_APPLICATION.FirstOrDefaultAsync(x => x.LOANAPPLICATIONID == id && x.DELETED == false);
        ...
        public async Task<bool> UpdateLoanApplicationTagsLMS(LoanApplicationTagsLMSViewModel model, int id, UserInfo user)
        {
            var entity =await this.context.TBL_LMSR_APPLICATION.FindAsync(id);
            if (entity != null)
            {
                entity.ISPROJECTRELATED = model.isProjectRelated;
                entity.ISONLENDING = model.isOnLending;
        ...
        public async Task<IEnumerable<RevisedProcessFlowModel>> getFacilityApplicationRevisedProcessFlow()
        {
            var revisedProcessFlow = await (from c in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
        ...
        public async Task<IEnumerable<RevisedProcessFlowModel>> getFacilityApplicationRevisedProcessFlowByProductClassId(short productClassId, short productId, short productTypeId)
        {
            var productTypeFlow = await (from c in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
            ...
            var productClassFlow =await (from c in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
            ...
            var productFlow =await (from c in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
        ...
        public async Task<RevisedProcessFlowModel> getCashCollaterizedProcessFlowBy()
        {
            var cashCollaterzedFlow = await (from c in context.TBL_LOAN_APPLICATN_FLOW_CHANGE
        ...
        public async Task<IEnumerable<LoanApplicationViewModel>> GetFacilityByApplicationId(int loanApplicationId)
        {
            var queryOne = await(from x in context.TBL_LOAN_APPLICATION_DETAIL
            ...
            var queryTwo = await(from x in context.TBL_LMSR_APPLICATION_DETAIL
        ...
        public async Task<bool> LoanApplicationFlowChange(int loanApplicationId)
        {
            var detail =await context.TBL_LOAN_APPLICATION.Where(o => o.LOANAPPLICATIONID == loanApplicationId).Select(o => o).FirstOrDefaultAsync();
            var flowChange =await context.TBL_LOAN_APPLICATN_FLOW_CHANGE.Where(o => o.PLACEHOLDER == "FAM").FirstOrDefaultAsync();

            if (flowChange == null) { throw new ConditionNotMetException(TranslateHelper.get("Workflow Change to reroute not found. Contact admin." )); }
        ...
        public async Task<bool> DeleteLoanApplicationThatFailedRAC(int loanApplicationDetailId, int deletedBy)
        {
            var detail = await context.TBL_LOAN_APPLICATION_DETAIL.Where(o => o.LOANAPPLICATIONDETAILID == loanApplicationDetailId).Select(o => o).FirstOrDefaultAsync();
            if (detail == null)
            {
                detail.DELETED = true;
        ...
        public async Task<LoanApplicationFlowChangeViewModel> GetLoanAppicationFlowChange(int id)
        {
            var entity =await context.TBL_LOAN_APPLICATN_FLOW_CHANGE.FirstOrDefaultAsync(x => x.FLOWCHANGEID == id && x.DELETED == false);
        ...
        public async Task<IEnumerable<LoanApplicationFlowChangeViewModel>> GetLoanApplicationFlowChange()
        {
            return await context.TBL_LOAN_APPLICATN_FLOW_CHANGE.Where(x => x.DELETED == false)
        ...
        public async Task<bool> AddLoanApplicationFlowChange(LoanApplicationFlowChangeViewModel model)
        {
            ...
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"TBL_LOAN_APPLICATN_FLOW_CHANGE '{entity.ToString()}' {TranslateHelper.get("created by")} {auditStaff}",
                IPADDRESS =CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateLoanApplicationFlowChange(LoanApplicationFlowChangeViewModel model, int id, UserInfo user)
        {
            var entity = await this.context.TBL_LOAN_APPLICATN_FLOW_CHANGE.FindAsync(id);
           ...
           // entity.PLACEHOLDER = model.placeHolder;
            ...
            var auditStaff =await context.TBL_STAFF.Where(x => x.STAFFID == user.createdBy).Select(x => x.STAFFCODE).FirstOrDefaultAsync();
            // Audit Section ---------------------------
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"TBL_LOAN_APPLICATN_FLOW_CHANGE '{entity.ToString()}' {TranslateHelper.get("was updated by")} {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteLoanApplicationFlowChange(int id, UserInfo user)
        {
            var entity = await this.context.TBL_LOAN_APPLICATN_FLOW_CHANGE.FindAsync(id);
            entity.DELETED = true;
            ...
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"TBL_LOAN_APPLICATN_FLOW_CHANGE '{entity.ToString()}' {TranslateHelper.get("was deleted by")} {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        // for lien operations
        public async Task<LoanApplicationLienViewModel> GetApplicationDetailLien(int id)
        {
            return await (from x in context.TBL_APPLICATIONDETAIL_LIEN
        ...
        public async Task<IEnumerable<LoanApplicationLienViewModel>> GetLienByCollateralId(int collateralId)
        {
            return await(from x in context.TBL_APPLICATIONDETAIL_LIEN
        ...
        public async Task<IEnumerable<LoanApplicationLienViewModel>> GetApplicationDetailLien()
        {
            return await(from x in context.TBL_APPLICATIONDETAIL_LIEN
                    select new LoanApplicationLienViewModel
                    {
        ...
        public async Task<IEnumerable<LoanApplicationLienViewModel>> GetLienByApplicationDetailId(int applicationDetailId)
        {
            return await(from x in context.TBL_APPLICATIONDETAIL_LIEN
        ...
        public async Task<IEnumerable<LoanApplicationLienViewModel>> GetApplicationDetailLienByIsReleased(bool isReleased)
        {
            return await (from x in context.TBL_APPLICATIONDETAIL_LIEN
        ...
        public async Task<IEnumerable<LoanApplicationLienViewModel>> GetApplicationDetailLienByAccountNo(string accountNo)
        {
            return await(from x in context.TBL_APPLICATIONDETAIL_LIEN
        ...
        public async Task<bool> AddLoanApplicationDetailLien(LoanApplicationLienViewModel model)
        {
               var entity = new TBL_APPLICATIONDETAIL_LIEN
                ...
                this.auditTrail.AddAuditTrail(new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"TBL_APPLICATIONDETAIL_LIEN '{entity.ToString()}' {TranslateHelper.get("created by")} {auditStaff}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateLoanApplicationDetailLien(LoanApplicationLienViewModel model, int id, UserInfo user)
        {
            var entity =await this.context.TBL_APPLICATIONDETAIL_LIEN.FindAsync(id);
            if (entity != null)
            {
                ...
                this.auditTrail.AddAuditTrail(new TBL_AUDIT
                {
                    ...
                    STAFFID = user.createdBy,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = $"TBL_APPLICATIONDETAIL_LIEN '{entity.ToString()}' {TranslateHelper.get("was updated by ")}{auditStaff}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteLoanApplicationDetailLien(int id, UserInfo user)
        {
            var entity =await this.context.TBL_APPLICATIONDETAIL_LIEN.FindAsync(id);
            entity.DELETED = true;
            ...
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"TBL_APPLICATIONDETAIL_LIEN '{entity.ToString()}' {TranslateHelper.get("was deleted by")} {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<LoanDetailReviewTypeViewModel>> GetAllLoanDetailReviewTypes()
        {
            var reviewTypes = await (from x in context.TBL_LOAN_DETAIL_REVIEW_TYPE
        ...
        public async Task<IEnumerable<ApprovedTradeCycleViewModel>> GetAllApprovedTradeCycles()
        {
            var approvedCycles =await (from x in context.TBL_APPROVED_TRADE_CYCLE
        ...
        public async Task<IEnumerable<RetailRecoveryCustomerTransactionsViewModels>> GetRetailRecoveryReporting(DateTime startDate, DateTime endDate, int accreditedConsultantId, string customer)
        {
            ...
            if (int.Parse(customer) > 0)
            {
                 dataTermLoan = await (from lr in context.TBL_LOAN_RECOVERY_ASSIGNMENT
                 ...
                 dataExposureLoan = await (from lr in context.TBL_LOAN_RECOVERY_ASSIGNMENT
                                ...
                                select new RetailRecoveryCustomerTransactionsViewModels
                                {
                                    ...
                                    approvedAmount = (decimal)ln.LOANAMOUNTLCY,
                ...
                foreach (var xx in dataExposureLoan)
                {
                 ...
                 dataDigitalExposureLoan =await (from lr in context.TBL_LOAN_RECOVERY_ASSIGNMENT
                ...
                dataRevolvingLoan =await (from lr in context.TBL_LOAN_RECOVERY_ASSIGNMENT
            ...
            {
               records =await (from a in context.TBL_FINANCE_TRANSACTION
                             where 
