...
namespace FintrakBanking.Repositories.Credit
{
    public class CustomerCollateralRepository : ICustomerCollateralRepository
    {
        ...
        public async Task<int> AddCollateral(CollateralViewModel entity) //, 
        {
            ...
            if (collateralId > 0)
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Error has occured while creating this collateral"));
                }
                if (saved)
                {
        ...
        // UPDATE
        public async Task<IQueryable<CollateralViewModel>> GetCollateralReleaseAwaitingApproval(int companyId, int staffId)
        {
        ...
        private void ValidateJobRequestCheck(TBL_COLLATERAL_RELEASE collateralRelease)
        {
            ...
            if (record.Count > 0)
            {
                ...
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Kindly Complete The Legal Job Request Before You Proceed"));
                }
                if (collateralRelease.COLLATERALRELEASETYPEID == 1) //Final Release
                {
                    ...
                    {
                        throw new ConditionNotMetException(TranslateHelper.get("Kindly Complete The Middle Office Job Request Before You Proceed"));
                    }
                }
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Kindly Complete The Legal & Middle Office Job Request Before You Proceed"));
        ...
        private void PendingJobRequestCheck(TBL_COLLATERAL_RELEASE collateralRelease)
        {
            ...
            if (record.Where(x => x.a.TARGETID == collateralRelease.COLLATERALCUSTOMERID && x.a.JOBTYPEID != (short)JobTypeEnum.legal && (x.a.REQUESTSTATUSID == (short)JobRequestStatusEnum.pending || x.a.REQUESTSTATUSID == (short)JobRequestStatusEnum.processing)).Any())
                throw new ConditionNotMetException(TranslateHelper.get("There are pending Legal job request for this request."));
            if (collateralRelease.COLLATERALRELEASETYPEID == 1) //Final Release
            {
                if (record.Where(x => x.a.TARGETID == collateralRelease.COLLATERALCUSTOMERID && x.a.JOBTYPEID == (short)JobTypeEnum.middleOfficeVerification && x.a.REQUESTSTATUSID == (short)JobRequestStatusEnum.disapproved).Any())
                    throw new ConditionNotMetException(TranslateHelper.get("There are unapproved middle office request."));
                ...
                if (record.Where(x => x.a.TARGETID == collateralRelease.COLLATERALCUSTOMERID && x.a.JOBTYPEID == (short)JobTypeEnum.middleOfficeVerification && (x.a.REQUESTSTATUSID == (short)JobRequestStatusEnum.pending || x.a.REQUESTSTATUSID == (short)JobRequestStatusEnum.processing)).Any())
                    throw new ConditionNotMetException(TranslateHelper.get("There are pending middle office job request for this request.."));
            }

            //}
        }

        public async Task<bool> FinalReleaseCollateralApproval(ApprovalViewModel entity, int staffId)
        {
            ...
            var release = await context.TBL_COLLATERAL_RELEASE.Where(a => a.COLLATERALRELEASEID == entity.targetId).FirstOrDefaultAsync();
            var customerCollateral = await context.TBL_COLLATERAL_CUSTOMER.Where(a => a.COLLATERALCUSTOMERID == release.COLLATERALCUSTOMERID).FirstOrDefaultAsync();


            var mainCollateral = await (from x in context.TBL_COLLATERAL_CUSTOMER
                                  ...
                                  select new { x.COLLATERALTYPEID, x.COLLATERALCUSTOMERID, t.REQUIREINSURANCEPOLICY, t.REQUIREVISITATION, x.COLLATERALCODE, x.COLLATERALVALUE }).FirstOrDefaultAsync();
            ...
            if (mainCollateral.COLLATERALTYPEID > 0)
            {
                ...
                if (mainCollateral.COLLATERALTYPEID == (int)CollateralTypeEnum.CASA)
                {
                    ...
                    var collateral = await context.TBL_COLLATERAL_CASA.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == mainCollateral.COLLATERALCUSTOMERID);
                ...
                if (mainCollateral.COLLATERALTYPEID == (int)CollateralTypeEnum.FixedDeposit)
                {
                    ...
                    var collateral = await context.TBL_COLLATERAL_DEPOSIT.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == mainCollateral.COLLATERALCUSTOMERID);
                    if (collateral != null) securityValue = collateral.SECURITYVALUE;
                }
                ...
                {
                    var existingLien = await context.TBL_CASA_LIEN.FirstOrDefaultAsync(x => x.SOURCEREFERENCENUMBER == mainCollateral.COLLATERALCODE && x.LIENTYPEID == (int)LienTypeEnum.CollateralCreation);
                ...
                var tempRelease = await context.TBL_COLLATERAL_RELEASE_DOC.Where(a => a.COLLATERALRELEASEID == entity.targetId).ToListAsync();
                ...
                this.auditTrail.AddAuditTrail(new TBL_AUDIT
                {
                    ...
                    BRANCHID = (short)entity.BranchId,
                    DETAIL = $"{TranslateHelper.get("Collateral Release Approval")} '{ customerCollateral.COLLATERALCODE }' ",
        ...
        public async Task<bool> TemporaryReleaseCollateralApproval(ApprovalViewModel entity, int staffId)
        {
            //var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.FinalCollateralRelease).ToList();
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.TemporalCollateralRelease).ToList();
            var levelsettings = context.TBL_APPROVAL_LEVEL_SETTING.Where(x => ids.Contains(x.APPROVALLEVELID));
            ...
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.BranchId,
                DETAIL = $"{TranslateHelper.get("Collateral Release Approval")} '{ collateralRecord.COLLATERALCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<ApprovalResponse> ReleaseCollateralGoForApproval(ApprovalViewModel model)
        {
            ApprovalResponse response = new ApprovalResponse();
            var release = await context.TBL_COLLATERAL_RELEASE.Where(a => a.COLLATERALRELEASEID == model.targetId).FirstOrDefaultAsync();
            ...
            if (workflow.NewState != (int)ApprovalState.Ended)
            {
                if (workflow.StatusId != (int)ApprovalStatusEnum.Referred)
                {
                    ...
                    {
                        var tempRelease = await context.TBL_COLLATERAL_RELEASE_DOC.Where(a => a.COLLATERALRELEASEID == release.COLLATERALRELEASEID).ToListAsync();
            ...
            if (workflow.NewState == (int)ApprovalState.Ended && workflow.StatusId != (int)ApprovalStatusEnum.Disapproved)
            {
                if (workflow.StatusId != (int)ApprovalStatusEnum.Referred)
                {
                    if (release.COLLATERALRELEASETYPEID == (int)CollateralReleaseType.FinalRelease) //OperationsEnum.FinalCollateralRelease
                    {
                        output = await FinalReleaseCollateralApproval(model, model.createdBy);

                    }
                    else if (release.COLLATERALRELEASETYPEID == (int)CollateralReleaseType.TemporaryRelease) //OperationsEnum.TemporalCollateralRelease
                    {
                        output = await TemporaryReleaseCollateralApproval(model, model.createdBy);
            ...
            if (output)
            {
                ...
                bool rec = await context.SaveChangesAsync() > 0;
        ...
        public async Task<bool> ReleaseCollateralJobRequest(CollateralViewModel entity)
        {
            var release = await context.TBL_COLLATERAL_RELEASE.Where(a => a.COLLATERALRELEASEID == entity.collateralReleaseId).FirstOrDefaultAsync();
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Collateral Release Action")} '{ context.TBL_COLLATERAL_CUSTOMER.Find(release.COLLATERALCUSTOMERID).COLLATERALCODE }' Job Request Done ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            if (saved)
            {
                ...
                workflow.Comment = TranslateHelper.get("Request for collateral release");
        ...
        public async Task<bool> ReleaseCollateral(CollateralViewModel entity)
        {

            var collateral = await context.TBL_COLLATERAL_CUSTOMER.FindAsync(entity.collateralId);

            var validate = await context.TBL_COLLATERAL_RELEASE.Where(a => a.COLLATERALCUSTOMERID == collateral.COLLATERALCUSTOMERID && (a.APPROVALSTATUSID == (int)ApprovalStatusEnum.Pending || a.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing)).FirstOrDefaultAsync();
            if (validate != null)
            {
                if (entity.releaseType == 2) //Temporary Release
                {
                    ...
                    {
                        var record = await (from a in context.TBL_COLLATERAL_RELEASE_DOC
                                      ...
                                      select new { a }).FirstOrDefaultAsync();
                        if (record != null)
                        {
                            throw new ConditionNotMetException(TranslateHelper.get("Document Release is Already Undergoing Approval For Release."));
                        }
                    }
                ...
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Collateral is Already Undergoing Approval For Release."));
                }
            ...
            if (entity.releaseType == 1) //Final Release
            {
                ...
                var document = await documentContext.TBL_MEDIA_COLLATERAL_DOCUMENTS.Where(a => a.COLLATERALCUSTOMERID == release.COLLATERALCUSTOMERID).ToListAsync();
                if (document.Count() > 0)
                {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Collateral Release Action")} '{ context.TBL_COLLATERAL_CUSTOMER.Find(release.COLLATERALCUSTOMERID).COLLATERALCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<int> AddReleaseDocument(CollateralViewModel model, byte[] file)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Release Document with Collateral Code")} : '{ model.collateralCode }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
            ...
            return await context.SaveChangesAsync() == 0 ? 1 : 2;
        }

        public async Task<IEnumerable<CollateralViewModel>> GetCollateralReleaseDocument(int releaseId)
        {
            string createdBy = string.Empty;

            var data = await (from x in this.documentContext.TBL_DOC_COLLATERAL_RELEASE
            ...
            {
                createdBy = await context.TBL_STAFF.Where(a => a.STAFFID == x.createdBy).Select(r => r.FIRSTNAME + " " + r.MIDDLENAME + " " + r.LASTNAME).FirstOrDefaultAsync();
                //systemDateTime = x.SYSTEMDATETIME,
        ...
        public async Task<CollateralViewModel> GetReleaseSupportingDocument(int documentId)
        {
            string createdBy = string.Empty;
            ...
            createdBy = await context.TBL_STAFF.Where(a => a.STAFFID == data.createdBy).Select(r => r.FIRSTNAME + " " + r.MIDDLENAME + " " + r.LASTNAME).FirstOrDefaultAsync();
        ...
        private int AddCollateralMainFormForGurantee(CollateralViewModel model)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("The specified Collateral Code is already used in the system!"));
            }
        ...
        private void AddTempEquipmentCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainPlant != null)
                {
                    ...
                    comment = TranslateHelper.get("Update plant and equipment collateral through loan application");
                }
                else
                {
                    ...
                    comment = TranslateHelper.get("New plant and equipment collateral created through loan application");
                }
            }
            else
            {
                ...
                comment = TranslateHelper.get("New plant and equipment collateral created through Setup");
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
        ...
        public async Task<bool> AddNewItemInsurancePolicy(InsurancePolicy entity)
        {
            ...
            if (await context.SaveChangesAsync() > 0)
            {
                ...
                workflow.Comment = TranslateHelper.get("Request for item policy approval");
        ...
        public async Task<bool> checkInsurancePolicy(InsurancePolicy model)
        {
            try
            {
                var result = await context.TBL_COLLATERAL_ITEM_POLICY.Where(ip => ip.COLLATERALCUSTOMERID == model.collateraalId
                                                                    && ip.APPROVALSTATUSID != (short)ApprovalStatusEnum.Approved
                                                                    && ip.APPROVALSTATUSID != (short)ApprovalStatusEnum.Disapproved).ToListAsync();
        ...
        public async Task<bool> UpdateInsurancePolicy(int id, CollateralInsurancePolicyViewModel model)
        {
            var entity = await this.context.TBL_COLLATERAL_ITEM_POLICY.FindAsync(id);
        ...
        public async Task<bool> AddInsurancePolicy(CollateralInsurancePolicyViewModel entity)
        {
            var result = await context.TBL_COLLATERAL_ITEM_POLICY.Where(ip => ip.COLLATERALCUSTOMERID == entity.collateraalId
                                                                    && ip.APPROVALSTATUSID != (short)ApprovalStatusEnum.Approved
                                                                    && ip.APPROVALSTATUSID != (short)ApprovalStatusEnum.Disapproved).ToListAsync();

            if (result.Count > 0) throw new SecureException(TranslateHelper.get("An Insurance Request for this Collateral is currently Undergoing Approval"));
        ...
        public async Task<bool> InsuranceRequestGoForApproval(CollateralViewModel model)
        {
            if (model.approvalStatusId == (short)ApprovalStatusEnum.Referred)
            {
                ...
                using (var transaction = context.Database.BeginTransaction())
                {
                    ...
                    workflow.Comment = TranslateHelper.get("Update has been applied, Request for Insurance Policy Approval");
                    workflow.OperationId = (int)OperationsEnum.IsurancePolicyApproval;
                    workflow.DeferredExecution = true;
                    workflow.LogActivity();
                    try
            ...
            else
            {
                ...
                var entity2 = await context.TBL_INSURANCE_REQUEST.FirstOrDefaultAsync(ir => ir.INSURANCEREQUESTID == model.insuranceRequestId);
                if (entity2 == null) return false;

                var entity = await (from ir in context.TBL_INSURANCE_REQUEST
                              ...
                              select cip).ToListAsync();

                if (entity.Any()) throw new SecureException(TranslateHelper.get("Collateral Item already undergoing Insurance Request Approval"));
                ...
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = model.insuranceRequestId;
                workflow.Comment = TranslateHelper.get("Request for Insurance Policy approval");
                workflow.OperationId = (int)OperationsEnum.IsurancePolicyApproval;
                workflow.DeferredExecution = true; // false by default will call the internal SaveChanges()
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
        ...
        public async Task<string> GetLastComment(int targetId, int operationId)
        {
            var result = await (from ir in context.TBL_INSURANCE_REQUEST
        ...
        public async Task<IEnumerable<CollateralViewModel>> GetInsuranceRequests(int staffId)
        {

            var initiator =await  context.TBL_APPROVAL_TRAIL.Where(o => o.OPERATIONID == (int)OperationsEnum.IsurancePolicyApproval).OrderBy(o => o.APPROVALTRAILID).Select(o => o.REQUESTSTAFFID).FirstOrDefaultAsync();

            var result1 = await (from ir in context.TBL_INSURANCE_REQUEST
        ...
        public async Task<bool> UpdateInsurancePolicyRequest(CollateralInsuranceRequestViewModel model, int id)
        {
            var entity = await context.TBL_INSURANCE_REQUEST.FindAsync(id);
        ...
        public async Task<bool> AddInsurancePolicyRequest(CollateralInsuranceRequestViewModel model, int? id)
        {
            var data = await context.TBL_INSURANCE_REQUEST.FirstOrDefaultAsync(d => d.COLLATERALCUSTOMERID == model.collateralCustomerId &&
                                                                        d.APPROVALSTATUSID == (short)ApprovalStatusEnum.Processing
                                                                        || d.APPROVALSTATUSID == (short)ApprovalStatusEnum.Referred);

            if (data != null) throw new SecureException(TranslateHelper.get("Collateral Item is already Undergoing Insurance Request Approval"));

            var data2 = await context.TBL_INSURANCE_REQUEST.FirstOrDefaultAsync(d => d.COLLATERALCUSTOMERID == model.collateralCustomerId &&
                                                                        d.APPROVALSTATUSID == (short)ApprovalStatusEnum.Pending);

            if (data2 != null) throw new SecureException(TranslateHelper.get("A Pending Insurance Request already exists for this Collateral"));
            ...
            {
                var rejectedInsurance = await context.TBL_INSURANCE_REQUEST.FindAsync(id);

                if (rejectedInsurance == null || (rejectedInsurance != null && rejectedInsurance.APPROVALSTATUSID != (short)ApprovalStatusEnum.Disapproved))
                {
                    throw new SecureException(TranslateHelper.get("An Error Occured, Please Contact the System Administrator"));
                }
        ...
        public async Task<bool> SaveCollateralMainDocument(CollateralViewModel model, int collateralId, byte[] file)
        {
            ...
            else
            {
                ...
                try
                {
                    return await documentContext.SaveChangesAsync() != 0;
                }
                catch (Exception ex) { }
            }
        ...
        // GET MAIN INFO
        public async Task<CollateralViewModel> GetCustomerCollateralInformation(int colateralcustomerId, int companyId)
        {
            ...
            var collaterals = await context.TBL_COLLATERAL_CUSTOMER.Where(x => x.DELETED == false && x.COLLATERALCUSTOMERID == colateralcustomerId)
                .GroupJoin(
                ...
                (
                    ...
                    ;
            decimal usage = 0;
            var mappings = await context.TBL_LOAN_COLLATERAL_MAPPING.Where(m => m.COLLATERALCUSTOMERID == collaterals.collateralId && m.DELETED == false && m.ISRELEASED == false).ToListAsync();
        ...
        public async Task<List<CollateralViewModel>> GetCustomerPropertyCollaterals(int? customerId, int companyId)
        {
            var company = context.TBL_COMPANY.Find(companyId);
            if (customerId != null)
            {
                var collaterals = await (from cc in context.TBL_COLLATERAL_CUSTOMER
        ...
        public async Task<IEnumerable<OriginalDocumentSubmissionByFacilityViewModel>> GetCustomerFacility(int customerId)
        {

            var application = await (from c in context.TBL_LOAN_APPLICATION_DETAIL
        ...
        public async Task<IEnumerable<CollateralViewModel>> GetCustomerCashCollateral(int customerId, int? applicationId, int companyId, bool isLMS = false)
        {
            var typeIds = new List<int>();
            var company = context.TBL_COMPANY.Find(companyId);
            var baseCurrencyId = company.TBL_CURRENCY.CURRENCYID;
            ...
            if (applicationId != null && applicationId != 0)
            {
                if (isLMS)
                {
                    productIds = await context.TBL_LMSR_APPLICATION_DETAIL
                    .Where(x => x.LOANAPPLICATIONID == applicationId)
                    .Select(x => x.PRODUCTID)
                ...
                {
                    productIds = await context.TBL_LOAN_APPLICATION_DETAIL
                    .Where(x => x.LOANAPPLICATIONID == applicationId)
        ...
        public async Task<IEnumerable<CollateralCashReleaseViewModel>> GetCustomerCashCollateralApplications(int id)
        {

            var collateralsA = await (from x in context.TBL_LOAN_APPLICATION_COLLATERL
                                join b in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals b.COLLATERALCUSTOMERID
                                //join c in context.TBL_APPLICATIONDETAIL_LIEN on x.LOANAPPLICATIONDETAILID equals c.APPLICATIONDETAILIDS
                                where
                               ...
                               //&& c.ISRELEASED == false
                                ...
                                select new CollateralCashReleaseViewModel
                                {
                                    ...
                                    //lienAmount = c.AMOUNT,
                                    facility = (from p in context.TBL_PRODUCT join a in context.TBL_LOAN_APPLICATION_DETAIL on p.PRODUCTID equals a.APPROVEDPRODUCTID where a.LOANAPPLICATIONDETAILID == x.LOANAPPLICATIONDETAILID select p.PRODUCTNAME).DefaultIfEmpty("N/A").FirstOrDefault(),
        ...
        public async Task<IEnumerable<CollateralCoverageViewModel>> GetProposedCustomerCollateralByCustomerIdLMS(int customerId, bool getAll = false)
        {
            ...
            if (getAll)
            {
                collaterals = await (from x in context.TBL_LOAN_APPLICATION_COLLATERL
                               join c in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals c.COLLATERALCUSTOMERID
                               join a in context.TBL_COLLATERAL_TYPE on c.COLLATERALTYPEID equals a.COLLATERALTYPEID
                               join s in context.TBL_COLLATERAL_TYPE_SUB on c.COLLATERALSUBTYPEID equals s.COLLATERALSUBTYPEID
        ...
        public async Task<IEnumerable<CollateralCoverageViewModel>> GetProposedCustomerCollateralByLoanApplicationDetailId(int loanApplicationDetailId)
        {
            ...
            var collaterals = await (from x in context.TBL_LOAN_APPLICATION_COLLATERL
                               join c in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals c.COLLATERALCUSTOMERID
                               join a in context.TBL_COLLATERAL_TYPE on c.COLLATERALTYPEID equals a.COLLATERALTYPEID
                               join s in context.TBL_COLLATERAL_TYPE_SUB on c.COLLATERALSUBTYPEID equals s.COLLATERALSUBTYPEID
        ...
        public IEnumerable<CollateralViewModel> GetCustomerCollateralReport(string searchParam, int companyId)
        {
            ...
            var collaterals = (
                               ...
                               select new CollateralViewModel
                               {
                                   ...
                                   camRefNumber = context.TBL_LOAN_APPLICATION.FirstOrDefault(x => x.CUSTOMERID == d.CUSTOMERID.Value).APPLICATIONREFERENCENUMBER,
                                   ...
                                   availableCollateralValue = 0,
                                   accountNumber = context.TBL_CASA.FirstOrDefault(x => x.CUSTOMERID == d.CUSTOMERID.Value).PRODUCTACCOUNTNUMBER,
        ...
        public async Task<CollateralViewModel> GetCustomerCollateralByCustomerCollateralId(int customerCollateralId)
        {
            var collateral = await context.TBL_COLLATERAL_CUSTOMER.Where(x => x.DELETED == false && x.COLLATERALCUSTOMERID == customerCollateralId)
            .Select(x => new CollateralViewModel
            {
                collateralId = x.COLLATERALCUSTOMERID,
                collateralTypeId = x.COLLATERALTYPEID,
        ...
        public async Task<CollateralViewModel> GetCollateralTypeByCollateralId(int collateralId, int typeId)
        {
            ...
            try
            {
                switch (typeId)
                {
                    case (int)CollateralTypeEnum.FixedDeposit: data = await GetCollateralDeposit(collateralId); break;
                    case (int)CollateralTypeEnum.PlantAndMachinery: data = await GetCollateralMachinery(collateralId); break;
                    case (int)CollateralTypeEnum.Miscellaneous: data = await GetCollateralMiscellaneous(collateralId); break;
                    case (int)CollateralTypeEnum.Gaurantee: data = await GetCollateralGuarantee(collateralId); break;
                    case (int)CollateralTypeEnum.CASA: data = await GetCollateralCasa(collateralId); break;
                    case (int)CollateralTypeEnum.Property: data = await GetCollateralImmovableProperty(collateralId); break;
                    case (int)CollateralTypeEnum.TreasuryBillsAndBonds: data = await GetCollateralMarketableSecurities(collateralId); break;
                    case (int)CollateralTypeEnum.InsurancePolicy: data = await GetCollateralPolicy(collateralId); break;
                    case (int)CollateralTypeEnum.PreciousMetal: data = await GetCollateralPreciousMetal(collateralId); break;
                    case (int)CollateralTypeEnum.MarketableSecurities_Shares: data = await GetCollateralStock(collateralId); break;
                    case (int)CollateralTypeEnum.Vehicle: data = await GetCollateralVehicle(collateralId); break;
                    case (int)CollateralTypeEnum.Promissory: data = await GetCollateralPromissory(collateralId); break;
                    case (int)CollateralTypeEnum.ISPO: data = await GetISPOCollateral(collateralId); break;
                    case (int)CollateralTypeEnum.DomiciliationContract: data = await GetContractDomiciliationCollateral(collateralId); break;
                    case (int)CollateralTypeEnum.DomiciliationSalary: data = await GetContractDomiciliationSalary(collateralId); break;
                    case (int)CollateralTypeEnum.Indemity: data = await GetIndemityCollateral(collateralId); break;
                    default:
                        break;
                }
            }
        ...
        private async Task<CollateralViewModel> GetCollateralMiscellaneous(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_MISCELLANEOUS.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
        ...
        private async Task<CollateralViewModel> GetCollateralMachinery(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_PLANT_AND_EQUIP.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        private async Task<CollateralViewModel> GetCollateralDeposit(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_DEPOSIT.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
        ...
        public async Task<List<InsurancePolicy>> GetCollateralInsurancePolicies(int collateralId)
        {
            var insurance = await context.TBL_COLLATERAL_ITEM_POLICY.Where(x => x.COLLATERALCUSTOMERID == collateralId
        ...
        public async Task<InsurancePolicy> GetInsurancePolicy(int collateralId)
        {
            var insurance = await context.TBL_COLLATERAL_ITEM_POLICY.Where(x => x.COLLATERALCUSTOMERID == collateralId
                                                                        && x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing
                                                                        || x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Referred
                ...
                }).OrderByDescending(ip => ip.policyId).FirstOrDefaultAsync();

            return insurance;
        }

        public async Task<List<InsurancePolicy>> GetTempCollateralInsurancePoliciesWaitingForApproval(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.IsurancePolicyApproval).ToList();

            var insurance = await (from x in context.TBL_TEMP_COLLATERAL_ITEM_POLI
        ...
        public async Task<List<InsurancePolicy>> GetCollateralInsurancePoliciesWaitingForApproval(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.IsurancePolicyApproval).ToList();
            ...
            //                         && x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing
            //                         && atrail.RESPONSESTAFFID == null
            var insurance = await (from x in context.TBL_INSURANCE_REQUEST
                             join s in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals s.COLLATERALCUSTOMERID
        ...
        public async Task<IEnumerable<InsurancePolicy>> Explore(string searchString)
        {
            ...
            var operations = await (from x in context.TBL_INSURANCE_REQUEST
                              join s in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals s.COLLATERALCUSTOMERID
                              join atrail in context.TBL_APPROVAL_TRAIL on x.INSURANCEREQUESTID equals atrail.TARGETID
                              join c in context.TBL_CUSTOMER on s.CUSTOMERID equals c.CUSTOMERID
        ...
        private void AddTempStockCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainStock != null)
                {
                    ...
                    comment = $"New stock collateral type has been update through loan application by staff: {entity.createdBy} ";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New stock collateral type has been created through loan application by staff")}: {entity.createdBy}";
                }
            }
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp stock collateral type has been created by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
                workflow.Comment = TranslateHelper.get("Request for stock collateral approval");
                workflow.OperationId = (int)OperationsEnum.CollateralApproval;
                workflow.DeferredExecution = true; // false by default will call the internal SaveChanges()
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
        ...
        private async Task<CollateralViewModel> GetCollateralStock(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_STOCK.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        private void AddPromissoryCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainVehicle != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("New promissory collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("Promissory collateral type has been created through loan application by staff")}: {entity.createdBy}";
            ...
            else
            {
                ...
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Promisory Note Has Already Been Used Before."));
                }
                ...
                comment = $"{TranslateHelper.get("temp Promissory collateral type has been created by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private void AddISPOCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (collateral != null)
                {
                    ...
                    collateral.REMARK = entity.remark;
                    collateral.DESCRIPTION = entity.description;
                    comment = $"{TranslateHelper.get("ISPO collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("ISPO collateral type has been created through loan application by staff")}: {entity.createdBy}";
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New ISPO collateral type has been created by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private void AddContractDomiciliationCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (collateral != null)
                {
                    ...
                    collateral.REMARK = entity.remark;
                    collateral.DESCRIPTION = entity.description;
                    comment = $"{TranslateHelper.get("New Salary Domiciliation collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                ...
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New Salary Domiciliation collateral type has been created through loan application by staff")}: {entity.createdBy}";
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp domiciliation contract collateral type has been created by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private void AddSalaryDomiciliationCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (collateral != null)
                {
                    ...
                    collateral.REMARK = entity.remark;
                    collateral.DESCRIPTION = entity.description;
                    comment = $"{TranslateHelper.get("New Salary Domiciliation collateral type has been updated through loan application by staff")} {entity.createdBy} staffid";
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New Temp Salary Domiciliation collateral type has been created by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private void AddIndemityCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (collateral != null)
                {
                    ...
                    collateral.DESCRIPTION = entity.description;
                    comment = $"{TranslateHelper.get("New Indemnity collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New Indemnity collateral type has been created through loan application by staff")}: {entity.createdBy}";
                }
            }
            else
            {
                ...
                comment = $"{TranslateHelper.get("New Temp Indemnity collateral type has been created by staff")}: {entity.createdBy}";
        ...
        private async Task<CollateralViewModel> GetCollateralPromissory(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_PROMISSORY.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        private void AddVehicleCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainVehicle != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("Vehicle collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New vehicle collateral type has been created through loan application by staff")}: {entity.createdBy}";
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New vehicle collateral type has been updated by staff")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private async Task<CollateralViewModel> GetCollateralVehicle(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_VEHICLE.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
                ...
                dateOfManufacture = (DateTime)specifics.MANUFACTUREDDATE,
        ...
        private async Task<CollateralViewModel> GetISPOCollateral(int collateralId)
        {
            var collateral = await context.TBL_COLLATERAL_ISPO.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
        ...
        private async Task<CollateralViewModel> GetContractDomiciliationCollateral(int collateralId)
        {
            var collateral = await context.TBL_COLLATERAL_DOMICILIATION.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = collateral.COLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetContractDomiciliationSalary(int collateralId)
        {
            var collateral = await context.TBL_COLLATERAL_DOMICILIATION.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
        ...
        private async Task<CollateralViewModel> GetIndemityCollateral(int collateralId)
        {
            var collateral = await context.TBL_COLLATERAL_INDEMNITY.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = collateral.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        private void AddTempPreciousMetalCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainMetal != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("New precious metal collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("Precious metal collateral type has been created through loan application by staff")}: {entity.createdBy}";
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp precious metal collateral type has been created by staff")}: {entity.createdBy}";
        ...
        private async Task<CollateralViewModel> GetCollateralPreciousMetal(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_PRECIOUSMETAL.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        private async Task<CollateralViewModel> GetCollateralCasa(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_CASA.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            if (specifics == null)
            {
                return null;
            }
        ...
        private void AddTempGuaranteeCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
               ...
               .FirstOrDefault();
                ...
                if (guarantee != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("guarantee collateral type has been updated through loan application by staff")}: {entity.createdBy}";
                ...
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New guarantee collateral type has been update through loan application by staff")}:{entity.createdBy}";
                }
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp guarantee collateral type has been update through loan application by staff Id")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        public async Task<List<CollateralViewModel>> AddGuaranteeJoinCollateral(CollateralViewModel entity, byte[] buffer)
        {
            int collateralId = 0;
            List<CollateralViewModel> listOfJoinCollateralGuarantee = new List<CollateralViewModel>();

            if (await context.TBL_TEMP_COLLATERAL_CUSTOMER.Where(x => x.TEMPCOLLATERALCUSTOMERID == entity.collateralId).AnyAsync() != true)
            {
                ...
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
                workflow.Comment = TranslateHelper.get("Request for join Guarantee collateral approval");
                workflow.OperationId = (int)OperationsEnum.CollateralApproval;
                workflow.DeferredExecution = true; // false by default will call the internal SaveChanges()
                workflow.ExternalInitialization = true;
                workflow.LogActivity();
                ...
                if (buffer != null) { await SaveCollateralMainDocument(entity, collateralId, buffer); }
                listOfJoinCollateralGuarantee = await GetCollateralJoinGuarantiee(collateralId);
            }
            else
            {
                if (entity.collateralId > 0)
                {
                    ...
                    entity.TargetId = guarantee.TEMPCOLLATERALGAURANTEEID;
                    if (buffer != null) { await SaveCollateralMainDocument(entity, entity.collateralId, buffer); }
                    listOfJoinCollateralGuarantee = await GetCollateralJoinGuarantiee(entity.collateralId);
        ...
        private async Task<List<CollateralViewModel>> GetCollateralJoinGuarantiee(int collateralId)
        {

            var guarantee = await (from x in context.TBL_TEMP_COLLATERAL_GAURANTEE
        ...
        private async Task<CollateralViewModel> GetCollateralGuarantee(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_GAURANTEE.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetCollateralImmovableProperty(int collateralId)
        {
            var details = await context.TBL_COLLATERAL_IMMOVE_PROPERTY.Where(x => x.COLLATERALCUSTOMERID == collateralId).Select(x => new CollateralViewModel
            {
                collateralId = x.COLLATERALCUSTOMERID,
        ...
        private void AddTempMarketableSecuritiesCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainMarket != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("Market security collateral type has been update through loan application by staffid")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New market security collateral type has been update through loan application by staffid")}: {entity.createdBy}";
                }
            }
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New market security collateral type has been created by staffid")}: {entity.createdBy}";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private async Task<CollateralViewModel> GetCollateralMarketableSecurities(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_MKT_SECURITY.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
        ...
        private void AddTempPolicyCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainPolicy != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("Policy collateral type has been updated through loan application by staffid")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New Policy collateral type has been created through loan application by staffid")}: {entity.createdBy}";
                }
            }
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp Policy collateral type has been created through loan application by staffid")}: {entity.createdBy}";
        ...
        private async Task<CollateralViewModel> GetCollateralPolicy(int collateralId)
        {
            var specifics = await context.TBL_COLLATERAL_POLICY.FirstOrDefaultAsync(x => x.COLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.COLLATERALCUSTOMERID,
                collateralSubTypeId = context.TBL_COLLATERAL_CUSTOMER.Find(collateralId).COLLATERALSUBTYPEID,
        ...
        public async Task<List<CollateralDocumentViewModel>> GetPropertyVistation(int collateralId)
        {
            ...
            var specifics = await (from x in context.TBL_COLLATERAL_VISITATION
                             where x.COLLATERALCUSTOMERID == collateralId
        ...
        public async Task<List<CollateralDocumentViewModel>> GetTempPropertyVistation(int collateralId)
        {
            List<CollateralDocumentViewModel> response = new List<CollateralDocumentViewModel>();
            var summary = context.TBL_COLLATERAL_CUSTOMER.FirstOrDefault(c => c.COLLATERALCUSTOMERID == collateralId)?.COLLATERALSUMMARY;
        ...
        public async Task<IEnumerable<LoanApplicationCollateralViewModel>> MapApplicationCollateral(ApplicationCollateralMapping entity)
        {

            var proposed = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(o => o.COLLATERALCUSTOMERID == entity.collateralId && o.LOANAPPLICATIONDETAILID == entity.loanApplicationDetailId && o.DELETED == false).FirstOrDefaultAsync();
            if (proposed != null)
            {
        ...
        public async Task<bool> IsCollateralMapped(ApplicationCollateralMapping entity)
        {
        ...
        public async Task<IEnumerable<ActiveCustomerCollateralViewModel>> GetLoanCollateral(int loanId, int productTypeId)
        {
            try
            {
                var l = context.TBL_LOAN.Where(x => x.TERMLOANID == loanId).FirstOrDefault();
                var r = context.TBL_LOAN_REVOLVING.Where(x => x.REVOLVINGLOANID == loanId).FirstOrDefault();
                var con = context.TBL_LOAN_CONTINGENT.Where(x => x.CONTINGENTLOANID == loanId).FirstOrDefault();
                if (l != null)
                {
                    var collaterals = await (from //x in context.TBL_LOAN_COLLATERAL_MAPPING join
                            c in context.TBL_COLLATERAL_CUSTOMER //on x.COLLATERALCUSTOMERID equals c.COLLATERALCUSTOMERID
                ...
                else 
                {
                    var collaterals = await (from //x in context.TBL_LOAN_COLLATERAL_MAPPING join
                            c in context.TBL_COLLATERAL_CUSTOMER //on x.COLLATERALCUSTOMERID equals c.COLLATERALCUSTOMERID
                                             join ct in context.TBL_COLLATERAL_TYPE on c.COLLATERALTYPEID equals ct.COLLATERALTYPEID
                                             join cs in context.TBL_COLLATERAL_TYPE_SUB on c.COLLATERALSUBTYPEID equals cs.COLLATERALSUBTYPEID
                                             where c.CUSTOMERID == con.CUSTOMERID
                                             && c.DELETED == false
                                             select new ActiveCustomerCollateralViewModel
                                             {
                                                 collateralType = ct.COLLATERALTYPENAME,
        ...
        public async Task<bool> ReleaseCollateral(int collateralMappingId, int staffId, GeneralEntity model)
        {
            var mapping = await context.TBL_LOAN_COLLATERAL_MAPPING.FindAsync(collateralMappingId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Collateral Release Action")} '{ mapping.TBL_COLLATERAL_CUSTOMER.COLLATERALCODE }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> ApproveCollateralRelease(ApprovalViewModel entity, int staffId, GeneralEntity user)
        {
            ...
            var mapping = await context.TBL_LOAN_COLLATERAL_MAPPING.FindAsync(entity.targetId);
            ...
            if (workflow.NewState == (int)ApprovalState.Ended && workflow.StatusId == (int)ApprovalStatusEnum.Approved)
            {
                ...
                if (mainCollateral.COLLATERALTYPEID > 0)
                {
                    var description = "Collateral lien release";
                    decimal securityValue = 0;

                    if (mainCollateral.COLLATERALTYPEID == (int)CollateralTypeEnum.CASA)
                    {
                        description = "CASA collateral lien release";
                    ...
                    if (mainCollateral.COLLATERALTYPEID == (int)CollateralTypeEnum.FixedDeposit)
                    {
                        description = "Deposit collateral lien release";
                    ...
                    this.auditTrail.AddAuditTrail(new TBL_AUDIT
                    {
                        ...
                        BRANCHID = (short)user.userBranchId,
                        DETAIL = $"{TranslateHelper.get("Collateral Release Approval")} '{ mapping.TBL_COLLATERAL_CUSTOMER.COLLATERALCODE }' ",
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<ActiveCustomerCollateralViewModel>> GetPendingCustomerCollateralRelease(int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.OfferLetterApproval).ToList();

            var pending = await (from x in context.TBL_LOAN_COLLATERAL_MAPPING
                           join c in context.TBL_COLLATERAL_CUSTOMER on x.COLLATERALCUSTOMERID equals c.COLLATERALCUSTOMERID
        ...
        public bool AssignCollateral(ActiveCustomerCollateralViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Collateral Assignment")} :: LoanApplicationId:'{ assignment.LOANID }' CollateralCustomerId:'{ assignment.COLLATERALCUSTOMERID }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            try {

                {
                    ...
                    else
                    {
                        ...
                        comment = $" Fixed Deposit collateral type has been created through loan application by {entity.createdBy} staffid";
        ...
        private ICollection<TBL_COLLATERAL_ITEM_POLICY> AddCollateralCustomerPolicy(int collateralTypeId, CollateralCustomerPolicyViewModel entity)
        {
            ...
            if (!type.REQUIREINSURANCEPOLICY)
                return null;
            if (entity == null)
                throw new InvalidOperationException(TranslateHelper.get("This collateral type requires insurance policy which was not submitted"));
        ...
        public async Task<bool> UpdateCollateralValuer(CollateralValuersViewModel entity, int id)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"Updated collateral valuer with Id: {entity.collateralValuerId} ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<CollateralValuersViewModel>> GetCollateralValuer(int companyId)
        {
            return await (from m in context.TBL_ACCREDITEDCONSULTANT
        ...
        public async Task<List<CollateralValuerTypeViewModel>> GetCollateralValuerType()
        {
            return await (from m in context.TBL_COLLATERAL_VALUER_TYPE
        ...
        public async Task<bool> DeleteProposedCollateral(CollateralCoverageViewModel model)
        {
            var data = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(o => o.LOANAPPCOLLATERALID == model.loanAppCollateralId).Select(o => o).FirstOrDefaultAsync();
            if (data.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved)
            {
                throw new Exception(TranslateHelper.get("Cannot Delete An Already Approved Collateral Mapping"));
            }
            data.DELETED = true;
        ...
        public async Task<bool> DeleteDuplicatedCollateral(CollateralViewModel model)
        {
            bool status = false;
            var data = await context.TBL_COLLATERAL_CUSTOMER.Where(o => o.COLLATERALCUSTOMERID == model.collateralCustomerId).Select(o => o).FirstOrDefaultAsync();
            var data2 = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(o => o.COLLATERALCUSTOMERID == model.collateralCustomerId && o.DELETED == false).Select(o => o).FirstOrDefaultAsync();
        ...
        public CollateralHistory getCollateralHistoryUsage(int collateralId)
        {
            ...
            var termLoansTiedToCollateral = (from c in context.TBL_COLLATERAL_CUSTOMER
                                             ...
                                             select new CollateralHistoryList
                                             {
                                                 ...
                                                 dateProposed = lc.DATETIMECREATED == null ? c.DATETIMECREATED : lc.DATETIMECREATED,
                                                 dateUsed = l.DISBURSEDATE == null ? c.DATETIMECREATED : l.DISBURSEDATE,
        ...
        //STCK PRICE
        public async Task<IEnumerable<StockCompanyViewModel>> getStockPrice()
        {
            var stock = await (from c in context.TBL_STOCK_COMPANY
        ...
        private void AddTempImmovablePropertyCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (property != null)
                {
                    ...
                    comment = $"Property collateral type has been updated through loan application by staffid: {entity.createdBy}";
                ...
                else
                {
                    ...
                    comment = $"{TranslateHelper.get("New property collateral type has been created through loan application by staffid")}: {entity.createdBy}";
                    if (context.SaveChanges() != 0)
                    {
                        var collateral = context.TBL_COLLATERAL_CUSTOMER.FirstOrDefault(c => c.COLLATERALCUSTOMERID == collateralId);
                        NotifyForCollateralStatusUpdate(collateral, entity.perfectionStatusId);
            ...
            else
            {
                ...
                comment = $"{TranslateHelper.get("New temp property collateral type has been created by staffid")}: {entity.createdBy} ";
                workflow.StaffId = entity.createdBy;
                workflow.CompanyId = entity.companyId;
                workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                workflow.TargetId = collateralId;
        ...
        private void AddTempCasaCollateral(int collateralId, CollateralViewModel entity)
        {
            ...
            if (entity.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                if (mainCasa != null)
                {
                    ...
                    comment = $"{TranslateHelper.get("New CASA collateral type has been updated through loan application by staffid")}: {entity.createdBy}";
                }
                else
                {
                    ...
                    //else
                    {
                        ...
                        comment = $"{TranslateHelper.get("New CASA collateral type has been created through loan application by staffid")}: {entity.createdBy}";
            ...
            else
            {
                ...
                //else
                {
                    ...
                    comment = $"{TranslateHelper.get("New Temp CASA collateral type has been created by staffid")}: {entity.createdBy}";
                    workflow.StaffId = entity.createdBy;
                    workflow.CompanyId = entity.companyId;
                    workflow.StatusId = (int)ApprovalStatusEnum.Processing;
                    workflow.TargetId = collateralId;
        ...
        public async Task<IEnumerable<CollateralViewModel>> GetTempCustomerCollateralForApproval(int companyId, int staffId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CollateralApproval).ToList();

            var collaterals = await (from x in context.TBL_TEMP_COLLATERAL_CUSTOMER
        ...
        public IEnumerable<CollateralViewModel> GetCustomerCollateralByCollateralId(int companyId, int collaterId)
        {
            ...
            //var collaterals = (from valPre in context.TBL_COLLATERAL_VALUATION_PRE //Added
            //                   join q in context.TBL_COLLATERAL_VALUATION on valPre.COLLATERALVALUATIONID equals q.COLLATERALVALUATIONID //Added
            //                   join x in context.TBL_COLLATERAL_CUSTOMER on q.COLLATERALCUSTOMERID equals x.COLLATERALCUSTOMERID //Added
                               join c in context.TBL_COLLATERAL_TYPE on x.COLLATERALTYPEID equals c.COLLATERALTYPEID
                               join a in context.TBL_CUSTOMER on x.CUSTOMERID equals a.CUSTOMERID
                               let ColSubType = context.TBL_COLLATERAL_TYPE_SUB.Where(c => c.COLLATERALSUBTYPEID == x.COLLATERALSUBTYPEID).Select(c => c.COLLATERALSUBTYPENAME).FirstOrDefault()
                               ...
                               select new CollateralViewModel
                               {
                                   ...
                                   //referenceNumber = valPre.REFERENCENUMBER, //Added
                                   customerAccount = context.TBL_CASA.Where(c => c.CUSTOMERID == a.CUSTOMERID).Select(c => c.PRODUCTACCOUNTNUMBER).FirstOrDefault(),
                                   securityCollateralValue = (double)x.COLLATERALVALUE - ((double)x.COLLATERALVALUE * (x.HAIRCUT * 0.01))
                               }).ToList();
        ...
        private int AddTempCollateralMainForm(CollateralViewModel model)
        {
            ...
            //if(customer == null)
            //{
            //    customer = context.TBL_CUSTOMER_GROUP.Where(x => x.CUSTOMERGROUPID == model.customerGroupId).Select(x => x).FirstOrDefault();
            //}
            ...
            if (model.isRegistrationDoneViaLoanApplication == (int)CollateralRegistrationTypeEnum.isRegistrationDoneViaLoanApplication)
            {
                ...
                else
                {
                    ...
                    var collateral = context.TBL_COLLATERAL_CUSTOMER.Add(new TBL_COLLATERAL_CUSTOMER
                    {
                        ...
                        CUSTOMERID = model.customerId ?? model.customerGroupId,
                        CAMREFNUMBER = model.camRefNumber,
                        CREATEDBY = model.createdBy,
                        DATETIMECREATED = genSetup.GetApplicationDate(),
            ...
            else
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("The specified Collateral is edited and is going through approval!"));
                }
        ...
        public async Task<bool> AddCollateralInsuranceTrackingForm(int accountOfficer, CollateralInsuranceTrackingViewModel model)
        {
                ...
                var insurancePolicy = await context.TBL_COLLATERAL_INSURANCE_TRACKING.Where(x => x.POLICYNUMBER.Trim() == model.referenceNumber.Trim() && x.DELETED == false).Select(x => x).FirstOrDefaultAsync();

                if (insurancePolicy != null)
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Insurance Reg/Ref Number Already Exists, Kindly enter a unique code or leave the field blank for Auto-Generation"));

                }

                var insurancePolicy2 = await context.TBL_COLLATERAL_INSURANCE_TRACKING.Where(x => x.LOANAPPLICATIONDETAILID == model.loanApplicationDetailId && x.COLLATERALCUSTOMERID == model.collateralCustomerId && x.DELETED == false).Select(x => x).FirstOrDefaultAsync();

                if (insurancePolicy2 != null)
                {
                    throw new ConditionNotMetException(TranslateHelper.get("Policy already captured and been assigned to this facility"));
        ...
        public async Task<bool> UpdateCollateralInsuranceTrackingForm(int accountOfficer, int id, CollateralInsuranceTrackingViewModel model)
        {


            if (id == 0)
            {
                throw new ConditionNotMetException(TranslateHelper.get("Tracking Reference ID is Null"));

            }
            
                var cit = await context.TBL_COLLATERAL_INSURANCE_TRACKING.FindAsync(id);
        ...
        public async Task<bool> GetCustomerCollateralInsuranceDetailsConfirmation(int getStaffId, int id)
        {
            ...
            var cit2 = await context.TBL_COLLATERAL_INSURANCE_TRACKING.Where(x=>x.LOANAPPLICATIONDETAILID == cit.LOANAPPLICATIONDETAILID).ToListAsync();
            if(cit2.Count() > 0)
            {
        ...
        public async Task<bool> DeleteCustomerCollateralInsuranceDetails(int getStaffId, int id)
        {
        ...
        public void NotifyForCollateralVisitation(TBL_COLLATERAL_CUSTOMER collateral, bool saveInternally = false)
        {
            ...
            if (property != null)
            {
                ...
                alertSubject = TranslateHelper.get("NOTIFICATION COLLATERAL VALUATION REMINDER FROM FINTRAK ALERT");
                ...
                messageBody = $"{TranslateHelper.get("Dear")} {staffFullName} <br /><br />," +
                               $"{TranslateHelper.get("This is to inform you that the collateral")}, {collateral?.COLLATERALSUMMARY} {TranslateHelper.get("belonging to")} {customer?.LASTNAME + ", " + customer?.FIRSTNAME + " " + customer?.MIDDLENAME}" +
                               $"(Customer ID: {collateral?.CUSTOMERCODE}) {TranslateHelper.get("with OMV")} {collateral?.TBL_CURRENCY?.CURRENCYCODE} {String.Format("{0:0,0.00}", property?.OPENMARKETVALUE)} {TranslateHelper.get("and FSV of")} {collateral?.TBL_CURRENCY?.CURRENCYCODE} {String.Format("{0:0,0.00}", property?.FORCEDSALEVALUE)} {TranslateHelper.get("is due for revaluation on")} {nextVisit} <br /><br />" +
                               $"{TranslateHelper.get("Kindly inform the customer, and initiate request for revaluation on Fintrak")} <br /><br />" +
                               $"{TranslateHelper.get("Regards")}"
                               ;
        ...
        public void NotifyForCollateralRevaluation(TBL_COLLATERAL_CUSTOMER collateral, DateTime lastValuationDate, bool saveInternally = false, int? valuationCycle = null)
        {
            ...
            if (property != null)
            {
                ...
                var customer = context.TBL_CUSTOMER.FirstOrDefault(s => s.CUSTOMERID == collateral.CUSTOMERID);
                alertSubject = TranslateHelper.get("NOTIFICATION COLLATERAL VALUATION REMINDER FROM FINTRAK");
                recipients = "John.Adeonojobi@ACCESSBANKPLC.com,Fayokemi.Akintunde@ACCESSBANKPLC.com,OLUKAYODE.AJAYI@ACCESSBANKPLC.com,paul.asiemo@accessbankplc.com";
                messageBody = $"{TranslateHelper.get("Dear")} {staffFullName} <br /><br />," +
                               $"{TranslateHelper.get("This is to inform you that the collateral")}, {collateral?.COLLATERALSUMMARY} {TranslateHelper.get("belonging to")} {customer?.LASTNAME + ", " + customer?.FIRSTNAME + " " + customer?.MIDDLENAME}" +
                               $"(Customer ID: {collateral?.CUSTOMERCODE}) {TranslateHelper.get("with OMV")} {collateral?.TBL_CURRENCY?.CURRENCYCODE} {String.Format("{0:0,0.00}", property?.OPENMARKETVALUE)} {TranslateHelper.get("and FSV of")} {collateral?.TBL_CURRENCY?.CURRENCYCODE} {String.Format("{0:0,0.00}", property?.FORCEDSALEVALUE)} is due for revaluation on {valuationDate.ToShortDateString()} <br /><br />" +
                               $"{TranslateHelper.get("Kindly inform the customer, and initiate request for revaluation on Fintrak")} <br /><br />" +
                               $"Regards";
        ...
        public void NotifyForCollateralStatusUpdate(TBL_COLLATERAL_CUSTOMER collateral, byte? perfectionStatusId, bool saveInternally = false)
        {
            ...
            if (property != null)
            {
                ...
                var customer = context.TBL_CUSTOMER.FirstOrDefault(s => s.CUSTOMERID == collateral.CUSTOMERID);
                alertSubject = TranslateHelper.get("COLLATERAL STATUS UPDATE FROM FINTRAK360 ALERT");
                recipients = "John.Adeonojobi@ACCESSBANKPLC.com,Fayokemi.Akintunde@ACCESSBANKPLC.com,OLUKAYODE.AJAYI@ACCESSBANKPLC.com,paul.asiemo@accessbankplc.com";
                messageBody = $"{TranslateHelper.get("Dear")} {staffFullName}, <br /><br />" +
                               $"{TranslateHelper.get("This is to inform you that")}, <br /><br />" +
                               $"{TranslateHelper.get("The collateral")}, {collateral.COLLATERALSUMMARY} {TranslateHelper.get("of")} {customer?.LASTNAME} {customer?.FIRSTNAME} {customer?.MIDDLENAME} {TranslateHelper.get("with customerId")} {customer?.CUSTOMERCODE} {TranslateHelper.get("of value")} {collateral?.TBL_CURRENCY?.CURRENCYCODE} {String.Format("{0:0,0.00}", collateral?.COLLATERALVALUE)}" +
                               $"{TranslateHelper.get("has it's perfection status updated as")} {perfectionStatus}"
                               ;
                LogEmailAlert(messageBody, alertSubject, recipients, jobReQuestCode, targetId);
            }
            if (saveInternally)
        ...
        public void NotifyForCollateralValidity(TBL_COLLATERAL_CUSTOMER collateral, DateTime? newValidityDate, bool saveInternally = false)
        {
            ...
            var customer = context.TBL_CUSTOMER.FirstOrDefault(s => s.CUSTOMERID == collateral.CUSTOMERID);
            alertSubject = TranslateHelper.get("COLLATERAL VALIDITY UPDATE FROM FINTRAK360 ALERT");
            recipients = "John.Adeonojobi@ACCESSBANKPLC.com,Fayokemi.Akintunde@ACCESSBANKPLC.com,OLUKAYODE.AJAYI@ACCESSBANKPLC.com,paul.asiemo@accessbankplc.com";
            messageBody = $"{TranslateHelper.get("Dear")} {staffFullName}, <br /><br />" +
                           $"{TranslateHelper.get("This is to inform you that")}, <br /><br />" +
                           $"{TranslateHelper.get("The collateral")}, {collateral.COLLATERALSUMMARY} {TranslateHelper.get("of")} {customer?.LASTNAME} {customer?.FIRSTNAME} {customer?.MIDDLENAME} {TranslateHelper.get("with customerId")} {customer?.CUSTOMERCODE} {TranslateHelper.get("of value")} {currency.CURRENCYCODE} {String.Format("{0:0,0.00}", collateral.COLLATERALVALUE)}" +
                           $" {TranslateHelper.get("now has a validity period that lasts till")} {newValidityDate.Value}"
                           ;
            LogEmailAlert(messageBody, alertSubject, recipients, jobReQuestCode, targetId);
        ...
        public async Task<int> GoForApproval(ApprovalViewModel model)
        {
            TwoFactorAutheticationViewModel twoFADetails = new TwoFactorAutheticationViewModel
            {
            ...
            using (var transaction = context.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    responce = await context.SaveChangesAsync();
                    transaction.Commit();
        ...
        public async Task<int> GoForPolicyApproval(ApprovalViewModel model)
        {
            using (var transaction = context.Database.BeginTransaction())
            {
                workflow.StaffId = model.createdBy;
                ...
                try
                {
                    if (workflow.NewState == (int)ApprovalState.Ended)
                    {
                        ...
                        {
                            TBL_TEMP_COLLATERAL_ITEM_POLI data = await context.TBL_TEMP_COLLATERAL_ITEM_POLI.Where(x => x.TEMPPOLICYID == model.targetId).FirstOrDefaultAsync();
                            UpdateItemPolicyApproval(data);
                        }
                    }

                    int responce = await context.SaveChangesAsync();
                    transaction.Commit();
                ...
                catch (Exception ex)
                {
                    ...
                    throw new SecureException(TranslateHelper.get("Error has occured while approving this insurance policy, kindly try again"));
                }
            }
        }

        public async Task<WorkflowResponse> GoForInsurancePolicyApproval(ApprovalViewModel model)
        {
            using (var transaction = context.Database.BeginTransaction())
            {
                bool responce;
                ...
                try
                {
                    if (workflow.NewState == (int)ApprovalState.Ended)
                    {
                        var data = await context.TBL_INSURANCE_REQUEST.Where(ir => ir.INSURANCEREQUESTID == model.targetId).FirstOrDefaultAsync();
                        var data2 = await context.TBL_COLLATERAL_ITEM_POLICY.Where(x => x.COLLATERALCUSTOMERID == data.COLLATERALCUSTOMERID
                                                                                && x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing)
                                                                                .FirstOrDefaultAsync();
                    ...
                    responce = await context.SaveChangesAsync() > 0;
                    transaction.Commit();
                ...
                else
                {
                    ...
                    else if (mainCollateral.COLLATERALTYPEID == (int)CollateralTypeEnum.FixedDeposit)
                    {
                        ...
                        CasaLienViewModel model = new CasaLienViewModel
                        {
                            ...
                            description = TranslateHelper.get("Term deposit collateral creation"),
                            lienTypeId = (int)LienTypeEnum.CollateralCreation,
                            sourceReferenceNumber = mainCollateral.COLLATERALCODE,
                            dateTimeCreated = DateTime.Now,
                            createdBy = ApprovalModel.createdBy,
        ...
        private void UpdatePromissoryCollateral(int tempCollateralId, string collateralcode, int newCollateralId)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Promissory Note Has Been Used Before"));
            }
        ...
        public async Task<CollateralViewModel> GetTempCollateralTypeByCollateralId(int collateralId, int typeId)
        {
            var data = new CollateralViewModel();
            switch (typeId)
            {
                case (int)CollateralTypeEnum.FixedDeposit: data = await GetTempCollateralDeposit(collateralId); break;
                case (int)CollateralTypeEnum.PlantAndMachinery: data = await GetTempCollateralMachinery(collateralId); break;
                case (int)CollateralTypeEnum.Miscellaneous: data = await GetTempCollateralMiscellaneous(collateralId); break;
                case (int)CollateralTypeEnum.Gaurantee: data = await GetTempCollateralGuarantee(collateralId); break;
                case (int)CollateralTypeEnum.CASA: data = await GetTempCollateralCasa(collateralId); break;
                case (int)CollateralTypeEnum.Property: data = await GetTempCollateralImmovableProperty(collateralId); break;
                case (int)CollateralTypeEnum.TreasuryBillsAndBonds: data = await GetTempCollateralMarketableSecurities(collateralId); break;
                case (int)CollateralTypeEnum.InsurancePolicy: data = await GetTempCollateralPolicy(collateralId); break;
                case (int)CollateralTypeEnum.PreciousMetal: data = await GetTempCollateralPreciousMetal(collateralId); break;
                case (int)CollateralTypeEnum.MarketableSecurities_Shares: data = await GetTempCollateralStock(collateralId); break;
                case (int)CollateralTypeEnum.Vehicle: data = await GetTempCollateralVehicle(collateralId); break;
                case (int)CollateralTypeEnum.Promissory: data = await GetTempCollateralPromissory(collateralId); break;
                case (int)CollateralTypeEnum.ISPO: data = await GetTempCollateralISPO(collateralId); break;
                case (int)CollateralTypeEnum.DomiciliationContract: data = await GetTempCollateralDomiciliationContract(collateralId); break;
                case (int)CollateralTypeEnum.DomiciliationSalary: data = await GetTempCollateralDomiciliationSalary(collateralId); break;
                case (int)CollateralTypeEnum.Indemity: data = await GetTempCollateralIndemnity(collateralId); break;
        ...
        private async Task<CollateralViewModel> GetTempCollateralDeposit(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_DEPOSIT.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
        ...
        private async Task<CollateralViewModel> GetTempCollateralMachinery(int collateralId)
        {
            var specifics = await (from x in context.TBL_TEMP_COLLATERAL_PLANT_EQUP
                             where x.TEMPCOLLATERALCUSTOMERID == collateralId
        ...
        private async Task<CollateralViewModel> GetTempCollateralMiscellaneous(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_MISCELLAN.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralGuarantee(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_GAURANTEE.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralCasa(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_CASA.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
                collateralCustomerId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralImmovableProperty(int collateralId)
        {
            var details = await (from x in context.TBL_TEMP_COLLATERAL_IMMOV_PROP
                           where x.TEMPCOLLATERALCUSTOMERID == collateralId
                           select new CollateralViewModel
                           {
                               collateralId = x.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralMarketableSecurities(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_MKT_SEC.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).FirstOrDefaultAsync();
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralPolicy(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_POLICY.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralPreciousMetal(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_PREC_METAL.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralStock(int collateralId)
        {
            var specifics = await (from x in context.TBL_TEMP_COLLATERAL_STOCK
                             where x.TEMPCOLLATERALCUSTOMERID == collateralId
                             select (x)).FirstOrDefaultAsync();
        ...
        private async Task<CollateralViewModel> GetTempCollateralPromissory(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_PROMISSORY.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralISPO(int collateralId)
        {
            var collateral = await context.TBL_TEMP_COLLATERAL_ISPO.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).Select(x => x).FirstOrDefaultAsync();
            var details = new CollateralViewModel
            {
        ...
        private async Task<CollateralViewModel> GetTempCollateralDomiciliationContract(int collateralId)
        {
            var collateral = await context.TBL_TEMP_COLLATERAL_DOMCLTN.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).Select(x => x).FirstOrDefaultAsync();
            var details = new CollateralViewModel
            {
                collateralId = collateral.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralDomiciliationSalary(int collateralId)
        {
            var collateral = await context.TBL_TEMP_COLLATERAL_DOMCLTN.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).Select(x => x).FirstOrDefaultAsync();
            var details = new CollateralViewModel
            {
        ...
        private async Task<CollateralViewModel> GetTempCollateralIndemnity(int collateralId)
        {
            var collateral = await context.TBL_TEMP_COLLATERAL_INDEMNITY.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).Select(x => x).FirstOrDefaultAsync();
            var details = new CollateralViewModel
            {
                collateralId = collateral.TEMPCOLLATERALCUSTOMERID,
        ...
        private async Task<CollateralViewModel> GetTempCollateralVehicle(int collateralId)
        {
            var specifics = await context.TBL_TEMP_COLLATERAL_VEHICLE.FirstOrDefaultAsync(x => x.TEMPCOLLATERALCUSTOMERID == collateralId);
            var details = new CollateralViewModel
            {
                collateralId = specifics.TEMPCOLLATERALCUSTOMERID,
        ...
        public async Task<List<InsurancePolicy>> GetTempCollateralInsurancePolicy(int collateralId)
        {
            var insurance = await (context.TBL_TEMP_COLLATERAL_ITEM_POLI.Where(x => x.COLLATERALCUSTOMERID == collateralId)
        ...
        public async Task<List<InsurancePolicy>> GetCollateralInsurancePolicy(int collateralId)
        {
            
                var insurance = await (context.TBL_COLLATERAL_INSURANCE_TRACKING.Where(x => x.COLLATERALCUSTOMERID == collateralId && x.DELETED == false)
                    .Select(x => new InsurancePolicy
                    {
        ...
        public string GetInsurancePolicyCollateralReport(int trackingId)
        {
            ...
            result = result + $@"
                <table style='font face: arial; size:12px' border=1 width=900 cellpadding=0 cellspacing=0>
                    ...
                    <tr>
                        <td><b>{TranslateHelper.get("INSURANCE POLICY REPORT")}</b></td>
            ...
            result = result + $@"
                <table style='font face: arial; size:12px' border=1 width=900 cellpadding=0 cellspacing=0>
                    ...
                    <tr>
                        <th><b>{TranslateHelper.get("ACCOUNT OFFICER NAME")}:</b></th>
                        <th>{staff?.FIRSTNAME} {staff?.MIDDLENAME} {staff?.LASTNAME}</th>
                        <th><b>{TranslateHelper.get("ACCOUNT OFFICER EMAIL")}:</b></th>
                        <th>{staff?.EMAIL}</th>
                    </tr>
                     <tr>
                        <th><b>{TranslateHelper.get("TEAM")}:</b></th>
                        <th>{teamName}</th>
                        <th><b>{TranslateHelper.get("GROUP HEAD")}:</b></th>
                        <th>{gh?.FIRSTNAME} {gh?.MIDDLENAME} {gh?.LASTNAME}</th>
                    </tr>
                      
                    <tr>
                        <td>{TranslateHelper.get("DIVISION")}:</td>
                        <td>{divisionName}</td>
                        <td>{TranslateHelper.get("CUSTOMER NAME")}:</td>
                        <td>{customer?.FIRSTNAME} {customer?.MIDDLENAME} {customer?.LASTNAME}</td>
                    </tr>
                    <tr>
                        <th><b>{TranslateHelper.get("CUSTOMER EMAIL")}:</b></th>
                        <th>{customer?.EMAILADDRESS} </th>
                        <th><b>{TranslateHelper.get("CUSTOMER PHONE NUMBER")}:</b></th>
                        <th>{customer?.PHONENUMBEROFSIGNATORY}</th>
                    </tr>
                    <tr>
                        <th><b>{TranslateHelper.get("CUSTOMER ADDRESS")}:</b></th>
                        <th>{customerAddress} </th>
                        <th><b></b></th>
                        <th></th>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("ACCOUNT NUMBER")}:</td>
                        <td>{customerAccount}</td>
                        <td>{TranslateHelper.get("CUSTOMER ID")}:</td>
                        <td>{customer?.CUSTOMERCODE}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("COLLATERAL DETAILS")}:</td>
                        <td>{insurance?.COLLATERALDETAILS}</td>
                        <td>{TranslateHelper.get("SUM INSURED")}</td>
                        <td>{string.Format("{0:#,##.00}", Convert.ToDecimal(insurance.SUMINSURED))}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("PREMIUM PAID")}:</td>
                        <td>{string.Format("{0:#,##.00}", Convert.ToDecimal(insurance.PREMIUMPAID))}</td>
                        <td></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("INSURANCE START DATE")}:</td>
                        <td>{insurance?.INSURANCESTARTDATE.Value.ToString("dd-MM-yyyy")}</td>
                        <td>{TranslateHelper.get("INSURANCE END DATE")}:</td>
                        <td>{insurance?.INSURANCEENDDATE.Value.ToString("dd-MM-yyyy")}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("INSURANCE POLICY TYPE")}:</td>
                        <td>{insurancePolicyType}</td>
                        <td>{TranslateHelper.get("VALUATION START DATE")}:</td>
                        <td>{insurance?.VALUATIONSTARTDATE.Value.ToString("dd-MM-yyyy")}</td>
                        </tr>
                    <tr>
                        <td>{TranslateHelper.get("VALUATION END DATE")}:</td>
                        <td>{insurance?.VALUATIONENDDATE.Value.ToString("dd-MM-yyyy")}</td>
                        <td>{TranslateHelper.get("VALUATION OPEN MARKET VALUE")}:</td>
                        <td>{insurance?.OMV}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("VALUATION OPEN MARKET VALUE")}:</td>
                        <td>{insurance?.FSV}</td>
                        <td>{TranslateHelper.get("VALUER NAME")}:</td>
                        <td>{valuer}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("LOAN AMOUNT")}:</td>
                        <td>{loanApplicationDetail?.APPROVEDAMOUNT}</td>
                        <td>{TranslateHelper.get("LOAN TYPE")}:</td>
                        <td>{loanTypeName}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("INSURANCE STATUS")}:</td>
                        <td>{insuranceStatus}</td>
                        <td>{TranslateHelper.get("POLICY NUMBER")}:</td>
                        <td>{insurance?.POLICYNUMBER}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("INSURANCE COMPANY")}:</td>
                        <td>{insuranceCompany?.COMPANYNAME}</td>
                        <td>{TranslateHelper.get("LOAN AMOUNT")}:</td>
                        <td>{loanAmount}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("LOAN STATUS")}:</td>
                        <td>{loanStatus}</td>
                        <td>{TranslateHelper.get("SECURITY RELEASE STATUS")}:</td>
                        <td>{securityReleaseStatus}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("TAX IDENTIFICATION NUMBER")}:</td>
                        <td>{taxNumber}</td>
                        <td>{TranslateHelper.get("RC NUMBER")}:</td>
                        <td>{rcNumber}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("FIRST LOSS PAYEE")}:</td>
                        <td>{firstLossPayee}</td>
                        <td>{TranslateHelper.get("INSURABLE VALUE")}:</td>
                        <td>{string.Format("{0:#,##.00}", Convert.ToDecimal(insurance.INSURABLEVALUE))}</td>
                    </tr>
                    <tr>
                        <td>{TranslateHelper.get("COMMENT")}:</td>
        ...
        public async Task<List<InsurancePolicy>> GetCollateralInsurancePolicyReport(DateTime? startDate, DateTime? endDate, string searchString)
        {
                ...
                {
                    insurance = await (context.TBL_COLLATERAL_INSURANCE_TRACKING.Where(x => DbFunctions.TruncateTime(x.INSURANCESTARTDATE).Value >= DbFunctions.TruncateTime(startDate).Value && DbFunctions.TruncateTime(x.INSURANCEENDDATE).Value <= DbFunctions.TruncateTime(endDate).Value && x.DELETED == false)
                       .Select(x => new InsurancePolicy
                       {
                       ...
                       })).OrderBy(x => x.collateralInsuranceTrackingId).ToListAsync();
        ...
        public async Task<CasaLienViewModel> GetAccountLienDetail(string AccountNumber)
        {
            return await (context.TBL_CASA_LIEN.Where(x => x.PRODUCTACCOUNTNUMBER == AccountNumber)
        ...
        public async Task<IEnumerable<CollateralViewModel>> GetCollateralStampToCoverValues(int customerId)
        {
            var collaterals = await (from x in context.TBL_COLLATERAL_CUSTOMER
                               join c in context.TBL_COLLATERAL_TYPE on x.COLLATERALTYPEID equals c.COLLATERALTYPEID
                               join a in context.TBL_CUSTOMER on x.CUSTOMERID equals a.CUSTOMERID
                               let ColSubType = context.TBL_COLLATERAL_TYPE_SUB.Where(c => c.COLLATERALSUBTYPEID == x.COLLATERALSUBTYPEID).Select(c => c.COLLATERALSUBTYPENAME).FirstOrDefault()
        ...
        public async Task<bool> RejectProposedCollateralForUsage(int collateralCustomerId)
        {
            var collateral = await context.TBL_COLLATERAL_CUSTOMER.Where(x => x.COLLATERALCUSTOMERID == collateralCustomerId).Select(x => x).FirstOrDefaultAsync();
            if (collateral != null)
        ...
        public async Task<bool> ProposeCollateralForUsage(CollateralCoverageViewModel model)
        {
            ...
            var facility = await context.TBL_LOAN_APPLICATION_DETAIL.Where(o => o.LOANAPPLICATIONDETAILID == model.loanApplicationDetailId).Select(o => o).FirstOrDefaultAsync();
            ...
            var collateral = await context.TBL_COLLATERAL_CUSTOMER.Where(o => o.COLLATERALCUSTOMERID == model.collateralId).Select(o => o).FirstOrDefaultAsync();

            var collateralCoverage = await context.TBL_COLLATERAL_COVERAGE.Where(o => o.COLLATERALSUBTYPEID == collateral.COLLATERALSUBTYPEID && o.DELETED == false && o.CURRENCYID == facilityCurrencyId).Select(o => o.COVERAGE).FirstOrDefaultAsync();
            ...
            else
            {
                ...
                var collateralMappedToFacilities = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(o => o.COLLATERALCUSTOMERID == model.collateralId && o.DELETED == false).Select(o => o).ToListAsync();
                // o.COLLATERALCUSTOMERID != model.collateralId
                var facilityMappedToCollaterals = await context.TBL_LOAN_APPLICATION_COLLATERL.Where(o => o.LOANAPPLICATIONDETAILID == model.loanApplicationDetailId && o.DELETED == false).Select(o => o).ToListAsync();
                ...
                if (availableCollateralValues > 0)
                {
                    ...
                    throw new Exception(TranslateHelper.get("It's fully in use"));
            }
            return false;
        }

        public async Task<bool> ProposeCollateralForUsageLMS(CollateralCoverageViewModel model)
        {
        ...
        {
            var recordExist = context.TBL_COLLATERAL_COVERAGE.Where(x =>
            x.COLLATERALSUBTYPEID == model.collateralSubTypeId && x.COVERAGE == model.coverage && x.CURRENCYID == model.currencyId).Any();
        ...
        public async Task<IEnumerable<CollateralCoverageViewModel>> GetCollateralCoverage(int collateralSubTypeId)
        {
            return await (context.TBL_COLLATERAL_COVERAGE.Where(o => o.COLLATERALSUBTYPEID == collateralSubTypeId && o.DELETED == false).Select(o => new CollateralCoverageViewModel
            {
        ...
        public async Task<bool> DeleteCollateralCoverage(int collateralCoverageId, int createdById)
        {
            var data = await context.TBL_COLLATERAL_COVERAGE.Where(o => o.COLLATERALCOVERAGEID == collateralCoverageId).Select(o => o).FirstOrDefaultAsync();
        ...
        public async Task<bool> DeleteAddedValuer(int valuerId, int createdById)
        {
            var data = await context.TBL_VALUATION_REPORT.FindAsync(valuerId);
        ...
        public async Task<bool> DeleteInsuranceRequest(int insuranceRequestId)
        {
            var entity = await context.TBL_INSURANCE_REQUEST.FirstOrDefaultAsync(ir => ir.INSURANCEREQUESTID == insuranceRequestId);
        ...
        public async Task<IEnumerable<CollateralCoverageViewModel>> CalculateCoverateOfCollateral(CollateralCoverageViewModel model)
        {
            var list = new List<CollateralCoverageViewModel>();
            ...
            var facility = await context.TBL_LOAN_APPLICATION_DETAIL.FindAsync(model.loanApplicationDetailId);
            ...
            if (data == null)
                throw new Exception(TranslateHelper.get("Collateral Coverage has not been set!"));
        ...
        public async Task<IEnumerable<CollateralCoverageViewModel>> CalculateCoverateOfCollateralLMS(CollateralCoverageViewModel model)
        {
        ...
        public async Task<InsuranceCompanyViewModel> GetInsuranceCompany(int id)
        {
            var entity = await context.TBL_INSURANCE_COMPANY.FirstOrDefaultAsync(x => x.INSURANCECOMPANYID == id && x.DELETED == false);
        ...
        public async Task<IEnumerable<InsuranceCompanyViewModel>> GetInsuranceCompanies()
        {
            return await context.TBL_INSURANCE_COMPANY.Where(x => x.DELETED == false)
        ...
        public async Task<bool> DeleteInsuranceCompany(int id, UserInfo user)
        {
            var entity = await this.context.TBL_INSURANCE_COMPANY.FindAsync(id);
            entity.DELETED = true;
        ...
        public async Task<bool> UpdateInsuranceCompany(InsuranceCompanyViewModel model, int id, UserInfo user)
        {
            var entity = await this.context.TBL_INSURANCE_COMPANY.FindAsync(id);
        ...
        public async Task<CollateralInsurancePolicyViewModel> GetAddedInsuranceById(int id)
        {
            var insurance = await context.TBL_COLLATERAL_ITEM_POLICY.Where(x => x.COLLATERALCUSTOMERID == id
                                                                     && x.APPROVALSTATUSID == (short)ApprovalStatusEnum.Processing).FirstOrDefaultAsync();
        ...
        public async Task<InsuranceTypeViewModel> GetInsuranceType(int id)
        {
            var entity = await context.TBL_INSURANCE_TYPE.FirstOrDefaultAsync(x => x.INSURANCETYPEID == id && x.DELETED == false);
        ...
        public async Task<IEnumerable<InsuranceTypeViewModel>> GetInsuranceTypes()
        {
            var data = await context.TBL_INSURANCE_TYPE.Where(x => x.DELETED == false)
        ...
        public async Task<IEnumerable<CollateralTypeViewModel>> GetCollateralTypes()
        {
            var data = await context.TBL_COLLATERAL_TYPE.Where(x => x.DELETED == false)
        ...
        public async Task<IEnumerable<CollateralSubTypeViewModel>> GetCollateralSubTypes(int collateralTypeId)
        {
            var data = await context.TBL_COLLATERAL_TYPE_SUB.Where(x => x.DELETED == false && x.COLLATERALTYPEID == collateralTypeId)
        ...
        public async Task<IEnumerable<InsuranceStatusViewModel>> GetInsuranceStatus()
        {
            var data = await context.TBL_COLLATERAL_INSURANCE_STATUS.Where(x => x.DELETED == false)
        ...
        public async Task<IEnumerable<InsurancePolicyTypeViewModel>> GetInsurancePolicyTypes()
        {
            return await context.TBL_INSURANCE_POLICY_TYPE.Where(x => x.DELETED == false)
        ...
        public async Task<bool> DeleteInsuranceType(int id, UserInfo user)
        {
            var entity = await this.context.TBL_INSURANCE_TYPE.FindAsync(id);
            entity.DELETED = true;
            entity.DELETEDBY = user.createdBy;
            entity.DATETIMEDELETED = genSetup.GetApplicationDate();
            return context.SaveChanges() != 0;
        ...
        public async Task<bool> DeleteInsurancePolicyType(int id, UserInfo user)
        {
            var entity = await this.context.TBL_INSURANCE_POLICY_TYPE.FindAsync(id);
            entity.DELETED = true;
            entity.DELETEDBY = user.createdBy;
            entity.DATETIMEDELETED = genSetup.GetApplicationDate();
            return context.SaveChanges() != 0;
        ...
        public async Task<bool> UpdateInsuranceType(InsuranceTypeViewModel model, int id, UserInfo user)
        {
            var entity = await this.context.TBL_INSURANCE_TYPE.FindAsync(id);
        ...
        public async Task<bool> UpdateInsurancePolicyType(InsurancePolicyTypeViewModel model, int id, UserInfo user)
        {
            if (model.valuation == "0")
            {
                model.valuationRequired = false;
        ...
        public bool AddInsurancePolicyFile(InsurancePolicy model)
        {
            ...
            this.auditTrail.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"Insurance policy type '{entity.ToString()}' created by {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteInsurancePolicy(int id, UserInfo user)
        {
            var entity = await this.context.TBL_COLLATERAL_ITEM_POLICY.FindAsync(id);
            entity.DELETED = true;
            entity.DELETEDBY = user.createdBy;
        ...
        public async Task<IEnumerable<CollateralSwapViewModel>> GetAllCollateralSwaps(int staffId)
        {
            ...
            var swapsInProgress = await (from s in context.TBL_COLLATERAL_SWAP_REQUEST
                                   ...
                                   select new CollateralSwapViewModel
                                   {
                                       ...
                                       .Where(l => (l.approvalStatusId == (int)ApprovalStatusEnum.Disapproved)
                                                ...
                                                && staffs.Contains(l.loopedStaffId ?? 0))).ToListAsync();

            var swapsNotStarted = await (from s in context.TBL_COLLATERAL_SWAP_REQUEST
                                   join c in context.TBL_CUSTOMER on s.TBL_LOAN_APPLICATION_COLLATERL.CUSTOMERID equals c.CUSTOMERID
                                   join d in context.TBL_LOAN_APPLICATION_DETAIL on s.TBL_LOAN_APPLICATION_COLLATERL.LOANAPPLICATIONDETAILID equals d.LOANAPPLICATIONDETAILID
        ...
        public async Task<IEnumerable<CollateralSwapViewModel>> GetCollateralSwapsForApproval(int staffId)
        {
            ...
            var collateralSwapsForApproval = await (from s in context.TBL_COLLATERAL_SWAP_REQUEST
                                              join c in context.TBL_CUSTOMER on s.TBL_LOAN_APPLICATION_COLLATERL.CUSTOMERID equals c.CUSTOMERID
                                              join t in context.TBL_APPROVAL_TRAIL on s.COLLATERALSWAPID equals t.TARGETID
                                              join d in context.TBL_LOAN_APPLICATION_DETAIL on s.TBL_LOAN_APPLICATION_COLLATERL.LOANAPPLICATIONDETAILID equals d.LOANAPPLICATIONDETAILID
                                              ...
                                              select new CollateralSwapViewModel
                                              {
                                                ...
                                                .Select(g => g.OrderByDescending(b => b.approvalTrailId).FirstOrDefault()).ToListAsync();
        ...
        public async Task<CollateralSwapViewModel> GetCollateralSwap(int collateralSwapId)
        {
            var swap = await (from s in context.TBL_COLLATERAL_SWAP_REQUEST
        ...
        public async Task<IEnumerable<CollateralSwapViewModel>> SearchCollateralSwap(string searchString)
        {
        ...
        public String ResponseMessage(WorkflowResponse response, string itemHeading)
        {
            if (response.stateId != (int)ApprovalState.Ended)
            {
                if (response.statusId == (int)ApprovalStatusEnum.Referred)
                {
                    ...
                    {
                        return $"{itemHeading}: {TranslateHelper.get("The request has been REFERRED to")} {response.nextPersonName}";
                    }
                    else
                    {
                        return $"{itemHeading}: {TranslateHelper.get("The request has been REFERRED to")} {response.nextLevelName}";
                    }
                }
                else
                {
                    if (response.nextPersonId > 0)
                    {
                        return $"{itemHeading}: {TranslateHelper.get("The request has been SENT to")} {response.nextPersonName}";
                    }
                    else
                    {
                        return $"{itemHeading}: {TranslateHelper.get("The request has been SENT to")} {response.nextLevelName}";
                    }
                }
            }
            else
            {
                ...
                {
                    return $"{itemHeading}: {TranslateHelper.get("The request has been APPROVED successfully")}";
                }
                else
                {
                    return $"{itemHeading}: {TranslateHelper.get("The request has been DISAPPROVED successfully")}";
                }
            }
        ...
        public WorkflowResponse CollateralSwapMemorandum(CollateralSwapViewModel model)
        {
            ...
            placeholders.customerName = $"<br />{TranslateHelper.get("CUSTOMER NAME")}: " + c?.FIRSTNAME + " " + c?.MIDDLENAME + " " + c?.LASTNAME;
            placeholders.referenceNumber = $"<br />{TranslateHelper.get("COLLATERAL SWAP REFERENCENUMBER")}: " + cs?.SWAPREF;
            placeholders.facilityType = $"<br />{TranslateHelper.get("COLLATERAL SWAP")}";
            placeholders.operationName = $"<br />{TranslateHelper.get("OPERATION NAME: COLLATERAL SWAP")}";
            placeholders.branchName = $"<br />{TranslateHelper.get("BRANCH NAME")}: " + c?.TBL_BRANCH.BRANCHNAME;
        ...
        public async Task<int> GetNextLevelForCollateralSwapAsync(int collateralSwapId, int createdBy, int companyId)
        {
            var approvalModel = new ForwardViewModel
            {
                ...
                comment = TranslateHelper.get("Get next level for collateral swap"),
                operationId = (short)OperationsEnum.CollateralSwap,
                forwardAction = (int)ApprovalStatusEnum.Pending,
            };

            var response = await drawdownRepo.LogApprovalForMessage(approvalModel, true);
        ...
        public async Task<bool> UpdateCollateralSwap(CollateralSwapViewModel model, int id, UserInfo user)
        {
            var swap = await context.TBL_COLLATERAL_SWAP_REQUEST.FindAsync(id);
        ...
        public async Task<bool> DeleteCollateralSwap(int collateralSwapId, UserInfo user)
        {
            var swap = await context.TBL_COLLATERAL_SWAP_REQUEST.FindAsync(collateralSwapId);
            if (swap == null) return false;
            swap.DELETED = true;
        ...
        public async Task<IEnumerable<LoanApplicationDetailViewModel>> GetCollateralMappingDetails(int loanAppCollateralId)
        {
            var data = await (from m in context.TBL_LOAN_APPLICATION_COLLATERL
