...
namespace FintrakBanking.Repositories.Credit
{
    public class CollateralDocumentRepository : ICollateralDocumentRepository
    {
        ...
        public async Task<bool> AddCollateralDocument(CollateralDocumentViewModel model, byte[] file)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Collateral Document")} '{ model.documentTitle }' ",
        ...
        public async Task<bool> AddTempCollateralDocument(CollateralDocumentViewModel model, byte[] file)
        {
        ...
        public async Task<bool> UpdateCollateralDocument(CollateralDocumentViewModel model, int documentId)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Collateral Document")} '{ model.documentTitle }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetAllCollateralDocument()
        {
            return await this.context.TBL_MEDIA_COLLATERAL_DOCUMENTS.Select(x => new CollateralDocumentViewModel
            {
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetTempAllCollateralDocument(int collateralId)
        {
            string docName = "";
            var docMedia = await this.context.TBL_TEMP_MEDIA_COLLATERAL_DOCS.Where(x => x.TEMPCOLLATERALCUSTOMERID == collateralId).Select(x => new CollateralDocumentViewModel
            {
        ...
        public async Task<CollateralDocumentViewModel> GetCollateralDocument(int documentId)
        {
            var data =await this.context.TBL_MEDIA_COLLATERAL_DOCUMENTS.FindAsync(documentId);
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetCustomerCollateralReleaseDocument(int collateralId)
        {
            List<int> record = new List<int>();
            var data2 =await (from a in bankingContext.TBL_COLLATERAL_RELEASE
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetCustomerCollateralDocument(int collateralId)
        {
           var inVaultNAme=  bankingContext.TBL_COLLATERAL_RELEASE_STATUS.Find((int)CollateralReleaseStatus.InVault).COLLATERALRELEASESTATUSNAME;
            ...
            var data =await (from x in context.TBL_DOCUMENT_USAGE 
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetTempCustomerCollateralDocument(int tempCollateralId)
        {
        ...
        public async Task<IEnumerable<CollateralDocumentViewModel>> GetCollateralGuaranteeDocument(int targetId)
        {
            var collateralDocuments = await this.GetAllCollateralDocument();
            return collateralDocuments.Where(x => x.targetId == targetId).ToList();
        }

        public async Task<CollateralVisitationDocumentViewModel> GetCollateralVisitationDocument(int collateralVisitationId)
        {
        ...
        public async Task<CollateralVisitationDocumentViewModel> GetTempCollateralVisitationDocument(int collateralVisitationId)
        {
            var data = (from x in this.context.TBL_DOC_COLLATERAL_VISITATION
                        where x.COLLATERALVISITATIONID == collateralVisitationId
        ...
        public async Task<bool> AddCollateralVisitation(CollateralDocumentViewModel model, byte[] file)
        {
          var visitationId =  coll.AddPropertyVistation(model);
           ...
           var audit = new TBL_AUDIT
           {
               ...
               STAFFID = model.createdBy,
               BRANCHID = (short)model.userBranchId,
               DETAIL = $"{TranslateHelper.get("Added Collateral Visitation File")} '{ model.documentTitle }' ",
               IPADDRESS = CommonHelpers.GetLocalIpAddress(),
               URL =model.applicationUrl,
               APPLICATIONDATE = general.GetApplicationDate(),
               SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddTempCollateralVisitation(CollateralDocumentViewModel model, byte[] file)
        {
          var visitationId =  coll.AddPropertyVistation(model);
            if (visitationId > 0)
            {
