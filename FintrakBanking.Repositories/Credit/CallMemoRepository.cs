...
namespace FintrakBanking.Repositories.Credit
{
    public class CallMemoRepository : ICallMemoRepository
    {
        ...
        public async Task<IQueryable<CallMemoLoanSearchViewModel>> SearchForCallMemoLoan(int staffId, string searchQuery)
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchQuery.Trim()))
            {
                ...
                var JobRole = await (from a in _context.TBL_STAFF where a.STAFFID == staffId select a.STAFFROLEID).FirstOrDefaultAsync();

                if (JobRole > 0)
                {
                    var memoLimit = await (from b in _context.TBL_CALL_MEMO_LIMIT where b.JOBTITLEID == JobRole select b).FirstOrDefaultAsync();
                    if (memoLimit != null)
                    {
                    ...
                    {
                        var jobName = await _context.TBL_STAFF_JOBTITLE.Where(x => x.JOBTITLEID == JobRole).FirstOrDefaultAsync();
                        throw new ConditionNotMetException($"{TranslateHelper.get("Kindly Setup Call Memo Limit For Job Title")} : {jobName.JOBTITLENAME}  {TranslateHelper.get("for This Staff")}");
                    }
                }
            }
        ...
        public async Task<IEnumerable<CallMemoTypeViewModel>> GetCallLimitType()
        {
            var data = await (from a in _context.TBL_CALL_MEMO_TYPE
                        orderby a.NAME
        ...
        public async Task<IEnumerable<CallLimitViewModel>> GetAllCallLimit(int companyId)
        {
            var data = await (from a in _context.TBL_CALL_MEMO_LIMIT
        ...
        public async Task<List<CallLimitViewModel>> GetCallLimitByTypeId(int limitId)
        {
            var data = await (from a in _context.TBL_CALL_MEMO_LIMIT
        ...
        public async Task<bool> AddCallLimit(CallLimitViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added tbl_Call_Limit")} '{ data.CALLLIMITID }' ",        
        ...
        public async Task<bool> UpdateCallLimit(int limitId, CallLimitViewModel model)
        {
            try { 
            var data = _context.TBL_CALL_MEMO_LIMIT.Where(c => c.CALLLIMITID == limitId).FirstOrDefault();
        ...
        public async Task<bool> DeleteCallLimit(int limitId, UserInfo user)
        {
        ...
        #region "Call Memo"
        public async Task<IEnumerable<CallMemoViewModel>> GetCustomerCallMemo(int staffId, int customerId)
        {
            var data = await (from a in _context.TBL_CALL_MEMO
        ...
        public async Task<IEnumerable<ApprovalTrailCallMemoViewModel>> GetAppraisalMemorandumTrailCallMemo(int operationId)
        {
            var data = await (from a in _context.TBL_APPROVAL_GROUP_MAPPING
        ...
        public async Task<IEnumerable<CallMemoViewModel>> GetCustomerApprovedCallMemo(int staffId, int customerId)
        {
            var data = await (from a in _context.TBL_CALL_MEMO
                        join c in _context.TBL_CUSTOMER on a.CUSTOMERID equals c.CUSTOMERID
        ...
        public async Task<IEnumerable<CallMemoViewModel>> SearchCallMemo(int staffId, CallMemoViewModel model)
        {
            var staffs = from s in _context.TBL_STAFF select s;
            var initiator =await  _context.TBL_APPROVAL_TRAIL.Where(o => o.OPERATIONID == (int)OperationsEnum.CallMemo).OrderBy(o => o.APPROVALTRAILID).Select(o => o.REQUESTSTAFFID).FirstOrDefaultAsync();
            //&& a.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved
            var customer = model.customerName;
            var firstQuery = await (from a in _context.TBL_CALL_MEMO
                        join c in _context.TBL_CUSTOMER on a.CUSTOMERID equals c.CUSTOMERID
            ...
            var secondQuery = await (from x in _context.TBL_CALL_MEMO
        ...
        public async Task<IEnumerable<CallMemoViewModel>> GetAllCallMemo(int staffId)
        {
            var staffs = from s in _context.TBL_STAFF select s;
            var initiator = await _context.TBL_APPROVAL_TRAIL.Where(o => o.OPERATIONID == (int)OperationsEnum.CallMemo).OrderBy(o => o.APPROVALTRAILID).Select(o => o.REQUESTSTAFFID).FirstOrDefaultAsync();
                ...
                var firstQuery = await (from a in _context.TBL_CALL_MEMO
                                  ...
                                  select new CallMemoViewModel
                                  {
                                      ...
                                      operationId = (int)OperationsEnum.CallMemo
                                  }).OrderByDescending(a => a.callMemoId).ToListAsync();
        ...
        public async Task<int> AddCallMemo(CallMemoViewModel model)
        {
        ...
        public async Task<CallMemoViewModel> GetCallMemoById(int callMemoID)
        {
            var data = await (from a in _context.TBL_CALL_MEMO
        ...
        public async Task<bool> UpdateCallMemo(int limitId, CallMemoViewModel model)
        {
        ...
        public async Task<bool> GoForCallMemoApproval(CallMemoViewModel entity)
        {
            var callMemos = await _context.TBL_CALL_MEMO.Where(O => O.CALLMEMOID == entity.callMemoId 
                                                         && O.APPROVALSTATUSID == (int) ApprovalStatusEnum.Pending).Select(O => O).ToListAsync();
            ...
            try
            {
                ...
                if (callMemos != null)
                {
                    ...
                    _workflow.Comment = TranslateHelper.get("Request for call memo approval");
                    ...
                    if (_workflow.NewState == (int)ApprovalState.Ended) 
                    {
                        if (_workflow.StatusId == (int)ApprovalStatusEnum.Approved)
                        {
                            ...
                            var accountOfficer = await _context.TBL_STAFF.Where(s => s.STAFFID == memo.CREATEDBY).FirstOrDefaultAsync();
                            var emailList = await GetBusinessUsersEmails(accountOfficer.MISCODE) +";"+memo.CC;
        ...
        private async Task<string> GetBusinessUsersEmails(string accountOfficerMIsCode)
        {
            string emailList = "";
            ...
            if (accountOfficer != null)
            {
                ...
                {
                    var relationshipManager = await _context.TBL_STAFF.Where(x => x.STAFFID == accountOfficer.SUPERVISOR_STAFFID).FirstOrDefaultAsync();
                    if (relationshipManager != null)
                    {
                        emailList = emailList + ";" + relationshipManager.EMAIL;
                        if (relationshipManager.SUPERVISOR_STAFFID != null)
                        {
                            var zonalHead = await _context.TBL_STAFF.Where(x => x.STAFFID == relationshipManager.SUPERVISOR_STAFFID).FirstOrDefaultAsync();
                            if (zonalHead != null)
                            {
                                emailList = emailList + ";" + zonalHead.EMAIL;

                                var groupHead = await _context.TBL_STAFF.Where(x => x.STAFFID == zonalHead.SUPERVISOR_STAFFID).FirstOrDefaultAsync();
        ...
        public async Task<bool> SubmitApproval(CallMemoViewModel model)
        {
            bool response = false;
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                try
                {
                    if (_workflow.NewState == (int) ApprovalState.Ended)
                    {
                        var callMemos = await _context.TBL_CALL_MEMO.Where(O => O.CALLMEMOID == model.callMemoId 
                                                && O.APPROVALSTATUSID == (int) ApprovalStatusEnum.Processing).Select(O => O).ToListAsync();
                    ...
                    response = await _context.SaveChangesAsync() > 0;
                    transaction.Commit();
                    return response;
                }
                catch (Exception ex)
        ...
        public async Task<IEnumerable<CallMemoViewModel>> GetCallMemoWaitingForApproval(int staffId)
        {
