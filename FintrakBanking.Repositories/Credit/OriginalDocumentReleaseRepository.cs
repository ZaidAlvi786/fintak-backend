    ...
    public class OriginalDocumentReleaseRepository : IOriginalDocumentReleaseRepository
    {
        ...
        public bool AddOriginalDocumentRelease(IEnumerable<OriginalDocumentReleaseViewModel> model)
        {
            ...
            foreach (var mod in model)
            {
                ...
                if(docCheck) throw new SecureException(TranslateHelper.get("Collateral Document(s) currently undergoing Approval"));
                ...
                var result = _context.TBL_ORIGINAL_DOCUMENT_RELEASE.Where(x => x.DOCUMENTUPLOADID == mod.documentUploadId
                    ...
                    if (result == true) throw new SecureException(TranslateHelper.get("One of the Selected Documents is currently Undergoing Approval"));
                ...
                if (inUseCollateral.Any()) throw new SecureException(TranslateHelper.get("Cannot release document for a Collateral with existing Exposure(s)"));
        ...
        public bool AddOriginalDocumentGuaranteeRelease(IEnumerable<OriginalDocumentReleaseViewModel> model)
        {
            ...
            foreach (var mod in model)
            {
                ...
                if (docCheck) throw new SecureException(TranslateHelper.get("Collateral Documents is currently undergoing Approval"));
        ...
        public async Task<IEnumerable<OriginalDocumentReleaseViewModel>> GetLeaseDocumentForApproval(int staffId)
        {
            ...
            var record =  await (from dr in _context.TBL_ORIGINAL_DOCUMENT_RELEASE
        ...
        public async Task<IEnumerable<CollateralCashReleaseViewModel>> GetCashSecurityReleaseForApproval(int staffId)
        {
            var ids = _general.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.GuaranteeReleaseApproval).ToList();
            var staffs = _general.GetStaffRlieved(staffId);

            var record = await (from dr in _context.TBL_CASH_SECURITY_RELEASE_APPROVAL
                         ...
                         where
                          (atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing
                            ...
                            || atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Authorised
                            || atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Referred)
                            && (atrail.OPERATIONID == (int)OperationsEnum.GuaranteeReleaseApproval || atrail.OPERATIONID == (int)OperationsEnum.CashSecurityRelease)
                            && ids.Contains((int)atrail.TOAPPROVALLEVELID)
                            && atrail.LOOPEDSTAFFID == null
                         ...
                         select new CollateralCashReleaseViewModel
                         {
                             ...
                             facility = (from p in _context.TBL_PRODUCT join a in _context.TBL_LOAN_APPLICATION_DETAIL on p.PRODUCTID equals a.APPROVEDPRODUCTID where a.LOANAPPLICATIONDETAILID == dr.LOANAPPLICATIONDETAILID select p.PRODUCTNAME).DefaultIfEmpty("N/A").FirstOrDefault(),
            ...
            var result = record.GroupBy(r => r.targetId)
                               .Select(p => p.OrderByDescending(r => r.approvalTrailId).FirstOrDefault());
            return result;
        }


        public async Task<IEnumerable<OriginalDocumentReleaseViewModel>> GetSecurityReleaseSearch(string  searchString)
        {
        ...
        public async Task<IEnumerable<CollateralCashReleaseViewModel>> GetCashSecurityReleaseSearch(string searchString)
        {
            searchString = searchString.Trim().ToLower();
        ...
        public async Task<IEnumerable<OriginalDocumentReleaseViewModel>> GetRejectedAndReferredSecurityRelease(int staffId)
        {
            //var ids = _general.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.SecurityRelease).ToList();
            ...
            var record = await (from oda in _context.TBL_ORIGINAL_DOCUMENT_APPROVAL
        ...
        public async Task<IEnumerable<CollateralCashReleaseViewModel>> GetRejectedAndReferredCashSecurityRelease(int staffId)
        {
        ...
        public bool reinitiateSecurityRelease(int id, int staffId, int companyId)
        {
            ...
            if(rejected != null)
            {
                ...
                if (result)
                {
                    ...
                    _workflow.Comment = TranslateHelper.get("Request for security release approval");
            ...
            if (recordReferred != null)
            {
                ...
                foreach (var x in recordReferred)
                {
                    using (var transaction = _context.Database.BeginTransaction())
                    {
                        ...
                        _workflow.Comment = TranslateHelper.get("Update has been applied, Request for Security Release Approval");
                        _workflow.OperationId = (int)OperationsEnum.SecurityRelease;
                        _workflow.DeferredExecution = true;
                        _workflow.LogActivity();
                        try
        ...
        public async Task<WorkflowResponse> GoForApproval(OriginalDocumentReleaseViewModel entity)
        {
            //var record = entity.GroupBy(x => x.originalDocumentApprovalId).Select(x => x.FirstOrDefault()).Where(x => x.approvalStatusId == (short)ApprovalStatusEnum.Pending); 
            //var recordReferred = entity.GroupBy(x => x.originalDocumentApprovalId).Select(x => x.FirstOrDefault()).Where(x => x.approvalStatusId == (short)ApprovalStatusEnum.Referred );
            ...
            if (entity.approvalStatusId == (short)ApprovalStatusEnum.Referred)
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _workflow.StatusId = (short)ApprovalStatusEnum.Processing;
                    _workflow.TargetId = entity.originalDocumentApprovalId;
                    _workflow.Comment = TranslateHelper.get("Update has been applied, Request for Security Release Approval");
                    _workflow.OperationId = (int)OperationsEnum.SecurityRelease;
                    _workflow.DeferredExecution = true;
                    _workflow.LogActivity();
            ...
            if (entity.approvalStatusId == (short)ApprovalStatusEnum.Pending)
            {
                //foreach (var x in record)
                //{
                var data = _context.TBL_ORIGINAL_DOCUMENT_RELEASE.Where(t => t.ORIGINALDOCUMENTAPPROVALID == entity.originalDocumentApprovalId
                                                                            && t.APPROVALSTATUSID == (int)ApprovalStatusEnum.Pending).ToList();
                _workflow.StaffId = entity.createdBy;
                _workflow.CompanyId = entity.companyId;
                _workflow.StatusId = (short)ApprovalStatusEnum.Processing;
        ...
        public async Task<WorkflowResponse> GoForGuaranteeApproval(IEnumerable<OriginalDocumentReleaseViewModel> entity)
        {
            var record = entity.GroupBy(x => x.originalDocumentApprovalId).Select(x => x.FirstOrDefault()).Where(x => x.approvalStatusId == (short)ApprovalStatusEnum.Pending);
            var recordReferred = entity.GroupBy(x => x.originalDocumentApprovalId).Select(x => x.FirstOrDefault()).Where(x => x.approvalStatusId == (short)ApprovalStatusEnum.Referred);
            ...
            if (recordReferred != null)
            {
                ...
                foreach (var x in recordReferred)
                {
                    using (var transaction = _context.Database.BeginTransaction())
                    {
                        ...
                        _workflow.StatusId = (short)ApprovalStatusEnum.Processing;
                        _workflow.TargetId = x.originalDocumentApprovalId;
                        _workflow.Comment = TranslateHelper.get("Update has been applied, Request for Guarantee Release Approval");
            ...
            if (record != null)
            {
                foreach (var x in record)
                {
                    ...
                    _workflow.StatusId = (short)ApprovalStatusEnum.Processing;
                    _workflow.TargetId = x.originalDocumentApprovalId;
                    _workflow.Comment = TranslateHelper.get("Request for gurantee security release approval");
                    _workflow.OperationId = (int)OperationsEnum.GuaranteeReleaseApproval;
                    _workflow.DeferredExecution = true;
                    _workflow.ExternalInitialization = true;
                    _workflow.LogActivity();
        ...
        public async Task<WorkflowResponse> GoForCashSecurityApproval(CollateralCashReleaseViewModel entity)
        {
            if (entity.approvalStatusId == (int)ApprovalStatusEnum.Referred)
            {
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    ...
                    _workflow.OperationId = (int)OperationsEnum.CashSecurityRelease;
                    _workflow.DeferredExecution = true;
                    _workflow.LogActivity();
                    try
                    {
        ...
        public async Task<WorkflowResponse> GoForGuaranteeCashApproval(CollateralCashReleaseViewModel entity)
        {
            if (entity.approvalStatusId == (int)ApprovalStatusEnum.Referred)
            {
                var cashRelease = _context.TBL_CASH_SECURITY_RELEASE_APPROVAL.Find(entity.cashSecurityReleaseIseId);
        ...
        public async Task< WorkflowResponse> SubmitApproval(OriginalDocumentReleaseViewModel model)
        {
            bool responce = false;
