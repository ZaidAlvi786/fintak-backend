...
namespace FintrakBanking.Repositories.credit
{
    public class FacilityModificationRepository : IFacilityModificationRepository
    {
        ...
        public async Task<IEnumerable<FacilityModificationViewModel>> GetFacilityModificationsForApproval(int staffId)
        {
            ...
            var reliefIds = general.GetStaffRlieved(staffId);
                ...
                var modifications = await (from x in context.TBL_FACILITY_MODIFICATION
        ...
        public async Task<FacilityModificationViewModel> GetFacilityModification(int facilityModificationId)
        {
            var entity = await context.TBL_FACILITY_MODIFICATION.FirstOrDefaultAsync(x => x.FACILITYMODIFICATIONID == facilityModificationId && x.DELETED == false);
        ...
        public async Task<WorkflowResponse> AddFacilityModification(FacilityModificationViewModel model)
        {
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                workflow.Vote = 2;
                workflow.StatusId = 1;
                workflow.Comment = TranslateHelper.get("Kindly approve this facility modification");
        ...
        public async Task<WorkflowResponse> ApproveFacilityModification(ForwardViewModel model)
        {
            using (var trans = this.context.Database.BeginTransaction())
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Facility Modification doesn't exist!"));
                }
                workflow.OperationId = (int)OperationsEnum.FacilityModificationApproval;
                workflow.StaffId = model.createdBy;
