...
namespace FintrakBanking.Repositories.Credit
{
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<List<DashboardViewModel>> LoanApplicationsBySector(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();

            var data = await (from x in context.TBL_LOAN_APPLICATION_DETAIL
                         ...
                         select new { x, l, sb, s })?.ToListAsync();
        ...
        public async Task<List<DashboardReportItem>> LoanPerformance(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            int count = 0;

            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();

            var data = await (from a in context.TBL_LOAN
        ...
        public async Task <List<LoanDisburseByType>> LoanDisbursedByType(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            int count = 0;
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();

            var loanDetails = await (from a in context.TBL_LOAN
                               where
                                a.EFFECTIVEDATE >= startDate && a.EFFECTIVEDATE <= endDate && a.COMPANYID == companyId
                               select new LoanViewModel { relationshipManagerId= a.RELATIONSHIPMANAGERID, loanSystemTypeId=a.LOANSYSTEMTYPEID, branchId=a.BRANCHID,})?.ToListAsync();

            var od = await (from a in context.TBL_LOAN_REVOLVING
                      where
                       a.EFFECTIVEDATE >= startDate && a.EFFECTIVEDATE <= endDate && a.COMPANYID == companyId
                      select new LoanViewModel { relationshipManagerId = a.RELATIONSHIPMANAGERID, loanSystemTypeId = a.LOANSYSTEMTYPEID, branchId = a.BRANCHID, })?.ToListAsync();

            var contingent = await (from a in context.TBL_LOAN_CONTINGENT
                              where 
                               a.EFFECTIVEDATE >= startDate && a.EFFECTIVEDATE <= endDate && a.COMPANYID == companyId
                              select new LoanViewModel { relationshipManagerId = a.RELATIONSHIPMANAGERID, loanSystemTypeId = a.LOANSYSTEMTYPEID, branchId = a.BRANCHID, })?.ToListAsync();
        ...
        public async Task<List<DashboardViewModel>> LoanOnThePipeline(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();
        ...
        public async Task<DashboardViewModel> GetLoanInThePipelineLms(int operationId, int staffId, int companyId, int branchId, int? classId)
        {
            var staff = await context.TBL_STAFF.FirstOrDefaultAsync(O => O.STAFFID == staffId);

            var approvalOperations = await context.TBL_OPERATIONS.Where(x => x.OPERATIONTYPEID == (short)OperationTypeEnum.LoanReviewApplication)
                .Select(x => x.OPERATIONID).ToListAsync();

            approvalOperations = approvalOperations.Where(id => id != (short)OperationsEnum.LoanReviewApprovalAvailment && id != (short)OperationsEnum.LoanReviewDrawdownForExtension && id != (short)OperationsEnum.OverdraftReviewDrawdownForExtension && id != (short)OperationsEnum.ContingentReviewDrawdownForExtension).ToList();
            ...
            var query = await context.TBL_LMSR_APPLICATION.Where(x => x.BRANCHID == branchId || ignoreBranch)
        ...
        public async Task<DashboardViewModel> GetApprovedLoansLms(int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();
            var approvalLevel = levelStaffRepo.GetAllAssignedApprovalLevelStaff(companyId).Where(c => c.staffId == staffId || c.staffRoleId == staff.STAFFROLEID).ToList();
            var staffApprovalLevels = approvalLevel.Select(x => x.approvalLevelId).Distinct();

            var data = await (from l in context.TBL_LMSR_APPLICATION_DETAIL
        ...
        public async Task<List<DashboardViewModel>> ExpotureByRiskRating(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();
        ...
        public async Task <List<DashboardViewModel>> CollateralCoverage(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();

            var termLoan = await (from x in context.TBL_LOAN
                           ...
                           select new LoanCollateralMappingViewModel { amount = x.PRINCIPALAMOUNT, exchangeRate = x.EXCHANGERATE, collateralCustomerId = c.COLLATERALCUSTOMERID, note="Term Loan" , userBranchId=x.BRANCHID})?.ToListAsync();
                         
            var contingent = await (from x in context.TBL_LOAN_CONTINGENT
                           ...
                           where x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved
                                  ...
                                  && x.EFFECTIVEDATE >= startDate && x.EFFECTIVEDATE <= endDate
                             select new LoanCollateralMappingViewModel { amount = x.CONTINGENTAMOUNT, exchangeRate = x.EXCHANGERATE, collateralCustomerId = c.COLLATERALCUSTOMERID, note = "Contingent", userBranchId = x.BRANCHID })?.ToListAsync();


            var overdraft = await (from x in context.TBL_LOAN_REVOLVING
                             ...
                             && c.ISRELEASED==false
                                    ...
                                    && x.EFFECTIVEDATE >= startDate && x.EFFECTIVEDATE <= endDate
                            select new LoanCollateralMappingViewModel { amount = x.OVERDRAFTLIMIT, exchangeRate = x.EXCHANGERATE, collateralCustomerId = c.COLLATERALCUSTOMERID , note = "Overdraft" , userBranchId = x.BRANCHID })?.ToListAsync();
        ...
        public async Task<List<DashboardViewModel>> ApprovedLoan(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();
            var approvalLevel = levelStaffRepo.GetAllAssignedApprovalLevelStaff(companyId).Where(c => c.staffId == staffId || c.staffRoleId == staff.STAFFROLEID).ToList();
            var staffApprovalLevels = approvalLevel.Select(x => x.approvalLevelId).Distinct();
        ...
        public async Task<List<DashboardViewModel>> TotalRiskExposure(DateTime startDate, DateTime endDate, int companyId, int staffId)
        {
            var staff = await context.TBL_STAFF.Where(o => o.STAFFID == staffId).Select(o => o).FirstOrDefaultAsync();

            var termLaon = await (from x in context.TBL_LOAN
                           where x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved
                                && x.COMPANYID == companyId
                                && x.DATETIMECREATED >= startDate && x.DATETIMECREATED <= endDate
            ...
            var OD = await (from x in context.TBL_LOAN_REVOLVING
                     where x.APPROVALSTATUSID == (int)ApprovalStatusEnum.Approved
                            && x.COMPANYID == companyId
                            && x.DATETIMECREATED >= startDate && x.DATETIMECREATED <= endDate
       ...
       public async Task<DashboardViewModel> GetCountryCurrency(int companyId)
        {
            var curr =  await (from c in context.TBL_COMPANY.Where(c => c.COMPANYID == companyId)
                       select new DashboardViewModel
                       {
