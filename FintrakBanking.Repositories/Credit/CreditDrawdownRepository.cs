...
namespace FintrakBanking.Repositories.Credit
{
    public class CreditDrawdownRepository : ICreditDrawdownRepository
    {
        ...
        public async Task<IEnumerable<TransactionDynamicsViewModel>> GetLoanTransactionDynamics(int loanApplicationDetailId)
        {
        ...
        public async Task<WorkflowResponse> LogApprovalForMessage(ForwardViewModel model, bool externalInitialization, bool saveChanges = false)
        {
        ...
        public async Task<CurrentCustomerExposure> GetCurrentCompanyExposure()
        {
        ...
        public async Task<WorkflowResponse> GoForBookingRequestApproval(ApprovalViewModel entity, int loanBookingRequestId)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                if (entity.approvalStatusId != (int)ApprovalStatusEnum.Referred)
                {
                    ...
                    if (classifiedTrail != null && previousTrail == null)
                    {
                        ...
                        var approvalModel = new ForwardViewModel
                        {
                            ...
                            comment = TranslateHelper.get("A request for booking needs your attention"),
                            amount = request.AMOUNT_REQUESTED,
                        };
                        application.APPLICATIONSTATUSID = (short)LoanApplicationStatusEnum.BookingRequestCompleted;

                        if (operationId > 0) await LogApproval(approvalModel, classifiedTrail.DESTINATIONOPERATIONID ?? 0, true, (short)ApprovalStatusEnum.Pending);
                        context.SaveChanges();
                        trans.Commit();
                ...
                workflow.ProductId = entity.productId;
                workflow.DeferredExecution = true;
                workflow.ExternalInitialization = false;
                ...
                //|| (workflow.NewState == (int)ApprovalState.Ended && isContingent == true))
                {
                    ...
                    if (operationId > 0) await LogApproval(approvalModel, operationId, true, (short)ApprovalStatusEnum.Pending);
                    application.APPLICATIONSTATUSID = (short)LoanApplicationStatusEnum.BookingRequestCompleted;
                ...
                else
                {
                    ...
                    if (workflow.Response.nextLevelName == "AVAILMENT" || workflow.Response.nextLevelName == "CREDIT AVAILMENT") { request.APPROVALSTATUSID = (short)ApprovalStatusEnum.Processing; }
                    context.SaveChanges();
                    trans.Commit();
                    return workflow.Response;
                    //return 1;
        ...
        public async Task<bool> setLineFacilityLegalDocumentStatus(RecommendedCollateralViewModel entity, int loanBookingRequestId, bool value)
        {
            ...
            if (request != null)
            {
                ...
                if (facility != null)
                {
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        DETAIL = $"{TranslateHelper.get("Line facility Legal Document set to")} '{ value }' {TranslateHelper.get("for loan application ref")} '{facility.TBL_LOAN_APPLICATION.APPLICATIONREFERENCENUMBER}{TranslateHelper.get(" on facility")} {facility.TBL_PRODUCT.PRODUCTNAME}'",
        ...
        public IEnumerable<CamProcessedLoanViewModel> GetBookingRequestAwaitingAvailment(int staffId, int companyId, bool isInitiation = false)
        {
            List<int> operationIds = new List<int>();
            operationIds.Add((int)OperationsEnum.CorporateDrawdownRequest);
            operationIds.Add((int)OperationsEnum.IndividualDrawdownRequest);
            ...
            data = (from req in context.TBL_LOAN_BOOKING_REQUEST
                    ...
                    where operationIds.Contains(atrail.OPERATIONID)
                            && m.APPLICATIONSTATUSID != (short)LoanApplicationStatusEnum.CAMInProgress
                            //&& m.APPLICATIONSTATUSID == (short)LoanApplicationStatusEnum.AvailmentInProgress
                            && (atrail.TOSTAFFID == null || staffs.Contains((int)atrail.TOSTAFFID))
                            && m.APPLICATIONSTATUSID != (short)LoanApplicationStatusEnum.CancellationCompleted
                            && req.ISUSED == false && atrail.RESPONSESTAFFID == null
                            && ((atrail.APPROVALSTATUSID != (short)ApprovalStatusEnum.Approved)
                                            && (atrail.APPROVALSTATUSID != (short)ApprovalStatusEnum.Disapproved)
                                            && (atrail.APPROVALSTATUSID != (short)ApprovalStatusEnum.Finishing))
                            && (req.DELETED == false && req.APPROVALSTATUSID == (short)ApprovalStatusEnum.Processing)
                            && ((levelIds.Contains((int)atrail.TOAPPROVALLEVELID) && atrail.LOOPEDSTAFFID == null)
                              || (!levelIds.Contains((int)atrail.TOAPPROVALLEVELID) && staffs.Contains((int)atrail.LOOPEDSTAFFID)))
                    //|| (isInitiation == true && req.APPROVALSTATUSID == (short)ApprovalStatusEnum.Disapproved && req.DELETED == false)
        ...
        public async Task<int> GetDrawdownOperationId(int applicationDetailId)
        {
            ...
            {
                if (await GetRevolvingTrancheDisbursementOperationId(applicationDetailId))
                {
        ...
        private Task<bool> GetRevolvingTrancheDisbursementOperationId(int applicationDetailId)
        {
            var loanApplicationDetails = context.TBL_LOAN_APPLICATION_DETAIL.Find(applicationDetailId);
        ...
        public async Task<IEnumerable<CamProcessedLoanViewModel>> GetGlobalEmployerLoansDueForInitiateBooking(int companyId, string searchString)
        {
            ...
            data = await PerformNecessaryDrawdownInitiationCalculations(data);
            return data;
        }

        public async Task<List<CamProcessedLoanViewModel>> PerformNecessaryDrawdownInitiationCalculations(List<CamProcessedLoanViewModel> data)
        {
            foreach (var item in data)
            {
        ...
        public async Task<IEnumerable<CamProcessedLoanViewModel>> GetAvailedLoanApplicationsDueForInitiateBooking(int companyId, int staffId, int branchId, int customerId, bool getAll = false)
        {
            var systemDate = generalSetup.GetApplicationDate();
            var company = context.TBL_COMPANY.Find(companyId);
        ...
        public async Task<IEnumerable<CamProcessedLoanViewModel>> getApplicationsToBeAdhocApprovedForInitiateBooking(int companyId, int staffId, int branchId)
        {
            var systemDate = generalSetup.GetApplicationDate();
            var company = context.TBL_COMPANY.Find(companyId);
        ...
        private async Task<decimal> getDisbursableAmount(int operationId, int loanApplicationDetailId)
        {
        ...
        public async Task<WorkflowResponse> AddLoanBookingRequest(int applicationStatusId, List<LoanBookingRequestViewModel> models)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                var productClassId = 0;
                ...
                {
                    productClassId = context.TBL_PRODUCT.Where(p => p.PRODUCTID == model.productId).FirstOrDefault().PRODUCTCLASSID;
                    if (model.approvalStatusId != (short)ApprovalStatusEnum.Referred)
                    {
                        model.productClassId = productClassId;
                        if (!await AddLoanBookingRequests(applicationStatusId, model))
                        {
                    ...
                    {
                        if (!await UpdateLoanBookingRequests(applicationStatusId, model))
                        {
                            trans.Rollback();
                            workflow.Response = null;
                            return workflow.Response;
        ...
        public async Task<int> GetNextLevelForBookingRequest(int applicationStatusId, List<LoanBookingRequestViewModel> entities)
        {
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                if (requestedFacility.PRODUCTCLASSID == (short)ProductClassEnum.Creditcards)
                {
                    ...
                    await LogApprovalForMessage(approvalModel, true);
                }
                else if (loanApplicationDetails.TBL_CUSTOMER.CUSTOMERTYPEID == (short)CustomerTypeEnum.Individual)
                {
                ...
                {
                    if (await GetRevolvingTrancheDisbursementOperationId(loanApplicationDetails.LOANAPPLICATIONDETAILID))
                    {
        ...
        private async Task<bool> AddLoanBookingRequests(int applicationStatusId, LoanBookingRequestViewModel entity)
        {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("Requested Amount cannot be greater than the approved amount"));
            }
            ...
            if (entity.customerId == null)
            {
                ...
                {
                    throw new SecureException(TranslateHelper.get("Please select a Group Member"));
                }
                else
                {
            ...
            {
                throw new ConditionNotMetException(TranslateHelper.get("This facility already has a tranche disbursement request for this customer currently undergoing approval."));
            }
            ...
            if (entity.amount_Requested > await getDisbursableAmount(operationId, entity.loanApplicationDetailId))
            {
                throw new ConditionNotMetException(TranslateHelper.get("Requested Amount cannot be greater than the disbursable amount"));
            }

            bool cleared = await OfferLetterChecklistValidation(loanApplicationDetails.LOANAPPLICATIONID, 1);
            if (cleared == false) throw new SecureException(TranslateHelper.get("Checklist not cleared to go further!"));
            ...
            if (requestedFacility.PRODUCTCLASSID == (short)ProductClassEnum.Creditcards)
            {
                await LogApproval(approvalModel, (short)OperationsEnum.CreditCardDrawdownRequest, true, (int)ApprovalStatusEnum.Pending);
            ...
            else if (loanApplicationDetails.TBL_CUSTOMER.CUSTOMERTYPEID == (short)CustomerTypeEnum.Individual)
            {
                ...
                {
                    await LogApproval(approvalModel, (short)OperationsEnum.CorporateDrawdownRequest, true, (int)ApprovalStatusEnum.Pending);
                    request.OPERATIONID = (short)OperationsEnum.CorporateDrawdownRequest;
                }
                else
                {
                    await LogApproval(approvalModel, (short)OperationsEnum.IndividualDrawdownRequest, true, (int)ApprovalStatusEnum.Pending);
            ...
            else if (loanApplicationDetails.TBL_CUSTOMER.CUSTOMERTYPEID == (short)CustomerTypeEnum.Corporate)
            {
                if (await GetRevolvingTrancheDisbursementOperationId(loanApplicationDetails.LOANAPPLICATIONDETAILID))
                {
                    await LogApproval(approvalModel, (short)OperationsEnum.RevolvingTranchDisbursement, true, (int)ApprovalStatusEnum.Pending);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                BRANCHID = (short)entity.userBranchId,
                TARGETID = request.LOAN_BOOKING_REQUESTID,
                DETAIL = $"{TranslateHelper.get("Request to book loan of amount")} '{ entity.amount_Requested }' {TranslateHelper.get("for customer id")}'{loanApplicationDetails.TBL_CUSTOMER.CUSTOMERCODE}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        private async Task<bool> OfferLetterChecklistValidation(int id, int type)
        {
            int count = 0;
            if (type == 1)
            {
        ...
        private async Task<TBL_LOAN_BOOKING_REQUEST> addBookingRequest(multipleDisbursementOutputViewModel entity, short? approvalStatusid, UserInfo user)
        {
            var request = new TBL_LOAN_BOOKING_REQUEST
            {
        ...
        private async Task<bool> UpdateLoanBookingRequests(int applicationStatusId, LoanBookingRequestViewModel model)
        {
            ...
            if (request != null)
            {
                ...
                workflow.Comment = model.comment != null ? model.comment : TranslateHelper.get("Kindly proceeed. Update has been applied");
