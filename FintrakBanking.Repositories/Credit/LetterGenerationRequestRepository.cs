...
namespace FintrakBanking.Repositories.Credit
{
    public class LetterGenerationRequestRepository : ILetterGenerationRequestRepository
    {
        ...
        public async Task<IEnumerable<AuthorisedSignatoryViewModel>> GetLetterGenerationSignatory(int requestId)
        {
            var signatories =  await (from a in context.TBL_AUTHORISED_SIGNATORY
        ...
        public async Task<IEnumerable<LetterGenerationRequestViewModel>> GetLetterGenerationRequests(int staffId)
        {
            var requestsInProgress =  await (from x in context.TBL_LETTER_GENERATION_REQUEST
        ...
        public async Task<IEnumerable<LetterGenerationRequestViewModel>> GetLetterGenerationRequestsForApproval(int staffId)
        {
            ...
            //                  select b).ToList();
            // query
            var query =  await (from l in context.TBL_LETTER_GENERATION_REQUEST
                         where
                            (l.DELETED == false
        ...
        public async Task<LetterGenerationRequestViewModel> GetLetterGenerationRequest(int id)
        {
            ...
            return new LetterGenerationRequestViewModel
            {
                ...
                letterGenerationsignatories = await (from a in context.TBL_AUTHORISED_SIGNATORY
                                               join y in context.TBL_OPERATION_SIGNATORY on a.SIGNATORYID equals y.SIGNATORYID
                ...
                letterGenerationCamsolList = await (from a in context.TBL_LOAN_CAMSOL
                                          join y in context.TBL_OPERATION_CAMSOL_LIST on a.LOAN_CAMSOLID equals y.LOAN_CAMSOLID
                                          join z in context.TBL_LOAN_CAMSOL_TYPE on a.CAMSOLTYPEID equals z.CAMSOLTYPEID
                                          where y.OPERATIONID == (int)OperationsEnum.LetterGenerationRequest && entity.LETTERGENERATIONREQUESTID == y.TARGETID
                                          select new CamsolLoanDocumentViewModel()
        ...
        public async Task<LetterGenerationRequestViewModel> AddLetterGenerationRequest(LetterGenerationRequestViewModel model)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                var aud = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"TBL_LETTER_GENERATION_REQUEST '{entity.ToString()}' {TranslateHelper.get("created by")} {auditStaff}",
        ...
        public LetterGenerationRequestViewModel UpdateLetterGenerationRequest(LetterGenerationRequestViewModel model, int id, UserInfo user)
        {
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                DETAIL = $"TBL_LETTER_GENERATION_REQUEST '{entity.ToString()}' {TranslateHelper.get("was updated by")} {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public bool DeleteLetterGenerationRequest(int id, UserInfo user)
        {
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"TBL_LETTER_GENERATION_REQUEST '{entity.ToString()}' {TranslateHelper.get("was deleted by")} {auditStaff}",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<List<CamsolLoanDocumentViewModel>> GetCamsolLoansByCustomerCode(string customerName, string customerCode)
        {
            string[] str = {" "};
            ...
            var data = await (from O in context.TBL_LOAN_CAMSOL
        ...
        public async Task<string> GetCamsolLoanDocument(int typeId, LetterGenerationRequestViewModel model)
        {
            if (typeId == 1) {
                ...
                var camsolList = await (from c in context.TBL_OPERATION_CAMSOL_LIST
        ...
        public string GetLetterOfIndebtedness(LetterGenerationRequestViewModel model)
        {
            ...
            var accountNumber = context.TBL_CASA.Where(p => p.CUSTOMERID == model.customerId).FirstOrDefault()?.PRODUCTACCOUNTNUMBER;
            ...
            string result = $"<p style='font face:arial;size:12px;'><b>{model.requestRef}</b></p> " +
                ...
                $"<p style='font face:arial;size:12px;'><b>{TranslateHelper.get("LETTER OF INDEBTEDNESS ")}– {fullName} - {accountNumber}</b></p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("We hereby confirm that")} <b>{fullName}</b>, {TranslateHelper.get("with account number")} {accountNumber} {TranslateHelper.get("is indebted to our Bank as at")} {asAtDate}, {TranslateHelper.get("to the tune of")} N {debtAmount.ToString("#,##")}.</p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("Please note that interest will continue to accrue on the above amount on a daily basis until the facility is fully liquidated.")}</p> " +
                $"<p style='font face:arial;size:12px;'><b>{TranslateHelper.get("This report is given in strict confidence and without liability on the part of Access Bank Plc or any of its staff or agent.")}</b></p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("Thank you")}.</p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("Yours faithfully")},</p> <p style='font face:arial;size:12px;'><b>For:</b> ACCESS BANK PLC</p> " +
        ...
        public string GetLetterOfNonIndebtedness(LetterGenerationRequestViewModel model)
        {
           ...
           // var reference = "<p style='font face:arial;size:12px;'>ABP/ROG/OA/BO/03/2016/0061</p>";
            ...
            string result = $"<p style='font face:arial;size:12px;'><b>REF: {model.requestRef}</b></p> " +
                $"<p style='font face:arial;size:12px;'><b>{asAtDate}.</b></p> " +
                $"<p style='font face:arial;size:12px;'><b>{fullName},</b> <br/> {address}</p> " +
                $"<p style='font face:arial;size:12px;'><b>{TranslateHelper.get("Dear Sir/Ma")},</b></p> " +
                $"<p style='font face:arial;size:12px;'><b>{TranslateHelper.get("LETTER OF NON-INDEBTEDNESS")} – {fullName} - {accountNumber}</b></p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("We hereby confirm that")} <b>{fullName}</b>, {TranslateHelper.get("is not indebted to our Bank as at")} {asAtDate}.</p> " +
                $"<p style='font face:arial;size:12px;'><b>{TranslateHelper.get("Please note that this report is given in strict confidence and without liability on the part of Access Bank Plc or any of its staff or agent.</b></p>")} " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("Thank you.")}</font></p> " +
                $"<p style='font face:arial;size:12px;'>{TranslateHelper.get("Yours faithfully")},</p> <p style='font face:arial;size:12px;'><b>For:</b> ACCESS BANK PLC</p> " +              
                //$"<p></p><p><b>AUTHORISED SIGNATORY <br/> EMMANUELLA OGHOR <br/> ASSISTANT BRANCH MANAGER</b></p> " +
                //$"<p></p><p><b>AUTHORISED SIGNATORY <br/> IKECHUKWU ONYEMEM <br/> BRANCH MANAGER</b></p></font>";
                $"{GetLetterGenRequestSignatory(model.requestId)}</font>";
            ...
            result = result + $@"
                <table border=1 width=750 cellpadding=15 cellspacing=0 style='font face: arial; size:12px'>
                    <tr>
                        ...
                        <th><b>{TranslateHelper.get("Loan Amount")} (N)</b></th>
                        <th><b>{TranslateHelper.get("Outstanding Amount")} (N)</b></th>
                        <th><b>{TranslateHelper.get("Rate")}</b></th>
                        <th><b>{TranslateHelper.get("Value Date")}</b></th>
                        <th><b>{TranslateHelper.get("Maturity Date")}</b></th>
                    </tr>";
