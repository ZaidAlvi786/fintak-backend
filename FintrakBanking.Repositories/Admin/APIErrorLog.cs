...
namespace FintrakBanking.Repositories.Admin
{
    public class APIErrorLog : IAPIErrorLog
    {
        ...
        public async Task<List<APILogViewModel>> GetAPILog(DateTime startDate, DateTime endDate, string searchInfo)
        {
            var  apiLogList = await (from x in _context.TBL_CUSTOM_API_LOGS
        ...
        {
            var errorLog = await (from x in _context.TBL_ERRORLOG
