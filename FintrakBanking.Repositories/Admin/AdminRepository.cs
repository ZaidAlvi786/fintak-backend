...
namespace FintrakBanking.Repositories.Admin
{
    public class AdminRepository : IAdminRepository
    {
        ...
        {
            return await context.TBL_PROFILE_USER.AnyAsync(x => x.USERNAME.ToLower() == username.ToLower());
        }

        public async Task<bool> isStaffExist(long staffId)
        {
            return await context.TBL_PROFILE_USER.AnyAsync(x => x.STAFFID == staffId);
        }
        public async Task<bool> GoForApproval(ApprovalViewModel entity)
        {
            ...
            using (var trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var response = await ApproveUser(entity.targetId, (short)workFlow.StatusId, entity);
        ...
        private async Task<bool> ApproveUser(int userId, short approvalStatusId, UserInfo user)
        {
            var userRecord = await context.TBL_PROFILE_USER.FindAsync(userId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Approved user with username")} : '{userRecord?.USERNAME}'",
        ...
        public async Task<bool> CreateUser(AppUserViewModel user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added User with username:")} '{user.username}'",
                IPADDRESS = user.userIPAddress,
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now
        ...
        public async Task<bool> UpdateUser(int userId, AppUserViewModel user)
        {
            ...
            if (targetUser != null)
            {
                ...
                var targetGroups = await context.TBL_PROFILE_USERGROUP.Where(x => x.USERID == userId).ToListAsync();
                var targetActivities = await context.TBL_PROFILE_ADDITIONALACTIVITY.Where(x => x.USERID == userId).ToListAsync();
                if (targetGroups.Any())
                {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.createdBy,
                    BRANCHID = user.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Updated User with username:")} '{user.username}'",
                    IPADDRESS = user.userIPAddress,
                    URL = user.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now
        ...
        #endregion Users
        public async Task<GlobalSettingViewModel> GetAllGlobalSettings()
        {
            return await context.TBL_SETUP_GLOBAL.Select(x => new GlobalSettingViewModel
            {
        ...
        {
            return context.TBL_PROFILE_GROUP.Where(g => g.DELETED == false).Select(x => new AppGroupViewModel
            {
                groupId = x.GROUPID,
        ...
        public async Task<AppGroupViewModel> GetSingleGroup(int groupId)
        {
            return await context.TBL_PROFILE_GROUP.Where(g => g.GROUPID == groupId && g.DELETED == false).Select(x => new AppGroupViewModel
            {
                groupId = x.GROUPID,
                groupName = x.GROUPNAME
            }).FirstAsync(); 
        ...
        {
            return await context.TBL_PROFILE_GROUP.AnyAsync(x => x.GROUPNAME.ToLower() == groupName);
        }
        ...
        public bool AddGroup(AppGroupViewModel group)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added User group with name")} : '{group.groupName}' ",
        ...
        {
            var targetGroup = await context.TBL_PROFILE_GROUP.FindAsync(groupId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Update User group with name")} : '{groupModel.groupName}' ",
        ...
        public async Task<bool> deleteGroup(short groupId, AppGroupViewModel groupModel)
        {
            //DELETING WORKFLOW GROUP IS NOT RECOMMENDED - AUSTIN
            var targetGroup = await context.TBL_PROFILE_GROUP.FindAsync(groupId);

            if (context.TBL_APPROVAL_LEVEL.Where(x => x.GROUPID != targetGroup.GROUPID).Any() || context.TBL_APPROVAL_GROUP_MAPPING.Where(x => x.GROUPID != targetGroup.GROUPID).Any())
            {
                throw new Exception("The group "+targetGroup.GROUPNAME +" has already been mapped or in use and cannot be deleted");
            }
            if (targetGroup != null)
            {
                targetGroup.DELETED = true;
                targetGroup.DATETIMEUPDATED = DateTime.Now;
                targetGroup.LASTUPDATEDBY = groupModel.createdBy;
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = (int)groupModel.createdBy,
                    BRANCHID = (short)groupModel.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Deleted User group with name")} : '{groupModel.groupName}' ",
                    IPADDRESS = groupModel.userIPAddress,
                    URL = CommonHelpers.GetLocalIpAddress(),// groupModel.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> StaffHasActivity(int staffId, string activity)
        {
            var userId = await (from a in context.TBL_PROFILE_USER
                          where a.STAFFID == staffId
                          select a.USERID).FirstOrDefaultAsync();
        ...
        public bool UpdateUserAccountStatus(int userId, short approvalStatusId, UserInfo user)
        {
            ...
            if (tempData == null) throw new ConditionNotMetException(TranslateHelper.get("Could not resolve user account status update."));
            ...
            {
                data.LASTLOCKOUTDATE = tempData.LASTLOCKOUTDATE;
            }
        ...
        public bool LogUserStatusUpdateRequest(ActiveUserDetails entity, out string message)
        {
            using (var trans = context.Database.BeginTransaction())
            {
                ...
                if (tempData != null)
                {
                    throw new ConditionNotMetException(TranslateHelper.get("There is already a User Account status update currently undergoing approval"));
                }
                else
                {
                    ...
                    if (entity.lockStatus)
                    {
                        ...
                        {
                            temProfileUser.LASTLOCKOUTDATE = DateTime.Now;
                        }
                    }
                    ...
                    {
                        trans.Rollback();
                        throw new SecureException(TranslateHelper.get("Failed to Save"));
                    }
