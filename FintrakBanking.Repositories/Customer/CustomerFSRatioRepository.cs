...
namespace FintrakBanking.Repositories.Customer
{
    ...
    public class CustomerFSRatioRepository : ICustomerFSRatioRepository
    {
        ...
        public async Task<IEnumerable<CustomerFSRatioCaptionViewModel>> GetFSRatioCaption(int companyId)
        {


            var data =await (from a in context.TBL_CUSTOMER_FS_CAPTION
        ...
        public async Task<IEnumerable<CustomerFSRatioCaptionViewModel>> GetFSRatioCaptionByFSCaptionGroupId(int companyId, int fSCaptionGroupId)
        {
            var data =await (from a in context.TBL_CUSTOMER_FS_CAPTION
        ...
        public async Task<List<CustomerFSRatioCaptionViewModel>> GetFSRatioCaptionById(short ratioCaptionId)
        {
            var data = await(from a in context.TBL_CUSTOMER_FS_CAPTION
        ...
        public async Task<bool> UpdateFSRatioCaption(short ratioCaptionId, CustomerFSRatioCaptionViewModel model)
        {
        ...
        public async Task<bool> AddFSRatioCaption(CustomerFSRatioCaptionViewModel model)
        {
        ...
        public async Task<bool> DeleteFSRatioCaption(short ratioCaptionId, UserInfo user)
        {
            //var data = context.TBL_CUSTOMER_FS_RATIO_CAPTION.Find(ratioCaptionId);
            //data.DELETED = true;
        ...
        public async Task<bool> AddFSRatioDetail(CustomerFSRatioDetailViewModel model)
        {
            if (model != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Added FS Ratio Detail")} {data.TBL_CUSTOMER_FS_CAPTION} {TranslateHelper.get("with divisor type")} '{auditDivisor}' {TranslateHelper.get("and value type")} '{auditValue }' ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL =model.applicationUrl,
                    DEVICENAME = CommonHelpers.GetDeviceName(),
                    OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> AddMultipleFSRatioDetail(List<CustomerFSRatioDetailViewModel> model)
        {
            if (model.Count <= 0)
                return false;

            foreach (CustomerFSRatioDetailViewModel entity in model)
                await AddFSRatioDetail(entity);

            return true;
        }

        public async Task<IEnumerable<CustomerFSRatioDetailViewModel>> GetFSRatioDetail(short ratioCaptionId, short fsCaptionGroupId, int companyId)
        {
            var data = await (from a in context.TBL_CUSTOMER_FS_RATIO_DETAIL
        ...
        public async Task<CustomerFSRatioDetailViewModel> GetFSRatioDetailById(int ratioDetailId)
        {
            var data =await( from a in context.TBL_CUSTOMER_FS_RATIO_DETAIL
                       join b in context.TBL_CUSTOMER_FS_CAPTION on a.RATIOCAPTIONID equals b.FSCAPTIONID
        ...
        public async Task<List<CustomerFSRatioCaptionReportViewModel>> GetCustomerFSRatioValues(int customerId)
        {
            ...
            var ratioCaptions =await (from a in context.TBL_CUSTOMER_FS_CAPTION
            ...
            foreach (var item in ratioCaptions)
            {
                ...
                value.ratioValue1 = count >= 4 ? await GetCustomerFSRatio(item.ISRATIO, customerId, (short)item.FSCAPTIONID, lastFourDates[count - 4]) : "0.00";
                value.ratioValue2 = count >= 3 ? await GetCustomerFSRatio(item.ISRATIO, customerId, (short)item.FSCAPTIONID, lastFourDates[count - 3]) : "0.00";
                value.ratioValue3 = count >= 2 ? await GetCustomerFSRatio(item.ISRATIO, customerId, (short)item.FSCAPTIONID, lastFourDates[count - 2]) : "0.00";
                value.ratioValue4 = count >= 1 ? await GetCustomerFSRatio(item.ISRATIO, customerId, (short)item.FSCAPTIONID, lastFourDates[count - 1]) : "0.00";
        ...
        private async Task<string> GetCustomerFSRatio(bool isRatio, int customerId, short fsCaptionId, DateTime fsDate)
        {
            // it is not ratio (not derived)
            if (isRatio == false)
            {
                var fsAmount = await (from a in context.TBL_CUSTOMER_FS_CAPTION_DETAIL
            ...
            if (isRatio)
            {
                ...
                var detail = await (from c in context.TBL_CUSTOMER_FS_CAPTION
                              ...
                              select new { c.FSCAPTIONID, c.FSCAPTIONNAME, c.ISRATIO, d.AMOUNT }).FirstOrDefaultAsync();
        ...
        {
            //var sum = 0;
            var sum = CalculateFSRatioValue(entity.customerId, (short) entity.fsCaptionId, entity.fsDate).Result;
            return sum;
            //entity.amount = sum;
        ...
        private async Task<decimal> CalculateFSRatioValue(int customerId, short fsCaptionId, DateTime fsDate)
        {
            var customerFS =await (from a in context.TBL_CUSTOMER_FS_CAPTION_DETAIL
                             where a.CUSTOMERID == customerId && a.FSDATE == fsDate && a.DELETED == false
                             select a).ToListAsync();

            var ratios = await(from a in context.TBL_CUSTOMER_FS_RATIO_DETAIL
        ...
        public async Task<bool> UpdateFSRatioDetail(int ratioDetailId, CustomerFSRatioDetailViewModel model)
        {
            var data =await context.TBL_CUSTOMER_FS_RATIO_DETAIL.FindAsync(ratioDetailId);
            if (data == null) return false;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated FS Ratio Detail")} {data.TBL_CUSTOMER_FS_CAPTION} {TranslateHelper.get("with divisor type")} '{audit_divisor}' {TranslateHelper.get("and value typ")} '{audit_value }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> DeleteFSRatioDetail(int ratioDetailId, UserInfo user)
        {
            var data =await context.TBL_CUSTOMER_FS_RATIO_DETAIL.FindAsync(ratioDetailId);
            data.DELETED = true;
            data.DELETEDBY = (int)user.staffId;
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Deleted FS Ratio Detail")} {data.TBL_CUSTOMER_FS_CAPTION} {TranslateHelper.get("with divisor type")} '{audit_divisor}' {TranslateHelper.get("and value type")} '{audit_value }' ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> DeleteMultipleFSRatioDetail(List<int> ratioDetailId, UserInfo user)
        {
            if (ratioDetailId.Count <= 0)
                return false;

            foreach (int fsdetailId in ratioDetailId)
                await DeleteFSRatioDetail(fsdetailId, user);
        ...
        public async Task<IEnumerable<CustomerFSRatioDivisorTypeViewModel>> GetAllDivisorType()
        {
            var data =await (from a in context.TBL_CUSTOMER_FS_RATIO_DIVI_TYP
        ...
        public async Task<IEnumerable<CustomerFSRatioValueTypeViewModel>> GetAllValueType()
        {
            var data = await(from a in context.TBL_CUSTOMER_FS_RATIO_VALUETYP
                        where a.DELETED == false
