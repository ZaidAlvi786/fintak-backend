...
namespace FintrakBanking.Repositories.Customer
{
    public class KYCDocumentUploadRepository : IKYCDocumentUploadRepository
    {
        ...
        public async Task<bool> KYCDocumentUpload(CustomerDocumentUploadViewModel model, byte[] file)
        {
            ...
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("KYC Document")} '{ model.documentTitle }' ",
        ...
        public async Task<int> CheckListDocumentUpload(CheckListDocumentUploadViewModel model, byte[] file)
        {
            ...
            //{
                ...
                this.audit.AddAuditTrail(new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Added Checklist Document for item with ID:")} '{ model.checkListDefinitionId }' ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = general.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<CheckListDocumentUploadViewModel> CheckListDocumentUploadViewModel(int definitionId, int statusId, int detailId, bool isProductBased, int? customerId=null,int? checkListItemId = null, int? checkListTypeId=null, DateTime ? checklistDate = null)
        {
            if (checkListTypeId == (int)CheckListTypeEnum.RegulatoryChecklist)
            {
                ...
                {
                    var customerCreditBureauId = await (from q in _finContext.TBL_CUSTOMER_CREDIT_BUREAU 
                                                  ...
                                                  select q.CUSTOMERCREDITBUREAUID).FirstOrDefaultAsync();

                     var checklistDoc =  await(from ck in context.TBL_CUSTOMER_CREDIT_BUREAU
            ...
            if (isProductBased)
            {
                var checklistDoc = await (from ck in context.TBL_MEDIA_CHECKLIST_DOCUMENTS
            ...
            else
            {
                ...
                {
                    checklistDoc =  await (from ck in context.TBL_MEDIA_CHECKLIST_DOCUMENTS
                                    where ck.CHECKLISTDEFINITIONID == definitionId
                                     // && ck.CHECKLISTSTATUSID == statusId
        ...
        public async Task<ConditionsPrecedentUploadViewModel> GetLoanConditionDocumentByConditionId(int conditionId,int loanApplicationId)
        {
            var checklistDoc = await (from ck in context.TBL_LOAN_CONDITION_DOCUMENTS
                                where ck.CONDITIONID == conditionId
        ...
        public async Task<ConditionsPrecedentUploadViewModel> GetLoanConditionDocumentBydocumentId(int documentId)
        {
            var checklistDoc = await (from ck in context.TBL_LOAN_CONDITION_DOCUMENTS
        ...
        public async Task<bool> DeleteConditionDocumentBydocumentId(int documentId, int staffId)
        {
            var checklistDoc = await context.TBL_LOAN_CONDITION_DOCUMENTS.FindAsync(documentId);
            if(checklistDoc != null)
            {
        ...
        public async Task<IEnumerable<ConditionsPrecedentUploadViewModel>> GetDeletedLoanConditionDocumentByContionId(int conditionId)
        {
            
                var data = await (from x in context.TBL_LOAN_CONDITION_DOCUMENTS
                            where
        ...
        public async Task<bool> ConditionsPrecedentDocumentUpload(ConditionsPrecedentUploadViewModel model, byte[] file)
        {
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = model.createdBy,
                    BRANCHID = (short)model.userBranchId,
                    DETAIL = $"{TranslateHelper.get("Added Condition Precedent Document for condition with ID:")} '{ model.conditionId }' ",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = model.applicationUrl,
                    APPLICATIONDATE = general.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
