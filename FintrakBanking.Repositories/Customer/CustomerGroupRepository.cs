...
namespace FintrakBanking.Repositories.Customer
{
    public class CustomerGroupRepository : ICustomerGroupRepository
    {
        ...
        public async Task<IEnumerable<KYCItemViewModel>> GetKYCItems(int companyId)
        {
            var kycItem = await (from d in context.TBL_KYC_ITEM
        ...
        public async Task<bool> AddKycItem(KYCItemViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added KYC Item:")} { entity.item  } ",
        ...
        public async Task<bool> UpdatedKycItem(int kYCItemId, KYCItemViewModel entity)
        {
            var data = await context.TBL_KYC_ITEM.Where(c => c.KYCITEMID == kYCItemId).SingleOrDefaultAsync();
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated KYC Item:")} { entity.item  } ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> AddCustomerGroup(CustomerGroupViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added Customer Group:")} { entity.groupName } {TranslateHelper.get("with Code:")} { entity.groupCode } ( { entity.groupName } )",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public bool AddTempCustomerGroup(CustomerGroupViewModel custGroupModel)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Customer Group:")} { custGroupModel.groupName } {TranslateHelper.get("with Code:")} { custGroupModel.groupCode } ( { custGroupModel.groupName } )",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> DeleteCustomerGroup(int groupId, UserInfo user)
        {
            var group = await context.TBL_CUSTOMER_GROUP.FindAsync(groupId);
            group.DELETED = true;
            group.DELETEDBY = (int)user.createdBy;
            group.DATETIMEDELETED = genSetup.GetApplicationDate();
            // Audit Section ---------------------------
            var entity =await context.TBL_CUSTOMER_GROUP.FirstOrDefaultAsync(x => x.CUSTOMERGROUPID == groupId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Deleted Customer Group:")} { entity.GROUPNAME } {TranslateHelper.get("with Code:")} { entity.GROUPCODE } ( { entity.GROUPNAME })",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<CustomerGroupViewModel>> GetAllTempCustomerGroups()
        {
            var data = await (from a in context.TBL_TEMP_CUSTOMER_GROUP
                        //join b in context.TBL_CUSTOMER_RISK_RATING on a.RISKRATINGID equals b.RISKRATINGID
                        //into bb from b in bb.DefaultIfEmpty()
                        where a.DELETED == false
                        select new CustomerGroupViewModel
        ...
        public async Task<bool> UpdateCustomerGroup(int customerGroupId, CustomerGroupViewModel entity)
        {
            var group = await this.context.TBL_CUSTOMER_GROUP.FindAsync(customerGroupId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Customer Group:")} { entity.groupName } {TranslateHelper.get("with Code:")} { entity.groupCode } ( { entity.groupName })",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<bool> UpdateCustomerGroupForApproval(int customerGroupId, CustomerGroupViewModel entity)
        {
            if (entity == null)
                return false;

            var existingTempGroup = await context.TBL_TEMP_CUSTOMER_GROUP.FirstOrDefaultAsync(x => x.GROUPCODE.ToLower() ==
            ...
            {
                throw new SecureException(TranslateHelper.get("Customer group is already undergoing approval"));
            }
        ...
        public async Task<bool> GoForApproval(ApprovalViewModel entity)
        {
            ...
            {
                var result = await ApproveCustomerGroup(entity.targetId, (short)workFlow.StatusId, entity);
                return result;
            }
        ...
        public async Task<bool> GoForGroupMappingApproval(ApprovalViewModel entity)
        {
            ...
            if (workFlow.NewState == (int)ApprovalState.Ended)
            {
                return await ApproveCustomerGroupMapping(entity.targetId, (short)workFlow.StatusId, entity);
            }
        ...
        private async Task<bool> ApproveCustomerGroup(int customerGroupId, short approvalStatusId, UserInfo user)
        {
            var customerGroupModel = await context.TBL_TEMP_CUSTOMER_GROUP.FindAsync(customerGroupId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Approved Customer Group")} '{customerGroupModel.GROUPNAME}' {TranslateHelper.get("with group code")}'{customerGroupModel.GROUPCODE}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public async Task<IEnumerable<CustomerGroupViewModel>> GetCustomerGroupsAwaitingApprovals(int staffId, int companyId)
        {
            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CustomerGroupCreation).ToList();

            return await (from c in context.TBL_TEMP_CUSTOMER_GROUP
        ...
        public async Task<bool> AddCustomerGroupMapping(CustomerGroupMappingViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                //StaffId = entity.createdBy,
                //BranchId = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Added Customer Group Mapping to customer:")} { customer } {TranslateHelper.get("with code:")} {entity.customerCode } {TranslateHelper.get("to group")}  ( { groupName } ) ",
                //IPAddress = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
        ...
        public bool AddTempCustomerGroupMapping(CustomerGroupMappingViewModel model)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Customer Group Mapping to customer:")} { customer } {TranslateHelper.get("with code:")} {model.customerCode } {TranslateHelper.get("to group")}  ( { groupName } ) ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> AddMultipleCustomerGroupMapping(List<CustomerGroupMappingViewModel> groupCustomers, int createdBy, short userBranchId, int companyId)
        {
            ...
            var oldGroupCustomers = await this.context.TBL_TEMP_CUSTOMER_GROUP_MAPPNG.Where(x => x.CUSTOMERGROUPID == groupCustomer.customerGroupId
            ...
            foreach (CustomerGroupMappingViewModel item in groupCustomers)
            {
                ...
                if (oldGroupCustomer == null && oldGroupCustomers.Count == 0) {
                    if (item.relationshipTypeId <= 0) {
                        throw new SecureException(TranslateHelper.get("Kindly select the 'Relationship Type'"));
                    }
                ...
                var groupName =await (from gr in this.context.TBL_CUSTOMER_GROUP where gr.CUSTOMERGROUPID == groupCustomer.customerGroupId select gr.GROUPNAME).FirstOrDefaultAsync();
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Added Customer Group Mapping to customer:")} { customerName } {TranslateHelper.get("with code:")} { newGroupCustomer.TBL_CUSTOMER.CUSTOMERCODE } {TranslateHelper.get("to group")}  ( { groupName } ) ",
                    //IPAddress =CommonHelpers.GetLocalIpAddress() ,
                    //Url = entity.applicationUrl,
                    APPLICATIONDATE = genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
                ...
                if (oldGroupCustomer != null) {
                    ...
                    var allCust = context.TBL_CUSTOMER_GROUP_MAPPING.Where(O => O.CUSTOMERGROUPID == oldGroupCustomer.CUSTOMERGROUPID).ToList();
        ...
        public async Task<IEnumerable<CustomerGroupMappingViewModel>> GetCustomerGroupMapping()
        {
            var customerGroupMapping = await (from a in context.TBL_CUSTOMER_GROUP_MAPPING
                                              where a.DELETED == false
        ...
        public async Task<CustomerGroupMappingViewModel> GetCustomerGroupMappingByGroupMapId(int groupMapId)
        {
            var customerGroupMapping =await( from a in context.TBL_CUSTOMER_GROUP_MAPPING
        ...
        public async Task<IEnumerable<CustomerGroupMappingViewModel>> GetCustomerGroupMappingByGroupId(int customerGroupId)
        {
            var customerGroupMapping = await (from a in context.TBL_CUSTOMER_GROUP_MAPPING
        ...
        public async Task<IEnumerable<CustomerGroupMappingViewModel>> GetTempCustomerGroupMappingByGroupId(int customerGroupId)
        {
            var customerGroupMapping =await (from a in context.TBL_TEMP_CUSTOMER_GROUP_MAPPNG
                                       where a.CUSTOMERGROUPID == customerGroupId && a.DELETED == false
                                       select new CustomerGroupMappingViewModel
                                       {
                                           customerGroupMappingId = a.CUSTOMERGROUPMAPPINGID,
        ...
        public async Task<IEnumerable<GroupCustomerMembersViewModel>> GetGroupMembersByGroupId(int customerGroupId, int companyId)
        {
            try
            {
                List<GroupCustomerMembersViewModel> lstCustomer = new List<GroupCustomerMembersViewModel>();

                var data =await (from b in context.TBL_CUSTOMER_GROUP_MAPPING
        ...
        public async Task<bool> DeleteCustomerGroupMaping(int groupMapId, UserInfo user)
        {
            var groupMap = await context.TBL_CUSTOMER_GROUP_MAPPING.FindAsync(groupMapId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Deleted Customer Group Mapping for customer:")} { customer} {TranslateHelper.get("with Code:")} {groupMap.TBL_CUSTOMER.CUSTOMERCODE} {TranslateHelper.get("to group")} ({customerGroupName })",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateCustomerGroupMapping(int groupMapId, CustomerGroupMappingViewModel entity)
        {
            var groupMap =await context.TBL_CUSTOMER_GROUP_MAPPING.FindAsync(groupMapId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                ////StaffId = entity.createdBy,
                //BranchId = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Customer Group Mapping for customer:")} { customer }  {TranslateHelper.get("with code:")}  { groupMap.TBL_CUSTOMER.CUSTOMERCODE } {TranslateHelper.get("to group")} ( {groupName } ) ",
                //IPAddress = entity.userIPAddress,
                //Url = entity./*applicationUrl*/,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> UpdateCustomerGroupMappingForApproval(int groupMapId, CustomerGroupMappingViewModel model)
        {
            if (model == null)
                return false;
            ...
            var targetGroupMapping =await this.context.TBL_CUSTOMER_GROUP_MAPPING.FindAsync(groupMapId);
            ...
            {
                throw new SecureException(TranslateHelper.get("Customer group map is already undergoing approval"));
            }
            else
            {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Customer Group Mapping for customer:")} { customer }  {TranslateHelper.get("with code:")}  { targetGroupMapping.TBL_CUSTOMER.CUSTOMERCODE } {TranslateHelper.get("to group")} ( {groupName } ) ",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = model.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteCustomerGroupMapping(int groupMapId, UserInfo user)
        {
            var groupMap =await context.TBL_CUSTOMER_GROUP_MAPPING.FindAsync(groupMapId);
        ...
        private async Task<bool> ApproveCustomerGroupMapping(int customerGroupMapId, short approvalStatusId, UserInfo user)
        {
            var customerGroupMapModel = await context.TBL_TEMP_CUSTOMER_GROUP_MAPPNG.FindAsync(customerGroupMapId);
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Approved Customer Group Mapping")} '{customerGroupMapModel.CUSTOMERGROUPMAPPINGID}'",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<CustomerGroupMappingViewModel>> GetCustomerGroupMapsAwaitingApprovals(int staffId, int companyId)
        {
            /*var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CustomerGroupCreation).ToList();*/ 

            var ids = genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CustomerGroupMapping).ToList();

            var data =await (from c in context.TBL_TEMP_CUSTOMER_GROUP_MAPPNG
        ...
        public async Task<IEnumerable<LookupViewModel>> GetCustomerGroupRelationshipTypes()
        {
            return await ( from a in context.TBL_CUSTOMER_GROUP_RELATN_TYPE
        ...
        public async Task<bool> AddCustomerGroupRelationshipTypes(LookupViewModel model)
        {
            if (model.lookupId > 0)
            {
                var type = await context.TBL_CUSTOMER_GROUP_RELATN_TYPE.FirstOrDefaultAsync(x => x.RELATIONSHIPTYPEID == model.lookupId);
        ...
        private async Task<IQueryable<CustomerGroupViewModel>> GellAllCustomerGroupMappings()
        {
            var data = await (from a in context.TBL_CUSTOMER_GROUP
                        where a.DELETED == false
                        select new CustomerGroupViewModel
                        {
                            customerGroupId = a.CUSTOMERGROUPID,
        ...
        public async Task<IQueryable<CustomerGroupViewModel>> SearchForCustomerGroupRealtime(int companyId, string searchQuery)
        {
            ...
            {
                allGroups = await GellAllCustomerGroupMappings();
                    //.Where(c => c.companyId == companyId)
                    allGroups = allGroups.Where(x => x.customerGroupName.Contains(searchQuery)
        ...
        public async Task<List<CurrentCustomerExposure>> GetGroupExposureByGroupId(int customerGroupId, int companyId)
        { 
            var exposures = new List<CurrentCustomerExposure>();
            var customers =await  GetCustomerGroupMappingByGroupId(customerGroupId);
            customers= (IEnumerable<CustomerGroupMappingViewModel>)customers.Select(c => new { c.customerGroupId, c.customerName, c.customerId }).ToList();
        ...
        public async Task<List<CurrentCustomerExposure>> GetGroupExposureByCustomerId(int customerId, int companyId)
        {
            List<CurrentCustomerExposure> exposures = new List<CurrentCustomerExposure>();
            var customerGroups = await GetCustomerGroupMapping();
            customerGroups=customerGroups.Where(m => m.customerGroupId == customerId).ToList();
            ...
            {
                exposures.AddRange(await GetGroupExposureByGroupId(group.customerGroupId, companyId));
            }
            return exposures;
        }

        public async Task<IEnumerable<CustomerGroupViewModel>> SearchForCustomerGroup(int companyId, string searchQuery)
        {
            if (!string.IsNullOrWhiteSpace(searchQuery))
            {
        ...
        public async Task<IEnumerable<CustomerGroupMappingViewModel>> GetAllCustomerGroupMappingByGroupId(int customerGroupId)
        {
            var GroupMappings = await (from b in context.TBL_CUSTOMER_GROUP_MAPPING
                                 join c in context.TBL_CUSTOMER on b.CUSTOMERID equals c.CUSTOMERID
        ...
        {
            var customerGroups = GetAllCustomerGroups();
            if (!string.IsNullOrWhiteSpace(search))
            {
                customerGroups = (IQueryable<CustomerGroupViewModel>)customerGroups.Where(x =>
        ...
        public async Task<CustomerGroupViewModel> GetCustomerGroupDetailsByGroupId(int customerGroupId)
        {
            var data =await (from a in context.TBL_CUSTOMER_GROUP
