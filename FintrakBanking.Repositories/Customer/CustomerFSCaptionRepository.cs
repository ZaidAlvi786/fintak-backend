...
namespace FintrakBanking.Repositories.Customer
{
    ...
    public class CustomerFSCaptionRepository : ICustomerFSCaptionRepository
    {
        ...
        public async Task<bool> AddCustomerFSCaption(CustomerFSCaptionViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Customer FS Caption :")} { data.FSCAPTIONNAME } {TranslateHelper.get("in group")} ( {auditInfo.FSCAPTIONGROUPNAME })",
        ...
        public async Task<bool> DeleteCustomerFSCaption(int fsCaptionId, UserInfo user)
        {
            var data =await context.TBL_CUSTOMER_FS_CAPTION.FindAsync(fsCaptionId);
            data.DELETED = true;
            data.DELETEDBY = (int)user.createdBy;
            data.DATETIMEDELETED = _genSetup.GetApplicationDate();

            // Audit Section ---------------------------
            var auditInfo =await context.TBL_CUSTOMER_FS_CAPTION_GROUP.FirstOrDefaultAsync(x => x.FSCAPTIONGROUPID == data.FSCAPTIONGROUPID);
            var audit = new TBL_AUDIT
            {
                AUDITTYPEID = (short)AuditTypeEnum.CustomerFSCaptionDeleted,
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("Deleted Customer FS Caption:")} { data.FSCAPTIONNAME } {TranslateHelper.get("in group")} ( {auditInfo.FSCAPTIONGROUPNAME })",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<IEnumerable<CustomerFSCaptionViewModel>> GetCustomerFSCaptionByGroupId(short fsCaptionGroupId)
        {
        ...
        public async Task<IEnumerable<CustomerFSCaptionViewModel>> GetCustomerFSCaptions()
        {
            var data =await (from a in context.TBL_CUSTOMER_FS_CAPTION
        ...
        public async Task<CustomerFSCaptionViewModel> GetCustomerFSCaptionById(int fsCaptionId)
        {
            var data = await (from a in context.TBL_CUSTOMER_FS_CAPTION
        ...
        public async Task<IEnumerable<CustomerFSCaptionViewModel>> GetUnmappedCustomerFSCaption(short fsCaptionGroupId, int customerId, DateTime fsDate)
        {
            var dataList =await (from data in context.TBL_CUSTOMER_FS_CAPTION_DETAIL
                            ...
                            select data.FSCAPTIONID).ToListAsync();

            var captions =await (from a in context.TBL_CUSTOMER_FS_CAPTION
            ...
            if (dataList.Any())
            {
                captions = (List<CustomerFSCaptionViewModel>)captions.Where(x => !dataList.Contains(x.fsCaptionId));
            }

            return captions;
        }

        public async Task<IEnumerable<CustomerFSCaptionViewModel>> GetUnmappedCustomerGroupFSCaption(short fsCaptionGroupId, int customerGroupId, DateTime fsDate)
        {
            var dataList = await (from data in context.TBL_CUSTOMER_GRP_FS_CAPTN_DET
        ...
        public async Task<bool> UpdateCustomerFSCaption(int fsCaptionId, CustomerFSCaptionViewModel entity)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = entity.createdBy,
                BRANCHID = (short)entity.userBranchId,
                DETAIL = $"{TranslateHelper.get("Updated Customer FS Caption :")} { data.FSCAPTIONNAME }  {TranslateHelper.get("in group")} ( {auditInfo.FSCAPTIONGROUPNAME })",
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = entity.applicationUrl,
                DEVICENAME = CommonHelpers.GetDeviceName(),
                OSNAME = CommonHelpers.FriendlyName(),
