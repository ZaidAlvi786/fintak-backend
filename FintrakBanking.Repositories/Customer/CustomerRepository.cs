...
namespace FintrakBanking.Repositories.Customer
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public async Task<dynamic> GetCustomerRating(int custormerId)
        {

            var data = await (from c in context.TBL_CUSTOMER  
        ...
        public string AddCustomer(CustomerViewModels entity)
        {
            ...
            {
                throw new ConditionNotMetException($"{TranslateHelper.get("Customer with customer code")} {entity.customerCode} {TranslateHelper.get("already exist")}");
            }
            if (ValidateModifiedCustomerRecord(entity.customerId))
            {
                throw new ConditionNotMetException(TranslateHelper.get("Customer General Information is already undergoing approval."));
            }
            ...
            if (entity.customerTypeId == (short)CustomerTypeEnum.Individual)
            {
                ...
                else if (entity.maritalStatus.ToLower() == "1")
                {
                    maritalStatus = 1;
                }
                else if(entity.maritalStatus.ToLower() == "2")
                {
                    maritalStatus = 2;
                }
                else if(entity.maritalStatus.ToLower() == "3")
                {
                    maritalStatus = 3;
                }
                else if(entity.maritalStatus.ToLower() == "4")
                {
                    maritalStatus = 4;
                }
                else
            ...
            var customer = new TBL_CUSTOMER
            {
                ...
                CUSTOMERNIN = entity.customerNationalId,
                ...
                CUSTOMERGLOBALNUMBER = entity.customerGlobalNumber,
                ISGLOBALCUSTOMER = entity.isGlobalCustomer,
                ISGLOBALNUMBERAUTOMATICALLYGENERATED = entity.isGlobalNumberAutomaticallyGenerated
            };
            ...
            var audit = new TBL_AUDIT
            {
                ...
                DETAIL = $"{TranslateHelper.get("Added Customer")}  '{entity.customerName}' {TranslateHelper.get("with Code:")} {entity.prospectCustomerCode}",
            ...
            try
            {
                ...
                if (ex.Message.Contains(TranslateHelper.get("This customer does not have an account linked"))) {
                    return result;
                }
        ...
        public async Task<bool> refreshCustomerAccount(int customerId)
        {
            bool result = false;

            var customer = await context.TBL_CUSTOMER.Where(x => x.CUSTOMERID == customerId).FirstOrDefaultAsync();
        ...
        public bool UpdateCustomerInformation(string customerCode, string accountNumber, int createdBy)
        {
            try
            {
                ...
                {
                    accountNumber = context.TBL_CASA.Where(c=>c.CUSTOMERID == currentCustomer.CUSTOMERID).FirstOrDefault().PRODUCTACCOUNTNUMBER;
        ...
        public async Task<bool> AddCustomerAddresses(CustomerAddressViewModels entity)
        {
            ...
            if (entity != null)
            {
                try
                {
                    ...
                    // {
                    address = await context.TBL_CUSTOMER_ADDRESS.Where(x=>x.ADDRESSID==entity.addressId).FirstOrDefaultAsync();
                    var customer = await context.TBL_CUSTOMER.FindAsync(entity.customerId);
                    ...
                    if (address != null && accountCompleted == false)
                    {
                        ...
                        address.DIGITALADDRESS = entity.digitalAddress;
                        address.STATEID = entity.stateId;
                        ...
                        auditDetail = $"{TranslateHelper.get("Updated new Customer Address for customer:")} {customer.CUSTOMERCODE}.  {TranslateHelper.get("from")}   {existingAddrss}   {TranslateHelper.get("to")}  {entity.address}";
                    ...
                    else if (address == null && accountCompleted == false)
                    {
                        ...
                        auditDetail = $"{TranslateHelper.get("Added new Customer Address for customer ID:")} + (" + entity.customerId + ") ";
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        ) //if customer address information has existing record being modified and approved, update it with the new change
                        {
                            ...
                            temp.DIGITALADDRESS = entity.digitalAddress;
                            temp.STATEID = entity.stateId;
        ...
        public async Task<bool> AddCustomerBvn(CustomerBvnViewModels entity)
        {
            if (entity != null)
            {
                try
                {
                    ...
                    {
                        customerBvn = await context.TBL_CUSTOMER_BVN.FindAsync(entity.customerBvnid);
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        STAFFID = entity.createdBy,
                        BRANCHID = (short)entity.userBranchId,
                        DETAIL = TranslateHelper.get("Added new Customer BVN for customer ID: + (") + entity.customerId + TranslateHelper.get(") "),
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                        URL = entity.applicationUrl,
                        APPLICATIONDATE = _genSetup.GetApplicationDate(),
                        SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddCustomerChildren(List<CustomerChildrenViewModel> models, int staffId, short BranchId)
        {
            if (models != null)
            {
                try
                {
                    ...
                    foreach (CustomerChildrenViewModel entity in models)
                    {
                        ...
                        {
                            child = await context.TBL_CUSTOMER_CHILDREN.FindAsync(entity.customerChildrenId);
                            if (child != null)
                            {
                        ...
                        var audit = new TBL_AUDIT
                        {
                            ...
                            STAFFID = staffId,
                            BRANCHID = BranchId,
                            DETAIL = TranslateHelper.get("Added new CUSTOMER CHILDREN for customer ID: + (") + entity.customerId + TranslateHelper.get(") "),
                            IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                            URL = entity.applicationUrl,
                            APPLICATIONDATE = _genSetup.GetApplicationDate(),
                            SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddCustomerPhoneContact(CustomerPhoneContactViewModels entity)
        {
            if (entity != null)
            {
                var auditDetail = string.Empty;
                ...
                try
                {
                    ...
                    phone = await context.TBL_CUSTOMER_PHONECONTACT.FindAsync(entity.phoneContactId);
                    ...
                    if (phone != null && accountCompleted == false)
                    {
                        ...
                        auditDetail = $"{TranslateHelper.get("Updated new Customer Phone Number for customer:")} {customer.CUSTOMERCODE}. {TranslateHelper.get("from")} {previousPhoneNumber} {TranslateHelper.get("to")} {entity.phoneNumber}";
                    ...
                    else if (phone == null && accountCompleted == false)
                    {
                        ...
                        context.TBL_CUSTOMER_PHONECONTACT.Add(phone);
                        auditDetail = TranslateHelper.get("Added new Customer Phone Number for customer ID:") + entity.customerId;
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        var existingTempPhone = await context.TBL_TEMP_CUSTOMER_PHONCONTACT.FirstOrDefaultAsync(x =>
        ...
        public async Task<bool> AddCustomerNextOfKin(CustomerNextOfKinViewModels entity)
        {
            if (entity != null)
            {
                var auditDetail = string.Empty;
                ...
                try
                {
                    ...
                    next = await context.TBL_CUSTOMER_NEXTOFKIN.FindAsync(entity.nextOfKinId);
                    ...
                    if (next != null && accountCompleted == false)
                    {
                        ...
                        auditDetail = $"{TranslateHelper.get("Updated new Customer Next Of Kin for customer:")} {customer.CUSTOMERCODE}. {TranslateHelper.get("from")} {previousNextOfKin} {TranslateHelper.get("to")} {entity.firstName}";
                    ...
                    else if (next == null && accountCompleted == false)
                    {
                        ...
                        auditDetail = TranslateHelper.get("Added Customer's Next Of Kin for customer ID:") + entity.customerId;
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        var existingTempNext = await context.TBL_TEMP_CUSTOMER_NEXTOFKIN.FirstOrDefaultAsync(x =>
        ...
        public async Task<bool> AddCustomerCompanyInfomation(CustomerCompanyInfomationViewModels entity)
        {
            if (entity != null)
            {
                try
                {
                    ...
                    {
                        company = await context.TBL_CUSTOMER_COMPANYINFOMATION.FindAsync(entity.companyInfomationId);
                        //If Customer main table ACCOUNTCREATIONCOMPLETE column equal false and entity.canModified equal true, record insert directly to the main table 
                        ...
                        else //If customer main table AccountCreationCompleted equals true then save record in temp table
                        {
                            ...
                            var existingTempCompany = await context.TBL_TEMP_CUSTOMER_COMPANYINFO.FirstOrDefaultAsync(x =>
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        STAFFID = entity.createdBy,
                        BRANCHID = (short)entity.userBranchId,
                        DETAIL = TranslateHelper.get("Added Customer's Company Information with CustomerId : ") + entity.customerId,
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                        URL = entity.applicationUrl,
                        APPLICATIONDATE = _genSetup.GetApplicationDate(),
                        SYSTEMDATETIME = DateTime.Now,
            ...
            int status)
        {
            ...
            foreach (var ent in entity)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = TranslateHelper.get("Added Customer's Customer Info for: ") + customer.FIRSTNAME + " "+ customer.LASTNAME +
                             TranslateHelper.get(" with Id: ") + info.CUSTOMERID + TranslateHelper.get(" to company") + info.COMPANYNAME + TranslateHelper.get(" on ") +
        ...
        public async Task<bool> AddCustomerCompanyDirector(CustomerCompanyDirectorsViewModels entity)
        {
            if (entity != null)
            {
                if (entity.companyDirectorTypeId == (int)CompanyDirectorTypeEnum.BoardMember)
                {
                    var promoterExistsForCompany = await context.TBL_CUSTOMER_COMPANY_DIRECTOR.Where(p => p.CUSTOMERID == entity.customerId && p.ISTHEPROMOTER == true).ToListAsync();
                    if (promoterExistsForCompany.Count() == 0 && !entity.isThePromoter)
                    {
                        throw new SecureException(TranslateHelper.get("A Director must be profiled as a promoter for this company!"));
                    }
                    if (promoterExistsForCompany.Count() == 1 && !entity.isThePromoter)
                    {
                        throw new SecureException(TranslateHelper.get("Only this Director is profiled as a promoter for this company"));
                    }
                }
                try
                {
                    TBL_CUSTOMER_COMPANY_DIRECTOR directors;
                    directors = await context.TBL_CUSTOMER_COMPANY_DIRECTOR.FindAsync(entity.companyDirectorId);
                    var accountCompleted = context.TBL_CUSTOMER.Find(entity.customerId).ACCOUNTCREATIONCOMPLETE;
                    ...
                    var CustomerRec = await context.TBL_CUSTOMER.FindAsync(entity.customerId);
                    //If Customer main table ACCOUNTCREATIONCOMPLETE column equal false, record insert directly to the main table 
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        var existingTempDirector = await context.TBL_TEMP_CUSTOMER_DIRECTOR.FirstOrDefaultAsync(x =>
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        BRANCHID = (short)entity.userBranchId,
                        DETAIL =
                            TranslateHelper.get("Added new TBL_CUSTOMER_IDENTIFICATION for customer ID: ") + entity.customerId,
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                        URL = entity.applicationUrl,
                        APPLICATIONDATE = _genSetup.GetApplicationDate(),
                        SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddCustomerIdentification(CustomerIdentificationViewModels entity)
        {
            if (entity != null)
            {
                try
                {
                    ...
                    {
                        identity =await  context.TBL_CUSTOMER_IDENTIFICATION.FindAsync(entity.identificationId);
                        if (identity != null)
                        {
        ...
        public async Task<bool> AddCustomerClientSupplier(CustomerClientOrSupplierViewModels entity)
        {
            if (entity != null)
            {
                try
                {
                    ...
                    clientSupplier = await context.TBL_CUSTOMER_CLIENT_SUPPLIER.FindAsync(entity.client_SupplierId);
                    var accountCompleted = context.TBL_CUSTOMER.Find(entity.customerId).ACCOUNTCREATIONCOMPLETE;
                    //If Customer main table ACCOUNTCREATIONCOMPLETE column equal false, record insert directly to the main table 
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        var existingTempCliSup = await context.TBL_TEMP_CUST_CLIENT_SUPPLIER.FirstOrDefaultAsync(x =>
                    ...
                    var audit = new TBL_AUDIT
                    {
                        ...
                        STAFFID = entity.createdBy,
                        BRANCHID = (short)entity.userBranchId,
                        DETAIL = TranslateHelper.get("Added new TBL_CUSTOMER_CLIENT_SUPPLIER for customer ID:") + entity.customerId,
                        IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                        URL = entity.applicationUrl,
                        APPLICATIONDATE = _genSetup.GetApplicationDate(),
                        SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddCustomerEmploymentHistory(CustomerEmploymentHistoryViewModels entity)
        {
            if (entity != null)
            {
                var auditDetail = string.Empty;
                ...
                try
                {
                    ...
                    history = await context.TBL_CUSTOMER_EMPLOYMENTHISTORY.FindAsync(entity.placeOfWorkId);
                    var accountCompleted = context.TBL_CUSTOMER.Find(entity.customerId).ACCOUNTCREATIONCOMPLETE;
                    //If Customer main table ACCOUNTCREATIONCOMPLETE column equal false, record insert directly to the main table 
                    if (history != null && accountCompleted == false)
                    {
                        ...
                        auditDetail = TranslateHelper.get("Updated Customer Employment History for customer ID:") + (entity.customerId);
                        auditType = (short)AuditTypeEnum.CustomerDetailUpdated;
                    }
                    else if (history == null && accountCompleted == false)
                    {
                        ...
                        history.APPROVEDEMPLOYERID = entity.approvedEmployerId;
                        context.TBL_CUSTOMER_EMPLOYMENTHISTORY.Add(history);
                        auditDetail = TranslateHelper.get("Added Customer Employment History for customer ID:") + (entity.customerId);
                        auditType = (short)AuditTypeEnum.CustomerDetailAdded;
                    ...
                    else //If customer main table AccountCreationCompleted equals true then save record in temp table
                    {
                        ...
                        var existingTempAddress = await context.TBL_TEMP_CUSTOMEREMPLOYMENT.FirstOrDefaultAsync(x =>
        ...
        public async Task<bool> DeleteChild(int childId, UserInfo user)
        {
            ...
            if (child != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    DETAIL = TranslateHelper.get("Deleted Child: ") + child.CHILDNAME + TranslateHelper.get(" of customer with  Code : ") + child.TBL_CUSTOMER.CUSTOMERCODE,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
        ...
        public async Task<bool> DeleteCustomer(int customerId, UserInfo user)
        {
            ...
            if (customer != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.staffId,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = TranslateHelper.get("Deleted Customer: ") + customer.LASTNAME + TranslateHelper.get(" with code: ") + customer.CUSTOMERCODE,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> DeleteUltimateBeneficial(int companyBeneficialId, UserInfo user)
        {
            var ultimate = await context.TBL_CUSTOMER_COMPANY_BENEFICIA.FindAsync(companyBeneficialId);
            ...
            if (ultimate != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.staffId,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = TranslateHelper.get("Deleted Company ultimate beneficial: ") + ultimate.SURNAME + " " + ultimate.SURNAME +TranslateHelper.get(" for: ") + ultimate.TBL_CUSTOMER_COMPANY_DIRECTOR.FIRSTNAME,
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<CustomerViewModels> GetCustomer(int custormerId)
        {
            var data = await (from a in context.TBL_CUSTOMER
                       ...
                       select new CustomerViewModels
                       {
                           ...
                           maritalStatus = a.MARITALSTATUS.Value == 1 ? "Single" : a.MARITALSTATUS.Value == 2 ? "Married" : a.MARITALSTATUS.Value == 3 ? "Divorced" : a.MARITALSTATUS.Value == 4 ? "Widowed" : null,
                           title = a.TITLE,
            ...
            return from a in context.TBL_CUSTOMER
                   ...
                   select
                       ...
                       new CustomerViewModels
                       {
                           ...
                           ownership = a.OWNERSHIP,
                           customerBVN = a.CUSTOMERBVN,
                           customerTypeName = context.TBL_CUSTOMER_TYPE.Where(c => c.CUSTOMERTYPEID == a.CUSTOMERTYPEID).FirstOrDefault().NAME,
                           nationality = context.TBL_COUNTRY.Where(c => c.COUNTRYID == a.NATIONALITYID).FirstOrDefault().NAME,
                           customerNationalId = a.CUSTOMERNIN,
        ...
        IQueryable<CustomerViewModels> GetCustomersLite()
        {
            return context.TBL_CUSTOMER.Where(x => x.DELETED == false).Select(a => new CustomerViewModels
            {
                ...
                lastName = a.LASTNAME,
                maidenName = a.MAIDENNAME,
                maritalStatus = a.MARITALSTATUS.Value.ToString(), //a.MARITALSTATUS.Value == 1 ? "Single" : a.MARITALSTATUS.Value == 2 ? "Married" : a.MARITALSTATUS.Value == 3 ? "Divorced" : a.MARITALSTATUS.Value == 4 ? "Widowed" : null,
                title = a.TITLE,
                middleName = a.MIDDLENAME,
                ...
                corr = a.CORR,
                ownership = a.OWNERSHIP,
                customerGlobalNumber = a.CUSTOMERGLOBALNUMBER,
                isGlobalCustomer = a.ISGLOBALCUSTOMER,
                isGlobalNumberAutomaticallyGenerated = a.ISGLOBALNUMBERAUTOMATICALLYGENERATED,
                phoneNumber = context.TBL_CUSTOMER_PHONECONTACT.Where(c=>c.CUSTOMERID==a.CUSTOMERID).Select(c=>c.PHONENUMBER).FirstOrDefault(),
            });
        }

        public async Task<IEnumerable<CustomerViewModels>> GetCustomerInGroupByGroupId(int groupId)
        {
            var data = await (from cs in context.TBL_CUSTOMER_GROUP_MAPPING
        ...
        public async Task<IEnumerable<CustomerTypeViewModels>> GetCustomerType()
        {
            var type = await (from a in context.TBL_CUSTOMER_TYPE
                       select new CustomerTypeViewModels
                       {
                           isHybrid = a.ISHYBRID,
                           name = a.NAME,
                           customerTypeId = a.CUSTOMERTYPEID
                       }).Where(x => x.isHybrid == false).ToListAsync();
            return type;
        }

        public async Task<IEnumerable<CorporateCustomerTypeViewModels>> GetCorporateCustomerType()
        {
            var type = await (from a in context.TBL_CORPORATE_CUSTOMER_TYPE
        ...
        public async Task<IEnumerable<CustomerTypeViewModels>> GetCustomerTypeWithHybrid()
        {
            var type = await (from a in context.TBL_CUSTOMER_TYPE
                       select new CustomerTypeViewModels
                       {
        ...
        public async Task<IEnumerable<KYCDocumentTypeViewModel>> GetKYCDocumentType()
        {
            var type = await (from a in context.TBL_KYC_DOCUMENTTYPE
        ...
        public async Task<bool> UpdateCustomer(int customerId, CustomerViewModels entity)
        {
            ...
            //get customer record by id
            var customerMain = await context.TBL_CUSTOMER.FindAsync(customerId);
            ...
            if (customerMain != null && customerMain.ACCOUNTCREATIONCOMPLETE == false)// && entity.canModified == true)
            {
                ...
                customerMain.MARITALSTATUS = entity.maritalStatus == "Single" ? 1 : entity.maritalStatus == "Married" ?  2 : entity.maritalStatus == "Divorced" ? 3 : entity.maritalStatus == "Widowed" ? 4 : entity.maritalStatus == "" ? 0 : Convert.ToInt16(entity.maritalStatus);
                ...
                customerMain.CUSTOMERNIN = entity.customerNationalId;
                ...
                customerMain.CUSTOMERGLOBALNUMBER = entity.customerGlobalNumber;
                customerMain.ISGLOBALCUSTOMER = entity.isGlobalCustomer;
                customerMain.ISGLOBALNUMBERAUTOMATICALLYGENERATED = entity.isGlobalNumberAutomaticallyGenerated;
            ...
            else
            {
                ...
                var existingTempCustomer = await context.TBL_TEMP_CUSTOMER.FirstOrDefaultAsync(x =>
                ...
                if (existingTempCustomer != null)
                {
                    ...
                    customer.MARITALSTATUS = entity.maritalStatus == "Single" ? 1 : entity.maritalStatus == "Married" ? 2 : entity.maritalStatus == "Divorced" ? 3 : entity.maritalStatus == "Widowed" ? 4 : entity.maritalStatus == "" ? 0 : Convert.ToInt16(entity.maritalStatus);
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = entity.createdBy,
                    BRANCHID = (short)entity.userBranchId,
                    DETAIL = $"{TranslateHelper.get("The Customer First Name for")} '{ entity.customerName }' {TranslateHelper.get("with code:")} '{entity.customerCode}' " +
                    $"{TranslateHelper.get("on")}  ({ entity.customerId })  {TranslateHelper.get("Has Been Updated from")} '{previousCustomerName}' {TranslateHelper.get("to")} '{entity.firstName}' ",
                    URL = entity.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
                    DEVICENAME = CommonHelpers.GetDeviceName(),
        ...
        public async Task<IEnumerable<CustomerViewModels>> SearchRandomCustomerBySearchQuery(string searchQuery)
        {
            var customers = await (from x in GetCustomersLite()
        ...
        public async Task<IEnumerable<CustomerViewModels>> SearchRandomCustomersBySearchQuery(string searchQuery)
        {
            var singleCustomers = await SearchRandomSingleCustomersBySearchQuery(searchQuery);
        ...
        public async Task<IEnumerable<CustomerViewModels>> SearchRandomSingleCustomersBySearchQuery(string searchQuery)
        {
            var customerGroup = await (from m in context.TBL_CUSTOMER_GROUP_MAPPING
        ...
        public async Task<CustomerViewModels> GetSingleCustomerGeneralInfo(string customerCode)
        {
            //int? maritalStatus = null;
            ...
            var data = await (from a in context.TBL_CUSTOMER
                        ...
                        select new CustomerViewModels
                        {
                            ...
                            maidenName = a.MAIDENNAME,
                            maritalStatus = a.MARITALSTATUS.Value == 1 ? "Single" : a.MARITALSTATUS.Value == 2 ? "Married" : a.MARITALSTATUS.Value == 3 ? "Divorced" : a.MARITALSTATUS.Value == 4 ? "Widowed" : null,
                            nationality = context.TBL_COUNTRY.Where(f => f.COUNTRYID == a.NATIONALITYID).Select(f => f.NAME).FirstOrDefault(),
                            title = a.TITLE,
                            middleName = a.MIDDLENAME,
                            ...
                            relationshipOfficerName = context.TBL_STAFF.Where(f => f.STAFFID == a.RELATIONSHIPOFFICERID)
                                .Select(f => f.FIRSTNAME + " " + f.LASTNAME).FirstOrDefault(),
                            riskRatingName = a.TBL_CUSTOMER_RISK_RATING.RISKRATING,
                            customerBVN = a.CUSTOMERBVN,
        ...
        public CustomerViewModels GetSingleCustomerGeneralInfoByCustomerId(int customerId)
        {
            var data = (from a in context.TBL_CUSTOMER
                        ...
                        select new CustomerViewModels
                        {
                            ...
                            emailofSignatories = a.EMAILOFSIGNATORY,
                            bvnNumberofSignatories = a.BVNNUMBEROFSIGNATORY,
                            customerGlobalNumber = a.CUSTOMERGLOBALNUMBER == null ? null : a.CUSTOMERGLOBALNUMBER,
                            isGlobalCustomer = a.ISGLOBALCUSTOMER,
                            isGlobalNumberAutomaticallyGenerated = a.ISGLOBALNUMBERAUTOMATICALLYGENERATED
                        }).FirstOrDefault();
            //if (USE_THIRD_PARTY_INTEGRATION)
        ...
        public CustomerEmploymentHistoryViewModels GetSingleCustomerRelatedEmployer(int customerId)
        {
            ...
            {
                throw new SecureException(TranslateHelper.get("There cannot be more than one active current employer setup for a customer!"));
            }

            var relatedEmployer = employers.Where(e => e.active && e.employerId > 0).FirstOrDefault();
            if (relatedEmployer == null)
            {
                throw new SecureException(TranslateHelper.get("There is no active Related Employer setup for this customer!"));
            }
        ...
        public bool CustomerInformationCompleted(int customerId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Updated customer information for :") + data.FIRSTNAME + " " + data.LASTNAME,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<CustomerInformationApprovalViemModel>> GetAllCustomerInformationAwaitingApproval(int staffId,
            int companyId)
        {
            var ids = _genSetup.GetStaffApprovalLevelIds(staffId, (int)OperationsEnum.CustomerInformationApproval)
                .ToList();

            return await (from a in context.TBL_CUSTOMER_MODIFICATION
        ...
        private bool ApproveGeneralInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            if (entity != null) //Update existing customer information with temp record
            {
                ...
                detail = $"{TranslateHelper.get("Approved General Customer Information for customer with code:")} {entity.CUSTOMERCODE} {TranslateHelper.get("has been updated by")} {staff.FIRSTNAME} {staff.LASTNAME} ({staff.STAFFCODE})";
        ...
        private bool ApproveCompanyInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            {
                detail = $"{TranslateHelper.get("Approved Company Information for customer with code:")} : {customer.CUSTOMERCODE} {TranslateHelper.get("has been updated by")} {staff.FIRSTNAME} {staff.LASTNAME} ({staff.STAFFCODE})";
            ...
            else
            {
                ...
                corporateInfo.NOOFFEMALEEMPLOYEES = temp.NOOFFEMALEEMPLOYEES;
                corporateInfo.ISSTARTUP = temp.ISSTARTUP;
                corporateInfo.ISFIRSTTIMECREDIT = temp.ISFIRSTTIMECREDIT;
                corporateInfo.NUMBEROFEMPLOYEES = temp.NUMBEROFEMPLOYEES;
                corporateInfo.TOTALASSETS = temp.TOTALASSETS;
                corporateInfo.CORPORATECUSTOMERTYPEID = temp.CORPORATECUSTOMERTYPEID;
        ...
        private bool ApproveAddressInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            if (modified.MODIFICATIONTYPEID == (int)CustomerInformationTrackerEnum.Address_Addition)
            {
                    ...
                    if (temp != null) //If temp record is not null select the information from the main table
                    {
                        ...
                        tempModel.digitalAddress = temp.DIGITALADDRESS;
                        ...
                        detail = $"{TranslateHelper.get("Customer address for customer with code:")} : {temp.TBL_CUSTOMER.CUSTOMERCODE} {TranslateHelper.get("has been added.")} <br> {TranslateHelper.get("New address:")} <br>{recentData}";
                        ...
                        entity.DIGITALADDRESS = temp.DIGITALADDRESS;
                        entity.STATEID = temp.STATEID;
            ...
            else if (modified.MODIFICATIONTYPEID == (int)CustomerInformationTrackerEnum.Address_Modification)
            {
                ...
                if (entity != null)
                {
                    ...
                    entityModel.digitalAddress = entity.DIGITALADDRESS;
                    entityModel.stateId = entity.STATEID;
                    ...
                    if (temp != null) //If temp record is not null select the information from the main table
                    {
                        ...
                        detail = $"{TranslateHelper.get("Customer address for customer with code:")} : {temp.TBL_CUSTOMER.CUSTOMERCODE} {TranslateHelper.get("has been updated.")} {TranslateHelper.get("Existing data :")}<br> {existingData}. <br> {TranslateHelper.get("New Data:")} <br>{recentData}";
        ...
        private bool ApprovePhoneContactInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Customer Address Information:  with Id: ") + entity.PHONECONTACTID,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Customer Employment History Information:  with Id: ") + entity.PLACEOFWORKID,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        private bool ApproveNextOfKinInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Customer Next of Kin Information:  with Id: ") + entity.NEXTOFKINID,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
            ...
            UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Customer Top Client Supplier Information:  with Id: ") + entity.CLIENT_SUPPLIERID,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        private bool ApproveDirectorInformation(int modifiedId, int targetId, short approvalStatusId, UserInfo user)
        {
            ...
            var audit = new TBL_AUDIT
            {
                ...
                STAFFID = user.staffId,
                BRANCHID = (short)user.BranchId,
                DETAIL = TranslateHelper.get("Approved Customer Top Client Supplier Information:  with Id: ") + entity.COMPANYDIRECTORID,
                IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                URL = user.applicationUrl,
                APPLICATIONDATE = _genSetup.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSLegalStatus()
        {

            var data = await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.LegalStatusType).Select(x => new LookupViewModel()
            {
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSCompanySize()
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.CompanySize).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupcustomerId = x.CUSTOMERTYPEID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSRelationshipType()
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.RelationshipType).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupcustomerId = x.CUSTOMERTYPEID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSLegalStatusByType(int type)
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.LegalStatusType && x.CUSTOMERTYPEID == type).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupcustomerId = x.CUSTOMERTYPEID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
        ...
        public async Task<IEnumerable<LookupViewModel>> GetAllCRMSRelationshipTypeByType(int type)
        {
            return await context.TBL_CRMS_REGULATORY.Where(x => x.CRMSTYPEID == (int)RegulatoryTypeEnum.RelationshipType && x.CUSTOMERTYPEID == type).Select(x => new LookupViewModel()
            {
                lookupId = (short)x.CRMSREGULATORYID,
                lookupcustomerId = x.CUSTOMERTYPEID,
                lookupName = x.CODE + "-" + x.DESCRIPTION
        ...
        public async Task<IEnumerable<CustomerRelatedPartyViewModel>> GetCustomerRelatedParty(int customerId) 
        {
            var related = await (from a in context.TBL_CUSTOMER_RELATED_PARTY
                           join b in context.TBL_CUSTOMER on a.CUSTOMERID equals b.CUSTOMERID
        ...
        public async Task<bool> DeleteRelatedParty(int relatedPartyId, UserInfo user)
        {
            var child = await context.TBL_CUSTOMER_RELATED_PARTY.FindAsync(relatedPartyId);
            var customer =await  context.TBL_CUSTOMER.FirstOrDefaultAsync(c => c.CUSTOMERID == child.CUSTOMERID);
            ...
            if (child != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.staffId,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = $"{TranslateHelper.get("Deleted Customer Related Party with Related Party ID:")} {child.RELATEDPARTYID}  {TranslateHelper.get("and companydirectorId")} {child.COMPANYDIRECTORID}  {TranslateHelper.get("and relationship")} {child.RELATIONSHIPTYPE} {TranslateHelper.get("createdBy")} {child.CREATEDBY}",
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> Deleteaddress(int addressId, UserInfo user)
        {
            var child = await context.TBL_CUSTOMER_ADDRESS.FindAsync(addressId);
            ...
            if (child != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.staffId,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = TranslateHelper.get("Deleted Customer Related Party with Related Party ID: "),
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> Deletcontact(int phoneContactId, UserInfo user)
        {
            var child = await context.TBL_CUSTOMER_PHONECONTACT.FindAsync(phoneContactId);
        ...
        public async Task<bool> DeleteEmployment(int placeOfWorkId, UserInfo user)
        {
            var child = await context.TBL_CUSTOMER_EMPLOYMENTHISTORY.FindAsync(placeOfWorkId);
        ...
        public async Task<bool> DeleteNextOfKin(int nextOfKinId, UserInfo user)
        {
            var nextOfKin = await context.TBL_CUSTOMER_NEXTOFKIN.FindAsync(nextOfKinId);
            ...
            if (nextOfKin != null)
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = user.staffId,
                    BRANCHID = (short)user.BranchId,
                    DETAIL = TranslateHelper.get("Deleted Customer Next of Kin with Next of Kin ID: "),
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = user.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        public async Task<bool> AddUpdateCustomerRelatedParty(CustomerRelatedPartyViewModel entity)
        {
            ...
            try
            {
                var customer = await context.TBL_CUSTOMER.Where(r => r.CUSTOMERID == entity.customerId).FirstOrDefaultAsync();
                TBL_CUSTOMER_RELATED_PARTY relParty;
                if (entity.relatedPartyId > 0)
                {
                    relParty = await context.TBL_CUSTOMER_RELATED_PARTY.Where(c=>c.RELATEDPARTYID == entity.relatedPartyId).FirstOrDefaultAsync();
                    ...
                    if (relParty != null)
                    {
                        ...
                        auditDetail = $"{TranslateHelper.get("Updated new Customer's Insider Related Party Relationship Type for customer:")} {customer.CUSTOMERCODE} {TranslateHelper.get("from")} {previousRelationshipType} {TranslateHelper.get("to")} {entity.relationshipType}";
                        auditType = (short)AuditTypeEnum.CustomerDetailUpdated;
                        customer.ISREALATEDPARTY = true;
                    }
                ...
                else
                {
                    ...
                    auditDetail = $"{TranslateHelper.get("Added Customer's Insider Related Party for customer ID:")} + ({entity.customerId})";
                    auditType = (short)AuditTypeEnum.CustomerDetailAdded;
                }
        ...
        public async Task<bool> ValidateRelatedPartyEntry(int customerId, int companyDirectorId)
        {
            var exist = await (from a in context.TBL_CUSTOMER_RELATED_PARTY
        ...
        public bool UpdatePropectToCustomer(int customerId, CustomerViewModels entity)
        {
            try
            {
                ...
                var audit = new TBL_AUDIT
                {
                    ...
                    STAFFID = entity.createdBy,
                    BRANCHID = (short)entity.userBranchId,
                    DETAIL = TranslateHelper.get("Updated TBL_CUSTOMER: ") + entity.customerName + TranslateHelper.get(" with code: ") + entity.customerCode +
                             TranslateHelper.get(" on") + TranslateHelper.get(" (") + entity.customerId + TranslateHelper.get(") "),
                    IPADDRESS = CommonHelpers.GetLocalIpAddress(),
                    URL = entity.applicationUrl,
                    APPLICATIONDATE = _genSetup.GetApplicationDate(),
                    SYSTEMDATETIME = DateTime.Now,
        ...
        #endregion
        public async Task<IEnumerable<CustomerRelatedDirectorViewModel>> DirectorRelatedCustomer(string bvn)
        {
            List<CustomerRelatedDirectorViewModel> relatedCust = new List<CustomerRelatedDirectorViewModel>();


            relatedCust = await (from a in context.TBL_CUSTOMER
            ...
            var ultimateBeneficial = await (from a in context.TBL_CUSTOMER
                                      join b in context.TBL_CUSTOMER_COMPANY_DIRECTOR on a.CUSTOMERID equals b.CUSTOMERID
                                      join c in context.TBL_CUSTOMER_COMPANY_BENEFICIA on b.COMPANYDIRECTORTYPEID equals c.COMPANYDIRECTORID
        ...
        public async Task<IEnumerable<CustomerViewModels>> GetCustomerGeneralInfoByLMSLoanId(int loanApplicationId)
        {
            var loanCust = await (from a in context.TBL_LMSR_APPLICATION_DETAIL
                            where a.LOANAPPLICATIONID == loanApplicationId
                            select a.CUSTOMERID).ToListAsync();
