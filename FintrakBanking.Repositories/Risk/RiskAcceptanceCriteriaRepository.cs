    ...
    public class RiskAcceptanceCriteriaRepository : IRiskAcceptanceCriteriaRepository
    {
        ...
        public bool AddRacDefinition(RacDefinitionViewModel model)
        {
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = model.createdBy,
                BRANCHID = (short)model.userBranchId,
                DETAIL = $"{TranslateHelper.get("RAC Definition with id")} '{entity.RACDEFINITIONID}' {TranslateHelper.get("created by")} {auditStaff}",
                IPADDRESS = model.userIPAddress,
                URL = model.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now
        ...
        public bool UpdateRacDefinition(RacDefinitionViewModel model, int id, UserInfo user)
        {
            ...
            this.audit.AddAuditTrail(new TBL_AUDIT
            {
                ...
                STAFFID = user.createdBy,
                BRANCHID = (short)user.BranchId,
                DETAIL = $"{TranslateHelper.get("RAC Definition with id")} '{entity.RACDEFINITIONID}' {TranslateHelper.get("was updated by")} {auditStaff}",
                IPADDRESS = user.userIPAddress,
                URL = user.applicationUrl,
                APPLICATIONDATE = general.GetApplicationDate(),
                SYSTEMDATETIME = DateTime.Now,
