...
namespace FintrakBanking.Repositories.WorkFlow
{
    public class Workflow : IWorkflow
    {
        ...
        private bool isExternalSystemApprover;
        private int? businessUnitId = null;
        private bool isGlobalCustomer = false;
        ...
        private bool standardGrade = false;
        private bool renewalLimit = false;
        ...
        private bool isRemoteCall = false;
        ...
        public string StaffRoleCode { get { return staffRoleCode; } set { staffRoleCode = value; } }
        ...
        public bool StandardGrade { set { standardGrade = value; } }
        public bool RenewalLimit { set { renewalLimit = value; } }
        ...
        public int? ThisLevelId { get { return thisLevelId; } set { thisLevelId = value; } }
        ...
        public bool IsExternalSystemApprover { get { return isExternalSystemApprover; } set { isExternalSystemApprover = value; } }
        public int? BusinessUnitId { get { return businessUnitId; } set { businessUnitId = value; } }
        public bool IsGlobalCustomer { get { return isGlobalCustomer; } set { isGlobalCustomer = value; } }
        ...
        public bool IsRemoteCall { get { return isRemoteCall; } set { isRemoteCall = value; } }
        ...
        public bool LogActivity()
        {
            ...
            else
            {
                ...
                this.thisLevelId = lastOpenRequest.TOAPPROVALLEVELID;
            ...
            if (lastOpenRequest != null)
            {
                if (isRemoteCall) 
                {
                    var levelRole = context.TBL_STAFF_ROLE.FirstOrDefault(x => x.STAFFROLEID == level.DefaultRoleId);
                    this.staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == levelRole.STAFFROLECODE).FirstOrDefault().REMOTENETWORKID; 
                };
            ...
            if (this.nextLevelId > 0)
            {
                ...
                response.isExternalSystemApprover = this.isExternalSystemApprover = level.ISEXTERNALSYSTEMAPPROVER;
            ...
            if (isRemoteCall) //{ this.staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == staffRoleCode).FirstOrDefault().REMOTENETWORKID; };
            {
                var levelRole = context.TBL_STAFF_ROLE.FirstOrDefault(x => x.STAFFROLEID == level.DefaultRoleId);
                this.staffId = context.TBL_STAFF_REMOTE_NT_IDS.Where(x => x.STAFFROLECODE == levelRole.STAFFROLECODE).FirstOrDefault().REMOTENETWORKID;
            };
            this.approvalTrail = context.TBL_APPROVAL_TRAIL.Add(new TBL_APPROVAL_TRAIL
            {
                ...
                ISEXTERNALSYSTEMAPPROVER = this.isExternalSystemApprover
            });
        ...
        private void ValidateVote()
        {
            ...
            else if (this.statusId == (int)ApprovalStatusEnum.Reroute)
            {
                this.vote = (int)ApprovalStatusEnum.Reroute;
            }
            else if (this.statusId == (int)ApprovalStatusEnum.Escalated)
            {
        ...
        private void ValidateSourceConfiguration()
        {
            ...
            {
                var staffRoleId = context.TBL_STAFF_ROLE.Where(x => x.STAFFROLECODE == staffRoleCode).FirstOrDefault()?.STAFFROLEID ?? 0;

                valid = general.GetStaffApprovalLevelIds((int)this.staffId, this.operationId, staffRoleId, isRemoteCall).ToList().Contains((int)this.fromLevelId);
        ...
        private bool ResolveLevelConfigurations()
        {
            ...
            //if(IsExternalSystemApprover == false)
            //{
            //    if(levelBusinessRule.isGlobalCustomerRelated ) { }
            //}
            ...
            if (this.fromLevelId > 0) // check if staff in level
            {
                ...
                TBL_STAFF_ROLE levelRole = null;
                if (staff.Any() == false)
                {
                    defaultRole = context.TBL_STAFF.FirstOrDefault(x => x.STAFFID == this.staffId &&  x.STAFFROLEID == level.DefaultRoleId );
                }

                //===================For Group office sTaffRoleCode check =======================
                if (isRemoteCall && defaultRole == null)  //&& isRemoteCall
                {
                    levelRole = context.TBL_STAFF_ROLE.FirstOrDefault(x => x.STAFFROLEID == level.DefaultRoleId);
                }

                TBL_STAFF_RELIEF relieverStaff = null;
                if (defaultRole == null && levelRole == null)
                {
                ...
                if (staff.Any() == false && (defaultRole == null && levelRole == null) && relieverStaff == null && IsClassifiedReferBack == false)
                {
                    throw new SecureException("You are not in the current workflow level " + level.Level.LEVELNAME);
                }
                var remoteStaffRoleRecord = context.TBL_STAFF_ROLE.Where(x => x.STAFFROLECODE == staffRoleCode).FirstOrDefault();
                //if (isRemoteCall == true && levelRole != remoteStaffRoleRecord && staff.Any() == false && relieverStaff == null && IsClassifiedReferBack == false)
                if (isRemoteCall == true && levelRole == null && staff.Any() == false && relieverStaff == null && IsClassifiedReferBack == false)
                {
                    throw new SecureException("You are not in the current workflow level " + level.Level.LEVELNAME);
                }
        ...
        {
            if (amount == 0) { return true; }
            if (renewalLimit == false) { return true; }
            if (renewalLimit == true && level.RENEWALLIMITAMOUNT >= amount) { return true; }
            return false;
        }
        ...
        {
            if (amount == 0) { return true; }
            if (standardGrade == false) { return true; }
            if (standardGrade == true && level.STANDARDGRADEAMOUNT >= amount) { return true; }
            return false;
        }
        ...
        private IEnumerable<WorkflowSetup> GetWorkflowSetup(int operationId, int? productClassId, int? productId)
        {
            ...
            levels = mappings
                           ...
                           .Select(x => new WorkflowSetup
                           {
                               ...
                               StandardGradeAmount = x.Level.STANDARDGRADEAMOUNT,
                               RenewalLimitAmount = x.Level.RENEWALLIMITAMOUNT,
                               ...
                               DefaultRoleCode = x.Level.TBL_STAFF_ROLE?.STAFFROLECODE, // (from y in context.TBL_STAFF_ROLE where y.STAFFROLEID == x.Level.STAFFROLEID select y.STAFFROLECODE ).FirstOrDefault(),
                               ...
                               IsExternalSystemApprover = x.Level.ISEXTERNALSYSTEMAPPROVER
                           })
            ...
            foreach (WorkflowSetup level in levels)
            {
                ...
                grid.Add(new WorkflowSetup
                {
                    ...
                    StandardGradeAmount = level.StandardGradeAmount,
                    RenewalLimitAmount = level.RenewalLimitAmount,
        ...
        private bool LevelBusinessRuleIsValid(TBL_APPROVAL_BUSINESS_RULE rule)
        {
            ...
            bool globalCustomerChecked = false;
            ...
            // ================= Initial One Bank Customer Adjustment================

            //var cust = context.TBL_CUSTOMER.Where(x=>x.CUSTOMERID == customerId).FirstOrDefault();
            //var isglobal = cust?.ISGLOBALCUSTOMER ?? false;
            if (rule.ISGLOBALCUSTOMERRELATED && IsGlobalCustomer == true)
            {
                globalCustomerChecked = true;
            }
            else if (IsExternalSystemApprover == true && rule.ISGLOBALCUSTOMERRELATED == false)
            {
                globalCustomerChecked = false;
                this.IsExternalSystemApprover = !rule.ISGLOBALCUSTOMERRELATED ? this.IsExternalSystemApprover = false : this.IsExternalSystemApprover = true;
                //return IsExternalSystemApprover;
            }
            else
            {
                globalCustomerChecked = false;
                this.IsExternalSystemApprover = false;
            }
            ...
            if (globalCustomerChecked) return globalCustomerChecked;
            if (this.IsExternalSystemApprover) return this.IsExternalSystemApprover;
        ...
        private bool ResolveStandardRules(TBL_WORKFLOW_ITEM_EXPRESSION expression, TBL_APPROVAL_BUSINESS_RULE rule)
        {
            ...
            if (ruleBase.CONTEXTNAME.ToUpper() == "CREDIT REQUEST")
            {
                if (ruleItem.DATAITEMNAME.ToUpper() == "CURRENCY TYPE")
                {
                    if (ruleItem.VALUETYPEID == list)
                    {
                        var comparisonString = context.TBL_OPERATORS.Where(x => x.OPERATORID == expression.COMPARISONID).FirstOrDefault()?.OPERATOR.ToString();
                        ...
                        if (expression.IDVALUE == null) expression.IDVALUE = 0;
                        return Compare(currencyId.Value, expression.IDVALUE.Value, comparisonString);
                    }
                }
            }
            //if (ruleBase.CONTEXTNAME.ToUpper() == "CREDIT REQUEST")
            //{
            //    if (ruleItem.DATAITEMNAME.ToUpper() == "AMOUNT")
            //    {
            //        if (ruleItem.VALUETYPEID == text)
            //        {
            //            var comparisonString = context.TBL_OPERATORS.Where(x => x.OPERATORID == expression.COMPARISONID).FirstOrDefault()?.OPERATOR.ToString();
            //            if (expression.TEXTVALUE == "@PREVIOUS")
            //            {
            //                return Compare(lastOpenRequest.AMOUNT, amount, comparisonString);
            //            }
            //            else if (expression.TEXTVALUE != null)
            //            {
            //                var x = decimal.TryParse(expression.TEXTVALUE, out decimal result);
            //                if (x) return Compare(lastOpenRequest.AMOUNT, result, comparisonString); else return false;
            //            }
            //        }
            //    }
            //}
                ...
                {   
                    ...
                    if (levelWorkflowNotification?.NOTIFYOFPENDINGAPPROVALS ?? false)
                    {
                        ...
                        if (alert != null)
                        {
                            ...
                            if(this.fromLevelId == null && this.operationId == (short)OperationsEnum.CreditAppraisal)
                            {
                                status = "INITIATED";
                                fromLevelName = context.TBL_APPROVAL_LEVEL.Find(this.nextLevelId)?.LEVELNAME;

                                var appl = context.TBL_LOAN_APPLICATION.Find(this.targetId);
                                placeholders = new AlertPlaceholders();
                            ...
                            if(reciever?.EMAIL == null)
                            {
                                foreach (var email in nextLevelStaffEmails){
                                    LogWorkflowNotifications(this.support, email, alert.TITLE, messageBody);
                                }
