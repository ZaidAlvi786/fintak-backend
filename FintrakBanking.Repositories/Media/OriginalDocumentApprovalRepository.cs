...
namespace FintrakBanking.Repositories.Media
{
    public class OriginalDocumentApprovalRepository : IOriginalDocumentApprovalRepository
    {
        ...
        public IEnumerable<OriginalDocumentApprovalViewModel> GetOriginalDocumentApprovals(int staffId)
        {
            ...
            var dataCollateral = (from x in context.TBL_ORIGINAL_DOCUMENT_APPROVAL
                    ...
                    select new OriginalDocumentApprovalViewModel
                    {
                        ...
                        productName = context.TBL_PRODUCT.Where(p => p.PRODUCTID == (context.TBL_LOAN_APPLICATION.Where(a => a.APPLICATIONREFERENCENUMBER == x.APPLICATIONREFERNECENUMBER).Select(a => a.PRODUCTID).FirstOrDefault())).Select(p => p.PRODUCTNAME).FirstOrDefault(),
                        businessUnitId = context.TBL_STAFF.Where(a => a.STAFFID == initiator).FirstOrDefault().TBL_PROFILE_BUSINESS_UNIT.BUSINESSUNITNAME,
                        productPriceIndex = context.TBL_PRODUCT.Where(p => p.PRODUCTID == (context.TBL_LOAN_APPLICATION.Where(a => a.APPLICATIONREFERENCENUMBER == x.APPLICATIONREFERNECENUMBER).Select(a => a.PRODUCTID).FirstOrDefault())).Select(p => p.PRODUCTPRICEINDEXID).FirstOrDefault(),
                        applicationDate = x.DATETIMECREATED
            ...
            var dataFacility = (from x in context.TBL_ORIGINAL_DOCUMENT_APPROVAL
                                join o in context.TBL_LOAN_APPLICATION_DETAIL on x.LOANAPPLICATIONID equals o.LOANAPPLICATIONID
                                join c in context.TBL_CUSTOMER on o.CUSTOMERID equals c.CUSTOMERID
                                join atrail in context.TBL_APPROVAL_TRAIL on x.ORIGINALDOCUMENTAPPROVALID equals atrail.TARGETID
                                where x.DELETED == false
                                && (atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Processing || atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Referred || atrail.APPROVALSTATUSID == (int)ApprovalStatusEnum.Finishing)
                                ...
                                select new OriginalDocumentApprovalViewModel
                                {
                                    businessUnitId = context.TBL_PROFILE_BUSINESS_UNIT.Where(b => b.BUSINESSUNITID == c.BUSINESSUNTID).Select(b => b.BUSINESSUNITNAME).FirstOrDefault(),
                                    ...
                                    productPriceIndex = context.TBL_PRODUCT.Where(p => p.PRODUCTID == o.PROPOSEDPRODUCTID).Select(p => p.PRODUCTPRICEINDEXID).FirstOrDefault(),
                                    originalDocumentApprovalId = x.ORIGINALDOCUMENTAPPROVALID,
                                    loanApplicationId = x.LOANAPPLICATIONID,
                                    description = x.DESCRIPTION,
                                    collateralCode = "",
                                    ...
                                    collateralTypeId = 0,
                                    approvalStatusId = (short)x.APPROVALSTATUSID,
                                    applicationReferenceNumber = o.TBL_LOAN_APPLICATION.APPLICATIONREFERENCENUMBER,
                                    referenceNumber = x.REFERENCENUMBER,
                                    dateTimeCreated = x.DATETIMECREATED,
                                    systemArrivalDateTime = atrail.SYSTEMARRIVALDATETIME,
                                    approvalStatusName = context.TBL_APPROVAL_STATUS.Where(o => o.APPROVALSTATUSID == atrail.APPROVALSTATUSID).Select(o => o.APPROVALSTATUSNAME).FirstOrDefault(),
            ...
            foreach(var item in dataCollateral)
            {
                if(item.originalDocumentApprovalId == dataFacility[0].originalDocumentApprovalId)
                {
                    item.productName = dataFacility[0].productName;
                    item.productPriceIndex = dataFacility[0].productPriceIndex;
                    item.interestRate = dataFacility[0].interestRate;
                    item.applicationAmount = dataFacility[0].applicationAmount;
                    item.applicationReferenceNumber = dataFacility[0].applicationReferenceNumber;
                }
            }

            var data = dataCollateral.Union(dataFacility);
            if (dataCollateral.Count() > 0 && dataFacility.Count() > 0)
            {
                //data = dataFacility;
                data = dataCollateral;
            }
            if (dataCollateral.Count() > 0 && dataFacility.Count() < 1)
            {
                data = dataCollateral;
            }
        ...
        public int updateOriginalDocumentApproval(OriginalDocumentApprovalViewModel model)
        {
            ...
            if(entity != null)
            {
                ...
                this.audit.AddAuditTrail(new TBL_AUDIT
                {
                    ...
                    DETAIL = $"{TranslateHelper.get("Original Document Approval")} '{entity.DESCRIPTION}' {TranslateHelper.get("updated by")} {auditStaff}",
        ...
        public int AddOriginalDocumentApproval(OriginalDocumentApprovalViewModel model)
        {
            ...
            var loanApplicationId = context.TBL_LOAN_APPLICATION_COLLATERL.Where(l => l.COLLATERALCUSTOMERID == model.collateralCustomerId).FirstOrDefault()?.LOANAPPLICATIONID;
            ...
            {
                model.loanApplicationId = (int)loanApplicationId;
            }
