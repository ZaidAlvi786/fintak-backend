...
PUSHES_PER_DAY = 7
START_HOUR = 23  # Temporarily set to current hour for testing
END_HOUR = 24    # Allow testing now
...
def test_push_single_file():
    """Test pushing one file at a time to the daily branch"""
    state = load_state()
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    branch_name = f"bug-fixes-{today}"
    
    print(f"🧪 TESTING AUTO PUSH SYSTEM")
    print(f"📅 Today: {today}")
    print(f"🌿 Branch: {branch_name}")
    print(f"⏰ Current time: {datetime.datetime.now().strftime('%H:%M')}")
    print(f"📊 Schedule: {START_HOUR}:00 - {END_HOUR}:00")
