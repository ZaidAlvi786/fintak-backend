        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            try
            {
               ...
               /* 
                ...
                ActiveUserDetails userInfo = await authRepo.GetUserAuthenticationInfo(userVm.username);
                ActiveUserDetails userInformation = await authRepo.GetUserInformation(userVm.username);
                ...
                if (await authRepo.ConcurrentUsers() > 2000) // 2000 setup somewhere
                {
                    context.SetError("invalid_grant", "Maximum Concurrent Users exceeded!");
                    return;
                }
                ...
                if (appSetup != null && appSetup.USE_ACTIVE_DIRECTORY && record.STAFFROLECODE != "SYSADM")
                {
                    ...
                    {
                        authRepo.SessionInfo = await authRepo.CheckSessionState(userVm.username.ToLower(), ipAddress);//.GetAwaiter().GetResult();
                        user = await authRepo.FindUserByUserName(userVm.username.ToLower());
                        // user = await Task.FromResult(authRepo.FindUserByUserNameAsync(userVm.username.ToLower())).Result;
                    }
                    else
                    {
