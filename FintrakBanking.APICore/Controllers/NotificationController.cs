    ...
    public class NotificationController : ApiControllerBase
    {
      ...
      [HttpGet] [ClaimsAuthorization]  
       ...
       [HttpPut] [ClaimsAuthorization]
        ...
        public async Task<HttpResponseMessage> UpdateNotificationState(int notificationId)
        {
            try
            {
                var data = await repo.UpdateNotificationState(notificationId);

                if (data)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("Notification state updated") });
                }

                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, message = TranslateHelper.get("Notification state not updated") });
            }
            catch (SecureException ex)
            {
                errorLogger.LogError(ex, Request.RequestUri.Host, token.GetUsername);
        ...
        public async Task<HttpResponseMessage> AddNotificationState(NotificationViewModel model)
        {
            try
            {
                var data = await repo.AddNotification(model);

                if (data)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("Notification added successfully") });
                }

                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, message = TranslateHelper.get("Notification not added successfully") });
            }
            catch (SecureException ex)
            {
                errorLogger.LogError(ex, Request.RequestUri.Host, token.GetUsername);
