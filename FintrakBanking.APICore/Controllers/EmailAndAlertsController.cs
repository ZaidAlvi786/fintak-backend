    ...
    public class EmailAndAlertsController : ApiControllerBase
    {
        ...
        public HttpResponseMessage SendAlertsForCovenantsApproachingDueDate()
        {
            try
            {
              // repo.SendAlertsForCovenantsApproachingDueDate();
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = TranslateHelper.get("Email sent successfully") });
            }
            catch (SecureException ex)
            {
                errorLog.LogError(ex, Common.CommonHelpers.GetUserIP(), token.GetUsername);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There were errors")});
            }
        }
      ...
      [HttpGet] [ClaimsAuthorization]  
        ...
        public HttpResponseMessage SendAlertsForCovenantsOverDue()
        {
            ...
            {
                errorLog.LogError(ex, Common.CommonHelpers.GetUserIP(), token.GetUsername);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There were errors") + ":" + ex.Message });
            }
        }
