...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class LetterGenerationRequestController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> GetLetterGenerationRequests()
        {
            IEnumerable<LetterGenerationRequestViewModel> response = await repo.GetLetterGenerationRequests(token.GetStaffId);
        ...
        public async Task<HttpResponseMessage> GetLetterGenerationSignatory(int requestId)
        {
            IEnumerable<AuthorisedSignatoryViewModel> response =  await repo.GetLetterGenerationSignatory(requestId);
            return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
        }
        ...
        public async Task<HttpResponseMessage> GetLetterGenerationRequestsForApproval()
        {
            IEnumerable<LetterGenerationRequestViewModel> response = await repo.GetLetterGenerationRequestsForApproval(token.GetStaffId);
            return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
        }
        ...
        public async Task<HttpResponseMessage> GetLetterGenerationRequest(int id)
        {
            LetterGenerationRequestViewModel response = await repo.GetLetterGenerationRequest(id);
        ...
        [ClaimsAuthorization]
        [Route("letter-generation-request")]
        public async Task<HttpResponseMessage> AddLetterGenerationRequest([FromBody] LetterGenerationRequestViewModel model)
        {
            ...
            try
            {
                var response = await repo.AddLetterGenerationRequest(model);
                if (response.requestId > 0) return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = TranslateHelper.get("The record has been created successfully") });
        ...
        public HttpResponseMessage UpdateLetterGenerationRequest([FromBody] LetterGenerationRequestViewModel model, int id)
        {
            ...
            try
            {
                ...
                if (response.requestId > 0) return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = TranslateHelper.get("The record has been updated successfully") });
        ...
        public async Task<HttpResponseMessage> GetCamsolLoansByCustomerCode(string customerName, string customerCode)
        {
            try
            {
                var camsolLoans = await repo.GetCamsolLoansByCustomerCode(customerName, customerCode);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = camsolLoans });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error fetchcing the record") });
            }
        }
        ...
        public async Task<HttpResponseMessage> GetCamsolLoanDocument(int typeId, [FromBody] LetterGenerationRequestViewModel model)
        {
            try
            {
                var docHtml = await repo.GetCamsolLoanDocument(typeId, model);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = docHtml });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error fetchcing the document") });
            }
        }
