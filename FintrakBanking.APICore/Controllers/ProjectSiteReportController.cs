    ...
    public class ProjectSiteReportController : ApiControllerBase
    {
        ...
        public HttpResponseMessage AddProjectSiteReport([FromBody] ProjectSiteReportViewModel model)
        {
            try
            {
                ...
                if (response != 0) return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = TranslateHelper.get("The record has been created successfully") });
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error creating this record") });
            }
            catch(SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, new { success = false, message = TranslateHelper.get(ex.Message) });

            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, new { success = false, message = TranslateHelper.get("There was an error creating this record") });
            }
        }
        ...
        public async System.Threading.Tasks.Task<HttpResponseMessage> AddPsrImagec()
        {
            ...
            catch (Exception ex) { return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error uploading this file")}); }
