    ...
    public class CashBackController : ApiControllerBase
    {
        ...
        public HttpResponseMessage UpdateCashbackSection([FromUri] int id, [FromBody] CashBackViewModel entity)
        {
            try
            {
                ...
                return Request.CreateResponse(HttpStatusCode.OK,
                   new { success = false, message = $"{TranslateHelper.get("There was an error updateding this record")}" });
            }
            catch (SecureException e)
            {
                return Request.CreateResponse(HttpStatusCode.OK,
                   new { success = false, message = $" {TranslateHelper.get("There was an error updateding this record")} {e.Message}" });
            }
        }
        #endregion
