...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class ApprovalReliefController : ApiControllerBase
    {
      ...
      [HttpGet] [ClaimsAuthorization]  
        [Route("approval-relief")]
        public async Task<HttpResponseMessage> GetAllApprovalRelief(HttpRequestMessage request)
        {
            try
            {
                var data = await repo.GetAllApprovalRelief(token.GetCompanyId);
                if (!data.Any())
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
       ...
       [HttpPut] [ClaimsAuthorization]
        ...
        public async Task<HttpResponseMessage> GetApprovalReliefAwaitingApproval()
        {
            try
            {
                var data = await repo.GetApprovalReliefAwaitingApprovals(token.GetStaffId, token.GetCompanyId);
        ...
        public  async Task<HttpResponseMessage> GetAllStaffRelief(int staffId)
        {
            try
            {
                var data = await repo.GetAllStaffRelief(token.GetCompanyId, staffId);
