    ...
    public class AdminController : ApiControllerBase
    {
        ...
        public HttpResponseMessage GetAllUsers()
        {
            try
            {
                ...
                if (!users.Any())
                {
                    return Request.CreateResponse(HttpStatusCode.NotFound, new { success = false, message = TranslateHelper.get("No Record Found") });
                }
        ...
        public async Task<HttpResponseMessage> AddUserAsync([FromBody]AppUserViewModel user)
        {
            try
            {
                ...
                {
                    if (await repo.isUserExist(user.username))
                    {
                        return Request.CreateResponse(HttpStatusCode.OK,
                           new { suucess = false, message = "A user with this username already exist" });
                    }
                    if (await repo.isStaffExist(user.staffId))
                    {
        ...
        [ClaimsAuthorization]
        [Route("group/add")]
        public async Task<HttpResponseMessage> AddGroup([FromBody] AppGroupViewModel group)
        {
            try
            {


                if (await repo.isGroupExist(group.groupName))
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
        ...
        public async Task<HttpResponseMessage> GetAllGlobalSettings()
        {
            try
            {
                var globalSettings = await repo.GetAllGlobalSettings();
        ...
        [ClaimsAuthorization]
        [Route("group/{id}")]
        public async Task<HttpResponseMessage> UpdateGroup([FromBody] AppGroupViewModel group, short id)
        {
            //[FromBody]
            ...
            try
            {
                ...
                var data = await repo.UpdateGroup(id, group);
                if (data)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
        ...
        [HttpDelete]
        [ClaimsAuthorization]
        [Route("remove-group/{id}")]
        public async Task<HttpResponseMessage> deleteGroup([FromBody] AppGroupViewModel group, short id)
        {
            //NOTE: DELETING WORKFLOW GROUP IS NOT RECOMMENDED - AUSTIN
            //[FromBody]
            var req = this.Request;
            group.createdBy = token.GetStaffId;
            group.userBranchId = (short)token.GetBranchId;
            ...
            var data = await repo.deleteGroup(id, group);
            if (data)
            {
                return Request.CreateResponse(HttpStatusCode.OK,
                   new { success = true, result = group, message = "Group has been deleted successfully" });
            }
        ...
        public async Task<HttpResponseMessage> GetGroupById(int id)
        {
            try
            {
                var group = await repo.GetSingleGroup(id);
        ...
        public async Task<HttpResponseMessage> StaffHasActivity( string activity)
        {
            try
            {
                var res = await repo.StaffHasActivity(token.GetStaffId,activity);
                return Request.CreateResponse(HttpStatusCode.OK,
        ...
        public async Task<HttpResponseMessage> GetProfileConfiguration()
        {
            try
            {
                var data = await profileSetup.GetProfileConfiguration();
        ...
        public HttpResponseMessage TwoFactorAuthentication(string staffCode, string passCode)
        {
            try
            {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                    new { success = true, message = "Two Factor Authentication not enabled", isTwoFAEnabled = false });
                }
                ...
                {
                       return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = true, result = data, message = data.message, isTwoFAEnabled = true });
                }
        ...
        [ClaimsAuthorization]
        [Route("api-log")]
        public async Task<HttpResponseMessage> GetAPILog([FromBody] DateRange range)
        {
            try
            {
                if (range != null)
                {
                    string message = string.Empty;
                    var data = await _log.GetAPILog(range.startDate, range.endDate, range.loanRefNo);
                    if (data!=null)
                        return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = false, result = data, message = "" });
                }
                ...
                return Request.CreateResponse(HttpStatusCode.OK,
                                     new { success = true, result = TranslateHelper.get("No Record Found") });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK,
        ...
        [ClaimsAuthorization]
        [Route("error-log")]
        public async Task<HttpResponseMessage> GetErrorLog([FromBody] DateRange range)
        {
            try
            {
                if (range != null)
                {
                    string message = string.Empty;
                    var data = await _log.GetErrorLog(range.startDate, range.endDate);
                    if (data != null)
                        return Request.CreateResponse(HttpStatusCode.OK,
                                              new { success = false, result = data, message = "" });
                }
        ...
        public HttpResponseMessage GetStaffADDetails(string staffCode, string passCode)
        {
            try
            {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                                            new { success = false, result = data, message = TranslateHelper.get("Staff Code not fund on Active Directory") });
                }
                else
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                                         new { success = true, result = data, message = TranslateHelper.get("Success") });
                }
            ...
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, (new { success = false, message = TranslateHelper.get("Action Failed") }));
            }
...
<!-- Auto-push timestamp: 2026-05-23 18:51:01 -->