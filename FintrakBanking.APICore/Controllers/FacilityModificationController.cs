...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class FacilityModificationController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> GetFacilityModificationsForApproval()
        {
            try
            {
                IEnumerable<FacilityModificationViewModel> response = await repo.GetFacilityModificationsForApproval(token.GetStaffId);
        ...
        public async Task<HttpResponseMessage> GetFacilityModification(int id)
        {
            try
            {
                FacilityModificationViewModel response = await repo.GetFacilityModification(id);
        ...
        public async Task<HttpResponseMessage> AddFacilityModification([FromBody] FacilityModificationViewModel model)
        {
            ...
            try
            {
                var response = await repo.AddFacilityModification(model);
                if (response != null)
                {
        ...
        public async Task<HttpResponseMessage> ApproveFacilityModification([FromBody] ForwardViewModel model)
        {
            ...
            try
            {
                var response = await repo.ApproveFacilityModification(model);
                if (response != null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = response.responseMessage });
                }
