...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class FacilityDetailSummaryController : ApiControllerBase
    {
        ...
        [HttpGet]
        [ClaimsAuthorization]
        [Route("lms-loan-convenantODF")]
        public HttpResponseMessage GetLMSLoanConvenantDetailODF(int loanId)
        {
            try
            {
                var data = repo.LMSLoanCovenantDetailODF(loanId);
                if (data == null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = false, message = TranslateHelper.get("No Record Found") });
        ...
        [HttpGet]
        [ClaimsAuthorization]
        [Route("loan-collateral-loanODF/{loanId}")]
        public HttpResponseMessage GetCollateralDetailLMSODF([FromUri] int loanId)
        {

            List<CollateralViewModel> data = repo.CollateralByLoanIdODF(loanId);
            if (data == null)
            {
                return Request.CreateResponse(HttpStatusCode.OK,
                   new { success = false, message = TranslateHelper.get("No Record Found") });
...
<!-- Auto-push timestamp: 2025-12-18 14:13:02 -->