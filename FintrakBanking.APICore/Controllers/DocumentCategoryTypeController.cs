...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class DocumentCategoryTypeController : ApiControllerBase
    {
        ...
        [ClaimsAuthorization]
        [Route("document-category-type")]
        public async Task<HttpResponseMessage> AddDocumentCategoryType([FromBody] DocumentCategoryTypeViewModel model)
        {
            ...
            var response = await repo.AddDocumentCategoryType(model);
        ...
        [ClaimsAuthorization]
        [Route("document-category-type/{id}")]
        public async Task<HttpResponseMessage> UpdateDocumentCategoryType([FromBody] DocumentCategoryTypeViewModel model, int id)
        {
            ...
            bool response = await repo.UpdateDocumentCategoryType(model,id,user);
        ...
        [ClaimsAuthorization]
        [Route("document-category-type/{id}")]
        public async Task<HttpResponseMessage> DeleteDocumentCategoryType(int id)
        {
            UserInfo user = new UserInfo()
            {
                BranchId = token.GetBranchId,
            ...
            bool response = await repo.DeleteDocumentCategoryType(id,user);
            return Request.CreateResponse(HttpStatusCode.OK, new { success = response, result = response, count = 1 });
        }
    }
}

<!-- Auto-push timestamp: 2026-03-17 16:52:42 -->