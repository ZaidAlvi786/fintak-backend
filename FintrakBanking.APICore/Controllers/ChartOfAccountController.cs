...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class ChartOfAccountController : ApiControllerBase
    {
      ...
      [HttpGet] [ClaimsAuthorization]   [Route("")]
        ...
        public async Task<HttpResponseMessage> GetAccountsByCategory(   short accountCategoryId)
        { 
                try
                {
                    var accounts = await repo.GetAccountsByCategory(accountCategoryId);
                    if (accounts == null)
                    {
                        return Request.CreateResponse(HttpStatusCode.OK,
                 new { success = false, message = TranslateHelper.get("No Record Found") });
      ...
      [HttpGet] [ClaimsAuthorization]  
        [Route("fs-captions")]
        public async Task<HttpResponseMessage> GetFinancialSatementCaptionLookup()
        { try
            {
                var accounts = await repo.GetFinancialSatementCaptionLookup();
                if (accounts == null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                      new { success = false, message = TranslateHelper.get("No Record Found") });
      ...
      [HttpGet] [ClaimsAuthorization]   [Route("{accountId}")]
        ...
        public async Task<HttpResponseMessage> AddTempAccount([FromBody] ChartOfAccountViewModel model)
        {
            try
            {
                if ( await repo.IsAccountCodeAlreadyExist(model.accountCode))
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = false, message = $"{TranslateHelper.get("An Account with")} {model.accountCode} {TranslateHelper.get("already exist")}" });
                }
                if ( await repo.IsTempAccountExist(model.accountCode))
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = false, message = $"{TranslateHelper.get("An Account with")} {model.accountCode} {TranslateHelper.get("already exist waiting for approval")}" } );
                }
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = true, result = account, message = TranslateHelper.get("Account has been created successfully, now waiting for approval") });
                }
                else
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = false, message = TranslateHelper.get("Account not created") });
            }
            catch (SecureException ex)
            {
       ...
       [HttpPut] [ClaimsAuthorization]
        [Route("{accountId}")]
        public async Task<HttpResponseMessage> UpdateAccount(short accountId, [FromBody] ChartOfAccountViewModel model)
        {
            try
            {
                var token = new TokenDecryptionHelper();
                ...
                var account = await repo.UpdateAccount(accountId, model);
                if (account)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = true, result = account, message = TranslateHelper.get("Account has been updated successfully, now waiting for approval") });
                }
                else
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = false, message = TranslateHelper.get("Account not created") });
        ...
        public HttpResponseMessage GoForApprovalAsync([FromBody]ApprovalViewModel entity)
        {
            try
            {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
                        new { success = true, message = TranslateHelper.get("Account record has been approved successfully") });
                }
                else
                    return Request.CreateResponse(HttpStatusCode.OK,
      ...
      [HttpGet] [ClaimsAuthorization]  
        ...
        public async Task<HttpResponseMessage> GetAccountsAwaitingApproval()
        {
            try
            {
                var token = new TokenDecryptionHelper();
                var accountInfo = await repo.GetAccountsAwaitingApprovals(token.GetStaffId, token.GetCompanyId);
      ...
      [HttpGet] [ClaimsAuthorization]  
        ...
        { 
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK,
                      new { success = true, result = accountId, message = TranslateHelper.get("account has been deleted successfully") });
                }
                catch (SecureException ex)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
      ...
      [HttpGet] [ClaimsAuthorization]  
        [Route("classes")]
        public async Task<HttpResponseMessage> GetChartOfAccountClasses()
        {
            try
            {
                var data = await repo.GetChartOfAccountClasses();
                ...
                return Request.CreateResponse(HttpStatusCode.OK,
                  new { success = false, result = data.ToList(), message = TranslateHelper.get("No records found!") });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK,
