...
{     
    ...
    public class ApprovalGroupController : ApiControllerBase
    {
        ...
        public async  Task<HttpResponseMessage> AddApprovalGroupMapping(  [FromBody] ApprovalGroupMappingViewModel model)
        { 
                try
                {
                ...
                var data = await repoMapping.AddApprovalGroupMapping(model);
                    if (data != -1)
                    {
      ...
      [HttpGet] [ClaimsAuthorization]  [Route("approval-group-mapping/{operationMappingId}")]
        public async Task<HttpResponseMessage> GetApprovalGroupMappingById(  int operationMappingId)
        {    try
                {
                  var token = new TokenDecryptionHelper( );

                    var  data = await repoMapping.GetApprovalGroupMapping(operationMappingId);
                    if (data == null)
                    {
        ...
        public async Task<HttpResponseMessage> GetApprovalGroupMapping(int operationId, short? productClassId, short? productId)
        {
            try
            {
                productClassId = (productClassId == -1) ? null : productClassId;

                var data = await repoMapping.GetApprovalGroupMapping(operationId, productClassId, productId);
                if (data== null)
                {
       ...
       [HttpPut] [ClaimsAuthorization] [Route("approval-group-mapping/{operationMappingId}")]
        ...
        { 
                ...
                applicationUrl = HttpContext.Current.Request.Path,
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = operationMappingId, message = TranslateHelper.get("record has been deleted successfully") });
                }
      ...
      [HttpGet] [ClaimsAuthorization]  [Route("approval-group")]
        public async Task<HttpResponseMessage> GetAllApprovalGroup(HttpRequestMessage request)
        { 
                try
                {
                    var token =  new TokenDecryptionHelper();
                    ...
                    var data = await repoGroup.GetAllApprovalGroup(token.GetCompanyId);
                    if (!data.Any())
                    {
                        return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = false, message = TranslateHelper.get("No Record Found") });
        ...
        { 
                ...
                applicationUrl = HttpContext.Current.Request.Path,
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = true, result = GroupId, message = TranslateHelper.get("Record has been deleted successfully") });
                }
                catch (SecureException ex)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
