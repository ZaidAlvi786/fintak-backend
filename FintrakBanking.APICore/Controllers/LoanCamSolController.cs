...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class LoanCamSolController : ApiControllerBase
    {
        ...
        [ClaimsAuthorization]
        [Route("loan-camsol")]
        public async Task<HttpResponseMessage> GetAllCamsol()
        {
            try
            {
                var data = await repo.GetCamSol();
        ...
        public async Task<HttpResponseMessage> GetLoanCamsolAwaitingApproval()
        {
            try
            {
                ...
                var data = await repo.CamSolAwaitingApproval(companyId, staffId);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        [ClaimsAuthorization]
        [Route("bulk-approval")]
        public async Task<HttpResponseMessage> GoForBulkApproval([FromBody]List<LoanCAMSOLViewModel> entity)
        {
            ...
            try
            {
                foreach(var data in entity)
                {
                    ...
                    var record = await repo.GoForBulkApproval(data);
                    var result = "";

                    if (record == true)
                    {
                       result = TranslateHelper.get("Record Approved");
                    }
                    else
                    {
                        result = TranslateHelper.get("Record Not Approved");
                    }
                ...
                if (val.Count() != 0)
                {
                    ...
                    new { success = true, message = TranslateHelper.get("Camsol record has been approved successfully") });
        ...
        public async Task<HttpResponseMessage> goForApproval([FromBody] LoanCAMSOLViewModel data)
        {
            try
            {
                data.companyId = token.GetCompanyId;
                data.createdBy = (short)token.GetStaffId;
                var val = await repo.goForApproval(data);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = val });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, result = $"{TranslateHelper.get("Error")}: {ex.Message}" });
            }
        }
        ...
        public async Task<HttpResponseMessage> GetLoanCamsolById(int id)
        {
            try
            {
                var data = await repo.GetCamSolByType(id);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetLoanCamsolByCustomerCode(string  customercode)
        {
            try
            {
                var data = await repo.GetCamSolByCustomerCode(customercode);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> ViewLoanCamsolById(int id)
        {
            try
            {
                var data = await repo.ViewCamSolByType(id);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetCamsolAwaitingApprovalById(int id)
        {
            try
            {
                var data = await repo.CamSolAwaitingApprovalById(id);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetLoanCansolType()
        {
            try
            {
                var data = await repo.GetCamSolType();
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> ApproveCamsol([FromBody] LoanCAMSOLViewModel updateOptions)
        {
            try
            {
                var data = await repo.ApproveCamsol(updateOptions);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("Camsol override was not successful") });
            }
        }
        ...
        public async Task<HttpResponseMessage> UploadCamsolData()
        {
            try
            {
                ...
                if (data != null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = data, message = TranslateHelper.get("Camsol document uploaded successfully") });
                }
