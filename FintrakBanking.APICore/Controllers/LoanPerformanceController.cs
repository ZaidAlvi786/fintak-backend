...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class LoanPerformanceController : ApiControllerBase
    {
      ...
      [HttpGet] [ClaimsAuthorization]  
        ...
        public async Task<HttpResponseMessage> GetPrudGuildlineType()
       {
            try
            {
                var type = await repo.GetPrudGuildlineStatus();
      ...
      [HttpGet] [ClaimsAuthorization]  
        ...
        public HttpResponseMessage GetAllLoanPerformance([FromUri] int page, [FromUri] int itemsPerPage)
        {
            ...
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error fetchcing the records") + "."+ TranslateHelper.get("Error") + "-" + TranslateHelper.get(ex.Message) });
            }
        ...
        public HttpResponseMessage SearchForFilteredLoanPerformance(string searchQuery)
        {
            ...
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("There was an error fetchcing the records")});
        ...
        public async Task<HttpResponseMessage> LoanPerformanceStatusChange([FromBody] PrudGuidelineStatusChangeViewModel entity)
        {
            try
            {
                ...
                var data = await repo.LoanPerformanceStatusChange(entity);
                if (data)
                {
                    return Request.CreateResponse(HttpStatusCode.OK,
