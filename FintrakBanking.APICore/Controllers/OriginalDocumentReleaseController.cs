...
namespace FintrakBanking.APICore.Controllers
{
    ...
    public class OriginalDocumentReleaseController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> GetReleaseApproval()
        {
            try
            {
                var response = await _repo.GetLeaseDocumentForApproval(token.GetStaffId);
        ...
        public async Task<HttpResponseMessage> GetCashReleaseApproval()
        {
            try
            {
                var response = await _repo.GetCashSecurityReleaseForApproval(token.GetStaffId);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetSecurityReleaseSearch(string searchString)
        {
            try
            {
                var response = await _repo.GetSecurityReleaseSearch(searchString);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetCashSecurityReleaseSearch(string searchString)
        {
            try
            {
                var response = await _repo.GetCashSecurityReleaseSearch(searchString);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetRejectedAndReferredSecurityRelease()
        {
            try
            {
                var response = await _repo.GetRejectedAndReferredSecurityRelease(token.GetStaffId);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
            }
            catch (SecureException ex)
            {
        ...
        public async Task<HttpResponseMessage> GetRejectedAndReferredCashSecurityRelease()
        {
            try
            {
                var response = await _repo.GetRejectedAndReferredCashSecurityRelease(token.GetStaffId);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, count = response.Count() });
            }
            catch (SecureException ex)
            {
        ...
        public HttpResponseMessage reinitiateSecurityRelease(int id)
        {
            try
            {
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, result = response, message = TranslateHelper.get("Error Occurred, Please Contact the System Administrator") });
            }
            catch (SecureException ex)
            {
        ...
        public HttpResponseMessage AddSecurityRelease([FromBody]IEnumerable<OriginalDocumentReleaseViewModel> model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("An Error Occurred, Kindly Contact the System Administrator") });
                    }
                }
        ...
        public HttpResponseMessage GoForApproval([FromBody] IEnumerable<OriginalDocumentReleaseViewModel> model)
        {
            ...
            try
            {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = false, result = response, message = TranslateHelper.get("One or More Document may currently be Undergoing Approval") });
                }
            //}
            }
            catch (SecureException ex)
        ...
        [HttpPost]
        [ClaimsAuthorization]
        [Route("cash-security-release-go-for-approval")]
        public async Task<HttpResponseMessage> GoForApproval([FromBody] OriginalDocumentReleaseViewModel model)
        {
            try
            {
                //  foreach(var x in model)
                ...
                //}
                var response = await _repo.GoForApproval(model);
                if (response != null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = response.responseMessage });
                }
        ...
        [HttpPost]
        [ClaimsAuthorization]
        [Route("cash-security-release-approval")]
        public async Task<HttpResponseMessage> GoForCashSecurityApproval([FromBody] CollateralCashReleaseViewModel model)
        {
            try
            {
                model.createdBy = token.GetStaffId;
                model.companyId = token.GetCompanyId;
                WorkflowResponse response = await _repo.GoForCashSecurityApproval(model);
                if (response != null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = response.responseMessage });
                }
        ...
        public async Task<HttpResponseMessage> GoForGuaranteeApproval([FromBody] CollateralCashReleaseViewModel model)
        {
            try
            {
                model.createdBy = token.GetStaffId;
                model.companyId = token.GetCompanyId;
                WorkflowResponse response = await _repo.GoForGuaranteeCashApproval(model);
                if (response != null)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = response.responseMessage });
                }
        ...
        public async Task<HttpResponseMessage> SecurityReleaseGoForApproval([FromBody] OriginalDocumentReleaseViewModel model)
        {
            try
            {
                ...
                WorkflowResponse response = await _repo.SubmitApproval(model);
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = response, message = response.responseMessage });
            }
            catch (SecureException ex)
            {
