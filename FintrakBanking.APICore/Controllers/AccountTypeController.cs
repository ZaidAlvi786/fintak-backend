    ...
    public class AccountTypeController : ApiControllerBase
    {
        ...
        public HttpResponseMessage AddAccountType([FromBody] AddAccountTypeViewModel model)
        {
            try
            {
                ...
                if (isCreated == true)
                {
                    var message = TranslateHelper.get("account type has been created successfully");
                    return Request.CreateResponse(HttpStatusCode.OK,
                       new { success = true, result = isCreated, message = message });
                }
                else
                {
                    var message = TranslateHelper.get("account type not created");
                    return Request.CreateResponse(HttpStatusCode.OK,
        ...
        public HttpResponseMessage UpdateAccountType(int accountTypeId, [FromBody]AccountTypeViewModel model)
        {
            ...
            {
                result = Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get(ex.Message) });
            }
            return result;
        }
