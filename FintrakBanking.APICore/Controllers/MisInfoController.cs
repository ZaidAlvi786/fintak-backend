    ...
    public class MisInfoController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> AddMisInfo(MisInfoViewModel entity)
        {
            try
            {
                ...
                if (misInfo)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = misInfo, message = TranslateHelper.get("mis has been created successfully") });
                }
                else
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("mis not created") });
            }
       ...
       [HttpPut] [ClaimsAuthorization][Route("misinfo/{misinfoid}")]
        ...
        public async Task<HttpResponseMessage> DeleteMisTypeAsync(int misInfoId)
        {
            try
            {
                ...
                if (misType)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = misType, message = TranslateHelper.get("mis has been created successfully") });
                }
                else
                    return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("mis not created") });
            }
      ...
      [HttpGet] [ClaimsAuthorization]  [Route("mistype/{mistypeid}")]
       ...
       [HttpPut] [ClaimsAuthorization][Route("mistype/{mistypeid}")]
        public async Task<HttpResponseMessage> UpdateMisType(int mistypeid, [FromBody] MisTypeViewModel entity)
        {
            if (entity == null)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get("Bad Request") });
            }
            ...
            try
            {
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { success = true, result = entity, message = TranslateHelper.get("account has been updated successfully") });
            }
            catch (SecureException ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { success = false, message = TranslateHelper.get(ex.Message) });
